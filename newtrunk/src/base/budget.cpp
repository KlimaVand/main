...
void budget::SetNames(string aClassName,
                      string aBudgetName) {
...
bool budget::Balance(double remain) {
    double diff        = input - output - remain;
    double maxRelError = 0.1;
    ...
    if ((fabs(diff) > maxRelError) && (fabs(diff - lastError) > maxRelError)) {
        ...
        outputString = ClassName + ":: Error in budget of " + BudgetName + ". Difference is " + conv.DoubleToChar(diff);

        if (theMessage) {
            theMessage -> Warning(outputString);
        } else {
...
void budget::Add(budget aBudget,
                 double fraction) {
