...
const int    AccmonthLength[12] = {
    0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334
};    // accumulated days
const double OldAdjust[13]      = {
    0, 1.21, 1.22, 1.22, 1.18, 1.15, 1.14, 1.12, 1.11, 1.13, 1.14, 1.16, 1.19
};    // accumulated days
const double NewAdjust[13]      = {
    0, 1.41, 1.42, 1.35, 1.24, 1.13, 1.11, 1.10, 1.10, 1.11, 1.14, 1.23, 1.37
};    // accumulated days
fileAccess   hd;
...
void climate::ReadLine() {
    ...
    switch (filetyp) {
        case 0 :    // CLM filer
            ...
            if (DailyCO2Values) {
				*file >> yr >> mnth >> d >> tmin >> tmax >> rad >> precip >> epot >> humidity >> windVelocity>>Tdew >> co2;
            } else {
				*file >> yr >> mnth >> d >> tmin >> tmax >> rad >> precip >> epot >> humidity >> windVelocity>> Tdew;
            }
			//cout << tmax << " " << theTime << " " << d << " " << mnth << endl;
		//	cout << yr << "\t year\t" << mnth << "\tmonth\t" << d << "\td\t" << tmin << "\ttmin\t" << tmax << "\ttmax\t" << rad << "\trad\t" << precip << "\tprecip\t" << epot << "\tepot\t" << humidity << "\thumidity\t" << windVelocity << "\twindVelocity\t" << Tdew << "\tTdew\t" << co2<<"\tco2\t"<<'\n';
        ...
        case 2 :
            ...
            *file >> date >> tmin >> tmax >> tmean >> precip >> rad >> humidity >> vel;
            ...
            windVelocity = GetWindAtHeight(10.0, vel, 2.0, 0.01);

            // calculates the velocity at 2m (vel is at 10m)
            file -> getline(buffer, strlength);
        ...
        case 3 :           // DAISY climate files
            ...
            rad = atof(c);
            rad = rad * 24.0 * 60.0 * 60.0 / 1000000.0;

            // Conversion Wm-2 to Jm-2. BMP added 4-6-1999
            file -> get(c, 7);
        ...
        case 4 :    // DAISY climate files but with year 2000 compatibility and extra

            // column with windspeed
            char buf[20];
        ...
        case 5 :    // Download format from Intranet
            ...
            yr           = (buffer[12] - 48) * 1000 + (buffer[13] - 48) * 100 + (buffer[14] - 48) * 10
                           + (buffer[15] - 48);
        ...
        case 8 :    // Excel CSV  file (; as separator)
            string line = "";
            ...
            tmin = atof(word.c_str());

            getline(strstr, word, ';');

            tmax = atof(word.c_str());
            ...
            precip = atof(word.c_str());

            getline(strstr, word, ';');

            epot       = atof(word.c_str());
 ...
 */
void climate::InitClimate(string afilename) {
	filename = afilename;
    Circular         = false;
    numPrecipDays    = 0;
	...
	ChangeMinPlantTemp = -2;
	data.FindItem("ChangeMinPlantTemp", &ChangeMinPlantTemp);
    ...
    while (!file -> eof()) {
        ...
        if ((yr > 0) && (FirstDate > TempDate)) {
            FirstDate = TempDate;
        }

        if ((yr > 0) && (LastDate < TempDate)) {
            LastDate = TempDate;
        }
    }

    if (i > 0) {
        YearlyAverage = YearlyAverage / i;
    }
 ...
 *
 */
void climate::InitClimate(string aDirectory,
                         string afilename) {
	filename = afilename;
	climateDirectory = aDirectory;
	hd.changeDir(climateDirectory);
...
double climate::SunDeclination() {
    ...
    return 0.006918 - 0.399912 * cos(Omega) + 0.070257 * sin(Omega) - 0.006758 * cos(2 * Omega)
           + 0.000907 * sin(2 * Omega) - 0.002697 * cos(3 * Omega) + 0.001480 * sin(3 * Omega);
}
...
double climate::e(double temprature)
{
...
double climate::Rnl()
{
	double latitude = theClimate->latitude;
	int jDay = theTime.GetJulianDay();
	double dr = 1 + 0.033*cos(2 * 3.14 / 365 * jDay);//Eq23
	double solarDeclination = 0.409*sin((2 * 3.1415) / 365 * jDay - 1.39);//Eq24

	double D2 = 3.1415 / 180 * latitude;
	double Ws = acos(-tan(solarDeclination)* tan(D2));
	double Gsc = 0.0820;
	double Ra = 1440 / 3.14 * 0.082* dr*((Ws)* sin(D2)*sin(solarDeclination) + cos(D2)*cos(solarDeclination)* sin(Ws));// Eq 28
	double Altitude = 0;
	commonData data = new commonData();
	data.readFile("environ.dat");
	data.setCritical(true);

	data.FindItem("Z", &Altitude);
	double Rso = (0.75 + 2 * (Altitude) / 100000)* Ra; //eq 37
	double q = 4.903 * pow(10.0, -9.0);
	double ea = GetsaturatedVapourPressure();//ctual vapour pressure
	double tmp =theClimate->rad / Rso;
	if (tmp > 1)
		tmp=1;
	double result = 4.903* pow(10.0, -9.0)*(pow((theClimate->tmax + 273.16), 4) + pow((theClimate->tmin + 273.16), 4)) / 2 * (0.34 - 0.14* pow(ea, 0.5))*(1.35*tmp - 0.35);//39
...
double climate::Rnet()
{
	//fomulars are from Faop 56
	
	
	


	double RnlResult = Rnl();
	double Rns = theClimate->rad*0.77;//Eq 38
	return Rns - RnlResult; //Eq 40
}
/*
...
double climate::PenmanEvaporation() {
	fileAccess hd;
	...
	double RNL = Rnet();//RN
	double slope = 4098 * (0.6108*exp(17.27 *tmean / (tmean + 237.3))) / (pow((tmean + 237.3), 2)); //represents the slope of the saturation vapour pressure temperature relationship,
	...
	double Y = 0.000665*P; //psychrometric constant
	
	double heatFlux = 0;
	double z = 0;
	double u2 = windVelocity;// *(4.87 / log(67.8*z - 5.42));
	double result = (0.408*slope*(RNL - heatFlux) + 900 * Y / (tmean + 273)* u2*(Es - Ea)) / (slope + Y*(1 + 0.34 *u2));

	if (used1 == false)
	{
		myfile1.open(globalSettings->getOutputDirectory() + "\\example2.xls");
		used1 = true;
		myfile1 <<"Penman"<<'\t'<< "RNL" << '\t' << "slope" << '\t' << "Es" << '\t' << "Ea" << '\t' << 'Y' << '\t' << "theTime" << '\n';
	}
	myfile1 << result << '\t' << RNL << '\t' << slope << '\t' << Es << '\t' << Ea << '\t' << Y << '\t' << theTime << '\n';
	myfile1.close();
	    return result;
}
 ...
 *
 */
double climate::MakkinkEvaporation(double globalRadiation,
                                   double temp) {
    const double evpHtWat   = 2.465;
    const double psykrConst = 0.667;
    double       result     = 0.7
                              * (DeltaVaporPress(temp) * globalRadiation
 ...
 */
double climate::Evaporation(double radiation,
                            double temp,
                            double humidity,
    ...
    double       result        = (Delta * radiation
                                  + air_density * specific_heat * vp * (1 - humidity)
 ...
 */
double climate::Evaporation(double temp,
                            double humidity,
                            double transferCoefficient) {
    return Evaporation(0, temp, humidity, transferCoefficient);
}
...
void climate::GetClimate(double & temp,
                         double & min_temp,
                         double & max_temp,
                         double & prec,
                         double & radiation,
...
void climate::Update(bsTime * aTime) {
    ...
    strcpy(buffer, hd.getCurrentPath().c_str());

    // get current directory - this procedure will divert to climate directory
    ...
    if (*aTime > LastDate) {
        if (!Circular) {
            theMessage -> WarningWithDisplay("Climate::Current date greather than last date - using circular dates!");
        }

        Circular = true;
        year     = FirstDate.GetYear()
                   + ((year - FirstDate.GetYear())
                      % (LastDate.GetYear() - FirstDate.GetYear()));    // ensures cirkular referencing if you run
    }
...
void climate::CalcAverageClimate() {
    ...
    strcpy(buffer,
           hd.getCurrentPath().c_str());    // get current directory - this procedure will divert to climate directory
...
void climate::CalcAverageClimateForPeriod(bsTime * startDate,
        int                                        numOfDays,
