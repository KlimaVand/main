...
template<class T>
void linkList<T>::MakeFirstNode(PS & P,
                                T *  E) {
    P            = new Snode;
    P -> next    = nullptr;
    P -> element = E;
...
template<class T>
void linkList<T>::PeekHead(PS & P) {
    P = Head;
}

template<class T>
void linkList<T>::PeekTail(PS & P) {
    P = Tail;
}

template<class T>
int linkList<T>::NumOfNodes() {
    return numOfNodes;
}

template<class T>
void linkList<T>::InsertAfter(PS & S,
                              PS & P,
                              T *  E) {
    if (!Head) {
        MakeFirstNode(P, E);
...
template<class T>
void linkList<T>::InsertLast(T * E) {
    PS S1, S2;
...
template<class T>
void linkList<T>::InsertBefore(PS & S,
                               PS & P,
                               T *  E) {
    if (!Head) {
        MakeFirstNode(P, E);
        ...
        if (Head == S) {
            ...
            if (PT) {
                ...
                cout << "linkList<T>::InsertBefore - function called with invalid pointer, element not inserted!"
                     << endl;
            }
        }
    }
...
template<class T>
void linkList<T>::ForEach(void (*funcp)(T *)) {
    PS P = Head;
...
template<class T>
double linkList<T>::AddForEach(double (*funcp)(T *)) {
    double sum = 0.0;
    PS     P   = Head;
...
template<class T>
void linkList<T>::FirstThat(PS & FT,
                            int (*funcp)(T *)) {
    PS P = Head;
...
// Returnerer nullptr n�r ikke fundet
template<class T>
void linkList<T>::LastThat(PS & FT,
                           int (*funcp)(T *)) {
    FT = nullptr;
...
template<class T>
void linkList<T>::SearchByNumber(PS & FT,
                                 int  num) {
    FT = Head;
...
template<class T>
T * linkList<T>::ElementAtNumber(int num) {
    PS  FT = Head;
    int i  = 0;
