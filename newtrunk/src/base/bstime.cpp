...
void bsTime::SetTime(string day,
                     string month,
                     string year) {
    if (day == "") {
        day = "1";
    ...
    if (year == "R1") {    // facilitates relative years (usefull in field plans)
        SetTime(conv.StringToInt(day), conv.StringToInt(month), theTime.GetYear() + 1);
    } else if ((year == "") || (year == "R0")) {
        SetTime(conv.StringToInt(day), conv.StringToInt(month), theTime.GetYear());
    } else {
        SetTime(conv.StringToInt(day), conv.StringToInt(month), conv.StringToInt(year));
    }
}

void bsTime::SetTime(int inityear,
                     int JulianDay) {
    year  = inityear;
    month = 1;

    while (JulianDay > tabDaysPerMonth[IsLeapYear(year)][month - 1]) {
        JulianDay = JulianDay - tabDaysPerMonth[IsLeapYear(year)][month - 1];
...
ostream & operator <<(ostream &      os,
    ...
    os << setw(2) << aTime.month << "/" << setw(2) << aTime.year << setiosflags(ios::left);
...
int bsTime::GetJulianDay(int aday,
                         int amonth,
                         int ayear) {
    int ret_val = 0;
