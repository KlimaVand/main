    ...
    public animal
{
    ...
    private:
        ...
        cattle() {
            ...
            fieldList          = nullptr;
            growth             = nullptr;
            lactating          = pregnant = false;
            mother             = nullptr;
            foetus             = nullptr;
            milk               = nullptr;
            calf               = nullptr;
            urineN             = weight = FEDemand = Grazed_DM = Grazed_FE = 0.0;
            daysFromConception = daysFromParturition = hoursHoused = lactationNumber = limitation = 0;
            aminoAcidDemand    = aminoAcidSupply = lactoseConc = 0.0;
            isFemale           = isSuckling = isYoung = isFoetus = false;
            DM_intake          = ME_intake = dungN = FE_intake = bodyFat = bodyProtein = age = acceptanceCoefficient = 0.0;
            milkFatConc        = milkProteinConc = rumenNsurplus = MBW = 0.0;
            lastMilkYield      = gestationPeriod = 0.0;
            feedPlanList       = new cloneList<product>;
            feedUseList        = new cloneList<product>;
        }

        // block use of default constructor from outside animal
#endif
    ...
    protected:
        ...
        virtual double Get_quality_factor(double digestibility) {
            return 0.0;
        }

        virtual double SucklingCalfMilkIntake() {
            return 0.0;
        }
		
        virtual void DoSelection(linkList<feed_resource> *,
                                 int * count);
    ...
    public:
        ...
        virtual void Setmother(cattle * aMother) {
            mother = aMother;
        }
		...
		linkList<field> * getField()
		{
			return fieldList;
		}
        virtual void Setage(int anAge) {
            age = anAge;
        }

        virtual void Setcode(int aCode) {
            code = aCode;
        }

        virtual void Setweight(double aWeight) {
            weight = aWeight;
        }

        virtual void SetisFemale(bool isaFemale) {
            isFemale = isaFemale;
        }

        virtual void SetfeedPlanList(cloneList<product> * aFeedPlan) {
            feedPlanList = aFeedPlan;
        }

        virtual void SetfeedUseList(cloneList<product> * aFeedUseList) {
            feedUseList = aFeedUseList;
        }

        virtual void SetisYoung(bool isItYoung) {
            isYoung = isItYoung;
        }

        virtual void SetisSuckling(bool isItSuckling) {
            isSuckling = isItSuckling;
        }

        virtual void SetisFoetus(bool itIsAFoetus) {
            isFoetus = itIsAFoetus;
        }

        virtual void SetbodyProtein(double aProtein) {
            bodyProtein = aProtein;
        }

        virtual void SetbodyFat(double abodyFat) {
            bodyFat = abodyFat;
        }

        void SetFoetus(cattle * aFoetus) {
            foetus = aFoetus;
        }

        virtual void SethoursHoused(double aValue) {
            hoursHoused = aValue;
        }

        virtual void SetfeedingGroup(int afeedingGroup) {
            feedingGroup = afeedingGroup;
        }

#ifndef _STANDALONE
        virtual void SetfieldList(linkList<field> * afieldList) {
        ...
        virtual cattle * Getmother() {
            return mother;
        }

        virtual bool GetisFemale() const {
            return isFemale;
        }

        virtual cloneList<product> * GetfeedPlanList() {
            return feedPlanList;
        }

        virtual feedItem * Getmilk() {
            return milk;
        }

        virtual bool GetisSuckling() {
            return isSuckling;
        }

        virtual bool GetisFoetus() {
            return isFoetus;
        }

        virtual double GetgestationPeriod() {
        ...
        virtual double GetbodyProtein() const {
            return bodyProtein;
        }

        virtual double GetbodyFat() const {
            return bodyFat;
        }
        ...
        virtual int GetfeedingGroup() {
            return feedingGroup;
        }

        double GetlastMilkYield() {
            return lastMilkYield;
        }

        void Setlactating(bool aLactation) {
            lactating = aLactation;
        }

        void Setpregnant(bool aPregnant) {
            pregnant = aPregnant;
        }

        void SetlactationNumber(int aNumber) {
            lactationNumber = aNumber;
        }

        void SetCalf(cattle * aCalf) {
            calf = aCalf;
        }

        void SetdaysFromParturition(double days) {
            daysFromParturition = days;
        }

        void SetdaysFromConception(double days) {
            daysFromConception = days;
        }

        bool isLactating() const {
            return lactating;
        }
        ...
        virtual bool GetisYoung() {
            return isYoung;
        }

        int GetlactationNumber() {
        ...
        double GetdaysFromParturition() const {
            return daysFromParturition;
        }

        double GetdaysFromConception() const {
            return daysFromConception;
        }

        virtual double GetPotentialMilkYield(int daysFromParturition) {
            return 0.0;
        }

        virtual double GetPotentialMilkYield() {
            return 0.0;
        }

        double GetaminoAcidSupply() {
            return aminoAcidSupply;
        }

        double GetaminoAcidDemand() {
            return aminoAcidDemand;
        }
        ...
        double GetrumenNsurplus() {
            return rumenNsurplus;
        }
        ...
        double GetPotentialIntake() {
            return 0.0;
        }

        virtual cattle * CheckBirth() {
            return nullptr;
        }
        ...
        virtual void Initialise(int    aCode,
                                int    anAge,
                                double LU,
                                double numPrCohort,
                                bool   isAYoung);
        ...
        virtual double GetFoetusWeight(double offset) {
            return 0.0;
        }
        ...
        virtual void GrowFoetus(double newFoetusProtein,
                                double newFoetusFat,
                                double newFoetus);

        virtual feedItem * Getgrowth() {
            return growth;
        }
        ...
        virtual void GrowGravidUterus(double newProtein,
                                      double newFat) {}
        ...
        virtual double GetPalatability(feed_resource * aFeed_resource) {
            return 0.0;
        }
        ...
        virtual double GetPotentialAnnualMilkProduction(int durationOfLactation) {
            return 0.0;
        }

        virtual double GetEnergyRequirement() {
            return 0.0;
        }
         ...
         */
        virtual void GetStandardComposition(double EBW,
                double *                           fat,
                double *                           protein);
		virtual double GetIntLivestockUnits()
		{
			SetIntLivestockUnits(weight/500);
			return animal::GetIntLivestockUnits();
		}
};
#endif
