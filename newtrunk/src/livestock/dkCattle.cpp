 ...
 */
void dk_cattle::DoBirth(double * afterBirthProtein,
                        double * afterBirthFat) {
    double gravidUterusProtein = GetbodyProtein();    // foetus and uterus protein
    double foetusProtein       =
        CalcFoetusProtein(0.0)
        / 1000.0;    // days since conception has been updated - use yesterday's to avoid false growth
...
dk_cattle * dk_cattle::CheckBirth() {
    if ((pregnant) && (daysFromConception >= GetgestationPeriod())) {
        ...
        calf -> SetanimalSection(GetanimalSection());
...
dk_cattle::dk_cattle(string       aName,
                     const int    aIndex,
                     const base * aOwner):
    cattle(aName,
           aIndex,
...
*   Copy Constructor - is shallow
*/
dk_cattle::dk_cattle(const cattle & anAnimal) :
cattle(anAnimal)
{
	herdMilkYieldConstant = 0;    // bes�tningsydelsen kg EKM
	q0 =0;
	q1 = 0;
	lactationEnergyFudgeFactor =0;
	fieldList = new linkList<field>();
}
dk_cattle::~dk_cattle() {}
 ...
 */
void dk_cattle::ReadParameters(commonData *& data) {
    cattle::ReadParameters(data);
    ...
    data -> FindSection(namestr, code);    // find the correct place in the input file
    data -> FindItem("herdMilkYieldConstant", &herdMilkYieldConstant);
    data -> FindItem("lactationEnergyFudgeFactor", &lactationEnergyFudgeFactor);
    data -> FindItem("q0", &q0);
    data -> FindItem("q1", &q1);
}
...
void dk_cattle::Partition() {
    ...
    if (growth -> GetAmount()
            == 0.0)    // do not calculate growth if there has been growth or remobilisation in connection with lactation
    {
    ...
    urineN = totalNinput
             - (dungN + pregnancyN + growthN
                + milkN);    // NB if growth is negative, urine N increases to reflect loss of remobilised N
    ...
    if (weight < 0.0) {
        theMessage -> FatalError(GetAlias(), " dk_cattle: weight has fallen below zero");
    }
}
...
double dk_cattle::GetbulkCapacity() {
    ...
    if (lactationNumber == 0) {
        bulkCapacity = (4.74 * log(weight) - 18.75)
                       * (3.125 - 0.6056 * log(weight) + 0.042 * log(weight) * log(weight));
        ...
        if (lactationNumber == 1) {
            ...
            bulkCapacity = (7.08 + z) - (0.4 * (7.08 + z)) * exp(-0.047 * 7 * weeksFromParturition)
                           - 0.0033 * 7 * weeksFromParturition;
        }
    }
...
double dk_cattle::GetPotentialMilkYield(int daysIntoLactation)    // Ykpot
{
    ...
    double potentialMilkYield = a * GetMilkYieldCapacity() * pow(weeksFromParturition, b)
                                * exp(-c * weeksFromParturition);
 ...
 */
double dk_cattle::DoPregnancy(double * avail_FE,
                              double * availableN) {
    ...
    if (pregnant) {
        ...
        *avail_FE -= foetusFEdemand;
        ...
        if (daysFromConception == 1)                           // just got pregnant
        {
            ...
            newGravidUterusProtein = CalcGravidUterusProtein(0.0) - CalcGravidUterusProtein(-1.0);
            newGravidUterusFat     = CalcGravidUterusFat(0.0) - CalcGravidUterusFat(-1.0);
        }
...
void dk_cattle::MakePregnant(bool isAFemale) {
    if (pregnant) {
        ...
        foetus -> SetFoetus(nullptr);
        foetus -> SetCalf(nullptr);
 ...
 * Calculate lactation
 */
void dk_cattle::DoLactation(double * avail_FE,
                            double * availableN,
                            double * milkN) {
    *milkN     = 0.0;
    limitation = 0;
    ...
    milk = new feedItem(*(feedItem *) theProducts -> GetProductElement(308));

    // deleted later in this routine (if no calf), otherwise deleted by calf
    ...
    if (calf) {    // see if has a calf (only has calf if calf is suckling)
        ...
        if (calfPotentialIntake < energyLimitedMilk) {
            energyLimitedMilk = calfPotentialIntake;
        }
    }

    *milkN       = energyLimitedMilk * milkProteinConc / N_to_protein;
    actMilkYield = energyLimitedMilk;
    lactationN   = energyLimitedMilk * milkProteinConc / N_to_protein;

    // DK recommendations for protein for milk production
    ...
    if (((lactationNumber == 1) && (daysFromParturition > 60))
            || ((lactationNumber > 1) && (daysFromParturition > 50))) {

        // stops milk production increasing after peak potential production, if feed supply increases
    ...
    *availableN   -= lactationN;
    ...
    //
    {
        ...
        theOutput -> AddIndicator(3320, "33.20 N in milk", "kg N", Number * *milkN / 1000.0);

        // theOutput->AddIndicator(555,"05.55 Milk sold","Dkr",Number * milk->GetAmount() * 1000.0 * value);    //proce is in litrs
        // theOutput->AddIndicator(1940,"19.40 Milk sold","litres",Number * milk->GetAmount() * 1000.0);
#endif
...
void dk_cattle::intake()
{
    ...
    if (fieldList) {
		...
		{
            fieldOfftakeList[i] = 0.0;
        }
    }
    ;
#endif

    double          DM_intake_this_resource;
    bool            animalIsFull = false;
    feed_resource * aFeed_resource;

    // for (int counter=0;counter<=Hi;counter++)
    // taken out until BPE sorts out Comsort
    int counter = Hi;
    ...
    while ((counter >= 0) && (!animalIsFull)) 
	{
        ...
        if (!animalIsFull) {
            ...
            if (aFeed_resource -> GetIsSupplement()) 
			{
				...
				//cout << aFeed_resource->GetName() << " " << aFeed_resource->GetAmount() << " " << aFeed_resource->GetdryMatter() <<endl;
			...
			{                                             // handle a grazed feed resource
                if (hoursHoused == 24.0) 
				{
                    theMessage -> WarningWithDisplay(
                        "dk_cattle: intake - grazed feed in diet but animal housed all day");
                }
                ...
                if (pot_availability > 0.0) 
				{
                    ...
                    animalIsFull            = GetAmountNextFeed(
                        aFeed_resource);                          // on return, the amount is the intake of this feed
                    DM_intake_this_resource = aFeed_resource -> GetAmount() * aFeed_resource -> GetdryMatter() * 1000.0;
                    Grazed_DM               += DM_intake_this_resource;

                    aFeed_resource -> Setamount(DM_intake_this_resource
                                                / (aFeed_resource -> GetdryMatter()
                                                   * 1000.0));    // convert from kg DM to tonnes FW
                    ...
                    *aFeed_resource * GetNumber();    // adjust for animal number prior to output of demand
                    ...
                    ((patch *) aFeed_resource -> GetOwner()) -> SetGrazed(0.0, GetNumber() * DM_intake_this_resource,
                            NGrazed);
                    theOutput -> AddIndicator(3304, "33.04 N grazed by cattle", "kg N", NGrazed);
                    theOutput -> AddIndicator(4304, "43.04 C grazed by cattle", "kg C", CGrazed);
                    *aFeed_resource * (1.0 / GetNumber());    // adjust back
            ...
            if (aFeed_resource -> GetAmount() > 0.0)
			{
                *currentDiet = *currentDiet + *aFeed_resource;
                ME_intake    += DM_intake_this_resource * aFeed_resource -> GetME()
                                / (aFeed_resource -> GetdryMatter() * 1000.0);
                FE_intake += DM_intake_this_resource * aFeed_resource -> GetfeedUnitsPerItemUnit()
                             / (aFeed_resource -> GetdryMatter() * 1000.0);
    ...
    } else if (FE_intake < 0.00001)
	{
        cout << endl << theTime << endl;

        theMessage -> FatalError("dk_cattle: error in current diet - too little energy for ", GetAlias());
    }
    ...
    if (isYoung)
	{    // output BEFORE adding milk (is internal to livestock)
        theOutput -> AddIndicator(3301, "33.01 N in feed for young cattle", "kg N", totalNinput);
        theOutput -> AddIndicator(4301, "43.01 C in feed for young cattle", "kg C", totalCinput);
    } 
	else
	{
        theOutput -> AddIndicator(3302, "33.02 N in feed for adult cattle", "kg N", totalNinput);
        theOutput -> AddIndicator(4302, "43.02 C in feed for adult cattle", "kg C", totalCinput);
    }
#endif
    ...
    if (isSuckling)
	{
        ...
        // feedPlanList=nullptr;  //otherwise, if calf is culled, feedPlanList will be deleted and mother will have no diet
        // this needs to be updated - what happens to suckled milk?
    }

    double feedNPerc = 100.0 * totalNinput
                       / (DM_intake * Number);    // totalNinput is total consumed by animals, DM_intake is per animal
...
#ifdef _STANDALONE
void dk_cattle::Initialise(double anAge,
...
bool dk_cattle::GetAmountNextFeed(feed_resource * aFeed_resource) 
{
    ...
    if ((aFeed_resource -> GetCode() >= 307)
            && (aFeed_resource -> GetCode() <= 311)) 
	{    // dealing with a milk substitute here
	...
	else 
	{
        ...
        double newFeedfillPerKg = Getfill(aFeed_resource)
                                  / 1000.0;    // fill value returned is appropriate to animal age (ie young or old)
        ...
        if (currentAmount == 0) 
		{    // aFeed_resource is the first feed item
		...
		else 
		{
            ...
            // NB - all content variables (x per kg) are on a fresh weight basis
            double currentfillPerKg  = Getfill(currentDiet) / 1000.0;
            double currentfillPerSFU = GetfillperSFU(currentDiet);
            double currentSFUperKg   = currentDiet -> GetfeedUnitsPerItemUnit() / 1000.0;
			...
			else
			{
                double a            = 0.44;
                double b            = 1.6;
                double constantTerm = currentAmount
                                      * (k0 * (a * currentSFUperKg + b * currentfillPerKg)
                                         - currentAmount * currentfillPerKg * currentSFUperKg);
                double linearTerm = k0 * (a * newFeedSFUperKg + b * newFeedfillPerKg)
                                    - currentAmount
                                      * (currentfillPerKg * newFeedSFUperKg + newFeedfillPerKg * currentSFUperKg);
 ...
 */
double dk_cattle::CalcFatInEBWchange(double energy,
        double                              protein) {
 ...
 */
void dk_cattle::GrowGravidUterus(double newProtein,
                                 double newFat) {
    if (isFoetus) {
        theMessage -> FatalError("cattle:GrowFoetus - this animal is a foetus!");
    } else {
        double newGravidUterus = (GetGravidUterusWeight(0.0) / 1000.0)
                                 - foetus -> Getweight();    // is actually weight of gravid uterus
...
void dk_cattle::CalcHeatProduction(manure * urine) {
    ...
    if (milk) {
        double milkEnergy = milk -> GetAmount() * 1000.0 * 0.4 * 7.89;

        // 0.4 = SFU/kg milk, 7.89 = MJ/SFU
 ...
 */
void dk_cattle::ChangeBodyComposition(double weightChange,
        double *                             avail_FE,
        double *                             availableN) {
    if (weightChange != 0.0) {
...
double dk_cattle::GetPotentialMilkSolidsIntake(int days) {
    return GetPotentialMilkYield(days + 1) * 0.125;

    // estimates milk intake as maximum milk yield * assumed solids content of 12.5%
}
...
double dk_cattle::GetFoetusPorteinRequirement() {
    ...
    // new foetus here includes associated pregnancy tissue such as additional uterus
    double ret_val = CalcGravidUterusProtein(1) - CalcGravidUterusProtein(0);
...
double dk_cattle::GetPotentialMilkIntake() {
    return pow((GetSize()), 0.75) * (0.42 + 0.58 * exp(-0.036 * age));
}
