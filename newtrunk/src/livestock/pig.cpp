...
pig::pig(string       aName,
    ...
    animal(aName,
           ...
           aOwner)
{
    NumberPrDay = aveWeight = NumberPrYear = variableCosts = 0.0;
...
void pig::DailyUpdate() {
    ...
    for (int i = 0; i < feedItemList -> NumOfNodes(); i++) {
        ...
        feedItem * storedfeedItem = (feedItem *) theProducts -> GetCopyStoredFeedProduct(
                                        afeedItem);    // returns a copy of the product if it is found in storage
        ...
        if ((storedfeedItem) && (storedfeedItem -> GetAmount() > 0.0)) {
            ...
            *currentfeed = *currentfeed + *storedfeedItem;
            ...
            if (afeedItem -> GetproteinN_digestibility() < 0.0) {
                cout << "attempt to access feed with protein digestibility less than zero, code = "
                     << afeedItem -> GetCode() << endl;
    ...
    *manurePrDay = *manurePrDay + *solidManurePrDay;
    ...
    double CinCH4     = 0.01 * GetGrossEnergyinDryMatter() * currentfeed -> GetAmount() * currentfeed -> GetdryMatter()
                        / 55.55;
...
void pig::ReceivePlan(commonData * data,
                      string       fileName) {
    ...
    cout << "------------ Reading " << (string) this -> GetName() << " feeding plan ------------" << " from "
         << fileName << endl;
    ...
    while ((index <= (first + num)) && (!gotit)) {
        data -> FindSection(namestr, index);    // find the correct place in the input file

        string animalName = GetAlias();
        string thisAlias;
        ...
        {
            gotit   = true;
            namestr = "Group(" + conv.IntToString(Index) + ")." + "Feed";
        } else {
            index++;
        }
    }
 ...
 */
double pig::ProduceManure(manure * fluidManure,
                          manure * solidManure) {
    string animalName          = this -> GetName();
    double surplus_digestibleN = 0.0,
           Nconc               = 0.0;
    ...
    if (NumberPrDay > 1e-10) {
        ...
        if (energyInput < 0.75 * (NumberPrDay * FE_need))    // BMP changed this january 2007
        {
            if (energyInput < 0.50 * (NumberPrDay * FE_need)) {
                cout << endl;
                cout << "Energy as percentage of requirement = " << (100.0 * energyInput / (NumberPrDay * FE_need))
                     << endl;

#ifdef NITROSCAPE
                theMessage -> Warning("pig::ProduceManure - ", Alias, " Very low amount of energy in feed");
#else
                theMessage -> FatalError("pig::ProduceManure - ", Alias, " Very low amount of energy in feed");
#endif

            } else {
                theMessage -> Warning("pig::ProduceManure - ", Alias, " Insufficient energy in feed");
            }
        }
        ...
        if (energyInput > 1.6 * (NumberPrDay * FE_need)) {
            ...
            theMessage -> WarningWithDisplay(Alias, energyPercent, "% excess energy in feed ");
        }
        ...
        if (surplus_digestibleN >= 0) {
            ...
            solidManure -> SetP_content(amountfeed * (currentfeed -> GetP_content())
                                        * (1.0 - currentfeed -> GetP_digest()) / totalAmountPrDay);
            solidManure -> SetK_content((amountfeed * (currentfeed -> GetK_content()) - NumberPrYear * K_growth)
                                        / totalAmountPrDay);
            ...
            fluidManure -> SetP_content((amountfeed * (currentfeed -> GetP_content()) * (currentfeed -> GetP_digest())
                                         - NumberPrDay * P_growth) / totalAmountPrDay);
            fluidManure -> SetK_content(0.0);    // rettes senere
            fluidManure -> SetpH(8.0);

            double balance = totalNInput
            ...
            theMessage -> FatalError("pig::ProduceManure - ", animalName,
                                     " required additional protein in feed");    // BMP modified this !!!
        }
    ...
    } else {    // sets the default values for manure
