...
animal(anAnimal)
{
	acceptanceCoefficient = aminoAcidDemand = aminoAcidSupply = dungN = Grazed_DM = Grazed_FE = limitation = rumenNsurplus = 0.0;
	urineN = anAnimal.urineN;
	...
	feedPlanList = new cloneList<product>();
	if (anAnimal.feedPlanList != nullptr)
	{
		for (int i = 0; i < anAnimal.feedPlanList->NumOfNodes(); i++)
		{
			product * newField = new feed_resource(*(feedItem *)anAnimal.feedPlanList->ElementAtNumber(i));
			feedPlanList->InsertLast(newField);
		}
	}
	else
	...
	if (anAnimal.feedUseList != nullptr)
	{
		for (int i = 0; i < anAnimal.feedUseList->NumOfNodes(); i++)
		{
			product * newField = new feed_resource(*(feedItem *)anAnimal.feedUseList->ElementAtNumber(i));
			feedUseList->InsertLast(newField);
		}
	}
	else
		feedUseList = nullptr;
	...
	/*fieldList = new linkList<field>();
	if (anAnimal.fieldList != nullptr)
	{

		for (int i = 0; i < anAnimal.fieldList->NumOfNodes(); i++)
		{
			field * newField = new field(*anAnimal.fieldList->ElementAtNumber(i));
			fieldList->InsertLast(newField);
		}
	}
	else*/
		fieldList = nullptr;
...
void cattle::Initialise() {
    ...
    isSuckling          = false;
    isYoung             = false;
    feedPlanList        = nullptr;    // must be set explicitly through
    feedUseList         = new cloneList<product>;
...
#ifndef _STANDALONE
    if (globalSettings -> DetailsData.getCowDetails()) {
...
void cattle::DailyUpdate() {
    ...
    if (growth) {
        ...
        // should really only be used for standalone (but is used generally until we can manage milking parlours)
        delete growth;
    ...
    double manureN = fluidManurePrDay -> GetAmount() * fluidManurePrDay -> GetAllN().n
                     + solidManurePrDay -> GetAmount() * solidManurePrDay -> GetAllN().n;
    double manureC = fluidManurePrDay -> GetAmount() * fluidManurePrDay -> GetC_content()
                     + solidManurePrDay -> GetAmount() * solidManurePrDay -> GetC_content();
    ...
    *solidManurePrDay * Number;
    ...
    if (isYoung) {
        theOutput -> AddIndicator(3341, "33.41 N in young cattle manure", "kg N", manureN * 1000.0);
        theOutput -> AddIndicator(4341, "43.41 4 in young cattle manure", "kg C", manureC * 1000.0);

        // theOutput->AddIndicator(1963,"19.63 Manure produced by young cattle","t",amountManure);
    } else {
        theOutput -> AddIndicator(3342, "33.42 N in dairy cattle manure", "kg N", manureN * 1000.0);
        theOutput -> AddIndicator(4342, "43.42 C in dairy cattle manure", "kg C", manureC * 1000.0);

        // theOutput->AddIndicator(1964,"19.64 Manure produced by dairy cattle","t",amountManure);
    }

    // excrete manure to the right location
    if (hoursHoused > 0.0) {
        animalSection * theSection = GetanimalSection();

        if (theSection) {
            manure * faecesToHouse = new manure(*solidManurePrDay);
            manure * urineToHouse  = new manure(*fluidManurePrDay);
			theSection->AddIntLivestockUnits(GetIntLivestockUnits());
            ...
            theOutput -> AddIndicator(3381, "33.81 Cattle manure N to animal housing", "kg N",
                                      manureNToHousing * 1000.0);
            theOutput -> AddIndicator(4381, "43.81 Cattle manure C to animal housing", "kg C",
                                      manureCToHousing * 1000.0);
            ...
            theMessage -> FatalError("cattle: no housing designated for receiving manure from ", GetAlias());
        }
    }
    ...
    if (hoursHoused < 24.0) {
        if (fieldList -> NumOfNodes() > 0) {
            double DMFromFields =
                0.0;    // used to partition manureNToField across the grazed fields in proportion to DM offtake
            ...
            {
                theMessage -> Warning(
                    "Cattle: No DM offtake from fields - dung and urine evenly distributed between grazed fields");
            ...
            for (int i = 0; i < fieldList -> NumOfNodes(); i++) {
                ...
                if (fieldOfftakeList[i] > 0.0) {
                    ...
                    faecesToField -> Setamount(solidManurePrDay -> GetAmount() * timeDistribution
                                               * proportionThisField);
                    ...
                    theOutput -> AddIndicator(3380, "33.80 Cattle manure N to pasture", "kg N",
                                              manureNToPasture * 1000.0);
                     ...
                     * ---------------------------------------------------------------------------------------
                     */
                    aField -> DepositManure(urineToField, faecesToField, area_urine,
                                            area_dung);                              // from McGehan and Topp 2004
                }
            ...
            theMessage -> FatalError("cattle: no fields grazed so nowhere for manure to go!!");
        }
    }
#endif
...
double cattle::Production() {
    ...
    if (urineN >= 0) {
        ...
        double urineCconc = urineNConc * 60.0
                            / 28.0;    // assumes all N in urine is urea (ratio of C to N in urea (2*(NH2) + CO))
        ...
        fluidManurePrDay -> SetP_content((amountfeed * (currentfeed -> GetP_content()) * (currentfeed -> GetP_digest())
                                          - P_growth) / totalAmountPrDay);
        ...
        solidManurePrDay -> SetNH4_content(0.0);
        solidManurePrDay -> SetNO3_content(0.0);
        solidManurePrDay -> SetDryMatter(0.15);
        solidManurePrDay -> SetC_content(faecalCconc);
        solidManurePrDay -> SetP_content(amountfeed * (currentfeed -> GetP_content())
                                         * (1.0 - currentfeed -> GetP_digest()) / totalAmountPrDay);
...
#ifndef _STANDALONE
        theOutput -> AddIndicator(4101, "41.01 C input into cattle", "kg C", Number * carbonIntake);
        theOutput -> AddIndicator(4104, "41.04 C in cattle faeces", "kg C", Number * faecalCarbon);
        theOutput -> AddIndicator(4103, "41.03 C in cattle urine", "kg C", Number * urineCarbon);
        theOutput -> AddIndicator(4107, "41.07 C retained in cattle body", "kg C", Number * CRetained);
        theOutput -> AddIndicator(4105, "41.05 C in cattle CO2 production", "kg C", Number * CO2_C);
        theOutput -> AddIndicator(4102, "41.02 C in cattle CH4 production", "kg C", Number * methaneCarbon);
        theOutput -> AddIndicator(4106, "41.06 C in milk production", "kg C", Number * milkCarbon);
#endif
        ...
        cout << urineN << " is the urine" << endl;
        cout << urineN << " is the urine " << theTime << endl;
...
void cattle::ProteinDigestion(double * availableN) {
    ...
    *availableN   = 0.0;
    ...
    dungN = 0.04 * N_intake + (DM_intake * DM_intake * 1.8 / 6.25)
            + DM_intake * 20.0
              / 6.25;    // note that DM_intake is updated to take account of milk intake in the intake function
...
feed_resource * cattle::GetFeedItem(product * menuPtr) {
    product * aStoredProduct =
        theProducts -> GetCopyStoredFeedProduct(menuPtr);    // returns a copy of the product if it is found in storage
 ...
 * Initialise cattle
 */
void cattle::Initialise(int    aCode,
                        int    anAge,
                        double LU,
                        double numPrCohort,
                        bool   isAYoung) {
 ...
 */
void cattle::DoSelection(linkList<feed_resource> * alist,
                         int *                     count) {
    int               counter;
    ...
    for (int i = 0; i < feedPlanList -> NumOfNodes(); i++) {
		...
		//cout << menuPtr->GetName() << " " << menuPtr->GetAmount() << " " << endl;
        aFeed_resource = GetFeedItem(
            menuPtr);    // returns copy of the feed resource. Gets stored feed if available, otherwise table values used
    ...
    if (fieldList) {
        if (hoursHoused < 24.0) {
            ...
            if (totGrazableFields == 0) {
                hoursHoused = 24.0;    // otherwise animal will try and send manure to non-existent fields - should not occur!!
            }

            // calculated grazedArea
            ...
            for (int i = 0; i < totGrazableFields; i++) {
                ...
                for (int j = 0; j < numOfPatches; j++) {
                    ...
                    // update my_resource
                    aPatch -> GetFeedResource(
                        0.0, 0.0,
                        1);    // parameters - stubble height as proportion of patch height, minimum stubble ht (m) and animal type (cattle = 1)

                    aFeed_resource = new feed_resource(*(feed_resource *) aPatch -> Getmy_resource());

                    double maxAvailThisSource = aFeed_resource -> GetAmount() * aFeed_resource -> Getarea_occupied()
                                                * beta / (grazedArea);
    ...
    if (Hi == 0) {
        theMessage -> FatalError("cattle: no feed available!!");
    }
...
void cattle::WriteCowOutput(bool header) {
    if (header) {
        *detailsFile << "date" << "\t" << "age" << "\t" << "emptyBodyWeight" << "\t" << "DM_intake" << "\t"
                     << "ME_intake" << "\t" << "FE_intake" << "\t" << "feedN" << "\t" << "AAT" << "\t" << "limit"
                     << "\t" << "foetusWt" << "\t" << "milkYield" << "\t" << "dungN" << "\t" << "urineN" << "\t"
                     << "milkN" << "\t" << "growthN" << "\t" << "feedGrp" << "\t" << "housed" << "\t" << "code1"
                     << "\t" << "DM1" << "\t" << "FE1" << "\t" << "code2" << "\t" << "DM2" << "\t" << "FE2" << "\t"
                     << "code3" << "\t" << "DM3" << "\t" << "FE3" << "\t" << "code4" << "\t" << "DM4" << "\t" << "FE4"
                     << "\t" << "code5" << "\t" << "DM5" << "\t" << "FE5" << "\t" << "code6" << "\t" << "DM6" << "\t"
                     << "FE6" << "\t" << "code7" << "\t" << "DM7" << "\t" << "FE7" << "\t" << "code8" << "\t" << "DM8"
                     << "\t" << "FE8" << endl;
    } else {
        ...
        *detailsFile << age << "\t" << weight << "\t" << DM_intake << "\t" << ME_intake << "\t" << FE_intake << "\t"
                     << feedN << "\t" << 1000.0 * currentDiet -> GetAmount() * currentDiet -> Getaat() << "\t"
                     << limitation << "\t" << foetusWeight << "\t" << milkYield << "\t" << dungN << "\t" << urineN
                     << "\t" << milkN << "\t" << growthN << "\t" << feedingGroup << "\t" << hoursHoused << "\t";
        ...
        for (int counter = 0; counter < feedUseList -> NumOfNodes(); counter++) {
            ...
            if (aFeedResource -> GetCode() < 1000) {
                *detailsFile << aFeedResource -> GetCode() << "\t"
                             << aFeedResource -> GetAmount() * aFeedResource -> GetdryMatter() * 1000.0 << "\t"
...
void cattle::CreateDetailFile(int Num) {
    ...
    stub            = name;
    detailsFileName = globalSettings -> getOutputDirectory();
    detailsFileName += stub;
    filename        = conv.IntToString(Num) + ".xls";
    detailsFileName += filename;

    detailsFile -> open(detailsFileName.c_str(), ios::out);
 ...
 */
void cattle::GrowFoetus(double newFoetusProtein,
                        double newFoetusFat,
                        double newFoetus)    // these should be in kg
{
    if (!isFoetus) {
...
double cattle::GetMethaneProduction() {
    ...
    methane = a + amountFeed
              * (79 * currentDiet -> Getcell() + 10 * currentDiet -> CalcNFE()
                 +    // from husdyrbrug rapport 54, vol 2, p336
...
double cattle::GetUrineVolume() {
    ...
    double urineVol        =
        1.3441 * DM_intake * (1.079 * sodiumPercent + 0.5380 * potassiumPercent + 0.1266 * nitrogenPercent)
...
double cattle::GetSensibleHeatProduction() {
    ...
    if (!isFemale) {    // male animals
        ret_val = 7.64 * pow(weight, 0.69)
                  + (13.3 * growthRate * ((23.0 / (currentDiet -> GetME() / 1000.0)) - 1)) * (57.27 + 0.302 * weight)
                    / (1 - 0.171 * growthRate);
    }
    ...
    if ((pregnant) && (lactationNumber == 0)) {    // pregnant heifers
        ret_val = 7.64 * pow(weight, 0.69)
                  + (13.3 * growthRate * ((23.0 / (currentDiet -> GetME() / 1000.0)) - 1)) * (57.27 + 0.302 * weight)
                    / (1 - 0.171 * growthRate) + 1.6 * 0.00001 * pow(daysFromConception, 3);
    }
    ...
    double temperature =
        theClimate ->
             GetMeanTemperature();    // assumes animals experience outside temperature - could be wrong in extreme climates
 ...
 */
void cattle::GetStandardComposition(double EBW,
        double *                           fat,
        double *                           protein) {
