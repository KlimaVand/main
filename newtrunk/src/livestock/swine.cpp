...
swine::swine():
    pig()
{
    pigletsPerLitter = FEperPiglet = maxParity = 0.0;
}

swine::swine(string       aName,
             const int    aIndex,
             const base * aOwner):
    pig(aName,
        aIndex,
 ...
 */
void swine::ReceivePlan(commonData * data,
                        string       FileName) {
 ...
 */
double swine::GetTotalHeatProduction(double weight) {
    double ret_val    = 0.0;
    int    animalType = 0;
 ...
 */
double swine::CalcMaintenanceEnergy(double weight) {
    return 4.85 * pow(weight, 0.75);
}
