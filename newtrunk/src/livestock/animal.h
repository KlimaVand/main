    ...
    public base
{
    protected:
        ...
        double          IntLivestockUnits;    // livestock units (1 LU = 500kg liveweight)
        double          variableCosts;

#ifndef _STANDALONE
        animalSection * theanimalSection;    // pointer to the section of the animal house where the animal is located when housed
#endif
...
#ifndef _STANDALONE
        animal() {
            amountFluid      = amountSolid = K_growth = 0;
            manurePrDay      = 0;
            N_growth         = C_growth = P_growth = 0;
            FE_need          = 0;
            heatProduction   = 0;
            Number           = 0;
            vetExpensesPrDay = fluidManurePrDay = solidManurePrDay = 0;
            variableCosts    = 0;
            LivestockUnits   = 0;
            IntLivestockUnits   = 0;
            theanimalSection = 0;
    ...
    public:
...
#ifndef _STANDALONE
        virtual void SetanimalSection(animalSection * aanimalSection) {
            theanimalSection = aanimalSection;
        }

        virtual animalSection * GetanimalSection() {
        ...
        virtual void SetN_growth(double aN_growth) {
            N_growth = aN_growth;
        }

        virtual double GetN_growth() {
            return N_growth;
        }

        virtual void SetC_growth(double aC_growth) {
            C_growth = aC_growth;
        }

        virtual double GetC_growth() {
            return C_growth;
        }

        virtual void SetthisProduct(animalProduct * ananimalProduct) {
            thisProduct = ananimalProduct;
        }

        virtual animalProduct * GetthisProduct() {
            return thisProduct;
        }

        virtual void SetNumber(double aNumber) {
            Number = aNumber;
        }
        ...
        virtual void SetAlias(string anAlias) {
            Alias = anAlias;
        }

        virtual void SetDE(double aVal) {
            LivestockUnits = aVal;
        }

        virtual double GetDE() {
            return Number * LivestockUnits;
        }

        virtual void SetIntLivestockUnits(double aVal) {
            IntLivestockUnits = aVal;
        }

        virtual double GetIntLivestockUnits() {
            return Number * IntLivestockUnits;
        }
        ...
        virtual feedItem * Getcurrentfeed() {
            return currentfeed;
        }

        virtual double GetamountSolid() {
            return amountSolid;
        }

        virtual double GetamountFluid() {
            return amountFluid;
        }

        virtual double GetCarbon(double fat,
                                 double protein);
};
#endif
