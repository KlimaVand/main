...
swineStock::swineStock(string       aName,
                       const int    aIndex,
                       const base * aOwner):
    animalStock(aName,
                aIndex,
                aOwner)
{
    ...
    newSows -> ReadParameters(data);
	if((animalProduct *) theProducts -> GetProductElement("NEWSOWS")==nullptr)
		cout<<"fdf"<<endl;
 ...
 *
 */
void swineStock::ReceivePlan(string fileExtension)
...
// Calculates number of animals to be imported + herd structure
{
    ...
    string     fileName = "Pigs." + fileExtension;
    commonData data;
     ...
     */
    data.FindSection(GetName(), GetIndex());
    data.FindItem("SOWS", &numSows);    // number of sows present per day
    data.FindItem("NEWSOWS", &numNewSows);    // production of new sows for breeding per year
    data.FindItem("BACONERS", &numBaconers);    // production of baconers per year
    data.FindItem("PIGLETS", &numPiglets);    // production of piglets per year

    double daysInYear = (double) theTime.daysInYear(1);

    // Get production cycle details from building
	double daysPregnant=-1;
	 double daysMating=-1;
	 double dayslactating=-1;
	if(numSows!=0)
	{
     daysPregnant  = ((pigstysection *) pregnantSwines -> GetanimalSection()) -> GetDaysInSection();
     daysMating    = ((pigstysection *) matingSwines -> GetanimalSection()) -> GetDaysInSection();
     dayslactating = ((pigstysection *) lactatingSwines -> GetanimalSection()) -> GetDaysInSection();
	
    SowProductionCycle = daysPregnant + daysMating + dayslactating;
	}
	double pigletProductionCycle =-1;
	double baconersProductionCycle =-1;
	double  newsowsProductionCycle=-1;
	if(piglets -> GetanimalSection()!=nullptr)
    pigletProductionCycle   = ((pigstysection *) piglets -> GetanimalSection()) -> GetDaysInSection();
	if(baconers -> GetanimalSection()!=nullptr)
    baconersProductionCycle = ((pigstysection *) baconers -> GetanimalSection()) -> GetDaysInSection();
	if(newSows -> GetanimalSection()!=nullptr)
    newsowsProductionCycle  = ((pigstysection *) newSows -> GetanimalSection()) -> GetDaysInSection();
    ...
    *newsowsInOutPrDay = *newSows -> GetthisProduct();
    ...
    animalsChange = numBaconers
            - numPiglets;    // note - calculate number of PIGLETS that must be bought or sold to get correct number of BACONERS
    ...
    newsowsInOutPrDay -> Setamount(-numNewSows * newsowsInOutPrDay -> GetEndWeight()
                                   / (daysInYear * 1000.0));    // only export newsows
    ...
    *castsowsPrDay = *matingSwines -> GetthisProduct();    // *theProducts->GetProductElement("SOWS");
    ...
    for (int i = 0; i < pointyPigs -> NumOfNodes(); i++) {
        ...
        if (aPig -> GetNumberPrDay() > 0.0) {
            ...
            heatOp = aPig -> GetNumberPrDay() * aPig -> GetTotalHeatProduction(aPig -> GetaveWeight());

            // double aheatOp=aPig->GetNumberPrDay()* aPig->CalcHeatProduction(aPig->GetaveWeight());

            /*
             *                   cout << aheatOp << endl;
             * aheatOp = aPig->GetNumberPrDay()* aPig->GetTotalHeatProduction(aPig->GetaveWeight());
             * cout << aheatOp << endl;
             */
            aPig -> GetanimalSection() -> SetAnimalParameters(aPig -> GetNumberPrDay(), faecesPerYear, urinePerYear,
                    heatOp);
        }
    }
}
 ...
 *
 */
void swineStock::ReceiveFeed(string fileExtension)
...
// Load feed and calculate growth N requirements
{
    ...
    if (!data -> readFile(fileName)) {
        ...
        fileName = "pigsfeed.fnn";

        data -> readFile("pigsfeed.fnn");
    }

    double startNContent, startWeight, endNContent, endWeight, NGrowth, startCContent, endCContent, CGrowth;

    if (numSows > 0) {
        pregnantSwines -> ReceivePlan(data, fileName);
        lactatingSwines -> ReceivePlan(data, fileName);
        matingSwines -> ReceivePlan(data, fileName);
        ...
        double NContent = ((animalProduct *) theProducts -> GetProductElement(
                              "SUCKLINGPIG")) -> GetN_content();    // suckler pig final N content
        double Weight = ((animalProduct *) theProducts -> GetProductElement(
                            "SUCKLINGPIG")) -> GetEndWeight();    // suckler pig final weight
        double totalSucklerPigletN = NContent * Weight
                                     * pregnantSwines -> GetpigletsPerLitter();    // total litter N per sow per pregnancy
        double sucklerDays = (((pigstysection *) lactatingSwines -> GetanimalSection()) -> GetDaysInSection()
                              +    // days taken for suckler to reach end of suckling
            ((pigstysection *) pregnantSwines -> GetanimalSection()) -> GetDaysInSection());

        NGrowth += totalSucklerPigletN
                   / (sucklerDays * 1000);    // add foetus and suckler growth demand to sow growth demand
        endCContent = ((animalProduct *) theProducts -> GetProductElement("SUCKLINGPIG")) -> GetC_content();

        double totalSucklerPigletC = endCContent * Weight
                                     * pregnantSwines -> GetpigletsPerLitter();    // total litter N per sow per pregnancy
    ...
    // new sows
    if (numNewSows > 0) {
        newSows -> ReceivePlan(data, fileName);

        // gets starting values from previous (younger) animal category
        double newsowsProductionCycle = ((pigstysection *) newSows -> GetanimalSection()) -> GetDaysInSection();
    ...
    // piglets
    if (numPiglets > 0) {
        piglets -> ReceivePlan(data, fileName);

        // gets starting values from previous (younger) animal category - here no instance exists so get from Products.dat
        double pigletProductionCycle = ((pigstysection *) piglets -> GetanimalSection()) -> GetDaysInSection();
    ...
    // baconers
    if (numBaconers > 0) {
        baconers -> ReceivePlan(data, fileName);

        double baconersProductionCycle = ((pigstysection *) baconers -> GetanimalSection()) -> GetDaysInSection();
     ...
     * theOutput->AddIndicator(306,"03.06 Value of livestock","Dkr",livestockPrice);
     * }
     */
    delete data;
}
...
void swineStock::DailyUpdate(void) {
     ...
     * theOutput->AddStateIndicator(1130,"11.30 Sows present","no",NumberPrDay);
     * NumberPrDay = baconers->GetNumberPrDay();
     * theOutput->AddStateIndicator(1131,"11.31 Baconers present","no",NumberPrDay);
     * NumberPrDay = piglets->GetNumberPrDay();
     * theOutput->AddStateIndicator(1132,"11.32 Piglets present","no",NumberPrDay);
     * NumberPrDay = newSows->GetNumberPrDay();
     * theOutput->AddStateIndicator(1133,"11.33 New sows present","no",NumberPrDay);
     */
    for (int i = 0; i < pointyPigs -> NumOfNodes(); i++) {
    ...
    ExportStock();
    theOutput -> AddStateIndicator(3460, "34.60 N bound in pigs", "kg N", 0.0);
}
 ...
 *
 */
void swineStock::InitSwines(commonData *& file,
	...
	if(numSows>0)
	{
    outstring = "Mating(" + conv.IntToString(Index)+")";

    matingSwines -> SetanimalSection(theBuildings -> PlaceAnimal(outstring));

    outstring = "Pregnant(" + conv.IntToString(Index)+")";

    pregnantSwines -> SetanimalSection(theBuildings -> PlaceAnimal(outstring));

    outstring = "Lactating(" + conv.IntToString(Index)+")";

    lactatingSwines -> SetanimalSection(theBuildings -> PlaceAnimal(outstring));

    outstring = "Weaner(" + conv.IntToString(Index)+")";

    piglets -> SetanimalSection(theBuildings -> PlaceAnimal(outstring));
	}
	if(numBaconers>0)
	{
    outstring = "Finisher(" + conv.IntToString(Index)+")";

    baconers -> SetanimalSection(theBuildings -> PlaceAnimal(outstring));
	}

	if(numNewSows>0)
	{
    outstring = "NewSow(" + conv.IntToString(Index)+")";

    newSows -> SetanimalSection(theBuildings -> PlaceAnimal(outstring));
	}
}

void swineStock::InformBuildings() {
    for (int i = 0; i < pointyPigs -> NumOfNodes(); i++) {

        // go through the animals in turn
        pig *           tempPig         = pointyPigs -> ElementAtNumber(i);
        animalSection * ananimalSection = tempPig -> GetanimalSection();
		if(ananimalSection!=nullptr)
		{
        // set urine production per pig per year
        ananimalSection -> SeturinePerYear(tempPig -> GetamountFluid());

        // set faecal production per pig per year
        ananimalSection -> SetfaecesPerYear(tempPig -> GetamountSolid());
		}
        // The average weight of the pig is set in ReceivePlans, which has not yet been called. Use information from Products instead

        /*
         *               string name;
         * name.assign(tempPig->GetAlias());//copy name
         * if ((name == "SOWS_MATING")||(name == "SOWS_PREGNANT")||(name == "SOWS_LACTATING"))
         * name ="SOWS";//the product name is only SOWS
         * double startWeight = ((animalProduct*) theProducts->GetProductElement(name))->GetStartWeight();
         * double endWeight = ((animalProduct*) theProducts->GetProductElement(name))->GetEndWeight();
         * double aveWeight = (endWeight+startWeight)/2;
         * ananimalSection->SetanimalHeatOp(tempPig->GetSensibleHeatProduction(aveWeight));
         */
    }
}
...
void swineStock::CheckNbalance() {
    ...
    double          totalImportN = aProduct -> GetAmount() * aProduct -> GetN_content() * theTime.daysInYear(1)
                                   * 1000.0;    // Nbudget.GetInput()*1000.0;
    ...
    for (int i = 0; i < pointyPigs -> NumOfNodes(); i++) {
        ...
        if (tempPig -> GetNumberPrDay() > 0.0) {
            ...
            manureN = theTime.daysInYear(1)
                      * (fluidManure -> GetAmount() * fluidManure -> GetAllN().n
                         + solidManure -> GetAmount() * solidManure -> GetAllN().n) * 1000.0;
            balance = feedN - (growthN + manureN);

            if (fabs(balance) > 1.0) {
                theMessage -> FatalError("Swinestock: ", tempPig -> GetName(),
                                         " error - N in feed > (growth N + manure N)");
            }
        }
    }
