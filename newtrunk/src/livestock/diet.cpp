 ...
 */
void diet::ReadParameters(commonData * infile,
                          bool         isAutoFeed) {
    ...
    double grazing    = 0.0;
    bool   gotGrazing = infile -> FindItem("Grazing", &grazing);    // get amount DM grazed in kg
    ...
    if ((grazing > 0.0) || (grazingSFU > 0.0) || ((propGrazed > 0.0) & (!zeroGrazing))) {
        ...
        if (feedCode > 0) {
            aFeedItem = (feedItem *) theProducts -> GetProductElement(
                feedCode);    // get default product info using feed code
        } else {
            aFeedItem = (feedItem *) theProducts -> GetProductElement(
                theItemName);    // get default product info using name
        }
    ...
    string  namestr  = GetName() + Indexstr + "." + "Normal.Supplement";
    int     first, num;
	...
	{
        infile -> FindSection(namestr, inx);                         // find the correct place in the input file
 ...
 */
feed_resource * diet::LoadFeedItem(commonData * infile,
                                   bool         autoFeed) {
    ...
    if ((amount > 0.0) || (autoFeed)) {
        ...
        theMessage -> FatalError("cattle diet: feed amount cannot be zero");
    }
 ...
 */
double diet::GetSilageRequired(double numberOfAnimals,
                               int    startCode,
                               int    endCode) {
    double amount = 0.0;
