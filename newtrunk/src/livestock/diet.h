    ...
    public base
{
    ...
    public:
        ...
        diet() {
            startDate              = nullptr;
            normalfeedItemList     = nullptr;
            deficiencyfeedItemList = nullptr;
            surplusfeedItemList    = nullptr;
            grazedItem             = nullptr;
            housed                 = propRoughage = propGrazed = 0.0;
            autoFeed               = false;
            zeroGrazing            = false;
        }
        ...
        cloneList<product> * GetnormalfeedItemList() {
            return normalfeedItemList;
        }

        cloneList<product> * GetdeficiencyfeedItemList() {
            return deficiencyfeedItemList;
        }

        cloneList<product> * GetsurplusfeedItemList() {
            return surplusfeedItemList;
        }
        ...
        void ReadParameters(commonData * infile,
                            bool         isAutoFeed);

        bool IsGrazing();

        feed_resource * LoadFeedItem(commonData * infile,
                                     bool         autoFeed);

        double GetGrazingFeedRequirement(bool zeroGrazed);

        void ShowDiet(fstream * fs,
                      bool      header);
        ...
        double GetSilageRequired(double numberOfAnimals,
                                 int    startCode,
                                 int    endCode);
        ...
        double GetpropRoughage() {
            return propRoughage;
        }

        // proportion of energy demand that should be satisfied by roughage
        double GetpropGrazed() {
            return propGrazed;
        }

        // proportion of roughage from grazing
        void SetpropGrazed(double aVal) {
            propGrazed = aVal;
        }
