    ...
    public animalStock
{
    ...
    private:
        dairyStock & operator = (
                const dairyStock & a);    // Dissable the compilers generation of default assignment operator.

        // Constructor
        dairyStock() {
            maxNumPerCohort = IDnumber = 0;
    ...
    public:
        ...
        void InitCattle(commonData *& file,
                        int           anAnimalModel);

        linkList<cattleGroup> * GetCattleGroup() const {
            return grouplist;
        }
