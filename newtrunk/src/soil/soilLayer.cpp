...
soilLayer::soilLayer(string       aName,
                     const int    aIndex,
                     const base * aOwner):
    base(aName,
         aIndex,
 ...
 * @param data
 * @param soilData
 * @param prevoisFileName
 */
void soilLayer::Initialize(double        startDep,
                           double        thick,
                           commonData *& data,
                           commonData *& soilData,
						   string *&     prevoisFileName, double newholdbackconstant) {
    startDepth = startDep;
    thickness  = thick;
	holdbackconstant = newholdbackconstant;
    ...
    while (!foundLayer && (i < (first + n))) {
        ...
        if (foundLayer) {
            ...
            if (!data -> FindItem("ClayContent", &clayContent)) {
                double JBC[] = {
                    0.025, 0.025, 0.075, 0.075, 0.125, 0.125, 0.20, 0.35, 0.50, 0.10
                };
                int    JB    = 5;
            ...
            if (!data -> FindItem("DrainageConstant", &drainageConstant)) {
                drainageConstant = 1.0271 - 3.02 * clayContent * clayContent;

                // Function from Addiscot & Whitmore 1991, Soil Use. Man., 94-102
                drainageConstant = min(1.0, max(0.1, drainageConstant));

                // Cut at [0.1;1.0]
                drainageConstant = min(1.0, drainageConstant * 15.462 * pow(thickness, -0.7));

                // Fitted to Fig. 2c, Addiscot & Whitmore 1991
            }
            ...
            pF -> ReadParameters(data, i, dryBulkDensity, clayContent, siltContent, carbon, TopSoil);
            ...
            if (fieldCapacity > MaxContent) {
                porosity = pF -> GetRelativeWater(0.0);
            }

            // Porosity set to the relative water content at pF zero
...
void soilLayer::SetIceContent(double aIceContent) {
    if (aIceContent > waterContent) {
        theMessage -> WarningWithDisplay(
            "soilLayer::SetIceContent - requsted ice content exceeds total amount of water");
    }
...
double soilLayer::GetPoolNitrogenN(int    Name,
                                   double startDepth,
                                   double thickness) {
    return OrganicMatter -> GetPoolNitrogenN(Name) * IntervalFraction(startDepth, thickness);
}

double soilLayer::GetPoolNitrogenN15(int Name,
        double                           startDepth,
        double                           thickness) {
 ...
 * addNitrate    - Nitrate to be added [g N/m2]
 * addAmmonium   - Ammonium to be added [g N/m2]
 */
void soilLayer::AddNutrient(nitrogen addNitrate,
...
double soilLayer::SpecWaterCapacity() {
    ...
    } else if (pF -> GetPressurePotential(relativeWater + 0.01) == pF1) {
...
double soilLayer::HeatConductivity() {
    ...
    double waterConcentration = max(1.0, waterContent) * 100
                                / thickness;    // converted mm to vol %. Minimum of 1% according to COUP figure p 31
    ...
    double k_hmi = b1 * pow(10.0, b2 * BulkDensity)
                   + b3 * waterConcentration / BulkDensity * pow(10.0, b4 * BulkDensity);
...
double soilLayer::MaxTranspiration(double rootRadius,
    ...
    if (rootLength <= 0) {
        ...
        double UptakeRate = -4.0 * M_PI
                            * (pF -> GetFluxPotential(pF -> GetpF(relWater)) - pF -> GetFluxPotential(rootpF))
...
void soilLayer::EquilibrateNitrogen(double mobileWater) {
	...
	double rateCoefficient = holdbackconstant;
    if (mobileWater > 0.0) {
        ...
        double concDiffAmmonium       = ammonium_mob.n / mobileWater
                                        - (ammonium_imob.n + ammonium_mob.n) / waterContent;
        ...
        Chloride_imob = Chloride_mob
                + Chloride_imob;    // if no mobile phase is present the mobile solutes are moved to the immobile phase
        Chloride_mob  = 0.0;
 ...
 *
 */
double soilLayer::MaxNitrogenFlux(bool            nitrate,
                                  rootStructure * root) {
    ...
    if (nitrate) {
        ...
        if (waterContent > 0) {
            concentration = max(0.0,
                                1000. * N_amount / waterContent * FracAmmoniumInWater() - root -> MinimumSoilAmmonium);
        }
    ...
    double rootLength    = root -> rootLengthList[Index];
    double maxFlux       = MaxNitrogenFlux(root -> rootRadius, uptake, rootLength, concentration, diffusionCoef,
                               bufferingCoef);
...
nitrogen soilLayer::SubtractNitrateUptake(double & nitrateUptake) {
    ...
    if ((nitrateUptake > 0) && (nitrate.n > 0)) {
        ...
        if ((nitrate_mob.n < -1e-6) || (nitrate_mob.n15 < -1e-6) || (nitrate_imob.n < -1e-6)
                || (nitrate_imob.n15 < -1e-6)) {
...
double soilLayer::UpdateOrganicMatter() {
    ...
    if ((ammonium_mob.n < -1E-6) || (ammonium_imob.n < -1E-6) || (nitrate_mob.n < -1E-6) || (nitrate_imob.n < -1E-6)
            || (OrganicMatter -> GetNitrogen().n15 < -1e-10)) {
...
nitrogen soilLayer::UpdateNitrification() {
    if ((ammonium_imob.n < -1e-10) || (ammonium_mob.n < -1e-10) || (ammonium_imob.n15 < -1e-10)
            || (ammonium_mob.n15 < -1e-10)) {
    ...
    if (ammonium_imob.n + ammonium_mob.n > 0.0) {
        N15RatioAmmonium = max(0.0,
                               min(1.0, (ammonium_imob.n15 + ammonium_mob.n15) / (ammonium_imob.n + ammonium_mob.n)));
    }

    // Old DAISY nitrification model
    double FirstOrderRate = nitrificationRate * AccesibleAmmonium();
    double Add            = FirstOrderRate * TemperatureNitrificationEffect() * WaterNitrificationEffect();
...
void soilLayer::UpdateN2OFromDenitrification(double TotalCO2Emission) {
    ...
    // kd estimation
    double DenPotClay = 0.286 + 0.0028 * clayContent * 100;
    ...
    if (Denitrification > 0) {
        ...
        if (nitrate_imob.n + nitrate_mob.n > 0.0) {
            N15RatioNitrate = max(0.0,
                                  min(1.0, (nitrate_imob.n15 + nitrate_mob.n15) / (nitrate_imob.n + nitrate_mob.n)));
        }
...
nitrogen soilLayer::UpdateN2Production() {
    ...
    double fWater = 1.0
                    - (0.0116
                       + 1.36 / (1.0 + exp(-(GetWaterFilledPorosity() - 0.815) / 0.0896)));    // inversed water factor
...
double soilLayer::RatioN2OEmissionNitri() {
    ...
    N2Oratio = 0.047 * tFactor
               * WaterFactor;    // Combined emission factor from er Ambus (2001), anual effect as from N2Oratio = 0.006;
...
double soilLayer::TemperatureNitrificationEffect() {
    ...
    if (temperature <= 1.) {
        ...
        F = 0.125 * (temperature - 1.);
    } else if (temperature <= 20) {
        F = 0.1 * temperature;
    } else {
        F = exp(0.4657 - 0.027259 * temperature + 0.0019315 * temperature * temperature);
    }
...
void soilLayer::AddProduct(organicProduct & product) {
    ...
    double   nitfrac      = 0.0;
    double   ammfrac      = 0.0;
    double   unprotecFrac = 0.0;    // Deals with protected NH4
    ...
    if ((ammonium_mob.n < -1E-6) || (ammonium_imob.n < -1E-6) || (nitrate_mob.n < -1E-6) || (nitrate_imob.n < -1E-6)
            || (OrganicMatter -> GetNitrogenN15() < -1e-10)) {
        theMessage -> WarningWithDisplay("soilLayer::UpdateOrganicMatter - negative n concentrations");
    }
...
void soilLayer::PrintAmmonium(int layer) {
    ...
    if (!ammoniumDisolvedUsed) {
        AmmoniumLayer.open((globalSettings -> getOutputDirectory() + "AmmoniumLayer.xls").c_str(), ios::out);

        AmmoniumLayer << theTime << "\t";

        ammoniumDisolvedUsed = true;
    } else {
        AmmoniumLayer.open((globalSettings -> getOutputDirectory() + "AmmoniumLayer.xls").c_str(), ios::app);
...
void soilLayer::PrintNitrate(int layer) {
    ...
    if (!NitLayer) {
        NitrateLayer.open((globalSettings -> getOutputDirectory() + "NitrateLayer.xls").c_str(), ios::out);

        NitrateLayer << theTime << "\t";

        NitLayer = true;
    } else {
        NitrateLayer.open((globalSettings -> getOutputDirectory() + "NitrateLayer.xls").c_str(), ios::app);
...
soilLayer::soilLayer(const soilLayer & Layer):
    base(Layer)
{
    nitrateBefore = orgNumber = ammoniumBefore = 0.0;
    next          = nullptr;
