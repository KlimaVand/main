...
void soil::Initialize(string soilFileName) {
    depth = 1000;

    cout << "Initialising soil no " << GetLongNameInNumbers() << " with the file " << soilFileName
         << endl;    // a problem with patches can occure at this point
    ...
    Surface -> Initialize(soilFileName);
    Profile -> Initialize(soilFileName);

    if (globalSettings -> DetailsData.getSoilDetails()) {
        ...
        for (unsigned int i = 0; i < globalSettings -> WaterData.size(); i++) {
            f << "\twater" << globalSettings -> WaterData[i].getStartDepth() << "-"
              << globalSettings -> WaterData[i].getEndDepth();
        }

        for (unsigned int i = 0; i < globalSettings -> NminData.size(); i++) {
            f << "\tNmin" << globalSettings -> NminData[i].getStartDepth() << "-"
              << globalSettings -> NminData[i].getEndDepth();
            f << "\tNO3-" << globalSettings -> NminData[i].getStartDepth() << "-"
              << globalSettings -> NminData[i].getEndDepth();
            f << "\tNH4-" << globalSettings -> NminData[i].getStartDepth() << "-"
              << globalSettings -> NminData[i].getEndDepth();
            f << "\tTotal15N-" << globalSettings -> NminData[i].getStartDepth() << "-"
              << globalSettings -> NminData[i].getEndDepth();
        }
        ...
        for (unsigned int i = 0; i < globalSettings -> SoilOrgCData.size(); i++) {
            f << "\tSMB-C-" << globalSettings -> SoilOrgCData[i].getStartDepth() << "-"
              << globalSettings -> SoilOrgCData[i].getEndDepth();
        }
	...
	if (globalSettings->DetailsData.getSoilDetails()) {
		...
		f << "\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2\tgN/m2";
		f << "\tgC/m2\tgC/m2\tgC/m2\tgC/m2\tgC/m2\tgC/m2\tgC/m2\tgC/m2\tgC/m2\tgC/m2\tgN/m2\tgC/m2\tgN/m2\tgN/m2";
		f << "\tg/m2\tg/m2\tg/m2\tg/m2\tratio pr day\tmm\tg N/m2\tmm/day\tmm/day";
		f << "\tg N/m2/day\tg N/m2/day\tg C/m2";
		f << "\t??\tday-1";
...
nitrogen soil::GetAmmonium() {
    nitrogen Ammonium = Surface -> GetAmmoniumSolid() + Surface -> GetAmmoniumDisolved() + Profile -> GetAmmonium();
...
nitrogen soil::GetMinNitrogen(double startDep,
                              double thick) {
    return GetAmmonium(startDep, thick) + GetNitrate(startDep, thick);
}

double soil::GetMinNitrogenN(double startDep,
                             double thick) {
    return GetAmmoniumN(startDep, thick) + GetNitrateN(startDep, thick);
}

double soil::GetMinNitrogenN15(double startDep,
                               double thick) {
...
void soil::Update(double                    surplus,
    ...
    if (MasterSoil
           ) {    // If more than one patch then speed up temperature calculation by using only one master patch
    ...
    double relativeSurfaceWater = min(1.0,
                                      Profile -> GetAvailWater(0., evaporationDepth)
                                      / Profile -> GetAvailCapacity(0., evaporationDepth));
    double maxEvaporation = Profile -> GetMaximumEvaporation();

    Surface -> Update(&surplus, EPotToSoil, relativeSurfaceWater, maxEvaporation, Profile -> GetEvaporationContent(),
                      airTemperature);
    ...
    surplus += Surface ->
         ExtractWater();    // Code review 2011 Tjek om det er korrekt regnet ud, eller om alt vand og N forsvinder på een gang!!!!!

    double Chloride = Surface -> ExtractChloride();

    // Water infiltration and nitrogen leaching
    Profile -> UpdateInfiltrationSWAT(&surplus, &surfaceNitrate, &surfaceAmmonium, &nitrateLeached, &ammoniumLeached,
                                      Chloride);
 ...
 *
 */
void soil::Incorporate(double MaxDepth,
                       double frac,
                       bool   tillage,
                       string tillagetype,
                       double MinDepth,
    ...
    if (((MaxDepth > 0) && (frac > 0))) {
        ...
        if (data.readFile("Tillage.dat")) {

            // Only comes in effect when tillage.dat is present
            if (tillage) {
                double p1 = 0.1;
                double p2 = 0.5;
        ...
        if (tillagetype == "Ploughing") {
            ...
            for (int i = 0; i < n; i++) {
                ...
                } else if ((CurrentDepth <= MaxDepth) && ((CurrentDepth - LayerThickness) < MinDepth)
                           && ((CurrentDepth - MinDepth) >= 0)) {
                    LengthList[i] = CurrentDepth - MinDepth;
                } else if ((CurrentDepth - LayerThickness < MaxDepth)
                           && ((CurrentDepth - LayerThickness) >= MinDepth)) {
                    LengthList[i] = MaxDepth - (CurrentDepth - LayerThickness);
                } else if ((CurrentDepth - LayerThickness) < MaxDepth && (CurrentDepth > MaxDepth)
                           && (CurrentDepth - LayerThickness) < MinDepth && (CurrentDepth > MinDepth)) {
...
/*
 * @param Fraction (0-1) : fraction of SMB killed
 * @param Depth mm
 */
void soil::KillSMB(double Fraction,
                   double Depth) {
...
/*
 *
 * @param Fraction (0-1) : fraction of AOM transferred to pools with a modified
 * @param TurnoverFactor: Factor for modification of turnover rate
 * @param Depth mm
 */
void soil::ModifyAOM(double Fraction,
                     double TurnoverFactor,
                     double Depth) {
    for (int i = 0; i < Profile -> GetNumberOfLayers(); i++) {
        if (Profile -> GetLayer(i) -> GetStartDepth() < Depth) {
    ...
    if (theTime.GetDateValue() == ObsDate) {
        ...
        while (currentLayer) {
            if ((currentLayer -> GetStartDepth() >= ObsStartDepth) && (currentLayer -> GetEndDepth() <= ObsEndDepth)) {
                currentLayer -> AddNutrient(currentLayer -> GetNitrate() * (factor - 1),
                                            currentLayer -> GetAmmonium() * (factor - 1));
            }
...
bool soil::EndBudget(double * NRemain,
                     double * WaterRemain) {
    bool retVal = true;
...
void soil::WriteOutput(string CropName) {
    if (globalSettings -> DetailsData.getSoilDetails()) {
        f << theTime.GetYear() << "-" << theTime.GetMonth() << "-" << theTime.GetDay();
		f << "\t" + CropName;
        for (unsigned int i = 0; i < globalSettings -> WaterData.size(); i++) {
            f << "\t"
              << GetWater(globalSettings -> WaterData[i].getStartDepth(),
                          globalSettings -> WaterData[i].getEndDepth()
                          - globalSettings -> WaterData[i].getStartDepth());
        }
        ...
        for (unsigned int i = 0; i < globalSettings -> PoreWaterData.size(); i++) {
            ...
            f << "\t" << 1000.0 * GetNitrate(depth - 5, 10).n / GetWater(depth - 5, 10) << "\t"
              ...
              << 1000.0 * (GetNitrate(depth - 5, 10).n + GetAmmonium(depth - 5, 10).n)
                 / GetWater(depth - 5, 10)                                          // Convert to mg/l
        ...
        f << "\t" << GetAmmonium(0, 250).n << "\t" << GetNitrate(0, 250).n << "\t" << GetAmmonium(0, 250).n15 << "\t"
          << GetNitrate(0, 250).n15;
        f << "\t" << GetPoolNitrogenN(0, 0, depth);
        f << "\t" << GetPoolNitrogenN(1, 0, depth) << "\t" << GetPoolNitrogenN(2, 0, depth) << "\t"
          << GetPoolNitrogenN(3, 0, depth) << "\t" << GetPoolNitrogenN(4, 0, depth);
        ...
        f << "\t" << GetPoolNitrogenN(6, 0, depth) << "\t" << GetPoolNitrogenN15(0, 0, depth) << "\t"
          << GetPoolNitrogenN15(1, 0, depth) << "\t" << GetPoolNitrogenN15(2, 0, depth) << "\t"
          << GetPoolNitrogenN15(3, 0, depth) << "\t" << GetPoolNitrogenN15(4, 0, depth) << "\t"
        ...
        if (Profile -> GetN2OFromNitrification().n + Profile -> GetN2OFromDenitrification().n > 1e-10) {
            N2ORatio = denitrification.n
                       / (Profile -> GetN2OFromNitrification().n + Profile -> GetN2OFromDenitrification().n);
        }

        f.precision(4);

        f << "\t" << GetPoolCarbon(6, 0, depth) << "\t" << GetPoolCarbon(0, 0, depth) << "\t"
          << GetPoolCarbon(1, 0, depth) << "\t" << GetPoolCarbon(2, 0, depth) << "\t" << GetPoolCarbon(3, 0, depth)
          << "\t" << GetPoolCarbon(4, 0, depth) << "\t" << GetTotalCarbon(0, 200) << "\t" << GetTotalCarbon(200, 200)
          << "\t" << GetTotalCarbon(400, 200) << "\t" << GetTotalNitrogen(0, 200).n << "\t" << GetTotalCarbon(0, 1000)
          << "\t" << GetTotalNitrogen(0, 500).n15 << "\t" << GetTotalNitrogen(2000).n15 << "\t"
          << NitrogenNetMineralisation.n << "\t" << Profile -> GetN2OFromNitrification().n << "\t"
          << Profile -> GetN2OFromDenitrification().n << "\t"
          << Profile -> GetN2OFromNitrification().n + Profile -> GetN2OFromDenitrification().n << "\t" << N2ORatio
          << "\t" << AccumulatedWaterDrainage << "\t" << AccumulatedNitrateDrainage << "\t" << soilEvaporation << "\t"
          << totalTranspiration << "\t" << denitrification.n << "\t" << nitrification.n << "\t" << CO2Evolution;

        double tempAdjust        = 1.0 / exp(-3.432 + 0.168 * 10 * (1.0 - 0.5 * 10 / 36.9));
        double TemperatureEffect = tempAdjust * exp(-3.432 + 0.168 * 20.0 * (1.0 - 0.5 * 20.0 / 36.9));

        // Note check if temperature effect on organic matter
        double PotC = 0.0;
        ...
        for (int i = 0; i < 7; i++) {
            ...
            double WaterEffect = max(0.0,
                                     min(1.0,
                                         0.0116
                                         + 1.36
                                           / (1 + exp(-(Profile -> GetLayer(i) -> GetWaterFilledPorosity() - 0.815)
                                               / 0.0896))));

            f << "\t" << WaterEffect;

            double tempAdjust        = 1.0 / exp(-3.432 + 0.168 * 10 * (1.0 - 0.5 * 10 / 36.9));
            double TemperatureEffect = tempAdjust
                                       * exp(-3.432
                                             + 0.168 * Profile -> GetLayer(i) -> GetTemperature()
                                               * (1.0 - 0.5 * Profile -> GetLayer(i) -> GetTemperature() / 36.9));

            f << "\t" << TemperatureEffect;

            double NitrateConc = Profile -> GetLayer(i) -> GetNitrate().n / Profile -> GetLayer(i) -> GetThickness()
                                 * 1000 / Profile -> GetLayer(i) -> GetDryBulkDensity() * 1e6;
        ...
        f << "\t" << Surface -> GetCarbon() << "\t" << Surface -> GetOrganicNitrogen().n << "\t"
          << Surface -> GetTotalNitrogen().n;
        f << endl;

        if (globalSettings -> DetailsData.DebugSoil) {
            WriteC();
 ...
 * @param rootLengthList
 */
void soil::AddDecomposable(decomposable * produc,
                           double *       rootLengthList) {
