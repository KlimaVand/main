    ...
    public base
{
    ...
    public:
        ...
        void SetMasterSoil(base * aMasterSoil) {
            MasterSoil = aMasterSoil;
        }

        // ! initialise soil module
        virtual void Initialize(string soilFileName);
        ...
        int GetLayers() {
            return Profile -> GetNumberOfLayers();
        }
        ...
        // Water functions
        double GetWater() {
            return Profile -> GetTotalWater();
        }

        double GetWater(double startDepth,
                        double thickness) {
            return Profile -> GetTotalWater(startDepth, thickness);
        }

        double GetSurfaceWater() {
            return Surface -> GetWater();
        }

        double GetAvailWater(double startDepth,
                             double thickness) {
            return Profile -> GetAvailWater(startDepth, thickness);
        }

        double GetAvailCapacity(double startDepth,
                                double thickness) {
            return Profile -> GetAvailCapacity(startDepth, thickness);
        }

        double GetFieldCapacity(double startDepth,
                                double thickness) {
            return Profile -> GetFieldCapacity(startDepth, thickness);
        }

        double GetWiltCapacity(double startDepth,
                               double thickness) {
            return Profile -> GetWiltCapacity(startDepth, thickness);
        }
        ...
        double GetDrainage(double depth) {
            return Profile -> GetDrainage(depth);
        }

        // Nitrogen functions
        nitrogen GetMineralNitrogenSurface() {
            return Surface -> GetAmmoniumDisolved() + Surface -> GetNitrateDisolved() + Surface -> GetAmmoniumSolid()
                   + Surface -> GetNitrateSolid();
        }

        nitrogen GetOrganicNitrogenSurface() {
            return Surface -> GetOrganicNitrogen();
        }

        nitrogen GetMineralNitrogenProfile() {
            return Profile -> GetAmmonium() + Profile -> GetNitrate();
        }

        nitrogen GetOrganicNitrogenProfile() {
            return Profile -> GetOrganicNitrogen();
        }

        nitrogen * GetNitrogenUptake() {
            return nitrogenUptake;
        }

        nitrogen GetNitrateLeached() {
            return nitrateLeached;
        }

        nitrogen GetAmmoniumLeached() {
            return ammoniumLeached;
        }

        nitrogen GetAmmonium();

        nitrogen GetAmmonium(double startDep,
                             double thick) {
            return Profile -> GetAmmonium(startDep, thick);
        }

        double GetAmmoniumN(double startDep,
                            double thick) {
            return Profile -> GetAmmoniumN(startDep, thick);
        }

        double GetAmmoniumN15(double startDep,
                              double thick) {
        ...
        nitrogen GetNitrate(double startDep,
                            double thick) {
            return Profile -> GetNitrate(startDep, thick);
        }

        double GetNitrateN(double startDep,
                           double thick) {
            return Profile -> GetNitrateN(startDep, thick);
        }

        double GetNitrateN15(double startDep,
                             double thick) {
            return Profile -> GetNitrateN15(startDep, thick);
        }

        nitrogen GetMinNitrogen();

        nitrogen GetMinNitrogen(double startDep,
                                double thick);

        double GetMinNitrogenN(double startDep,
                               double thick);

        double GetMinNitrogenN15(double startDep,
                                 double thick);

        nitrogen GetDenitrification() {
            return denitrification;
        }

        nitrogen GetN2OFromNitrification() {
            return Profile -> GetN2OFromNitrification();
        }

        nitrogen GetN2OFromDenitrification() {
            return Profile -> GetN2OFromDenitrification();
        }
        ...
        nitrogen GetTotalNitrogen(double startDep,
                                  double thick);

        double GetTotalNitrogenN(double startDep,
                                 double thick);

        double GetTotalNitrogenN15(double startDep,
                                   double thick);
        ...
        double GettotalTranspiration() {
            return totalTranspiration;
        }

        // Carbon functions
        double GetTotalCarbon() {
            return Surface -> GetCarbon() + Profile -> GetCarbon();
        }

        double GetTotalCarbon(double startDep,
                              double thick) {
        ...
        double GetCO2Evolution() {
            return CO2Evolution;
        }

        // Diverse get functions
        double GetSoilMass(double startDep,
                           double thick) {    // unit: g dry soil per m2
            return Profile -> GetSoilMass(startDep, thick);
        }

        soilSurface * GetSoilSurface() {
            return Surface;
        }

        soilProfile * GetSoilProfile() {
            return Profile;
        }
        ...
        double GetMaximumDepth() {
            return Profile -> GetMaximumDepth();
        }

        double GetMaxRootDepth() {
            return Profile -> GetMaxRootDepth();
        }

        // N functions
        nitrogen GetAmmoniumLeaching(double depth) {
            return Profile -> GetAmmoniumLeaching(depth);
        }

        nitrogen GetNitrateLeaching(double depth) {
            return Profile -> GetNitrateLeaching(depth);
        }

        nitrogen GetRunOff_N() {
            return Surface -> GetRunOff_N();
        }

        double GetRunOff() {
            return Surface -> GetRunOff();
        }

        double GetDiscreteDepth(double depth) {
            return Profile -> GetDiscreteDepth(depth);
        }

        double GetWaterInfiltration() {
            return Profile -> Getinfiltration();
        }
        ...
        bool EndBudget(double * N,
                       double * w);

        budget * GetWaterBudget() {
            return &WaterBudget;
        }
