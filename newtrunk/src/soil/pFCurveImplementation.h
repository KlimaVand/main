    ...
    public base
{
    private:
        ...
        pF_Curve_Implementation & operator = (
                const pF_Curve_Implementation & p);    // Dissable the compilers generation of default assignment operator.
    ...
    public:
        ...
        pF_Curve_Implementation(string Pname,
                                int    Intpol = 0);

        pF_Curve_Implementation(const pF_Curve_Implementation & pFSource);

        ~pF_Curve_Implementation();

        void MakeTables();

        void SetIntpol(int Intpol);

        void AddWaterItem(pF_WaterData data);

        void AddWaterData(double RelativeWater,
                          double pFValue);

        void AddConductionItem(pF_ConductionData data);

        void AddConductionData(double pFValue,
                               double logCond,
                               double fluxPoten);
        ...
        double GetNumberOfPFCurves() {
            return NumberOfPFCurves;
        }

        // NumberOfPFCurves indicates how many pF_curves that points on this implementation
        void AddOnePFCurve() {
            NumberOfPFCurves++;
        }

        void RemoveOnePFCurve() {
            NumberOfPFCurves--;
        }

        void AddExternalConductionData(double pFValue,
                                       double logCond);
