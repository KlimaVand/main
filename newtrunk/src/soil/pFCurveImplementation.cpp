...
pF_Curve_Implementation::pF_Curve_Implementation(string Pname,
        int                                             Intpol):
    base(Pname)
{
    InitVars(Intpol);
...
pF_Curve_Implementation::pF_Curve_Implementation(const pF_Curve_Implementation & pFSource) {
    numExternalConduc = pFSource.numExternalConduc;

    for (int i = 0; i < numExternalConduc - 1; i++) {
        ExternalConduc[i].pF        = pFSource.ExternalConduc[i].pF;
        ExternalConduc[i].logConduc = pFSource.ExternalConduc[i].logConduc;
    }

    pF_WaterData * pW = nullptr;

    IntpolMethod     = pFSource.IntpolMethod;
    NumberOfPFCurves = 0;
    ...
    while (pC) {
        AddConductionItem(*pC);

        pC = pC -> next;
    }

    tabulated = false;
    minpF     = pFSource.minpF;
    maxpF     = pFSource.maxpF;

    for (int i = 0; i < maxTablePoints; i++) {
        pFTable[i]            = pFSource.pFTable[i];
        relativeWaterTable[i] = pFSource.relativeWaterTable[i];
        conductivityTable[i]  = pFSource.conductivityTable[i];
        fluxPotentialTable[i] = pFSource.fluxPotentialTable[i];
    }
}
...
void pF_Curve_Implementation::AddWaterItem(pF_WaterData data) {
    ...
    while (p) {
        if (p -> relativeWaterContent == data.relativeWaterContent) {
            Ok = 0;
        }
    ...
    if (Ok) {
        if ((!firstW) || (data.relativeWaterContent < firstW -> relativeWaterContent)) {
            newlink -> next = firstW;
            firstW          = newlink;
        } else {
            p = firstW;
            ...
            while ((p -> next) && (data.relativeWaterContent > p -> relativeWaterContent)) {
                l = p;
                p = p -> next;
            }

            if (data.relativeWaterContent < p -> relativeWaterContent) {
                p = l;
            }
...
void pF_Curve_Implementation::AddWaterData(double RelativeWater,
        double                                    pFValue) {
    pF_WaterData p;
...
void pF_Curve_Implementation::AddConductionData(double pFValue,
        double                                         logCond,
        double                                         fluxPot)    // units: pf, log10(m/s), m2 s-1
{
    pF_ConductionData p;
...
double pF_Curve_Implementation::GetExternalConductivity(double pF) {
    ...
    do {
        ...
        for (int i = 0; i < numExternalConduc - 1; i++) {
            if (ExternalConduc[i + 1].pF < ExternalConduc[i].pF) {    // Swap
                unSorted = true;
...
void pF_Curve_Implementation::MakeHydraulicConductivity(double FieldConductivity) {
    ...
    if (numExternalConduc > 1) {
        ...
        for (i = -10; i <= 70; i++) {
            ...
            for (int j = i; j <= 70; j++) {
                sum += (2 * j + 1 - 2 * i) / (pow(10., 2 * i * 0.1) / 10000.);
            }
    ...
    for (i = -10; i <= 70; i++) {
        if (i > 0) {
            flux += (pow(10., i * 0.1) - pow(10., (i - 1) * 0.1)) / 100.0
                    * cond[i + 10];    // unit m2 s-1  (divides by 100 to convert cm to m)
        }
...
double pF_Curve_Implementation::CalcRelativeWater(double pFValue) {
    ...
    while (p) {
        ...
        if (n >= maxSplinePoints) {
            theMessage -> FatalError(
                "pF_Curve_Implementation::CalcRelativeWater - the maximum of pF-curve points exceeded");
        }
    ...
    if (yp <= 0) {
        ...
        theMessage -> FatalError(
            "pF_Curve_Implementation::CalcRelativeWater - relative water content can not be above 1");
    }
...
double pF_Curve_Implementation::CalcConductivity(double pFValue) {
    ...
    } else if (pFHigh == pFLow) {    // Linear interpolation
...
double pF_Curve_Implementation::GetPorosity() {
    double         por = -1;
    pF_WaterData * p;
...
void pF_Curve_Implementation::ReadParameters(commonData * data,
        int                                               index,
    ...
    data -> FindSection("SoilLayer", index);
    data -> setCritical(false);

    int Mualem       = 0;
    int vanGenuchten = 0;

    data -> FindItem("MualemVanGenucten", &Mualem);
    data -> FindItem("vanGenuchten", &vanGenuchten);
    ...
    if (Mualem || vanGenuchten) {
        ...
        if (vanGenuchten) {
            ...
            data -> FindItem("KS", &KS);    // cm d-1
            data -> FindItem("fis", &fis);
            data -> FindItem("fir", &fir);
            data -> FindItem("L", &L);
            data -> FindItem("alpha", &alpha);
            data -> FindItem("n", &N);
            data -> setCritical(false);
            ...
            clay = clay
                   / (1 - 0.01
                      * orgmat);    // Texture here calculated to sum to 100 WITHOUT orgmat, minor fault though - BMP !!!
            silt = silt
                   / (1 - 0.01
                      * orgmat);    // Texture here calculated to sum to 100 WITHOUT orgmat, minor fault though - BMP !!!
            fis = 0.7919 + (0.001691 * clay) - (0.29619 * BD) - (0.000001491 * silt * silt)
                  + (0.0000821 * orgmat * orgmat) + (0.02427 / clay) + (0.01113 / silt) + (0.01472 * log(silt))
                  - (0.0000733 * orgmat * clay) - (0.000619 * BD * clay) - (0.001183 * BD * orgmat)
            ...
            alpha = -14.96 + (0.03135 * clay) + (0.0351 * silt) + (0.646 * orgmat) + (15.29 * BD) - (0.192 * TopSoil)
                    - (4.671 * BD * BD) - (0.000781 * clay * clay) - (0.00687 * orgmat * orgmat) + (0.0449 / orgmat)
                    + (0.0663 * log(silt)) + 0.1482 * log(orgmat) - (0.0456 * BD * silt) - (0.4852 * BD * orgmat)
                    + (0.00673 * TopSoil * clay);
            alpha = exp(alpha);
            N     = -25.23 - (0.02195 * clay) + (0.0074 * silt) - (0.194 * orgmat) + (45.5 * BD) - (7.24 * BD * BD)
                    + (0.0003658 * clay * clay) + (0.002885 * orgmat * orgmat) - (12.81 / BD) - (0.1524 / silt)
                    - (0.01958 / orgmat) - (0.2876 * log(silt)) - (0.0709 * log(orgmat)) - (44.6 * log(BD))
                    - (0.02264 * BD * clay) + (0.0896 * BD * orgmat) + (0.00718 * TopSoil * clay);
            L = 0.0202 + (0.0006193 * clay * clay) - (0.001136 * orgmat * orgmat) - (0.2316 * log(orgmat))
                - (0.03544 * BD * clay) + (0.00283 * BD * silt) + (0.0488 * BD * orgmat);
            L  = 10.0 * ((exp(L) - 1.0)) / (1.0 + exp(L));
            KS = 7.755 + (0.0352 * silt) + (0.93 * TopSoil) - (0.967 * BD * BD) - (0.000484 * clay * clay)
                 - (0.000322 * silt * silt) + (0.001 / silt) - (0.0748 / orgmat) - (0.643 * log(silt))
                 - (0.01398 * BD * clay) - (0.1673 * BD * orgmat) + (0.02986 * TopSoil * clay)
        ...
        data -> setCritical(false);
        data -> FindVectorSize("RelativeWater", &first, &num);
        data -> FindVectorSize("PFValue", &f1, &n1);

        if ((first != f1) || (num != n1)) {
            theMessage -> FatalError(
                "pF_Curve_Implementation::ReadParameters-error in numbering of pF curve vector elements");
        }
        ...
        if (num > 0) {
            for (int i = first; i <= (num); i++) {
                ...
                data -> FindItem("RelativeWater", &relWater, i);
                data -> FindItem("PFValue", &pFValue, i);

                if ((relWater > 0) && (relWater < 1) && (pFValue >= -1) && (pFValue < 7)) {
                    AddWaterData(relWater, pFValue);
                } else if ((pFValue > 7)
                        || (relWater
                            < 0)) {    // pF = 7 and low relative water are ignored but no warning are given (point is inserted anyhow)
                    theMessage -> WarningWithDisplay(
                        "soilLayerDescriptor::Initialize-pF point outside boundaries ignored");
                }
            }
        }

        data -> FindItem("ConductivityFC", &conductivityFC);    // Unit log(m/s)  ?
