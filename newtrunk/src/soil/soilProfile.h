    ...
    public base
{
    private:
        ...
        double      amplitudeAirTemp;
        double      dayZero;                            // august 1th the hottest day january 30th the coldest day
        ...
        double      DistPreviousLayerInv[MaxSoilLayers],
                    DistNextLayerInv[MaxSoilLayers];    // Only used for speed optimization
        ...
        void EstimateTotalRoot(rootStructure *           TotalRoot,
                               linkList<rootStructure> * rootList);

        void ResourceCompetition(int      ResourceNumber,
                                 int      AttackerNumber,
                                 double * Demand,
                                 double * Apparancy,
                                 double * Resource,
        ...
        void checkWaterCont(double waterContent, double maxContent);
    public:
        soilProfile(string       Pname,
                    const int    Index,
                    const base * owner);
        ...
        virtual void AddNutrient(nitrogen soilNitrate,
                                 nitrogen soilAmmonium);

        virtual void UpdateFinDiffTemperature(double temp,
                double                               snowContent);

        virtual void Tridag(double        a[],
                            double        b[],
                            double        c[],
                            double        r[],
                            double        u[],
        ...
        virtual void UpdateInfiltrationSWAT(double * surplus,
        ...
        virtual double Getinfiltration() {
            return cum_infiltration;
        }
        ...
        nitrogen GetOrganicNitrogen(double startDep,
                                    double thick);

        double GetOrganicNitrogenN(double startDep,
                                   double thick);

        double GetOrganicNitrogenN15(double startDep,
                                     double thick);
        ...
        double GetChloride(double startDep,
                           double thick);

        // Get functions
        virtual double GetDiscreteDepth(
                double depth);    // Returns a number >= depth, s� that the returned depth does not "divide" two layers

        soilLayer * GetLayer(int i) {
            return soilLayerArray[i];
        }

        virtual void UpdateT(double * soilTemperature,
        ...
        virtual double GetTotalWater(double startDepth,
                                     double thickness);

        virtual double GetAvailWater(double startDepth,
                                     double thickness);

        virtual double GetAvailCapacity(double startDepth,
                                        double thickness);

        virtual double GetFieldCapacity(double startDepth,
                                        double thickness);

        virtual double GetWiltCapacity(double startDepth,
                                       double thickness);
        ...
        double GetOrganicCarbon(double startDep,
                                double thick);
        ...
        double GetEvaporationContent() {
            return EvaporationContent;
        }
        ...
        double GetClayContent(double startdepth,
                              double thick);
        ...
        void changeLayer(int starLayer,
                         int endLayer);
