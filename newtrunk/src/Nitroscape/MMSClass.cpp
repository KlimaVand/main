...
double MMSClass::GetMMSHouseAmmonia(int houseNo) {
    return houseEmission[houseNo].NH3Emission;
}
...
double MMSClass::GetMMSHouseN2O(int houseNo) {
    return houseEmission[houseNo].N2OEmission;
}
...
double MMSClass::GetMMSHouseNO(int houseNo) {
    return houseEmission[houseNo].NOEmission;
}
...
double MMSClass::GetMMSStoreAmmonia(int storeNo) {
    return storeEmission[storeNo].NH3Emission * 1000.0;
}
...
double MMSClass::GetMMSStoreN2O(int storeNo) {
    return storeEmission[storeNo].N2OEmission * 1000.0;
}
...
double MMSClass::GetMMSStoreNO(int storeNo) {
    return storeEmission[storeNo].NOEmission * 1000.0;
}
...
void MMSClass::initialise_MMS(int    farmNo,
                              string orgPath) {
    std::string ClimateDirectory, ClimateFileName;
    ...
    myInputDirectory            = globalSettings -> getInputDirectory();
    ...
    if (aBase.FindItem("WriteSoilTemperature", &WriteSoilTemperature)) {

        // Write soil temperature to file for later use in the above procedure (for speed reasons)
    ...
    string FN1 = InputDirectoryS + "warnings.txt";
    string FN2 = InputDirectoryS + "logfile.txt";
    ...
    if (chdir(myOutputDirectory.c_str()) != 0)

#ifdef __ANSICPP__

        // make output directory if not present

#if defined(BUILDER) || defined(__BCplusplus__)
    ...
    if (chdir(myInputDirectory.c_str()) != 0) {
        myMessage -> FatalError("Event:: input directory ", myInputDirectory, " not found!");
    }

    myEventControl -> Initialize(stopTime, myInputDirectory, ClimateDirectory, ClimateFileName, myOutputDirectory);
    InitialiseInterface(myEventControl);
}

void MMSClass::DoMMSDaily() {
    SetMMSGlobals();
    chdir(myInputDirectory.c_str());
    ...
    if ((theTime.GetDay() == 1) && (theTime.GetMonth() == 1)) {
        ...
        } else if (simYear < 1000) {
            sprintf(fileExtension, "f%03d", simYear);
        }

        // else (cppcheck error) Buffer is accessed out of bounds.
...
void MMSClass::SetEcoStatus(int                       patchNo,
                            double                    ReadyForHarvestOrLater,
                            double                    IrrigationDemand,
                            linkList<harvInfoClass> * theHarvestProducts) {
    myEventControl -> getCropRotation() -> GetFields() -> ElementAtNumber(farmNo) -> GetPatch(
        patchNo) -> getPatchInterface() -> SetEcoStatus(ReadyForHarvestOrLater, IrrigationDemand, theHarvestProducts);
}

bool MMSClass::GetMMSTillage(int      patchNo,
                             double * theType,
                             double * theDepth,
                             double * theFractionIncorporated) {
    patchInterfaceClass * apatchInterfaceClass = myEventControl -> getCropRotation() -> GetFields() -> ElementAtNumber(
                                                     farmNo) -> GetPatch(patchNo) -> getPatchInterface();
...
bool MMSClass::GetMMSAddOrganic(int      patchNo,
                                double * theType,
                                double * theamount,
                                double * theDepth,
                                double * theTotalN,
...
bool MMSClass::GetMMSDoFertilisation(int patchNo,
        double *                         theNH4N,
        double *                         theNO3N) {
    patchInterfaceClass * apatchInterfaceClass = myEventControl -> getCropRotation() -> GetFields() -> ElementAtNumber(
                                                     farmNo) -> GetPatch(patchNo) -> getPatchInterface();
...
bool MMSClass::GetMMSDoSowing(int      patchNo,
                              double * theSeedCode,
                              double * theSeedDM,
                              double * theSeedN) {
    patchInterfaceClass * apatchInterfaceClass = myEventControl -> getCropRotation() -> GetFields() -> ElementAtNumber(
...
bool MMSClass::GetMMSDoHarvesting(int      patchNo,
                                  double * theharvestCode,
                                  double * thepropGrainHarvested,
                                  double * thepropStrawHarvested) {
    patchInterfaceClass * apatchInterfaceClass = myEventControl -> getCropRotation() -> GetFields() -> ElementAtNumber(
                                                     farmNo) -> GetPatch(patchNo) -> getPatchInterface();
...
double MMSClass::GetMMSPatchAmmonia(int patchNo) {
    patchInterfaceClass * apatchInterfaceClass = myEventControl -> getCropRotation() -> GetFields() -> ElementAtNumber(
                                                     farmNo) -> GetPatch(patchNo) -> getPatchInterface();
    double ret_val = apatchInterfaceClass -> GetNH3Emission();
...
double MMSClass::GetMMSSpreadAmmonia() {
    ...
    for (int i = 0;
            i < myEventControl -> getCropRotation() -> GetFields() -> ElementAtNumber(
                farmNo) -> getPatchList() -> NumOfNodes();
            i++) {
