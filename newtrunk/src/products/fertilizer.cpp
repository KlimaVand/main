...
fertilizer::fertilizer(string       aName,
                       const int    aIndex,
                       const base * aOwner):
    product(aName,
            aIndex,
...
product & fertilizer::operator -(const product & someFertilizer) {
    ...
    if ((N_content != aFertilizer -> GetN_content()) || (P_content != aFertilizer -> GetP_content())
            || (K_content != aFertilizer -> GetK_content()) || (Ca_content != aFertilizer -> GetCa_content())) {
 ...
 */
void fertilizer::ReadParameters(commonData * data,
                                string       sectionName) {
    product::ReadParameters(data, sectionName);

    if (data -> FindSection(sectionName, Index)) {
        data -> FindItem("N_content", &N_content.n);
        data -> FindItem("P_content", &P_content);
        data -> FindItem("K_content", &K_content);
        data -> FindItem("Ca_content", &Ca_content);
        data -> FindItem("Cl_content", &Cl_content);

        MineralForm  = "SOLID";
        NH4_fraction = 0.0;

        data -> FindItem("NH4_fraction", &NH4_fraction);
        data -> FindItem("MineralForm", &MineralForm);
    }
}
