 ...
 */
organicProduct::organicProduct(string       Pname,
                               const int    Index,
                               const base * owner):
    base(Pname,
         Index,
...
bool organicProduct::ProductSimilarity(const organicProduct * product) {
    ...
    bool resemblance = ((allocationList[0].decayRate == product -> allocationList[0].decayRate)
                        ...
                        && (ReduceTurnover == product -> ReduceTurnover) && (fabs(deviation) < 0.05));
 ...
 */
void organicProduct::ReadParameters(commonData *& file,
        bool                                      ncfrac) {
    ...
    if (file -> FindSection(Name, Index)) {
        ...
        for (int i = first; i < (first + n); i++) {
            ...
            if (file -> FindItem("DecayRate", &allocationList[i].decayRate,
                                 i)) {    // Rate optional. If not set, takes rate from model definition file.
            }
