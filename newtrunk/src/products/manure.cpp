    ...
    decomposable(amanure)
{
    ...
    NH3emissionFactor      = amanure.NH3emissionFactor;
}
...
double manure::VolatAmmonia(double   temperature,
                            double   evap,
                            double   infiltration,
                            ...
                            double   duration,
                            double * volEvap,
                            manure * toSoil) {    // returns volatilisation in tonnes
    ...
    if (amount > 0.0) {
        ...
        *volEvap = evap * duration / 1000.0;                              // convert from mm to tonnes per sq metre
        ...
        double Kh = exp(160.559 - (8621.06 / (temperature + 273.15)) - 25.6767 * log(temperature + 273.15)
                        + 0.035388 * (temperature + 273.15));    // atm litre /mol
        ...
        if (duration * (infiltration + evap - precipitation) >= volume) {                   // dries completely
            ...
            volInfiltration = volume * (infiltration / (evap + infiltration))
                              / 1000.0;    // convert from mm to tonnes per sq metre;
 ...
 */
double manure::GetWaterEvap(double temperature,
                            double relHumidity,
                            double area,
                            double resistance,
                            double duration) {    // returns evaporation in tonnes
    ...
    // double result = (aClimate->Evaporation(temperature, relHumidity,
    // transferCoefficient) / 1000.0) * area * duration;
    ...
    // "manure::GetWaterEvap - resistance to water vapour transport must be greater than zero");
    //
    //
    // currently disabled
    double result = 0.0;
...
product & manure::operator +(const product & someManure) {
    ...
    if ((amount > 1E-6) && (aAmount > 1E-6)) {
        ...
        canopyStorageParameter = (canopyStorageParameter * amount + acanopyStorageParameter * aAmount)
                                 / (amount + aAmount);
...
product & manure::operator -(const product & someManure) {
    ...
    if ((Mg_content != aManure -> GetMg_content()) || (Cu_content != aManure -> GetCu_content())
            || (Rt_content != aManure -> GetRt_content()) || (sand != aManure -> Getsand())
            || (pH != aManure -> GetpH()) || (infiltFactor != aManure -> GetinfiltFactor())
            || (degRateConst != aManure -> GetdegRateConst()) || (maxDeg != aManure -> GetmaxDeg())
            || (degradableOM != aManure -> GetdegradableOM())) {
...
void manure::ReadParameters(commonData * data,
                            string       sectionName) {
    decomposable::ReadParameters(data, sectionName);

    if (data -> FindSection(sectionName, Index)) {
        data -> FindItem("Mg_content", &Mg_content);
        data -> FindItem("Cu_content", &Cu_content);
        data -> FindItem("Rt_content", &Rt_content);
        data -> FindItem("sand", &sand);
        data -> FindItem("pH", &pH);
    }
}
 ...
 */
void manure::VolatAndMinNitrogen(double   NH3EF,
                                 double   N2EF,
                                 double   N2OEF,
                                 double   MinF,
                                 double * NH3vol,
...
double manure::InstantaneousVolatAmmonia(double temperature,
        double                                  area,
        double                                  resistance) {    // returns instantaneous volatilisation of NH3-N
    ...
    if (amount > 0.0) {
        ...
        double q = KH / ((1 + pow(10, -pH) / KN) * resistance * volume
                         * 14.0);    // rate constant, per hourTemperature in degrees Celsius
 ...
 */
double manure::doDenitrification(double   temperature,
                                 double   duration,
                                 double * N2,
                                 double * N2O) {    // N2 and N2O emission in kg, placeholder for better funtion
    if (temperature <= 0.0) {
        *N2 = 0.0;
    } else {
 ...
 */
void manure::CalcOMdegradation(double   temperature,
                               double   duration,
                               double   degRateConst,
                               double * CO2,
                               double * CH4,
    ...
    double OMdeg     = degRateConst * duration * mesoTemperatureFunc(temperature) * MoistureFunc()
                       * currentOM;    // * maxDeg;
...
double manure::AdjustNH4N(double amountToAdjust) {
    double retVal       = 0;
    double NH4N_present = amount * NH4_content.n;
    double newNH4N      = NH4N_present - amountToAdjust;

    if (newNH4N < 0.0) {
        NH4_content = 0.0;
        retVal      = amountToAdjust - NH4N_present;
    } else {
        NH4N_present -= amountToAdjust;
        NH4_content  = NH4N_present / amount;
        retVal       = amountToAdjust;
    }
