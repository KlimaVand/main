...
feedItem::feedItem(string       aName,
                   const int    aIndex,
                   const base * aOwner):
    liveMatter(aName,
               aIndex,
...
product & feedItem::operator +(const product & someFeedItem) {
    ...
    if (aFeedItem -> GetObjType() == feedItemObj) {
        ...
        if (amount != 0) {
            proteinN_digestibility = (proteinN_digestibility * Getprotein_N() + aprotein_N * aproteinN_digestibility)
                                     / (aprotein_N + Getprotein_N());    // notice not relative to amount !!
            feedUnitsPerItemUnit = (amount * feedUnitsPerItemUnit + aAmount * afeedUnitsPerItemUnit)
                                   / (amount + aAmount);
            ...
            pigFeedUnitsPerItemUnit = (pigFeedUnitsPerItemUnit * amount + aAmount * apigFeedUnitsPerItemUnit)
                                      / (amount + aAmount);
            ...
            if ((amountProtein + aAmountProtein) > 0.0) {
                ...
                propUDPdigestible = (propUDPdigestible * amountUDP + aAmountUDP * apropUDPdigestible)
                                    / (amountUDP + aAmountUDP);
...
product & feedItem::operator -(const product & someFeedItem) {
    ...
    if ((proteinN_digestibility != aFeedItem -> GetproteinN_digestibility())
            || (feedUnitsPerItemUnit != aFeedItem -> GetfeedUnitsPerItemUnit())
            || (Ca_content != aFeedItem -> GetCa_content()) || (Na_content != aFeedItem -> GetNa_content())
            || (Mg_content != aFeedItem -> GetMg_content()) || (Cu_content != aFeedItem -> GetCu_content())
            || (Threonin != aFeedItem -> GetThreonin()) || (Methionin != aFeedItem -> GetMethionin())
            || (Lysin != aFeedItem -> GetLysin()) || (fat != aFeedItem -> Getfat())
            || (sugar != aFeedItem -> Getsugar()) || (starch != aFeedItem -> Getstarch())
            || (chew != aFeedItem -> Getchew()) || (aat != aFeedItem -> Getaat()) || (pbv != aFeedItem -> Getpbv())
            || (cell != aFeedItem -> Getcell()) || (fill != aFeedItem -> Getfill())
            || (fillU != aFeedItem -> GetfillU()) || (plantName != aFeedItem -> GetPlantName())
            || (P_digest != aFeedItem -> GetP_digest())
            || (pigFeedUnitsPerItemUnit != aFeedItem -> GetpigFeedUnitsPerItemUnit())) {
...
bool feedItem::ReadParams(fstream * file) {
    ...
    int    counter  = 0;
    string feedName = "";    // never used, DKfeedName = "";
    ...
    if (instring.length() > 0) {
        ...
        while ((int(instring[J]) != 9)) {

            // never used DKfeedName += instring[J];
            J++;
        }
        ...
        for (int I = J; I < int(instring.length());
                I++)                                                                          // parse line
        {
            testchar = instring[I];

            // int testint;
            if ((((int(instring[I]) != 9)) && (int(instring[I]) != 32)))    // if not tab
            {
                if ((testchar == ".") || (testchar == "-") || (testchar == " ")
                        || (isdigit(instring[I]))) {                                          // accept numerics only
                    outstring += testchar;
                }
            }
        ...
        if (orgN_content.n < 0.0) {
            theMessage -> FatalError("feedItem::ReadParams - orgN_content below zero");
        }
        ...
        ME         = 0.81 * inarray[51] * 1000.0
                * dryMatter;    // MJ per tonne FW, note feed table contains digestible energy, so use 0.81 to convert (SCA (1990))
        feedUnitsPerItemUnit = inarray[52] * 1000.0 * dryMatter;           // per tonne FW

        // this is a fudge; the feed table should be updated with real values
        if (inarray[66] <= 0) {
            if (inarray[71] == 1) {
                pigFeedUnitsPerItemUnit = 1.0;
            } else {
                pigFeedUnitsPerItemUnit = 0.2;
            }
        } else {
        ...
        if ((inarray[4] > 0.0) && (inarray[5] > 0.0)) {                    // Changed 67 to 5 - BMP 2007 !!!
            proteinN_digestibility = inarray[5]
                                     / inarray[4];    // for pigs. Changed from (inarray[67]/10.0)/inarray[4] BMP 2007 !!!
        } else {
            if (inarray[4] > 0.0) {
        ...
        feedCategory = int(inarray[74]);

        // should that be an or??? so it is alway false...
        // if ((feedCategory < 1) && (feedCategory > 4))
        // theMessage->FatalError("feedItem::ReadParams - no feed category");
        ...
        if (((code >= 2) && (code <= 270)) || ((code >= 295) && (code <= 297)) || ((code >= 301) && (code <= 350))
                || ((code >= 908) && (code <= 911))) {
            propAAinUDP = 0.85;
        } else {
            propAAinUDP = 0.65;
 ...
 */
void feedItem::AddPlantItem(const plantItem & somePlantItem,
                            int               feedCat) {
...
double feedItem::GetC() {
    ...
    if ((fat == 0.0) && (cell == 0.0) && (sugar == 0.0) && (starch == 0.0)) {
        ...
        C = 0.76 * fat + 0.52 * protein + 0.48 * cell + 0.421 * sugar + 0.444 * starch;
    }
...
void feedItem::CalcFill(int type) {
    switch (type) {
        ...
        // concentrates
        case 1 :
            fill  = 220.0 * dryMatter;    // fill per tonne FW
        ...
        case 2 :
            fill = 1000.0 * (0.85 - 0.44 * feedUnitsPerItemUnit / (1000.0 * dryMatter)) * dryMatter;    // fill per tonne FW
        ...
        // silage
        case 4 :
            fill = 1000.0 * (0.79 - 0.44 * feedUnitsPerItemUnit / (1000.0 * dryMatter)) * dryMatter;    // fill per tonne FW

            break;

        default :
            theMessage -> FatalError("pfeedit:CalcFill - type not found");
    }
}
