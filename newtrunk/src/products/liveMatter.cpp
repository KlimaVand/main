...
liveMatter::liveMatter(string       aName,
                       const int    aIndex,
                       const base * aOwner):
    product(aName,
            aIndex,
...
product & liveMatter::operator -(const product & someliveMatter) {
    ...
    if ((NH4_content != aliveMatter -> NH4_content) || (orgN_content != aliveMatter -> orgN_content)
            || (NO3_content != aliveMatter -> NO3_content) || (P_content != aliveMatter -> P_content)
            || (K_content != aliveMatter -> K_content) || (dryMatter != aliveMatter -> dryMatter)
            || (organic != aliveMatter -> Getorganic()) || (C_content != aliveMatter -> C_content)
            || (S_content != aliveMatter -> S_content) || (code != aliveMatter -> code)) {
...
void liveMatter::ReadParameters(commonData *& data,
                                string        sectionName) {
    product::ReadParameters(data, sectionName);

    if (data -> FindSection(sectionName, Index)) {
        data -> FindItem("orgN_content", &orgN_content.n);
        data -> FindItem("NH4_content", &NH4_content.n);
        data -> FindItem("NO3_content", &NO3_content.n);
        data -> FindItem("P_content", &P_content);
        data -> FindItem("K_content", &K_content);
        data -> FindItem("dryMatter", &dryMatter);
        data -> FindItem("S_content", &S_content);
        data -> FindItem("C_content", &C_content);
        data -> FindItem("organic", &organic);
        data -> FindItem("code", &code);
    }
}
