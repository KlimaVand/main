 ...
 */
double price::GetPseudo(double lrange,
                        double rrange,
                        double price) {
 ...
 */
void price::ReadParameters(commonData * file,
    ...
    file -> FindItem("pricename", &name);
    file -> setCritical(false);
    file -> FindItem("rightrange", &rightRange);
    file -> FindItem("leftrange", &leftRange);

    if ((file -> FindItem("sellYear0", &someyearprice))
            || (file -> FindItem("buyYear0", &someyearprice)))    // fixed prices
    {
        ...
        while (ayear < simulationLength) {
            ...
            string  tmp = "sellYear" + conv.IntToString(ayear);

            if (file -> FindItem(tmp, &someyearprice)) {
                sellPriceSet[ayear] = someyearprice;
            } else {
                sellPriceSet[ayear] = (ayear == 0) ? 0 : sellPriceSet[ayear - 1];    // no price given
            }

            tmp = "buyYear" + conv.IntToString(ayear);
        ...
        file -> FindItem("basesellprice", &baseSellPrice);
        file -> FindItem("basebuyprice", &baseBuyPrice);
    }
    ...
    file -> FindItem("alpha", &alpha);
    file -> FindItem("lambda", &lambda);
}
