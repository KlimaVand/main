    ...
    public base
{
    ...
    public:
        ...
        virtual double GetexcretaPartition() const {
            return excretaPartition;
        }

        // !Returns the number of stores on the floor
        virtual int GetOnfloorStoreNumber() const {
            return theOnFloorStores -> NumOfNodes();
        }

        // !Returns the number of nextstores on the floor
        virtual int GetNextStoreNumber() const {
            return theNextStores -> NumOfNodes();
        }
         ...
         */
        virtual manurestore * GetaStore(int aStoreNumber) const {
            return theOnFloorStores -> ElementAtNumber(aStoreNumber);
        }

        virtual manurestore * GetaNextStore(int aStoreNumber) const {
            return theNextStores -> ElementAtNumber(aStoreNumber);
        }

        // !Returned the cleaning interval in days
        virtual double GetcleaningInterval() {
            return cleaningInterval;
        }

        // !Return time since last cleaned in days
        virtual double GettimeUncleaned() {
            return timeUncleaned;
        }

        // !Returned the amount of Straw added  in kilograms per day
        virtual double GetStrawAdded() {
            return StrawAdded;
        }
