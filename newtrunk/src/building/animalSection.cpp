...
animalSection::animalSection(string        aName,
                             const int     aIndex,
                             const base *  aOwner,
                             commonData *& runData,
                             commonData *& original,
         ...
         aOwner)
{
    ...
    for (int inx = small; inx <= large; inx++) {
        floorstore * floorInst = new floorstore(floorName, inx, this, runData, original,
                                     (large - small) + 1);
    ...
    base(aAnimalSection)
{
	...
	IntLivestockUnits = aAnimalSection.IntLivestockUnits;
 ...
 *
 */
void animalSection::Volatize(double   airVelocity,
    ...
    for (int i = 0; i < flooring -> NumOfNodes(); i++) {
        ...
        double       tempwaterEvap    = aFloor -> GetAreaPartition() * waterEvapThisSection;

        aFloor -> Volatize(airVelocity, tempwaterEvap, temperature, &tempvolatAmmonia, &tempN2Emission,
                           &tempN2OEmission, &tempCH4Emission, &tempCO2Emission);
 ...
 */
void animalSection::Volatize(double * volatAmmonia,
                             double * N2Emission,
                             double * N2OEmission,
                             double * CH4Emission,
                             double * CO2Emission) {
...
manure * animalSection::CleanSection(string forceIt) {
    ...
    for (int i = 0; i < flooring -> NumOfNodes(); i++) {
        ...
        if ((forceIt == "force cleaning") || (aFloor -> GettimeUncleaned() + 1 >= aFloor -> GetcleaningInterval())) {
            ...
            aFloor -> SettimeUncleaned(aFloor -> GettimeUncleaned() + 1);
        }
    }
 ...
 */
void animalSection::RcvManure(manure * aUrine,
                              manure * aFaeces) {
...
void animalSection::CheckLinks() {
    for (int i = 0; i < flooring -> NumOfNodes(); i++) {
        ...
        // manurestore * aStore = floorInst -> GetaNextStore(0);
        if (floorInst -> GetNextStoreNumber() == 0) {
            ...
            theMessage -> FatalError(outString);
 ...
 */
product * animalSection::GetFromManureStore(double RequestedAmount,
        product *                                  p,
        bool                                       checkAccess) {
    manure *  aManure     = new manure();
...
void animalSection::DailyUpdate() {
    if (NrOfAnimals > 0.0) {
        ...
        for (int i = 0; i < flooring -> NumOfNodes(); i++) {
            ...
            someStraw -> Setamount(NrOfAnimals * floorInst -> GetStrawAdded() * floorInst -> GetexcretaPartition()
                                   / 1000.0);                // add bedding material
...
#ifndef _STANDALONE
            theOutput -> AddIndicator(3563, "35.63 N in bedding", "kg N",
                                      someStraw -> GetAmount() * someStraw -> GetAllN().n * 1000.0);
            theOutput -> AddIndicator(4563, "45.63 C in bedding", "kg C",
                                      someStraw -> GetAmount() * someStraw -> GetC_content() * 1000.0);
#endif
...
void animalSection::ScaleSize() {
    ...
    for (int i = 0; i < flooring -> NumOfNodes(); i++) {
        ...
        annualManureToFloor += floorInst -> GetexcretaPartition() * NrOfAnimals * 365.0
                               * (floorInst -> GetWashingWater() + SpiltDrinkingWater) / 1000.0;    // in cubic metres
 ...
 */
void animalSection::SetAnimalParameters(double numberOfAnimals,
