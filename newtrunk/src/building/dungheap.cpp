...
dungheap::dungheap(string        aName,
                   const int     aIndex,
                   const base *  aOwner,
                   commonData *& runData,
                   commonData *& original):
             ...
             original)
{
    ...
    surfaceLayer = new manurestore();

    surfaceLayer -> SetnextStore(this);
    surfaceLayer -> Setdepth(0.02);    // depth 20 cm

    /*
     *   manure * aManure=new manure(theStore->GetProductPointer()->Getname(),0,nullptr);
     *
     * theProducts->GiveProductInformation(aManure);
     * aManure->Setamount(0);
     * surfaceLayer->SetProductPointer(aManure);
     */
    ...
    ventilated = base::conv.StringToBool(isVentilated);

    original -> FindItem("Area", &surfaceArea);
...
void dungheap::DailyUpdate() {
    ...
    switch (theControlParameters -> GetvolatModel()) {
        ...
        case 1 :
            VolatizeDung(&volatAmmonia, &N2Emission, &N2OEmission, &CO2Emission, &CH4Emission);
 ...
 */
void dungheap::VolatizeDung(double * volatAmmonia,
                            double * N2Emission,
                            double * N2OEmission,
                            double * CO2Emission,
                            double * CH4Emission) {
    if (surfaceLayer -> GetAmountInManureStore() > 0.0) {
        ...
        resistance = 2000.0
                     * (rb + ra + coverResistance + specificResistance);    // rC is the crusts resistance to volatization

        surfaceLayer -> Volatize(Ameantemp, Ahumidity, Aprecip, surfaceArea, resistance, volatAmmonia, &waterEvap,
...
void dungheap::RcvManure(const manure * someManure) {
    ...
    if (theStore -> GetProductPointer() -> GetName() == "SLURRY") {
        theMessage -> FatalError(manurestore::GetName(), " ", "dungheap: attempt to add wrong manure type");
    }

    string theManureName = surfaceLayer -> GetManure() -> GetName();

    if ((tmpManure -> GetName() == "CATTLE-FYM") && (theManureName == "PIG-FYM")) {
        theMessage -> FatalError("dungheap::RcvManure - trying to add cattle FYM to pig FYM");
    }

    if ((tmpManure -> GetName() == "PIG-FYM") && (theManureName == "CATTLE-FYM")) {
    ...
    if ((tmpManure -> GetName() == "FLUID-MANURE") || (tmpManure -> GetName() == "SOLID-MANURE")
            ...
            || (tmpManure -> GetName() == "WATER") || (tmpManure -> GetName() == "PIG-FYM")
        ...
        if (tmpManure -> GetAmount() >= surfaceLayer -> GetCapacity()) {
            ...
            *aManure = *tmpManure;
            ...
            tmpManure -> Setamount(tmpManure -> GetAmount()
                                   - surfaceLayer -> GetCapacity());    // remove this amount from manure sent
            ...
            double surplus = surfaceLayer -> GetAmountManure() + tmpManure -> GetAmount()
                             - surfaceLayer -> GetCapacity();
            ...
            if (surplus > 0.0)                             // surface layer either full or will be filled
            {
                ...
                {
                    aManure = (manure *) surfaceLayer -> GetFromManureStore(surplus,
                            tmpManure);                    // make space for the new manure

                    theStore -> AddToStore(aManure);    // send to main store
                } else {
                    aManure = (manure *) surfaceLayer -> GetFromManureStore(tmpManure -> GetAmount(),
                            tmpManure);                 // make space for the new manure
...
void dungheap::EndBudget(bool show) {
    manure * aManure       = (manure *) theStore -> GetProductPointer();
    manure * surfaceManure = surfaceLayer -> GetManure();

    if ((aManure -> GetpH() < 1.0) || (aManure -> GetpH() > 12.0)) {

#ifdef _STANDALONE
        {
            cout << "dungheap: pH error";

            exit(0);
        }
#else
        theMessage -> FatalError(GetAlias(), " ", "dungheap: pH error");
#endif
    ...
    NRemaining += surfaceManure -> GetAmount() * surfaceManure -> GetAllN().n;
    ...
    CRemaining += surfaceManure -> GetAmount() * surfaceManure -> GetC_content();
