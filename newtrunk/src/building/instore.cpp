...
instore::instore(const instore & aInstore):
    manurestore(aInstore)
{
 ...
 */
void instore::Volatize(double * volatAmmonia,
                       double * N2Emission,
                       double * N2OEmission,
                       double * CH4Emission,
                       double * CO2Emission) {
 ...
 */
void instore::floorVolatize(double   airVelocity,
                            double   tempWaterEvap,
                            double   temperature,
                            double   areaSoiled,
                            double   duration,
                            double * volatAmmonia,
    ...
    *volatAmmonia = 0.0;
    ...
    double   resistance = 1 / (velocityConstant * pow(airVelocity, 0.8)
                               * pow(temperature + 273, -1.4));    // Aarninck model from 1997
    ...
    if (areaSoiled > 0.0) {
        ...
        *volatAmmonia = aManure -> InstantaneousVolatAmmonia(temperature, areaSoiled, resistance);
        *volatAmmonia *= duration * areaSoiled * 3600 / 1000.0;    // convert to kg NH3-N
        ...
        *volatAmmonia = aManure -> AdjustNH4N(*volatAmmonia / 1000.0);

        aManure -> SubtractWater(tempWaterEvap / 1000.0);
    }
 ...
 */
void instore::inhouseVolatize(double   airVelocity,
                              double   tempWaterEvap,
                              double   temperature,
                              double   duration,
                              double * volatAmmonia,
...
void instore::Scale(double annualAmount) {
    ...
    if ((getManureName() == "SLURRY") && (StorageCapacity <= 0)) {
        theMessage -> FatalError("instore:: StorageCapacity must be greater than zero if using scaling");
    }
