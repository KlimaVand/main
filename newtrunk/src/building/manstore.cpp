...
manurestore::manurestore(string       aName,
                         const int    aIndex,
                         const base * aOwner,
                         string       manureType) {
    locked            = false;
    CH4Emission       = CO2Emission = CH4Emissionfactor = CO2Emissionfactor = 0.0;
    N2Emissionfactor  = N2OEmissionfactor = N2Emission = N2OEmission = 0.0;
    NH3Emissionfactor = NOEmission = NH3Emission = NMinfactor = depth = 0.0;
    OMdegRateConst    = cumvolat = surfaceArea = 0.0;
    postCleanDepth    = false;
    manureToIndex     = 0;
    detailsFile       = nullptr;
    external          = 0;
    nextStore         = nullptr;
    ManureToID        = manureStoreID = 0;
    aBase             = new base(aName, aIndex, aOwner);
    theStore          = new storage();

    manure * aManure = new manure(manureType, 0, nullptr);
...
manurestore::manurestore(string        aName,
                         ...
                         commonData *& original) {
    aBase        = new base(aName, aIndex, aOwner);
    CH4Emission  = CO2Emission = N2OEmission = NH3Emission = 0.0;
    NOEmission   = N2Emission = 0.0;
    theStore     = new storage(aName, aIndex, aOwner, 0.0, runData, original);
    manureToName = " ";
    nextStore    = nullptr;
    ...
    original -> FindItem("ManureStoreID", &manureStoreID);
    ...
    original -> FindItem("ManureToIndex", &manureToIndex);
    ...
    original -> FindItem("Locked", &locked);
    ...
    switch (theControlParameters -> GetvolatModel()) {
        case 0 : {
            ...
            original -> FindItem("NMinfactor", &NMinfactor);
        ...
        case 1 : {
            ...
            original -> FindItem("Depth", &depth);
            runData -> FindItem("Depth", &depth);
            original -> FindItem("OMdegRateConst", &OMdegRateConst);
...
manurestore::manurestore(const manurestore & s) {
    NOEmission = N2OEmission = N2Emission = NH3Emission = 0.0;
    ManureToID = 0;

    theMessage -> FatalError("manurestore::manurestore has not been used for a while");

    aBase             = new base(s.aBase -> GetName(), s.aBase -> GetIndex(), s.aBase -> GetOwner());
    CH4Emission       = CO2Emission = depth = 0.0;
...
void manurestore::RcvManure(const manure * someManure) {
    ...
    if (!theStore -> GetProductPointer()) {
        manure * tmpManureConst = (manure *) someManure -> clone();

        theStore -> SetProductPointer(tmpManureConst);
    } else if (theStore -> GetProductPointer() -> GetName() == "SLURRY") {
        if ((tmpManure -> GetName() == "FLUID-MANURE") || (tmpManure -> GetName() == "SOLID-MANURE")
                || (tmpManure -> GetName() == "CATTLE-URINE") || (tmpManure -> GetName() == "CATTLE-FAECES")
                || (tmpManure -> GetName() == "PIG-URINE") || (tmpManure -> GetName() == "PIG-FAECES")
                || (tmpManure -> GetName() == "WASHWATER") || (tmpManure -> GetName(), "STRAW")
                || (tmpManure -> GetName() == "WATER") || (tmpManure -> GetName(), "SLURRY")
                || (tmpManure -> GetName() == "WHEATSTRAW")) {
            tmpManure -> Setname("SLURRY");
            theStore -> AddToStore(tmpManure);
        } else {
            cout << tmpManure -> GetName() << endl;
        ...
        if ((tmpManure -> GetName() == "FLUID-MANURE") || (tmpManure -> GetName() == "SOLID-MANURE")
                || (tmpManure -> GetName() == "WASHWATER") || (tmpManure -> GetName(), "STRAW")
                || (tmpManure -> GetName() == "CATTLE-URINE") || (tmpManure -> GetName() == "CATTLE-FAECES")
                || (tmpManure -> GetName() == "WHEATSTRAW") || (tmpManure -> GetName() == "WATER")
                || (tmpManure -> GetName() == "CATTLE-FYM")) {
 ...
 */
void manurestore::SendManure(const double amount,
                             manure *     aManure) {
    product * tmpManure;
    ...
    if (tmpManure) {
        ...
        *aManure + *tmpManure;
        ...
        Nbudget.AddOutput(aManure -> GetAmount() * aManure -> GetAllN().n);
        Cbudget.AddOutput(aManure -> GetAmount() * aManure -> GetC_content());
    }
}
 ...
 */
void manurestore::Volatize(double   Ameantemp,
                           double   relHumidity,
    ...
    if ((surfaceArea > 0.0) && (GetAmountInManureStore() > 0.0)) {
        ...
        *volatAmmonia = aManure -> VolatAmmonia(Ameantemp, evaporation, infiltration, precipitation, surfaceArea,
                resistance, duration, &evaporation, nullptr);
 ...
 */
product * manurestore::GetFromManureStore(double RequestedAmount,
        product *                                p,
        bool                                     checkAccess) {
    ...
    bool      permitAcccess   =
        true;    // gatekeeper - ensures that fields do not steal manure from floors, inhouse pits etc
    ...
    if (permitAcccess) {
        if (theStore -> ProductCompare(p)) {
            ...
            // manure *tempManure=(manure*) theStore->GetFromStore(RequestedAmount);
 ...
 */
void manurestore::Scale(double anAmount) {
    SetCapacity(GetCapacity() + anAmount);

    surfaceArea += anAmount / depth;
}
 ...
 */
void manurestore::Volatize(double * Nlost,
                           double * Clost,
                           double * volatAmmonia,
                           double * N2Emission,
                           double * N2OEmission,
    ...
    *Clost = 0.0;
    ...
    if (aManure -> GetAmount() > 0.0) {
        aManure -> VolatAndMinNitrogen(NH3Emissionfactor, N2Emissionfactor, N2OEmissionfactor, NMinfactor, &NH3vol,
                                       &N2vol, &N2Ovol, &MinN);
    }
 ...
 */
product * manurestore::EmptyManureStore(bool checkAccess) {
    product * aManure       = nullptr;
    ...
    if (permitAcccess) {
        ...
        if (doubleGreater(amountStored, 0, 0.0000001) != 0) {
            aManure = theStore -> GetFromStore(amountStored);

            Nbudget.AddOutput(aManure -> GetAmount() * ((manure *) aManure) -> GetAllN().n);
            Cbudget.AddOutput(aManure -> GetAmount() * ((manure *) aManure) -> GetC_content());
        }
    }
...
void manurestore::OpenDetailsFile() {
    ...
    namestr     += "(" + conv.IntToString(aBase -> GetIndex()) + ").xls";
...
void manurestore::ShowManureStore(fstream * afile,
                                  bool      header) {
    if (header) {
        *afile << "Day" << "\t" << "Name" << "\t" << "StoredAmount" << "\t" << "StoredDM" << "\t" << "StoredC" << "\t"
               << "StoredN" << "\t" << "CH4Emission" << "\t" << "CO2Emission" << "\t" << "volatAmmonia" << "\t"
               << "N2Emission" << "\t" << "N2OEmission" << "NOEmission" << "\t";
    } else {
        *afile << theTime.GetJulianDay() << "\t" << GetAlias() << "\t" << GetAmountManure() << "\t"
               << GetTotalDMStored() << "\t" << GetTotalCStored() << "\t" << GetTotalNStored() << "\t" << CH4Emission
