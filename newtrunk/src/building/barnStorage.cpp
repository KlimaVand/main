...
barnStorage::barnStorage(string        aName,
                         const int     aIndex,
                         const base *  aOwner,
                         commonData *& runData,
                         commonData *& original):
        ...
        if (!aStore -> ProductCompare(p) == 0) {
            ...
            if (aStore -> GetCapacity() < AmountStored) {
                if (scalable) {
                    ...
                    theMessage -> FatalError("barnStorage::SendToStore: not enoug room in store for product: ",
                                             p -> GetName());
                }
            }
        }
 ...
 */
product * barnStorage::GetFromStore(double RequestedAmount,
        product *                          p) {
    int       NrOfStores   = stores -> NumOfNodes();
