...
buildings::buildings(string       aName,
                     const int    aIndex,
                     const base * aOwner):
    base(aName,
         aIndex,
 ...
 */
animalSection * buildings::PlaceAnimal(
        string name) {    // Note - this allows more than one animal type to be placed in a given section
...
product * buildings::ExtractProduct(product * p) {
    ...
    double        requestedamount = p -> GetAmount(),
                  gottenamount    = 0;
    ...
    if (p -> GetObjType() != manureObj) {
        ...
        while ((stb) && (gottenamount < requestedamount)) {
    ...
    if (p -> GetObjType() == manureObj) {
        ...
        while ((slb) && (gottenamount < requestedamount)) {
        ...
        while ((s) && (gottenamount < requestedamount)) {
            aStable     = s -> element;
            ...
            if (tempProduct) {
                ...
                theOutput -> AddIndicator(3580, "35.80 N transf. from stable to store", "kg N",
                                          (((manure *) tempProduct) -> GetAllN().n * tempProduct -> GetAmount())
                                          * 1000.0);
                theOutput -> AddIndicator(4580, "45.80 C transf. from stable to store", "kg C",
                                          (((manure *) tempProduct) -> GetC_content() * tempProduct -> GetAmount())
                                          * 1000.0);
                *productFound + *tempProduct;
 ...
 */
void buildings::InitializeAll(string InputFilename,
                              bool   pigs,
                              bool   dairy,
                              bool   beef,
                              bool   ScaleBuildings) {
    ...
    if (runData -> readFile(InputFilename)) {
        ...
        if (pigs) {
            runData -> getSectionsNumbers("PigHousing", &small, &large);

            for (int inx = small; inx <= large; inx++) {
                PigHousing * stableInstance = new PigHousing("PigHousing", inx, this, runData, common);
