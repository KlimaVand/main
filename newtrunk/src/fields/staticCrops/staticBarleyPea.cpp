...
void staticBarleyPea::SpecifiedPlan(fieldOrder * PP,
        int                                      JB_NO,
        string                                   crop_id) {
    staticCrop::SpecifiedPlan(PP, JB_NO, crop_id);
}

cloneList<fieldOperationFields> * staticBarleyPea::MakeOpList(fieldOrder * PP,
        int                                                                JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        {       // Ploughing
            ...
            FO -> DefineOp(Ploughing, cropName, cropName, PP -> GetStartYear() + 1, 3, 28, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if ((PP -> GetK() > 0) || (PP -> GetP() > 0)) {    // Spread mineral fertilizer
            ...
            FO -> DefineMineralFertFieldsOp(MineralFertilizing, cropName, cropName, PP -> GetStartYear() + 1, 3, 29,
        ...
        {    // 1st seed bed harrowing
            ...
            FO -> DefineOp(SeedbedHarrowing, cropName, cropName, PP -> GetStartYear() + 1, 3, 30, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (JB_NO > 4) {    // 2nd seed bed harrowing on clayish soil
            ...
            FO -> DefineOp(SeedbedHarrowing, cropName, cropName, PP -> GetStartYear() + 1, 3, 31, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (PP -> GetFluidManureSpring() > 0) {    // Apply FLUID manure
            ...
            FO -> DefineAnimalFertFieldsOp(SpreadingLiquidManure, cropName, cropName, PP -> GetStartYear() + 1, 3, 16,
        ...
        {    // Sowing
            ...
            aSeed2 -> Setamount(83);
            FO -> DefineSowFieldsOp(Sowing, cropName, cropName, PP -> GetStartYear() + 1, 4, 1, PP -> GetArea(), 0,
                                    PP -> GetDistance(), PP -> GetPVStrat(), 0, aSeed1);
        ...
        if (PP -> GetPVStrat() == 0) {    // Weed harrowing
            ...
            FO -> DefineOp(WeedHarrowing, cropName, cropName, PP -> GetStartYear() + 1, 4, 15, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (PP -> GetPVStrat() == 0) {    // Weed harrowing
            ...
            FO -> DefineOp(WeedHarrowing, cropName, cropName, PP -> GetStartYear() + 1, 4, 29, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if ((JB_NO <= 4) && irrigation) {    // Irrigating 30 mm on sandy soil
            ...
            FO -> DefineIrrigateFieldsOp(Irrigating, cropName, cropName, PP -> GetStartYear() + 1, 6, 20,
        ...
        {    // Crop monitoring - hours for the spring & summer
            ...
            FO -> DefineManualLabourFieldsOp(MonitoringCrop, cropName, cropName, PP -> GetStartYear() + 1, 7, 30,
        ...
        {    // Combine harvesting
            ...
            FO -> DefineHarvestFieldsOper(CombineHarvestingShred, cropName, cropName, PP -> GetStartYear() + 1, 8, 15,
        ...
        {    // Stub harrowing
            ...
            FO -> DefineOp(StubHarrowing, cropName, cropName, PP -> GetStartYear() + 1, 8, 16, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
    }
...
double staticBarleyPea::NitrogenAmount(int JB_NO,
        double                             free_N,
        int                                N_level) {
...
double staticBarleyPea::ExpectedYield_(int JB_NO,
        int                                N_level,
        int                                PV_strat) {
