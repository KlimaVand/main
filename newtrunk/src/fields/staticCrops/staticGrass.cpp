...
void staticGrass::SpecifiedPlan(fieldOrder * PP,
                                int          JB_NO,
                                string       crop_id) {
...
cloneList<fieldOperationFields> * staticGrass::MakeOpList(fieldOrder * PP,
        int                                                            JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        if ((PP -> GetN() > 0) || (PP -> GetP() > 0) || (PP -> GetK() > 0)) {    // Apply mineral fertilizer
            ...
            FO -> DefineMineralFertFieldsOp(MineralFertilizing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 15,
