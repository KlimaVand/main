...
void staticSrape::SpecifiedPlan(fieldOrder * PP,
                                int          JB_NO,
                                string       crop_id) {
    staticCrop::SpecifiedPlan(PP, JB_NO, crop_id);

    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning("static_sprape::SpecifiedPlan - more than one harvest date specified");
    }
}

cloneList<fieldOperationFields> * staticSrape::MakeOpList(fieldOrder * PP,
        int                                                            JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        {    // Skårlægning
            ...
            aPlantItem -> Setamount(ExpectedYield_(JB_NO, PP -> GetNLevel(), PP -> GetPVStrat())
                                    + ExpectedStrawYield_(JB_NO, PP -> GetNLevel(), PP -> GetPVStrat()));

            harvestFields * FO = new harvestFields;

            FO -> DefineHarvestFieldsOper(Mowing, crop_id, cropName, PP -> GetStartYear() + 1, 7, 20, PP -> GetArea(),
                                          0, PP -> GetDistance(), 0, false, PP -> GetFractionStrawHarvested(0),
...
double staticSrape::NitrogenAmount(int    JB_NO,
                                   double free_N,
                                   int    N_level) {
    double N = Nopt(JB_NO);
...
double staticSrape::ExpectedYield_(int JB_NO,
                                   int N_level,
                                   int PV_strat) {
