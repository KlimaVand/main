...
void staticGrassEstablished::SpecifiedPlan(fieldOrder * PP,
        int                                             JB_NO,
        string                                          crop_id) {
    ...
    for (int i = 0; i < PP -> GetNumOfHarvestDates(); i++) {
        ...
        FO -> DefineHarvestFieldsOper(ForageHarvesting, crop_id, cropName, PP -> GetYear(date), PP -> GetMonth(date),
    ...
    for (int i = 0; i < PP -> GetNumOfGrazingStartDates(); i++) {
        ...
        FO -> DefinemakeGrazableOper(MakeGrazable, crop_id, cropName, PP -> GetYear(date) + yearOffset,
                                     PP -> GetMonth(date), PP -> GetDay(date), 0.0, 0, 0.0, true);
        AddOpList(FO);
    }
    ...
    for (int i = 0; i < PP -> GetNumOfGrazingStopDates(); i++) {
        ...
        FO -> DefinemakeGrazableOper(MakeGrazable, crop_id, cropName, PP -> GetYear(date) + yearOffset,
                                     PP -> GetMonth(date), PP -> GetDay(date), 0.0, 0, 0.0, false);
        AddOpList(FO);
    }
}

cloneList<fieldOperationFields> * staticGrassEstablished::MakeOpList(fieldOrder * PP,
        int                                                                       JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        if ((PP -> GetN() > 0) || (PP -> GetP() > 0) || (PP -> GetK() > 0)) {
            ...
            FO -> DefineMineralFertFieldsOp(MineralFertilizing, crop_id, cropName, PP -> GetStartYear() + 1, 5, 15,
                                            PP -> GetArea(), 0, PP -> GetDistance(), aFertilizer1, aFertilizer2,
                                            aFertilizer3);
            AddOpList(FO);
        ...
        if ((PP -> GetN() > 0) || (PP -> GetP() > 0) || (PP -> GetK() > 0)) {
            ...
            FO -> DefineMineralFertFieldsOp(MineralFertilizing, crop_id, cropName, PP -> GetStartYear() + 1, 7, 1,
                                            PP -> GetArea(), 0, PP -> GetDistance(), aFertilizer1, aFertilizer2,
                                            aFertilizer3);
            AddOpList(FO);
        ...
        //
        {
            ...
            FO -> DefinemakeGrazableOper(MakeGrazable, crop_id, cropName, PP -> GetStartYear(), 8, 1, PP -> GetArea(),
                                         0, 0.0, true);
            AddOpList(FO);

            FO = new makeGrazable();

            FO -> DefinemakeGrazableOper(MakeGrazable, crop_id, cropName, PP -> GetStartYear() + 1, 7, 31,
                                         PP -> GetArea(), 0, 0.0, false);
            AddOpList(FO);
        }
    }
