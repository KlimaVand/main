...
void staticRye::SpecifiedPlan(fieldOrder * PP,
                              int          JB_NO,
                              string       crop_id) {
    staticCrop::SpecifiedPlan(PP, JB_NO, crop_id);

    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning("static_rye::SpecifiedPlan - more than one harvest date specified");
    }
}

cloneList<fieldOperationFields> * staticRye::MakeOpList(fieldOrder * PP,
        int                                                          JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        if (PP -> GetSolidManure() > 0) {    // Apply solid manure
            ...
            FO -> DefineAnimalFertFieldsOp(SpreadingSolidManure, crop_id, cropName, PP -> GetStartYear(), 9, 30,
        ...
        {    // Plough
            ...
            FO -> DefineOp(Ploughing, crop_id, cropName, PP -> GetStartYear(), 10, 1, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        {    // 1st seed bed harrowing
            ...
            FO -> DefineOp(SeedbedHarrowing, crop_id, cropName, PP -> GetStartYear(), 10, 2, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (JB_NO > 4) {    // 2nd seed bed harrowing (only clayish soils)
            ...
            FO -> DefineOp(SeedbedHarrowing, crop_id, cropName, PP -> GetStartYear(), 10, 3, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        {                                                         // Sowing
            ...
            FO -> DefineSowFieldsOp(Sowing, crop_id, cropName, PP -> GetStartYear(), 10, 4, PP -> GetArea(), 0,
        ...
        {    // Plantprotection registrations - hours for the fall
            ...
            FO -> DefineManualLabourFieldsOp(MonitoringCrop, crop_id, cropName, PP -> GetStartYear(), 11, 1,
        ...
        if (PP -> GetPVStrat() == 2) {    // Spraying herbicide
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear(), 11, 15, PP -> GetArea(),
        ...
        if (PP -> GetPVStrat() == 0) {    // Weedharrowing
            ...
            FO -> DefineOp(WeedHarrowing, crop_id, cropName, PP -> GetStartYear(), 11, 15, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
...
double staticRye::NitrogenAmount(int    JB_NO,
                                 double free_N,
                                 int    N_level) {
    double N = Nopt(JB_NO);
...
double staticRye::ExpectedYield_(int JB_NO,
                                 int N_level,
                                 int PV_strat) {
...
double staticRye::ExpectedStrawYield_(int JB_NO,
        int                               N_level,
        int                               PV_strat) {
    if (gatheringStraw) {
