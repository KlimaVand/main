 ...
 */
void staticSeedGrass::SpecifiedPlan(fieldOrder * PP,
        int                                      JB_NO,
        string                                   crop_id) {
    staticCrop::SpecifiedPlan(PP, JB_NO, crop_id);

    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning("static_seedgrass::SpecifiedPlan - more than one harvest date specified");
    }
...
cloneList<fieldOperationFields> * staticSeedGrass::MakeOpList(fieldOrder * PP,
        int                                                                JB_NO) {
...
double staticSeedGrass::ExpectedYield_(int JB_NO,
        int                                N_level,
        int                                PV_strat) {
    return 1.0;
}
