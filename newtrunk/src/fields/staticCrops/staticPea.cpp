...
void staticPea::SpecifiedPlan(fieldOrder * PP,
                              int          JB_NO,
                              string       crop_id) {
    staticCrop::SpecifiedPlan(PP, JB_NO, crop_id);

    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning("static_pea::SpecifiedPlan - more than one harvest date specified");
    }
}

cloneList<fieldOperationFields> * staticPea::MakeOpList(fieldOrder * PP,
        int                                                          JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        if ((PP -> GetN() > 0) || (PP -> GetK() > 0) || (PP -> GetP() > 0)) {    // Spread mineral fertilizer
            ...
            FO -> DefineMineralFertFieldsOp(MineralFertilizing, crop_id, cropName, PP -> GetStartYear() + 1, 3, 29,
        ...
        {    // 1st seed bed harrowing
            ...
            FO -> DefineOp(SeedbedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 3, 30, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (JB_NO > 4) {    // 2nd seed bed harrowing on clayish soil
            ...
            FO -> DefineOp(SeedbedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 3, 31, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        {                                                         // Sowing
            ...
            FO -> DefineSowFieldsOp(Sowing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 1, PP -> GetArea(), 0,
        ...
        if (PP -> GetPVStrat() == 0) {    // Weed harrowing
            ...
            FO -> DefineOp(WeedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 15, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (PP -> GetPVStrat() > 0) {    // Spraying
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear() + 1, 4, 15,
        ...
        if (PP -> GetPVStrat() == 0) {    // Weed harrowing
            ...
            FO -> DefineOp(WeedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 29, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (PP -> GetPVStrat() > 0) {    // Spraying
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear() + 1, 4, 28,
                                          PP -> GetArea(), 0, PP -> GetDistance(), aChemical1, aChemical2, aChemical3,
                                          aWater);
            AddOpList(FO);
        ...
        if (PP -> GetPVStrat() > 1) {    // Spraying against pests
            chemical * aChemical1 = new chemical;
        ...
        if ((JB_NO <= 4) && irrigation) {    // Irrigating 30 mm on sandy soil
            ...
            FO -> DefineIrrigateFieldsOp(Irrigating, crop_id, cropName, PP -> GetStartYear() + 1, 6, 20,
        ...
        {    // Combine harvesting
            ...
            FO -> DefineHarvestFieldsOper(CombineHarvestingShred, crop_id, cropName, PP -> GetStartYear() + 1, 8, 15,
        ...
        {    // Stub harrowing
            ...
            FO -> DefineOp(StubHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 8, 16, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
    }
...
double staticPea::NitrogenAmount(int    JB_NO,
                                 double free_N,
                                 int    N_level) {
    return 0;
}

double staticPea::ExpectedYield_(int JB_NO,
                                 int N_level,
                                 int PV_strat) {
