...
void staticBeet::SpecifiedPlan(fieldOrder * PP,
                               int          JB_NO,
                               string       crop_id) {
    ...
    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning("static_beet::SpecifiedPlan - more than one harvest date specified");
    }

    if (PP -> GetNumOfHarvestDates() > 0) {
        string * date  = PP -> GetHarvestDate(0);
        int      year  = PP -> GetYear(date);
        ...
        FO -> DefineHarvestFieldsOper(BeetHarvesting, crop_id, cropName, year, month, day, PP -> GetArea(), 0,
...
cloneList<fieldOperationFields> * staticBeet::MakeOpList(fieldOrder * PP,
        int                                                           JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        if (PP -> GetPVStrat() == 0) {
            theMessage -> Warning("static_beet::MakeOpList - Hand weeding of beets not implemented yet");
        }
        ...
        if (PP -> GetSolidManure() > 0) {    // Spread solid manure
            ...
            FO -> DefineAnimalFertFieldsOp(SpreadingSolidManure, crop_id, cropName, PP -> GetStartYear() + 1, 3, 27,
        ...
        if (PP -> GetFluidManureSpring() > 0.0)    // Apply FLUID manure
        {
            ...
            FO -> DefineAnimalFertFieldsOp(SpreadingLiquidManure, crop_id, cropName, PP -> GetStartYear() + 1, 4, 4,
                                           PP -> GetArea(), 0, PP -> GetDistance(), aManure);
            AddOpList(FO);
        ...
        if (JB_NO <= 4) {    // 3. seed bed harrowing (only clayish soils)
            ...
            FO -> DefineOp(SeedbedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 8, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        {                                                         // Sowing
            ...
            FO -> DefineSowFieldsOp(SowingBeets, crop_id, cropName, PP -> GetStartYear() + 1, 4, 10, PP -> GetArea(),
                                    0, PP -> GetDistance(), PP -> GetPVStrat(), 0, aSeed);
            AddOpList(FO);
        ...
        if (PP -> GetPVStrat() <= 1)    // Row cultivating 1. time
        {
            soilTreatFields * FO = new soilTreatFields;

            FO -> DefineOp(RowCultivating, crop_id, cropName, PP -> GetStartYear() + 1, 5, 15, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }

        if (PP -> GetPVStrat() <= 1)    // Row cultivating 2. time
        {
            soilTreatFields * FO = new soilTreatFields;

            FO -> DefineOp(RowCultivating, crop_id, cropName, PP -> GetStartYear() + 1, 6, 2, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }

        if (PP -> GetPVStrat() <= 1)    // Row cultivating 3. time
        {
            soilTreatFields * FO = new soilTreatFields;

            FO -> DefineOp(RowCultivating, crop_id, cropName, PP -> GetStartYear() + 1, 6, 20, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }

        if (PP -> GetPVStrat() > 0)    // Spraying herbicide 1. time
        {                              // Mangler forskel mellem bånd- og bredsprøjtning. Er det rette kemikalier? !!!
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear() + 1, 5, 1, PP -> GetArea(),
        ...
        if (PP -> GetPVStrat() > 0)    // Spraying herbicide 2. time
        {                              // Mangler forskel mellem bånd- og bredsprøjtning. Er det rette kemikalier? !!!
            chemical * aChemical1 = new chemical;
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear() + 1, 5, 10,
                                          PP -> GetArea(), 0, PP -> GetDistance(), aChemical1, aChemical2, nullptr,
                                          aWater);
            AddOpList(FO);
        ...
        if (PP -> GetFluidManureSpring() > MaxGylle)    // Apply FLUID manure 2. time
        {
            ...
            FO -> DefineAnimalFertFieldsOp(SpreadingLiquidManure, crop_id, cropName, PP -> GetStartYear() + 1, 5, 31,
                                           PP -> GetArea(), 0, PP -> GetDistance(), aManure);
            AddOpList(FO);
        ...
        if (PP -> GetPVStrat() > 0)    // Spraying herbicide 3. time + insecticide 1. time
        {                              // Mangler forskel mellem bånd- og bredsprøjtning. Er det rette kemikalier? !!!
            chemical * aChemical1 = new chemical;
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear() + 1, 6, 1, PP -> GetArea(),
                                          0, PP -> GetDistance(), aChemical1, aChemical2, aChemical3, aWater);
            AddOpList(FO);
        ...
        if (PP -> GetPVStrat() > 0)    // Spraying insecticide 2. time
        {                              // Mangler forskel mellem bånd- og bredsprøjtning. Er det rette kemikalier? !!!
            chemical * aChemical1 = new chemical;
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear() + 1, 7, 1, PP -> GetArea(),
                                          0, PP -> GetDistance(), aChemical1, nullptr, nullptr, aWater);
            AddOpList(FO);
        ...
        if ((JB_NO <= 4) && irrigation) {    // Irrigate 30 mm on sandy soils
            ...
            FO -> DefineIrrigateFieldsOp(Irrigating, crop_id, cropName, PP -> GetStartYear() + 1, 7, 15,
        ...
        {    // Plantprotection registrations - hours for the whole season
            ...
            FO -> DefineManualLabourFieldsOp(MonitoringCrop, crop_id, cropName, PP -> GetStartYear() + 1, 10, 1,
                                             PP -> GetArea(), 0, PP -> GetDistance(), 0, 1.5);
            AddOpList(FO);
        }
        {    // Removing top
            theProducts -> GiveProductInformation(aPlantItem2);
            ...
            FO -> DefineHarvestFieldsOper(BeetTopChopping, crop_id, cropName, PP -> GetStartYear() + 1, 11, 1,
        ...
        {    // Harvesting beets
            ...
            FO -> DefineHarvestFieldsOper(BeetHarvesting, crop_id, cropName, PP -> GetStartYear() + 1, 11, 2,
                                          PP -> GetArea(), 0, PP -> GetDistance(), 0, true,
                                          PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
            FO -> SetMainProduct(aPlantItem1);
            FO -> SetStrawProduct(aPlantItem2);
...
double staticBeet::NitrogenAmount(int    JB_NO,
                                  double free_N,
                                  int    N_level) {
    double N = Nopt(JB_NO);
...
double staticBeet::ExpectedYield_(int JB_NO,
                                  int N_level,
                                  int PV_strat) {
...
#ifdef MEASCOPE
void staticBeet::SpecifiedPeriods(fieldOrder * PP,
                                  int          JB_NO,
                                  string       crop_id) {
    ...
    if (PP -> GetNumOfHarvestPeriods() > 0) {
        ...
        FO -> DefineHarvestFieldsOper(BeetHarvesting, crop_id, cropName, PP -> GetYear(sdate) + yearOffset,
    ...
    for (int i = 0; i < PP -> GetNumOfCuttingPeriods(); i++) {
        ...
        switch (processProduct) {
            case 0 :
            case 1 :
                FO -> DefineHarvestFieldsOper(BeetTopHarvesting, crop_id, cropName, PP -> GetYear(sdate) + yearOffset,
                ...
                FO1 -> DefineHarvestFieldsOper(GatheringSilage, crop_id, cropName, PP -> GetYear(sdate) + yearOffset,
