...
void staticPotato::SpecifiedPlan(fieldOrder * PP,
                                 int          JB_NO,
                                 string       crop_id) {
    staticCrop::SpecifiedPlan(PP, JB_NO, crop_id);

    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning("static_potato::SpecifiedPlan - more than one harvest date specified");
    }
...
cloneList<fieldOperationFields> * staticPotato::MakeOpList(fieldOrder * PP,
        int                                                             JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        {    // Sowing. Trivially wrong, as potatoes are planted, but works technically.
            ...
            FO -> DefineSowFieldsOp(SowingBeets, crop_id, cropName, PP -> GetStartYear() + 1, 4, 20, PP -> GetArea(),
        ...
        {    // Harvesting potatos.
            ...
            FO -> DefineHarvestFieldsOper(CombineHarvestingShred, crop_id, cropName, PP -> GetStartYear() + 1, 9, 15,
...
double staticPotato::NitrogenAmount(int JB_NO,
        double                          free_N,
        int                             N_level) {
    double N = Nopt(JB_NO);
...
double staticPotato::ExpectedYield_(int JB_NO,
        int                             N_level,
        int                             PV_strat) {
