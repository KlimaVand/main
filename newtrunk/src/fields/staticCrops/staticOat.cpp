...
void staticOat::SpecifiedPlan(fieldOrder * PP,
                              int          JB_NO,
                              string       crop_id) {
    ...
    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning(
            "static_oat::SpecifiedPlan - more than one harvest date specified");    // Might not be a problem, is oat with undersown clover/grass
    }
}

cloneList<fieldOperationFields> * staticOat::MakeOpList(fieldOrder * PP,
        int                                                          JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        {                                                         // Sowing
            ...
            if (underSown) {
                ...
                FO -> DefineSowFieldsOp(Sowing, tmp, cropName, PP -> GetStartYear() + 1, 4, 6, PP -> GetArea(), 0,
        ...
        {    // Combine harvesting
            ...
            if (gatheringStraw) {
                FO -> DefineHarvestFieldsOper(CombineHarvesting, crop_id, cropName, PP -> GetStartYear() + 1, 9, 1,
                                              PP -> GetArea(), 0, PP -> GetDistance(), 0, false,
                                              PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
            } else {
                FO -> DefineHarvestFieldsOper(CombineHarvestingShred, crop_id, cropName, PP -> GetStartYear() + 1, 9,
                                              1, PP -> GetArea(), 0, PP -> GetDistance(), 0, false,
                                              PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
            }
            ...
            if (gatheringStraw) {    // Gathering straw
                ...
                FO -> DefineHarvestFieldsOper(GatheringBales, crop_id, cropName, PP -> GetStartYear() + 1, 8, 16,
                                              PP -> GetArea(), 0, PP -> GetDistance(), 0, false,
                                              PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
                FO -> SetStrawProduct(aPlantItem2);
                AddOpList(FO);
...
double staticOat::NitrogenAmount(int    JB_NO,
                                 double free_N,
                                 int    N_level) {
    double N = Nopt(JB_NO);
...
double staticOat::ExpectedYield_(int JB_NO,
                                 int N_level,
                                 int PV_strat) {
 ...
 */
double staticOat::ExpectedStrawYield_(int JB_NO,
        int                               N_level,
        int                               PV_strat) {
    if (gatheringStraw) {
