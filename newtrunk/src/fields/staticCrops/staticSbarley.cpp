...
void staticSbarley::HarvestPlan(fieldOrder * PP,
                                int          JB_NO,
                                string       crop_id) {
    ...
    {    // Combine harvesting
        ...
        if (gatheringStraw) {
            FO -> DefineHarvestFieldsOper(CombineHarvesting, crop_id, cropName, year, month, day, PP -> GetArea(), 0,
                                          PP -> GetDistance(), 1.0, true, PP -> GetFractionStrawHarvested(0),
                                          PP -> GetFractionGrainHarvested(0));
        } else {
            FO -> DefineHarvestFieldsOper(CombineHarvestingShred, crop_id, cropName, year, month, day, PP -> GetArea(),
                                          0, PP -> GetDistance(), 1.0, true, PP -> GetFractionStrawHarvested(0),
                                          PP -> GetFractionGrainHarvested(0));
        }
        ...
        if (gatheringStraw) {    // Gathering straw
            ...
            FO1 -> DefineHarvestFieldsOper(GatheringBales, crop_id, cropName, bs.GetYear(), bs.GetMonth(), bs.GetDay(),
        ...
        for (int i = 1; i < PP -> GetNumOfHarvestDates(); i++) {

            // cooping with several harvesting dates (i.e. clover/grass)
            ...
            FO -> DefineHarvestFieldsOper(CutSetAside, crop_id, cropName, PP -> GetYear(date), PP -> GetMonth(date),
                                          PP -> GetDay(date), PP -> GetArea(), 0, PP -> GetDistance(), 1.0, true,
                                          PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
...
cloneList<fieldOperationFields> * staticSbarley::MakeOpList(fieldOrder * PP,
        int                                                              JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        if (PP -> GetFluidManureSpring() > 0) {    // Spread liquid manure
            ...
            FO -> DefineAnimalFertFieldsOp(SpreadingLiquidManure, crop_id, cropName, PP -> GetStartYear() + 1, 3, 27,
                                           PP -> GetArea(), 0, PP -> GetDistance(), aManure);
            AddOpList(FO);
        ...
        {    // Ploughing
            ...
            FO -> DefineOp(Ploughing, crop_id, cropName, PP -> GetStartYear() + 1, 3, 28, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        {    // 1st seed bed harrowing
            ...
            FO -> DefineOp(SeedbedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 4, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        if (JB_NO > 4) {    // 2nd seed bed harrowing on clayish soil
            ...
            FO -> DefineOp(SeedbedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 5, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        {                                                         // Sowing
            ...
            if (underSown) {
                ...
                FO -> DefineSowFieldsOp(Sowing, input, cropName, PP -> GetStartYear() + 1, 4, 6, PP -> GetArea(), 0,
                                        PP -> GetDistance(), PP -> GetPVStrat(), 0, aSeed);
            } else {
                FO -> DefineSowFieldsOp(Sowing, crop_id, cropName, PP -> GetStartYear() + 1, 4, 6, PP -> GetArea(), 0,
                                        PP -> GetDistance(), PP -> GetPVStrat(), 0, aSeed);
            }
        ...
        if (PP -> GetPVStrat() == 0) {    // Weed harrowing
            soilTreatFields * FO = new soilTreatFields;

            FO -> DefineOp(WeedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 5, 4, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }

        if (PP -> GetPVStrat() > 0) {    // Spraying
            ...
            FO -> DefinePlantCareFieldsOp(Spraying, crop_id, cropName, PP -> GetStartYear() + 1, 5, 6, PP -> GetArea(),
        ...
        if (PP -> GetPVStrat() == 0) {    // Weed harrowing
            ...
            FO -> DefineOp(WeedHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 5, 21, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
        ...
        {    // Crop monitoring - hours for the spring & summer
            ...
            FO -> DefineManualLabourFieldsOp(MonitoringCrop, crop_id, cropName, PP -> GetStartYear() + 1, 7, 30,
...
double staticSbarley::NitrogenAmount(int JB_NO,
        double                           free_N,
        int                              N_level) {
    double N = Nopt(JB_NO);
...
double staticSbarley::ExpectedYield_(int JB_NO,
        int                              N_level,
        int                              PV_strat) {
 ...
 */
double staticSbarley::ExpectedStrawYield_(int JB_NO,
        int                                   N_level,
        int                                   PV_strat) {
    if (gatheringStraw) {
