...
void staticCloverGrass::SpecifiedPlan(fieldOrder * PP,
        int                                        JB_NO,
        string                                     crop_id) {
    staticCrop::SpecifiedPlan(PP, JB_NO, crop_id);
}

cloneList<fieldOperationFields> * staticCloverGrass::MakeOpList(fieldOrder * PP,
        int                                                                  JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        if (PP -> GetFluidManureSpring() > 0) {    // Apply fluid manure
            ...
            FO -> DefineAnimalFertFieldsOp(SpreadingLiquidManure, crop_id, cropName, PP -> GetStartYear() + 1, 3, 16,
...

        {
            ...
            // ensure no grazing at end of year and over winter  //first harvest
            ...
            FO -> DefineHarvestFieldsOper(ForageHarvesting, crop_id, cropName, PP -> GetStartYear() + 1, 6, 1,
        ...
        {    // second harvest
            ...
            FO -> DefineHarvestFieldsOper(ForageHarvesting, crop_id, cropName, PP -> GetStartYear() + 1, 7, 15,
                                          PP -> GetArea(), 0, PP -> GetDistance(), 1.0, true,
                                          PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
            FO -> SetForageProduct(aPlantItem);
            AddOpList(FO);
        ...
        {    // third harvest
            ...
            FO -> DefineHarvestFieldsOper(ForageHarvesting, crop_id, cropName, PP -> GetStartYear() + 1, 9, 5,
                                          PP -> GetArea(), 0, PP -> GetDistance(), 1.0, true,
                                          PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
            FO -> SetForageProduct(aPlantItem);
            AddOpList(FO);
...
double staticCloverGrass::NitrogenAmount(int JB_NO,
        double                               free_N,
        int                                  N_level) {
    return 0;
}

double staticCloverGrass::ExpectedYield_(int JB_NO,
        int                                  N_level,
        int                                  PV_strat) {
