...
void staticMaize::SpecifiedPlan(fieldOrder * PP,
                                int          JB_NO,
                                string       crop_id) {
    ...
    if (PP -> GetNumOfHarvestDates() > 1) {
        theMessage -> Warning(
            "static_maize::SpecifiedPlan - more than one harvest date specified");    // Might not be a problem is barley with undersown clover/grass
    }
}

cloneList<fieldOperationFields> * staticMaize::MakeOpList(fieldOrder * PP,
        int                                                            JB_NO) {
    ...
    if (!PP -> GetAutomatic()) {
        ...
        {                                                         // Sowing
            ...
            if (underSown) {
                ...
                FO -> DefineSowFieldsOp(Sowing, tmp, cropName, PP -> GetStartYear() + 1, 5, 6, PP -> GetArea(), 0,
                                        PP -> GetDistance(), PP -> GetPVStrat(), 0, aSeed);
            } else {
                FO -> DefineSowFieldsOp(Sowing, crop_id, cropName, PP -> GetStartYear() + 1, 5, 6, PP -> GetArea(), 0,
                                        PP -> GetDistance(), PP -> GetPVStrat(), 0, aSeed);
            }
        ...
        {    // Combine harvesting
            ...
            if (gatheringStraw) {
                FO -> DefineHarvestFieldsOper(CombineHarvesting, crop_id, cropName, PP -> GetStartYear() + 1, 9, 20,
            ...
            if (gatheringStraw) {    // Gathering straw
                ...
                FO -> DefineHarvestFieldsOper(GatheringBales, crop_id, cropName, PP -> GetStartYear() + 1, 9, 20,
                                              PP -> GetArea(), 0, PP -> GetDistance(), 0, false,
                                              PP -> GetFractionStrawHarvested(0), PP -> GetFractionGrainHarvested(0));
                FO -> SetStrawProduct(aPlantItem2);
                AddOpList(FO);
        ...
        if (!underSown) {    // Stub harrowing
            ...
            FO -> DefineOp(StubHarrowing, crop_id, cropName, PP -> GetStartYear() + 1, 9, 21, PP -> GetArea(), 0,
                           PP -> GetDistance());
            AddOpList(FO);
        }
    }
...
double staticMaize::NitrogenAmount(int    JB_NO,
                                   double free_N,
                                   int    N_level) {
    double N = Nopt(JB_NO);
...
double staticMaize::ExpectedYield_(int JB_NO,
                                   int N_level,
                                   int PV_strat) {
 ...
 */
double staticMaize::ExpectedStrawYield_(int JB_NO,
        int                                 N_level,
        int                                 PV_strat) {
    if (gatheringStraw) {
