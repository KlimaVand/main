    ...
    public base
{
    protected:
        ...
        double nFixThisDay,        // !amount of nitrogen fixed during the day
               totalGrazed_DM,     // !total amount of dry matter grazed
               totalGrazed_N,      // !total amount nitrogen grazed
               grazingEffiency,    // !percentage of dry matter grazed (may be obsolete)
               leachingDepth,      // ! depth below which it is assumed that nitrate cannot be subsequently recovered by the crop (mm)
               balanceDepth,    // !depth below which nitrogen to use the system  (mm)
               area,            // !area of ecosystem (ha)                  // Present area of ecosystem, ha
               nLoad, k1;
        ...
        void AddHarvestedAreaToIndicators(string cid,
                                          double a);

        // !updates harvest yield indicators
        void AddYieldToIndicators(string cid,
                                  double y,
                                  double n);

        // !an expected yield to indicate the list
        // void AddExpectedYieldToIndicators(string cid, double y);

        /*
         * !
         * \param  surplus           - Water to be added to interception layer and resulting drainage from the layer [mm]
         * \param temperature       - Mean air temperature [ï¿½C]
         * \param globalRadiation   - Global solar radiation [MJ/mï¿½]
         ...
         */
        crop * AllocateCrop(string crop_id);

    public:
		// !default assignment operator
		virtual  ecosystem & operator = (const ecosystem & p);    // Dissable the compilers generation of default assignment operator
		// ! default constructor
		ecosystem(); 
         ...
         */
        void SetGrazed(double grazing_height,
                       double DMGrazed);
        ...
        double GetNFixationThisDay() {
            return nFixThisDay;
        }

        // !Get mass of dry matter (g DM/m**2)of ecosystem that is in excess of a given residual mass (gDM/m**2)
        double GetAvailableStandingDM(double mass);
        ...
        void AddDecomposable(decomposable * produc) {
            aSoil -> AddDecomposable(produc);
        }

        void Incorporate(double depth,
                         double frac,
                         bool   tillage,
                         string tillagetype) {
            aSoil -> Incorporate(depth, frac, tillage, tillagetype);
        }
		...
		void StrawToFieldfuncion(harvestFields * hrv);
