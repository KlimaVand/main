...
void mineralFertFields::DefineMineralFertFieldsOp(operationNames op,
        string                                                   crop,
        string                                                   cropNm,
        int                                                      yr,
        int                                                      mon,
...
bool mineralFertFields::CalcCost(linkList<operation> * aOperationList,
                                 ...
                                 bool                  update) {
    if (!update
            && ((mode == hoursTotalNotCombineNotFert) || (mode == hoursCombine) || (mode == hoursAnimalFert)
                || (mode == hoursPlantCare) || (mode == varCostTotalNotFert) || (mode == varCostAnimalFert))) {
        return false;
    }
    ...
    if (!aOperationNode) {
        theMessage -> WarningWithDisplay("mineralFertFields::CalcCost - Operation ", operationNameList[op_id],
                                         " not found in list");
        ...
        mineralFertTechInstance -> SetcropName(cropName);
        ...
        if (mineralFertTechInstance -> CreateGang(aTechFarmList)) {
            ...
            if (update) {
                ...
                mineralFertTechInstance -> CalcCost(mode);

                *cost = mineralFertTechInstance -> Getcost();
            }
        } else {    // Buy service
            if (update) {
                ...
                theOutput -> AddIndicator(708, "07.08 Cost of machine contractors", "Dkr",
                                          -mineralFertTechInstance -> GetMachinePoolPrice() * area);

                delete hiredService;
            } else if (mode == varCostMineralFert) {
                *cost = -mineralFertTechInstance -> GetMachinePoolPrice();
            } else {
                *cost = 0.0;
            }
        }
