 ...
 */
void animalFertFields::DefineAnimalFertFieldsOp(operationNames op,
        string                                                 crop,
        string                                                 cropNm,
        int                                                    yr,
        int                                                    mon,
...
bool animalFertFields::CalcCost(linkList<operation> * aOperationList,
                                ...
                                bool                  update) {
    if (!update
            && ((mode == hoursCombine) || (mode == hoursMineralFert) || (mode == hoursPlantCare)
                || (mode == varCostTotalNotFert) || (mode == varCostMineralFert))) {
        return false;
    }
    ...
    if (!aOperationNode) {
        theMessage -> WarningWithDisplay("animalFertFields::CalcCost - Operation ", operationNameList[op_id],
                                         " not found in list");
        ...
        if (animalFertTechInstance -> CreateGang(aTechFarmList)) {
            ...
            if (update) {
                ...
                theOutput -> AddIndicator(708, "07.08 Cost of machine contractors", "Dkr",
                                          -animalFertTechInstance -> GetMachinePoolPrice() * aManure -> GetAmount());

                delete hiredService;
            } else if (mode == varCostAnimalFert) {
...
fieldOperationTech * animalFertFields::GetfieldOperationTech(linkList<operation> * aOperationList) {
    ...
    if (!aOperationNode) {
        theMessage -> FatalError("animalFertFields::GetfieldOperationTech - Operation ",
                                 operationNameList[op_id], " not found in list");
...
void animalFertFields::Output(ofstream * fs) {
    *fs << " Manure applied " << aManure -> GetName() << " " << aManure -> GetAmount() << " t/ha "
        << (aManure -> GetAmount() * aManure -> GetAllN().n * 1000.0) << " kgN/ha ";
}
