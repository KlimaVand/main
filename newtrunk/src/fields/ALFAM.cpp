...
ALFAM::ALFAM() {
    ...
    b_met1Km   = 0.3888;
    b_met2Km   = 0.7024;
    applicRate = exposureTime = km = Nmax = TAN = timeElapsed = 0;
}

/*
 * manureType: 0 = cattle, 1 = pig
 */
void ALFAM::initialise(int    soilWet,
                       double aveAirTemp,
                       double aveWindspeed,
                       int    manureType,
                       double initDM,
    ...
    switch (appMeth) {
        case 1 :    // broadcast
            Nmax = exp(b_Nmx0 + b_sm1Nmx * soilWet + b_atNmx * aveAirTemp + b_wsNmx * aveWindspeed
...
double ALFAM::ALFAM_volatilisation(double duration) {
    if (timeElapsed >= exposureTime) {
        ...
        double ret_val = TAN * applicRate * Nmax
                         * ((timeElapsed + duration) / ((timeElapsed + duration) + km)
                            - timeElapsed / (timeElapsed + km));
