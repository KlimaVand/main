 ...
 */
void plan::HandPlanOver(cloneList<fieldOperationFields> * OP,
                        string                            crop_id,
                        int                               futrNum) {
    futureNum = futrNum;
    OpListP   = OP;
    ...
    if (!NextOperation) {
        theMessage -> Warning(
            "plan::HandPlanOver - no operations specified for this field");    // JB changed this 7/11-02 as might not be wrong (i.e. permanent grass)
    } else {
        cloneList<fieldOperationFields>::PS P;
        OpListP -> PeekHead(P);

        while (P) {
            if (P -> element -> GetCropId() == "ADJUSTAREA") {
 ...
 */
fieldOperationFields * plan::ReturnOperation(int year,
        int                                      month,
        int                                      day,
        bool                                     update) {
    if (!NextOperation) {
 ...
 */
bool plan::FirstOperationToday(int year,
                               int month,
                               int day) {
    cloneList<fieldOperationFields>::PS P;

    if ((OpListP) && (OpListP -> NumOfNodes() > 0)) {
        OpListP -> PeekHead(P);

        long int idx = year * 512 + month * 32 + day;
        long int operatindex;

        operatindex = P -> element -> GetYear() * 512 + P -> element -> GetMonth() * 32 + P -> element -> GetDay();
...
void plan::InsertAsNextOperation(fieldOperationFields * anOperation) {
    ...
    if (NextOperation) {
        ...
        for (int i = 0; i < OpListP -> NumOfNodes();
                i++)    // find where InsertBefore put the operation and make it NextOperation
        {
