...
fieldOrder::fieldOrder(string       aName,
                       const int    aIndex,
                       const base * aOwner):
    base(aName,
         aIndex,
 ...
 * function.
 */
void fieldOrder::ReadParameters(commonData *& data, int yearIndex) {
    ...
    SetCropId(st.substr(0, 2));

    if (master < 0) {
        field_no = conv.StringToInt(st.substr(7, 2));
    } else {
        field_no = Index;
    }
    ...
    int    unitType =
        theControlParameters -> GetFertiliserUnit();    // Default value for unitType = 0 -> fertiliser given per ha in file

    if ((unitType < 0) || (unitType > 2)) {
        theMessage -> FatalError("fieldOrder::ReadParameters - 'unitType' has illegal value");
    } else if (unitType
               == 1) {    // Total amount of fertiliser or manure applied to field, option for backward compatability
        factor = 1.0 / area;
    }
    ...
    if (Automatic == 0) {
        ...
        for (int i = first; i <= (size); i++) {
			...
			bsTime time;
			time.SetTime(*s);
			time.AddYear(yearIndex);
			s=new string(time.GetString2());
            ...
            if ((*s != "PLOUGH") && (*s != "HARROW")) {
                ...
                theMessage -> WarningWithDisplay(
                    "fieldOder::ReadParameters Fasset supports only PLOUGH and HARROW as TillageType");
        ...
        for (int i = first; i <= (size); i++) {
            ...
            if (data -> FindItem("SowStartDate", s, i)) {
                theMessage -> FatalError(
                    "fieldOrder::ReadParameters - not using the MEASCOPE compiler option, you pillock");
            }
            ...
            if (*type == "MINERAL") {
                ...
                *MineralFormString = "SOLID";
                ...
                if ((unitType != 1) && (aNitrogen -> n < 10.0)) {
                    ...
                    theMessage -> WarningWithDisplay(
                        "fieldOrder::ReadParameters - N from mineral fertiliser below 10 kg per ha");
                }
                ...
                if ((unitType != 1) && (aNitrogen -> n > 400.0)) {
                    ...
                    theMessage -> WarningWithDisplay(
                        "fieldOrder::ReadParameters - N from mineral fertiliser above 400 kg per ha");
                }
                ...
                *a = *a * factor;
                ...
                if (found == 0) {
                    theMessage -> FatalError(
                        "fieldOrder::ReadParameters - must specify N, P or K content of mineral fertilizer");
                }
            } else if ((*type == "CATTLE-SLURRY-FRESH") || (*type == "CATTLE-SLURRY-STORED")
                       || (*type == "CATTLE-SLURRY-ANAEROB") || (*type == "CATTLE-COMPOST") || (*type == "CATTLE-FYM")
                       || (*type == "PIG-SLURRY-STORED") || (*type == "PIG-SLURRY-ANAEROB") || (*type == "PIG-COMPOST")
                       || (*type == "PIG-FYM") || (*type == "PIG-SLURRY-FRESH") || (*type == "PIG-SLURRY-DEGAS")
                       || (*type == "SLURRY") || (*type == "LIQUID-MANURE") || (*type == "SOLID-MANURE")
                       || (type -> find("PIG-SLURRY-STORED") != std::string::npos)) {
                    ...
                    if (data -> FindItem("OrganicN", &orgN, i)) {    // FarmN project, minor screw-up
                        if (unitType == 2) {                         // FarmN, minor screw-up
                            ...
                            if (f > 5.0) {
                                ...
                                theMessage -> WarningWithDisplay(
                                    "fieldOrder::ReadParameters - adjustment factor for N in animal manure very high");
                            } else if (f < 0.2) {
                                cout << "f = " << f << endl;

                                theMessage -> WarningWithDisplay(
                                    "fieldOrder::ReadParameters - adjustment factor for N in animal manure very low");
                            }
                            ...
                            if ((Norg.n > 1.0) || (Nnh4.n > 1.0) || (Nno3.n > 1.0)) {
                                theMessage -> FatalError(
                                    "fieldOrder::ReadParameters - adjustment of N in animal manure above possible range");
                            }

                            if ((Norg.n + Nnh4.n + Nno3.n) > 0.2) {
                                theMessage -> WarningWithDisplay(
                                    "fieldOrder::ReadParameters - adjustment of N in animal manure above plausible range");
                            }
                            ...
                            theMessage -> FatalError(
                                "fieldOrder::ReadParameters - 'OrganicN' can not be used in this mode");
                        }
                    }
                }
                ...
                if (data -> FindItem("FertilizerN", &amount, i)) {
                    if (unitType != 0) {
                        theMessage -> FatalError(
                            "fieldOrder::ReadParameters - fertiliser unit type must be zero to use <FertiliserN> command");
                    }
                ...
                if (aManure -> GetmainUnitName() == 3) {
                    manureFW = amount
                               / (aManure -> GetAllN().n * 1000.0);    // convert from kg N/ha to tonnes fresh manure/ha
                } else {
                ...
                if (manureFW > 0.0) {
                    if (manureFW < 5.0) {
                        cout << endl << "Manure amount " << manureFW << " t/ha, containing "
                             << (1000.0 * manureFW * aManure -> GetTotalN().n) << " kg N/ha" << endl;

                        theMessage -> WarningWithDisplay(
                            "fieldOrder::ReadParameters - manure amount below 5 tonnes per ha");
                    } else if (manureFW > 100.0) {
                        cout << endl << "Manure amount " << manureFW << " t/ha, containing "
                             << (1000.0 * manureFW * aManure -> GetTotalN().n) << " kg N/ha" << endl;

                        theMessage -> WarningWithDisplay(
                            "fieldOrder::ReadParameters - manure amount above 100 tonnes per ha");
                    }

#ifdef TUPLE
                    if (area > 0.001) {
                        AddTuple(*s, aManure -> GetTotalN().n * manureFW * area * 1000.0, manureFW * area,
		...
		data->FindVectorSize("AnimalOnFieldStart", &first, &size);
		...
		for (int i = first; i <= (size); i++) {
			...
			data->FindItem("AnimalOnFieldStart", s, i);
			CheckDateStringForEmpty(s);
			bsTime time;
			time.SetTime(*s);
			time.AddYear(yearIndex);
			...
			AnimalOnFieldStart.InsertLast(s);

			

			
			string * a = new string;
			data->FindItem("AnimalOnFieldType", a, i);

			
			AnimalOnFieldType.InsertLast(a);

			int * amount= new int;
			data->FindItem("AnimalOnFieldAmount", amount, i);
			AnimalOnFieldAmount.InsertLast(amount);
		}

		data->FindVectorSize("AnimalOnFieldStop", &first, &size);
		...
		for (int i = first; i <= (size); i++) {
			...
			data->FindItem("AnimalOnFieldStop", s, i);
			CheckDateStringForEmpty(s);
			bsTime time;
			time.SetTime(*s);
			time.AddYear(yearIndex);
			...
			AnimalOnFieldStop.InsertLast(s);
			...
			string * a = new string;
			data->FindItem("AnimalOnFieldTypeStop", a, i);


			AnimalOnFieldTypeStop.InsertLast(a);

			int * amount = new int;
			data->FindItem("AnimalOnFieldAmountStop", amount, i);


			AnimalOnFieldAmountStop.InsertLast(amount);
        ...
        bool solidHugUsed = data -> FindItem("DUNG.PS", &solid_hug);
        ...
        if (data -> FindItem("SLUR.P00", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P01", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P02", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P03", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P04", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P05", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P06", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P07", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P08", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P09", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P10", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }

        if (data -> FindItem("SLUR.P11", &a)) {
            theMessage -> FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
        }
 ...
 */
void fieldOrder::AddTuple(string date,
                          double n,
                          double amount,
                          string op,
                          string type,
