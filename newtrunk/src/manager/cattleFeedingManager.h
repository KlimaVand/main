...
public:
    ...
    cattleFeedingManager(string       aName,
                         const int    aIndex,
                         const base * aOwner);

    ~cattleFeedingManager();

    void SetdairyPresent(bool isPresent)
    {
        dairyPresent = isPresent;
    }

    bool GetdairyPresent()
    {
        return dairyPresent;
    }

    void SetbeefPresent(bool isPresent)
    {
        beefPresent = isPresent;
    }
    ...
    cattleFeedingGroup * GetGroupMember(int elementNum)
    {
        return feedingGroups -> ElementAtNumber(elementNum);
    }
    ...
    double GetHerbageRequired(int  start,
                              int  end,
                              bool zeroGrazed);

    void weanCalves(int    herdNo,
    ...
    void AllocateFieldsForGrazing(linkList<field> * fields_avail_for_grazing,
                                  int               numOthers);

    bsTime * GetTurnoutDate()
    {
        return turnoutDate;
    }

    bsTime * GetHousingDate()
    {
        return housingDate;
    }
    ...
    double GetDairyHerbageRequired(bool zeroGrazed)
    {
        return GetHerbageRequired(dairyLo, dairyHi, zeroGrazed);
    }

    double GetFollowerHerbageRequired(bool zeroGrazed)
    {
        return GetHerbageRequired(0, dairyLo - 1, zeroGrazed);
    }

    int GetNumFeedingGroups()
    {
        return feedingGroups -> NumOfNodes();
    }
    ...
    void AutoFeed(int conceptionTime,
