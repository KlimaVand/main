                                ...
                                cattleFeedingManager * theCattleFeedingManagerptr)
{
    ...
    {
        if (theLivestock -> getanimalStock() -> ElementAtNumber(i) -> GetName() == "DairyStock")
        ...
        if (theLivestock -> getanimalStock() -> ElementAtNumber(i) -> GetName() == "BeefStock")
    ...
    currentPeriod = 0;
    if ((globalSettings -> DetailsData.getPastureDetails()) && (!grazmanfile))
    ...
    for (int i = 0; i < theCattle -> NumOfNodes(); i++)
    {
        for (int j = 0; j < theCattle -> ElementAtNumber(i) -> GetCattleGroup() -> NumOfNodes(); j++)
        {
            ...
            if (aNumberOfAnimals > 0)
            {
                ...
                aGroup -> SetanimalSection(theBuildings -> PlaceAnimal(aGroup -> GetAlias()));
                ...
                for (int k = 0; k < aGroup -> GetnumberOfCohorts(); k++)
                {
                    ...
                    anAnimal -> SetanimalSection(aGroup -> GetanimalSection());
                }
            }
        }
    }
    ...
    if (housingDate)
    {
        if ((initialised) && (theTime <= *housingDate))
        {
            for (int i = 0; i < theCropRotation -> GetFields() -> NumOfNodes(); i++)
            {
                ...
                {
                    if (("C1" == cropId) || ("C2" == cropId) || ("G1" == cropId) || ("G2" == cropId)
                            || ("K1" == cropId) || ("G4" == cropId) || ("G5" == cropId))
                    {
...
void grazingManager::ReceivePlan(commonData * data)
{
...
double grazingManager::GetHerbageAvailability(bool restricted)
{
    ...
    return herbageAvailable/1000.0;  //convert to tonnes
}
 ...
 */
int grazingManager::CalcGrazingPeriod(bsTime * aDate,
                                      int                                    growthPeriod)
{
    int      period   = 0;
 ...
 */
void grazingManager::InsertCut(field * aField,
                               bool    harvestCut)
{
    string          crop_id = "G4";
    ...
    if (harvestCut)
    {
        FO -> DefineHarvestFieldsOper(ForageHarvesting, crop_id, "Pasture", theTime.GetYear(), theTime.GetMonth(),
                                      theTime.GetDay(), aField -> GetArea(), 0, aField -> DistanceToField_(), true,
                                      residualDM, 0);    // @@@ Might be problems here regarding parameters !!!
    }
    else
    {
        FO -> DefineHarvestFieldsOper(CutSetAside, crop_id, "Pasture", theTime.GetYear(), theTime.GetMonth(),
                                      theTime.GetDay(), aField -> GetArea(), 0, aField -> DistanceToField_(), true,
                                      residualDM, 0);    // @@@ Might be problems here regarding parameters !!!
    }
...
void grazingManager::InitialiseGrowthEstimates()
{
    ...
    for (int j = 0; j < totalDays; j++)
    {
        ...
        for (int i = 0; i < theCropRotation -> GetFields() -> NumOfNodes(); i++)
        {
            ...
            maxGrowthRate = potentialGrowthFactor
                            * aField -> InitExpectedGrowthRate(theClimate -> GetnormTemp(month),
                                    theClimate -> GetnormRad(month));
        ...
        if ((!growthEnd) && (tempTime -> GetJulianDay(day, month, year) >= springCutoffDay)
                && (daysWithoutGrowth > 10))
...
void grazingManager::SetGrazing(bsTime * aDate,int duration,int *numOthers)
{
    ...
    *numOthers = fields_to_graze -> NumOfNodes();    // number of fields grazed by animals other than dairy cattle
    ...
    if (fields_to_graze -> NumOfNodes() > 0)
        theMessage -> LogEventWithTime("Fields grazed ", fields_to_graze -> NumOfNodes());
}
 ...
 * PRECISE LOOK INTO THE FUTURE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!@@@
 */
double grazingManager::GetExpectedGrowthRate(bsTime * aDate,
        int                                           fieldNo)
{
...
bool grazingManager::CheckGrazingTheshold()
{
    ...
    double grazedFeedReq = theCattleFeedingManager -> GetHerbageRequired(0,
                           theCattleFeedingManager -> GetNumFeedingGroups() - 1, false);
    grazedFeedReq += theCattleFeedingManager -> GetHerbageRequired(0,
                     theCattleFeedingManager -> GetNumFeedingGroups() - 1, true);
...
void grazingManager::GetStatus()
{
    ...
    if (globalSettings -> DetailsData.getPastureDetails())
    {
...
void grazingManager::InitialiseFileOutput()
{
...
#ifdef __BCplusplus
    filename    = "pasture.xls";
#else
    filename    = "pasture.xls";
#endif
    leader      += filename;
    grazmanfile -> open(leader.c_str(), ios::out);
    ...
    *grazmanfile << " " << "\t";
    for (int i = 0; i < theCropRotation -> GetFields() -> NumOfNodes(); i++)
    {
        *grazmanfile << " " << "\t" << "tDM/ha" << "\t" << "g/kg" << "\t" << " " << "\t" << "kgDM/ha/d" << "\t"
                     << "kgDM/ha/d" << "\t";
    }
    *grazmanfile << endl;
}
...
bool grazingManager::CheckConservationFields()
{
    ...
    for (int counter = 0; counter < numOfFields; counter++)
    {
        ...
        if (aField -> GetGrazingShutDown())
        {
            ...
            {
                // proteinConc = aField->GetAbovegroundCropN() * N_to_protein
                // / mass;
                //
