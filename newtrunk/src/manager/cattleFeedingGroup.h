...
public:
    ...
    cattleFeedingGroup(string       aName,
                       const int    aIndex,
                       const base * aOwner);
    ...
    void AddToGroup(cattle * anAnimal)
    {
        theGroup -> InsertLast(anAnimal);
    }

    // LEAK - probably stems from here (bmp) !!!
    ...
    void ShowGroupDiets(fstream * afile,
                        bool      header);
    ...
    double GetSilageRequired(int startCode,
                             int endCode);

    double GetPotentialMilkYield(int durationOfLactation);

    // void SetroughageEnergyAllowance();
    void CalcDefaultDiets(double    totalEnergyRequired,
