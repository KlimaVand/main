...
cattleFeedingManager::cattleFeedingManager(string aName,
        const int                                 aIndex,
        const base *                              aOwner):
    base(aName,
         aIndex,
...
void cattleFeedingManager::ReceivePlan()
{
    ...
    theMessage -> LogEvent(logString);
    fileAccess hd;
    cout << hd.getCurrentPath();
    ...
    {
        namestr       = "Group";
        aFeedingGroup = new cattleFeedingGroup(namestr, inx, this);    // Possible LEAK, not sure (bmp) !!!
    ...
    if ((globalSettings -> DetailsData.getHerdDetails()) && (!fHerdPerformance))
...
void cattleFeedingManager::SortAnimals(bool Cull)
{
    ...
    if (theCattle)
    {
        ...
        {
            cattleGroup * anAgeGroup = theCattle -> GetCattleGroup() -> ElementAtNumber(j);
            // get each age group in turn
            for (int k = 0; k < anAgeGroup -> GetnumberInstances(); k++)
            {
                ...
                if ((anAnimal -> Getage() > 365) && (anAnimal -> GetlactationNumber() == 0))
                {
                    ...
                    // Heifers3 - 1 to 2 year old heifers
                    anAnimal -> SetfeedingGroup(2);
                }
                if ((anAnimal -> isLactating())
                        && (anAnimal -> GetlactationNumber() == 1))      // if 1st lactation and lactating
                {
                    if (anAnimal -> GetdaysFromParturition() <= startLateLactation)
                    {
                        feedingGroups -> ElementAtNumber(3) -> AddToGroup(anAnimal);
                        // 1st lactation, early lactation
                        anAnimal -> SetfeedingGroup(3);
                    }
                    else                                                    // 1st lactation, late lactation
                    {
                        feedingGroups -> ElementAtNumber(4) -> AddToGroup(anAnimal);
                        // 1st lactation, late lactation
                        anAnimal -> SetfeedingGroup(4);
                    }
                }
                if ((anAnimal -> isLactating()) && (anAnimal -> GetlactationNumber() > 1))
                {
                    // if mature and lactating
                    if (anAnimal -> GetdaysFromParturition() <= startLateLactation)
                    {
                        feedingGroups -> ElementAtNumber(5) -> AddToGroup(anAnimal);
                        // mature, early lactation
                        anAnimal -> SetfeedingGroup(5);
                    }
                    else                                                    // mature, late lactation
                    {
                        feedingGroups -> ElementAtNumber(6) -> AddToGroup(anAnimal);
                        // mature, late lactation
                        anAnimal -> SetfeedingGroup(6);
                    }
                }
                if (!(anAnimal -> isLactating()) && (anAnimal -> GetlactationNumber() > 0))
                {
                    feedingGroups -> ElementAtNumber(7) -> AddToGroup(anAnimal);
                    // dairy dry animals
        ...
        if (theCattle)
        {
            ...
            for (int j = 0; j < theCattle -> GetCattleGroup() -> NumOfNodes(); j++)
            {
                cattleGroup * anAgeGroup =
                    theCattle -> GetCattleGroup() -> ElementAtNumber(j);    // get each age group in turn
                for (int k = 0; k < anAgeGroup -> GetnumberInstances(); k++)
                {
                    cattle * anAnimal = anAgeGroup -> GetGroupMember(k);
                    ...
                    if ((anAnimal -> Getage() >= 183) && (anAnimal -> Getage() <= 365))
                    {
                        // young animals 6 months to 1 year
                    ...
                    if ((anAnimal -> isLactating()) && (anAnimal -> GetlactationNumber() == 1))
                    {
                        // if 1st lactation and lactating
                        if (anAnimal -> GetdaysFromParturition() <= startLateLactation)
                        {
                            feedingGroups -> ElementAtNumber(9) -> AddToGroup(anAnimal);
                            // 1st lactation, early lactation
                            anAnimal -> SetfeedingGroup(9);
                        }
                        else        // 1st lactation, late lactation
                        {
                            feedingGroups -> ElementAtNumber(10) -> AddToGroup(
                                anAnimal);    // 1st lactation, late lactation
...
{
    double herbageRequired = GetHerbageRequired(0, feedingGroups -> NumOfNodes() - 1, zeroGrazed);
 ...
 */
double cattleFeedingManager::GetHerbageRequired(int start,
        int                                         end,
        bool                                        zeroGrazed)
{
 ...
 */
void cattleFeedingManager::weanCalves(int herdNo,
...
dairyStock * cattleFeedingManager::GetHerd(int whichHerd)
{
    ...
    {
        if (theLivestock -> getanimalStock() -> ElementAtNumber(i) -> GetName() == herdName)
            // if dairy animals are present
        {
 ...
 */
void cattleFeedingManager::AllocateFieldsForGrazing(linkList<field> * fields_to_graze,int numOthers)
{
...
{
    string filename;
    string leader = globalSettings -> getOutputDirectory();
    fCattleFeeding = new fstream();
#ifndef __BCplusplus__
    filename       = "cattlefeed.xls";
#else
    filename       = "cattlefeed.xls";
#endif
    leader         += filename;
    fCattleFeeding -> open(leader.c_str(), ios::out);
    fCattleFeeding -> precision(8);
    fHerdPerformance = new fstream();
    leader           = globalSettings -> getOutputDirectory();
#ifndef __BCplusplus__
    filename         = "herd.xls";
#else
    filename         = "herd.xls";
    ...
    *fCattleFeeding << "Group" << "\t";
    for (int i = 0; i <= feedingGroups -> NumOfNodes(); i++)
    {
        ...
        *fHerdPerformance << "kg/d" << "\t";
        *fHerdPerformance << "MJ/d" << "\t";
        *fHerdPerformance << "SFU/d" << "\t";
        /*
 ...
 */
void cattleFeedingManager::AutoFeed(int conceptionTime,
                                    int                             lengthOfLactation)
{
    for (int i = 1; i <= 2; i++)
    {
        ...
        if (theCattle)
        {
            ...
            for (int j = 0; j < theCattle -> GetCattleGroup() -> NumOfNodes(); j++)
            {
                ...
                case 9 :            // MBEEF3
                    ...
                    aFeedingGroup -> CalcDefaultDiets(energyRequired, proteinRequired, differentiation, false,
                                                      fCattleFeeding);
                ...
                case 3 :      // DAIRY and SUCKLER
                {
                    ...
                    proteinRequired = tempcow -> GetProteinRequirement();
                    energyRequired  = tempcow -> GetEnergyRequirement();
                    aFeedingGroup -> CalcDefaultDiets(energyRequired, proteinRequired, differentiation, true,
                                                      fCattleFeeding);
                    cout << "Initialised " << aFeedingGroup -> GetAlias() << endl;
                    // end of dry animals
