    ...
    base()
{
    ...
    meatExport        = milkQuota = 0.0;
    lengthOfLactation = ageAtWeaning = 0;
    ...
    if (theLivestock)
    {
        ...
        for (int i = 0; i < theLivestock -> getanimalStock() -> NumOfNodes(); i++)
        {
            if ((theLivestock -> getanimalStock() -> ElementAtNumber(i) -> GetName() == "DairyStock")
                    || (theLivestock -> getanimalStock() -> ElementAtNumber(i) -> GetName() == "BeefStock"))
            {
                ...
                grazing_manager -> Initialize(atheLivestock, atheCropRotation, atheBuildings,
                                              cattleFeedingManagerInstance);
                if ((theLivestock -> getanimalStock() -> ElementAtNumber(i) -> GetName() == "DairyStock"))
...
void operationsManager::daily_update()
{
    ...
    if (cattleFeedingManagerInstance)    // check to see if cattleFeedingManager is present
    {
        ...
        if (cattleFeedingManagerInstance -> GetbeefPresent())
        {
            ...
            cattleFeedingManagerInstance -> GetGroupMember(11) -> makePregnant(
                conceptionTime);         // suckler mature lact early
            cattleFeedingManagerInstance -> GetGroupMember(12) -> makePregnant(
                conceptionTime);    // suckler mature lact late
            ...
            cattleFeedingManagerInstance -> GetGroupMember(11) -> DryOff(
                lengthOfLactation);    // suckler  mature early lact
        ...
        double grazedHerbageRequired =
            cattleFeedingManagerInstance -> GetHerbageRequired(false);    // herbage required per day
        double zerograzedHerbageRequired =
            0.0;    // cattleFeedingManager->GetHerbageRequired(true);  //herbage required per day
        int status    = 1;
        int numOthers = 0;
        if (grazing_manager)                                                      // check to see if grazing manager is present
        {
            if (grazing_manager -> GetenableGrazing())
            {
                ...
                if ((grazedHerbageRequired > 0.0) || (zerograzedHerbageRequired > 0.0))
                {
                    ...
                    {
                        // Goldilocks test: find whether the amount of grass available is too little, too much or just right
...
void operationsManager::ReceivePlan(string fileExtension,string inputDir)
{
...
// void Operations_manager_class::Set(stringfileExtension, stringinputDir)
void operationsManager::ShowOpsDetails(bool header)
{
    if (header)
    {
        *detailsFile << "Date" << "\t";
        *detailsFile << "ConcDM" << "\t" << "ConcSFU" << "\t" << "RoughDM" << "\t" << "RoughSFU" << "\t" << "ManureFW"
                     << "\t" << "ManureN" << "\t";
        *detailsFile << endl;
        *detailsFile << " " << "\t" << "t" << "\t" << " " << "\t" << "t" << "\t" << " " << "\t" << "t" << "\t" << "kg"
                     << "\t";
        *detailsFile << endl;
    }
    else
    {
        ...
        if (cattleFeedingManagerInstance
                && (globalSettings -> DetailsData.getHerdDetails()
                   ))      // check to see if cattleFeedingManager is present
