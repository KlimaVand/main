...
cattleFeedingGroup::cattleFeedingGroup()
{
    ...
    wasFemale           = active = false;
}

cattleFeedingGroup::cattleFeedingGroup(string aName,
                                       const int                             aIndex,
                                       const base *                          aOwner):
    base(aName,
         aIndex,
...
void cattleFeedingGroup::ReadParameters(commonData * infile,
                                        bool                                         autoFeed)
{
    ...
    // Causes a LEAK, probably in base.cpp (hard to understand - BMP) !!!
    string  namestr = "Group(" + conv.IntToString(GetIndex()) + ")." + "Period";
    int     first, num;
    infile -> FindSection(GetName(), GetIndex());    // find the correct place in the input file
    infile -> getSectionsNumbers(namestr, &first, &num);    // count the number of periods for which a diet is defined
    infile -> setCritical(false);
    for (int index = first; index <= (first + num); index++)
    {
        diet * aDiet = new diet(namestr, index, this);    // LEAK (BMP) !!!
...
void cattleFeedingGroup::ShowGroupDiets(fstream * afile,
                                        bool                                      header)
{
    if (theGroupDiets)
    {
...
diet * cattleFeedingGroup::GetHousedDiet()
{
    ...
    cout<<theGroupDiets -> NumOfNodes()<<endl;
...
void cattleFeedingGroup::FeedCattle(int status)
{
    for (int i = 0; i < theGroup -> NumOfNodes(); i++)
    {
        ...
        {
        case 0 :
            tempcattle -> SetfeedPlanList(theCurrentDiet -> GetdeficiencyfeedItemList());
            currentFeedingGroup = 100 * tempcattle -> GetfeedingGroup() + 10 + theCurrentDiet -> GetIndex();
            break;
        case 1 :
            tempcattle -> SetfeedPlanList(theCurrentDiet -> GetnormalfeedItemList());
            currentFeedingGroup = 100 * tempcattle -> GetfeedingGroup() + 20 + theCurrentDiet -> GetIndex();
            break;
        case 2 :
            tempcattle -> SetfeedPlanList(theCurrentDiet -> GetsurplusfeedItemList());
            currentFeedingGroup = 100 * tempcattle -> GetfeedingGroup() + 30 + theCurrentDiet -> GetIndex();
            break;
        }
        tempcattle -> SetfeedingGroup(currentFeedingGroup);
        // 3 digit output format: 1st digit = group number,
        ...
        int                                                         stop)
{
    if (theCurrentDiet)
    {
        // could be nullptr if there are beef cattle but not dairy cattle present on farm
...
void cattleFeedingGroup::GetStatus(fstream * aFile)
{
    ...
    *aFile << Alias << "\t";
    if (currentFeedingGroup < 100)
        *aFile << "0" << currentFeedingGroup << "\t";
    else
        *aFile << currentFeedingGroup << "\t";
    *aFile << GetNumber() << "\t" << LW << "\t" << growth << "\t" << milk_prod << "\t" << ME_intake << "\t"
           << FE_intake << "\t" << N_urine << "\t" << N_faeces << "\t" << DM_intake << "\t" << Grazed_DM << "\t";
}
...
double cattleFeedingGroup::GetSilageRequired(int startCode,
        int                                      endCode)
{
    double ret_val = 0.0;
...
void cattleFeedingGroup::CalcDefaultDiets(double totalEnergyRequired,
        ...
        fstream *                                aFile)
{
    ...
    for (int j = 0; j < theGroupDiets -> NumOfNodes(); j++)
    {
        ...
        if (conservedRoughageEnergyAllowance > 0.0)
        {
            ...
            // roughageProtein = conservedRoughage->GetAmount()
            // * conservedRoughage->Getaat() * 1000.0;  //in kg
        }
        else
            conservedRoughage -> Setamount(0.0);
        if (grazedRoughageAllowance > 0.0)
        {
            feedItem * grazed = ((feedItem *) aDiet -> FindGrazedFeed());
            grazed -> Setamount(grazedRoughageAllowance / grazed -> GetfeedUnitsPerItemUnit());
            // roughageProtein += grazed->GetAmount() * grazed->Getaat() * 1000.0; //in kg
            //
        }
        feedItem * concentrate = ((feedItem *) aDiet -> FindConcentrateFeed());
        if (concentrateEnergyReq > 0.0)
            concentrate -> Setamount(concentrateEnergyReq / concentrate -> GetfeedUnitsPerItemUnit());
        else
            concentrate -> Setamount(0.0);
