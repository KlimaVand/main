...
operation::operation(string       aName,
                     const int    aIndex,
                     const base * aOwner):
    base(aName,
         aIndex,
...
void operation::ReadParameters(commonData *& file) {
    ...
    section = Name + "(" + conv.IntToString(Index) + ").mainTechEquip";
    ...
    section = Name + "(" + conv.IntToString(Index) + ").techEquip";
...
void operation::CalcGangMaintenance() {
    linkList<techEquip> * aTechEquipList;
    linkList<techEquip>::PS aTechEquip;

    techEquip aTechFarm("techFarm", 0, this);

    // double aGangMaintenance = 0.0;
    aTechEquipList = aGang -> GetTechEquipList();

    aTechEquipList -> PeekHead(aTechEquip);

    while (aTechEquip) {
        aTechFarm  = *(techEquip *) (aTechEquip -> element);
        aTechEquip = aTechEquip -> next;
    }

    // aGangMaintenance *= aGang->GetgangHours();
}
