    ...
    public fieldOperationTech
{
    ...
    private:
        ...
        harvestTech & operator = (
                const harvestTech & h);        // Dissable the compilers generation of default assignment operator.
    ...
    public:
        harvestTech(string       aName,
                    const int    aIndex = -1,
                    const base * aOwner = nullptr);

        virtual ~harvestTech();

        void SetoperationID(int aoperationID) {
            operationID = aoperationID;
        }

        virtual void ReadParameters(commonData *& file);

        virtual void SetMainProduct(int         i,
                                    plantItem * aMain) {
            MainProduct[i] = aMain;
        }

        virtual void SetStrawProduct(int         i,
                                     plantItem * aStraw) {
            StrawProduct[i] = aStraw;
        }

        virtual void SetForageProduct(int         i,
                                      plantItem * aForage) {
            ForageProduct[i] = aForage;
        }

        void ClearThePlantItems();
