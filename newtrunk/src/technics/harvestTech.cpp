...
harvestTech::harvestTech(string       aName,
                         const int    aIndex,
                         const base * aOwner):
...
void harvestTech::CalcCost(calcLPType mode) {
    techEquip aMainTechEquip("maintechequip", 0, this);

    switch (mode) {
        case hoursTotalNotCombineNotFert :
            if ((operationName == "CombineHarvesting") || (operationName == "CombineHarvestingShred")) {
                cost = 0.0;
            } else {
                CalcGangHours();
            }
...
void harvestTech::CalcGangHours() {
    ...
    while (aTechEquipNode) {
        if (aTechEquipNode -> element -> GetName() == "selfPropelled") {
        ...
        if ((aTechEquipNode -> element -> GetName() == "implement")
                || (aTechEquipNode -> element -> GetName() == "forageCutter")) {
    ...
    if (size == 0.0) {
        theMessage -> FatalError("harvestTech::CalcGangHours - size = 0 in operation ", operationName);
    }

    if ((operationID == CombineHarvesting) || (operationID == CombineHarvestingShred)) {
        if (netCapacityWheat == 0.0) {
            theMessage -> FatalError("harvestTech::CalcGangHours - netCapacityWheat = 0 in operation ", operationName);
        }
    }

    if (area == 0.0) {
        theMessage -> FatalError("harvestTech::CalcGangHours - area = 0 in operation ", operationName);
    }

    if (carryCapacity == 0.0) {
        theMessage -> FatalError("harvestTech::CalcGangHours - carryCapacity = 0 in operation ", operationName);
    }

    if (workingSpeed == 0.0) {
        theMessage -> FatalError("harvestTech::CalcGangHours - fieldSpeed = 0 in operation ", operationName);
    }

    if (roadSpeed == 0.0) {
        theMessage -> FatalError("harvestTech::CalcGangHours - roadSpeed = 0 in operation ", operationName);
    }

    if ((balesLoad == 0.0) && (operationID == GatheringBales)) {
        theMessage -> FatalError("harvestTech::CalcGangHours - balesLoad = 0 in operation ", operationName);
    }

    if ((operationID == Mowing) || (operationID == CutSetAside) || ForageHarvesting) {
        A = ((area * 600.0) / (workingSpeed * size) + (turningTime * sqrt(area * 10000.0 / 2.0)) / size
    ...
    if ((operationID == CombineHarvesting) || (operationID == CombineHarvestingShred)) {
        ...
        if (!MainProduct[0]) {
            theMessage -> FatalError("harvestTech::CalcGangHours - MainProduct[0] should not be nullptr here");
        } else if (MainProduct[0] -> GetName() == "PEAS") {
            cropFactor = 0.888;
        } else if (MainProduct[0] -> GetName() == "WNRPE") {
            cropFactor = 0.454;
        } else if (MainProduct[0] -> GetName() == "RYE") {
            cropFactor = 0.548;
        }

        if (calcAmount <= 0) {
            theMessage -> WarningWithDisplay(
                "harvestTech::CalcGangHours - amount harvested/gathered should be greater than zero in operation ",
                operationName);
        }

        A = ((area * calcAmount * 60.0) * cropFactor / netCapacityWheat
             + (max(0.0, turningTime * (sqrt(area * 10000.0 / 2.0) - (8.0 * size))) / size)
             +    /* Max function added, otherwise this will fall below zero at very low areas - BMP Dec. 2006 */
            turningHeadland + (disturbanceAllowance * area)) / area;
    }
    ...
    if (operationID == GatheringBales) {
        ...
        A = ((area * calcAmount * 60.0) / strawBalerCapacity + (turningTime * sqrt(area * 10000.0 / 2.0)) / size
             + turningHeadland + (disturbanceAllowance * area)) / area;
        ...
        if (baleType == 1) {
            ...
            if (A1 <= 0.0) {
                theMessage -> WarningWithDisplay(
                    "harvestTech::CalcGangHours - A1 should be greater than zero in operation ", operationName);
            }
        }
    }
    ...
    switch (operationID) {
        ...
        case CombineHarvestingShred :
            ...
            D1 = (prepReloadingHarvester * calcAmount * 1000.0) / ((9.12 * size - 17.83) * 100);
        ...
        case GatheringBales :
            ...
            if (baleType == 1) {
                ...
                L1 = (((loadCapacity * balesPerLoad) + reloading + (moweCarriage * balesPerLoad / 10.0))
    ...
    if ((operationID == CombineHarvesting) || (operationID == CombineHarvestingShred)) {
        B = (0.75 * sqrt(area * 10000.0 / 2.0) * calcAmount * 120.0) / (carryCapacity * workingSpeed);
    } else if ((operationID == GatheringBales) && (baleType == 0)) {
        B = 0.0;
    } else {
        B = (1.5 * sqrt(area * 10000.0 / 2.0) * calcAmount * 120.0) / (carryCapacity * workingSpeed);
    }
    ...
    if (operationID == BeetHarvesting) {
        ...
        B3 = (distance * calcAmount * 120.0) / (carryCapacity * roadSpeed) + stops * calcAmount;
        E  = stops * calcAmount;
    }
    ...
    if (operationID == GatheringBales) {
        if (baleType == 0) {
            P1 = ((prepUnloadingStorage * calcAmount * 1000.0) / carryCapacity) + (calcAmount * unloadCapacity);
        } else if (baleType == 1) {
            P1 = ((prepUnloadingStorage * calcAmount * 1000.0) / carryCapacity)
                 + ((calcAmount * 1000.0 * unloadCapacity) / balesLoad);
        }
    } else if ((operationID != BeetTopChopping) && (operationID != Mowing) && (operationID != CutSetAside)) {
        P1 = ((prepUnloadingStorage * calcAmount * 1000.0) / carryCapacity) + (calcAmount * unloadCapacity);
    }
    ...
    if (operationID == BeetHarvesting) {
        ...
        P2 = ((prepUnloadingRoothouse * calcAmount * 1000.0) / carryCapacity) + (calcAmount * unloadRoothouseCapacity);
    ...
    // Logical disjunction always evaluates to true: operationID != 12 || operationID != 13.
    // if (operationID != CombineHarvesting
    // || operationID != CombineHarvestingShred)
...
void harvestTech::CalcGangFuel() {
    ...
    while (aTechEquipNode) {
        ...
        } else if (aTechEquipNode -> element -> GetName() == "implement") {
            aImplement = *(implement *) (aTechEquipNode -> element);
