...
animalFertTech::animalFertTech(string       aName,
                               const int    aIndex,
                               const base * aOwner):
...
void animalFertTech::CalcCost(calcLPType mode) {
    techEquip aMainTechEquip("maintechequip", 0, this);

    switch (mode) {
        case hoursAnimalFert :
            CalcGangHours();
...
void animalFertTech::CalcGangHours() {
    ...
    if (spreadingCapacity == 0) {
        theMessage -> FatalError("animalFertTech::CalcGangHours - spreadingCapacity = 0 in operation ", operationName);
    }

    if (size == 0) {
        theMessage -> FatalError("animalFertTech::CalcGangHours - size = 0 in operation ", operationName);
    }

    if (carryCapacity == 0) {
        theMessage -> FatalError("animalFertTech::CalcGangHours - carryCapacity = 0 in operation ", operationName);
    }

    if (workingSpeed == 0) {
        theMessage -> FatalError("animalFertTech::CalcGangHours - fieldSpeed = 0 in operation ", operationName);
    }

    if (roadSpeed == 0) {
        theMessage -> FatalError("animalFertTech::CalcGangHours - roadSpeed = 0 in operation ", operationName);
    }

    double A1 = ((area * amount * 1000 / spreadingCapacity)
                 + (turningTime * (sqrt(area * 10000 / 2) - 2 * size) / size) + turningHeadland
                 + (disturbanceAllowance * area)) * (1 + relaxationAllowance) / area;
    double A2 = (((amount * 1000) / carryCapacity) * prepForSpreading) * (1 + relaxationAllowance);
    double B  = (((1.5 * sqrt(area * 10000 / 2)) * amount * 120) / (carryCapacity * workingSpeed))
                * (1 + relaxationAllowance);
    double T = ((distance * amount * 0.12 * 1000) / (carryCapacity * roadSpeed)) * (1 + relaxationAllowance);
    double L;

    if (j == 0) {    // i.e. solid manure
        L = (amount * 1000 / carryCapacity * prepLoading + amount * 1000 / loadingCapacity) * (1 + relaxationAllowance);
    } else {

        // i.e. FLUID manure
        L = (amount * 1000 / carryCapacity * prepLoading + amount * 60 / (-150.1 + 66.7 * pumpSize))
            * (1 + relaxationAllowance);
    }
...
void animalFertTech::CalcGangFuel() {
    ...
    while (aTechEquipNode) {
        ...
        if (aTechEquipNode -> element -> GetName() == "tractor") {
...
techEquip * animalFertTech::GetApplicationTech() {
    ...
    theMessage -> FatalError(
        "animalFertTech::GetApplicationTech () - Unable to find manure application equipment amongst farm's equipment");
