...
mineralFertTech::mineralFertTech(string       aName,
                                 const int    aIndex,
                                 const base * aOwner):
...
void mineralFertTech::SetTheFertilizers(fertilizer * aFertilizer1,
...
void mineralFertTech::CalcCost(calcLPType mode) {
    techEquip aMainTechEquip("maintechequip", 0, this);

    switch (mode) {
        case hoursMineralFert :
            CalcGangHours();
...
void mineralFertTech::CalcGangHours() {
    ...
    double I = ((fillingTimeSpreader * amount) / 100) + ((prepFilling * amount) / (vol * 0.95));
    double T = ((((distance * 0.12) / roadSpeed) + changingTrailer) * (amount / carryCapacity));
    double L = ((loadingTime * amount) / 100) + ((prepLoading * amount) / carryCapacity);
