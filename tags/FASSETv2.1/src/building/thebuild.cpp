	...
	:base()
{
   ...
   theStores = new linkList<B_storage2>;
...
/****************************************************************************\
   ...
   returns a pointer to the stable.
\****************************************************************************/
...
// could be made faster by dropping out when a store is found
   ...
   {
		B_storage2* store=theStores->ElementAtNumber(i);
		store->SendToStore(p);
   }
}
...
void buildings::DailyUpdate()
{
	...
	for (int i=0;i<theStores->NumOfNodes();i++)
   {
      B_storage2 *bs=theStores->ElementAtNumber(i);
      bs->DailyUpdate();
   }
...
void buildings::InitializeAll(char * InputFilename, bool pigs, bool dairy, bool beef)
{
  if (OpenInputFile(InputFilename))
  {
   ...
   // Build the product stores
      ...
      if ((dairy)||(beef))
      {
         ...
         for(int InstanceIndex=first;InstanceIndex<(first+num);InstanceIndex++)
         {
            catstable *stableInstance=new catstable(InstanceName,InstanceIndex,this,0);
            stableInstance->ReadParameters(file);
            theStables->InsertLast(stableInstance);
            if (theOutputControl->GetManureManageDetails())
            	stableInstance->OpenDetailsFile();
...
void buildings::CalcLPmanurestore(char* filename)                   // JB did this
{
...
void buildings::CalcLPmanurestorecap(char* filename)
{
...
void buildings::CalcLPstables(char* filename)
{
...
/****************************************************************************\
Check to see if all houusing is used. If not, disable calls to these houses.
This speeds up runs and prevents economic calculations for unused housing
Must be called AFTER livestock have been initialised
\****************************************************************************/

/*void buildings::CheckHouseUse()
{
   }
	for(int i=0;i<theStables->NumOfNodes();i++)
	{
	   stable* astable=theStables->ElementAtNumber(i);
	   astable->GetTotalArea();
   }
...
/****************************************************************************\
\****************************************************************************/
void buildings::GetAvailableStoredFeed(product *aProduct)
{
   int i=0;
...
/*void buildings::GetStoredConcentrateFeed(product *aProduct)
{
	return GetAmountStoredFeed(1);
};
double buildings::GetStoredRoughageFeed()
{
	return GetAmountStoredFeed(2);
};
double buildings::GetStoredBedding()
{
	return GetAmountStoredFeed(4);
};
  */
