...
/* Not implimented - need separate stores as liquid and solid stored separately but store info is currently
read in manurestore::Readparameters
      instore *aFluidStore = new instore("floorfluidStore",0,this);
	 	manure * aFluidManure=new manure();
	 	aFluidManure->Setname("FLUID-MANURE");
		aFluidStore->ReadParameters(file);
  	 	theProducts->GiveProductInformation(aFluidManure);
    	aFluidManure->Setamount(0);
    	aFluidStore->SetManure(aFluidManure);
      aFluidStore->SetpostCleanDepth(postCleanDepth);  //may need to be modified - with solids, effectively doubles Depth

      instore *aSolidStore = new instore("floorsolidStore",0,this);
	 	manure * aSolidManure=new manure();
	 	aSolidManure->Setname("SOLID-MANURE");
  	 	theProducts->GiveProductInformation(aSolidManure);
    	aSolidManure->Setamount(0);
    	aSolidStore->SetManure(aSolidManure);
      aSolidStore->SetpostCleanDepth(postCleanResidue);

      string fluidToName;      //name of storage receiving liquid manure
      int fluidToIndex;       //index of storage receiving liquid manure
      string solidToName;	//name of storage receiving solid manure
      int solidToIndex;        //index of storage receiving solid manure
      GetParameter("FluidToName",&fluidToName);
      GetParameter("FluidToIndex",&fluidToIndex);
      GetParameter("SolidToName",&solidToName);
      GetParameter("SolidToIndex",&solidToIndex);
    	aFluidStore->SetManureToName(fluidToName);
      aFluidStore->SetManureToIndex(fluidToIndex);
    	aSolidStore->SetManureToName(solidToName);
      aSolidStore->SetManureToIndex(solidToIndex);
     	theStores->InsertLast(aFluidStore);
     	theStores->InsertLast(aSolidStore);*/
   }
...
/*      else if ((aManure->Getname()=="SOLID-MANURE")||(aManure->Getname()=="FYM"))
      {
   	    theMessage->FatalError("FloorStore: attempt to volatilise from dung - needs modification");
	   	instore * adungstore=(instore *)theStores->ElementAtNumber(i);
   	   tempVolat = adungstore->VolatizeDung(ventilated);
         totalVolat+=tempVolat;
   	}
    else {theMessage->FatalError("FloorStore: attempt to volatilise from a manure type that is not supported");}*/
	}
...
/****************************************************************************\
	GiveIndicator, called daily from DailyUpdate.
\****************************************************************************/

void floorstore::GiveIndicator()
{
}
