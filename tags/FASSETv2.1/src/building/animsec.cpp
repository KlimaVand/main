...
/****************************************************************************\
Volatilise N from a section  - complex model
\****************************************************************************/
...
/*     fstream * filehandle = theMessage->GiveDebugHandle();
    *filehandle << GetAlias() << " "  << aManure->GetAmount() << " " << GetStoredN() << " " ;
...
/****************************************************************************\
	Get total area of section
\****************************************************************************/
/*double animalSection::SetTotalArea()
{
 double area=0.0;
 for(int i=0;i<flooring->NumOfNodes();i++)
 {
   floorstore * floorInst=flooring->ElementAtNumber(i);
   area += NrOfAnimals * floorInst->GetArea();
 }
 return area;
};
  */
...
ostream& operator<< (ostream& os,const animalSection& s)
{
  os << "***********************************" << endl;
  os << setiosflags(ios::left) << setw(40) << "Animal section details for  " << s.Alias << endl;
  os << setiosflags(ios::left) << setw(40) << "Number of places " << s.Places << endl;
 for(int i=0;i<s.flooring->NumOfNodes();i++)
 {
   floorstore * floorInst=s.flooring->ElementAtNumber(i);
   os << * floorInst;
 }
