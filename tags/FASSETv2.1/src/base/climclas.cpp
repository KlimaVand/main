...
//			f1 >> date >> tmin >> tmax >> tmean >> precip >> rad  >> humidity >> vel;
   ...
   if (rad<0.0)
   {
   	theMessage->WarningWithDisplay("climclas::Readline - incoming radiation cannot be negative");
      rad=0.0;
   }
   if (precip<0.0)
   {
   	theMessage->WarningWithDisplay("climclas::Readline - precipitation is negative");
      precip=0.0;
   }
   if ((d==1)&&(mnth==1))
      ResetdegreeDays();
...
\****************************************************************************/
void climate::InitClimate(char * filename, int filetype, int firstobs)
{
   Circular = false;
   NitrogenInRain=0.002;
  	DailyCO2Values = false;
   CO2Concentration = 377.0;
   latitude = 56.5;                                          // Denmark
	int correct;
   for (int i=0;i<13;i++)
   	CorrectionFactor[i] = 1.0;
	if (OpenInputFile("environ.dat"))
   {
	UnsetCritical();
   	GetParameter("CO2Concentration",&CO2Concentration);
    if (CO2Concentration==-1)
    DailyCO2Values = true;
    GetParameter("lattitude",&latitude);
    GetParameter("PrecipitationCorrection",&correct);
    GetParameter("NitrogenInRain",&NitrogenInRain);
    if (correct==1)
      	for (int i=0;i<13;i++)
...
void climate::InitClimate(char *aDirectory, char * filename)
{
  ...
  if (!FileExists(filename))
   	theMessage->FatalError("Climate file named [",filename,"] not found");

   CloseInputFile();
   if (date>2000)
   	InitClimate(filename,1,0);               // KML files
   else
   	InitClimate(filename,filetyp,0);         // CLM files
}
...
/****************************************************************************\
Obtains climate data for a specific date
Parameters:
   year, month, day
   min_temp           reference to minimum temperature (Celsius)
	max_temp           reference to maximum temperature (Celsius)
   precip             reference to precipitation (mm)
   radiation,         reference to radiation (MJ/(m2*day))
	EPot               reference to potential evaporation (mm)
   found              reference 1->date found, 0->date not found
\****************************************************************************/
void climate::GetClimate(int year, int month, int day, double &temp, double &min_temp,
								 ...
								 double &EPot, double &windSpeed,  double &relHumidity, int &found)
{
   ...
   {
      circular = true;
   	year = firstYear + (year - lastYear - 1) % (lastYear - firstYear + 1);                   // ensures cirkular referencing if you run
   }
   ...
   switch (EpotCalculationMethod)
   {
      ...
      case 3:
         EPot = epot;
	     	break;
	   case 1:
	   	EPot = PenmanEvaporation(radiation,tmean,windVelocity,humidity);
         break;
      case 2:
      	EPot = MakkinkEvaporation(radiation,tmean);
         break;
   }
   precip=max(0.0,precip);              // this is nessesary because of potential prcp = -1!
   if ((filetyp>=3)&&(filetyp!=6))
   	precip*=AdjustPrecip(month);
   if (EPot<0.0) EPot = 0.0;        // do not allow negativ epot
...
Parameters:
   year, month, day
\****************************************************************************/
bool climate::FindClimate(int year, int month, int day)
{
   char buffer[MAX_TXT];
   getcwd(buffer, MAX_TXT); //get current directory - this procedure will divert to climate directory
   bool circular = false;
   found = false;
   if (year<firstYear)
   	year = firstYear;
   ...
   if (epot<0.0)
      epot = 0.0;          // do not allow negativ epot
   precip=max(0.0,precip); // this is nessesary because of potential precip = -1!
   if (filetyp<=3)         //if filetype is 4 or above then there is no adjustment NJH April 2002
   	precip*=AdjustPrecip(month);
	found=(idx==searchindex);
   if (!found && circular && month==2 && day>=29)
   	found = true;                 // a fix to ensure correct circular referencing
   _chdir(buffer);   //revert to previous directory
   return found;
}
...
{
   char buffer[MAX_TXT];
   _getcwd(buffer, MAX_TXT); //get current directory - this procedure will divert to climate directory
   int year = theTime.GetYear();
   int month = theTime.GetMonth();
   int day = theTime.GetDay();
...
void climate::Update(bsTime *aTime)
{
   ...
   if (tmean>0.0)
	   degreeDays+=tmean;
  chdir(buffer);   //revert to previous directory
}
