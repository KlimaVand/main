...
//--------------Static data member initialization-----------------------------
 ...
 char* IndicatorOutput::character = NULL;
...
void IndicatorOutput::InitializeIndicators()
{
   ...
   AddIndicator(economicIndicator,     "05.30 Sows sold","Dkr",0);
	...
	AddIndicator(economicIndicator,     "05.70 Ha premium","Dkr",0);
   ...
   AddIndicator(economicIndicator,     "06.32 Costs feed and bedding","Dkr",0);
	...
	AddIndicator(economicIndicator,     "06.80 Nlevy1 (mineral fertilizer)","Dkr",0);
	AddIndicator(economicIndicator,     "06.81 Nlevy2 (mineral fertilizer + imported feed)","Dkr",0);
	AddIndicator(economicIndicator,     "06.82 Nlevy3 (import-export)","Dkr",0);
	AddIndicator(economicIndicator,     "06.83 Nlevy4 (import-export) above level","Dkr",0);
...
#ifdef COOLCROP
	AddIndicator(environmentalIndicator,"32.69 Organic N in soil 0-30 cm","kg N",0);
	AddIndicator(environmentalIndicator,"32.70 Mineral N in soil 0-30 cm","kg N",0);
	AddIndicator(environmentalIndicator,"32.71 Organic N in soil 30-90 cm","kg N",0);
	AddIndicator(environmentalIndicator,"32.72 Mineral N in soil 30-90 cm","kg N",0);
	AddIndicator(environmentalIndicator,"32.73 Organic N in soil 90-270 cm","kg N",0);
	AddIndicator(environmentalIndicator,"32.74 Mineral N in soil 90-270 cm","kg N",0);
#endif
...
#ifdef COOLCROP
	AddIndicator(environmentalIndicator,"40.62 Total C in soil 0-30 cm","kg C",0);
	AddIndicator(environmentalIndicator,"40.63 Total C in soil 30-90 cm","kg C",0);
	AddIndicator(environmentalIndicator,"40.64 Total C in soil 90-270 cm","kg C",0);
#endif
	AddIndicator(environmentalIndicator,"40.80 C from roots incorporated into soil","kg C",0);
	AddIndicator(environmentalIndicator,"40.81 C from straw incorporated into soil","kg N",0);
...
//   AddIndicator(environmentalIndicator,"40.99 Plant top res. C incorp. in org. matt.","kg C",0);
...
/****************************************************************************\
\****************************************************************************/
double IndicatorOutput::GetAmount(int indicatorType, char * aName)
{
   character=aName;
   linkList<item>::PS P;
   indicatorList[indicatorType][FindSubIndex(aName)]->FirstThat(P,FindString);
   if (P)
...
int IndicatorOutput::FindSubIndex(const char * aName)
{
   int indexNumber1 = 0;
   int indexNumber2 = 0;
   sscanf(aName,"%d.%d",&indexNumber1,&indexNumber2);
...
/****************************************************************************\
\****************************************************************************/
void IndicatorOutput::AddStateIndicator(int indicatorType, char * aName, char * aUnit, double aAmount)
{
#ifndef __ECOSYSTEM__
   timeConsumption->startProfiling(AddIndicatorToOutput);
#endif
   character=aName;
   linkList<item>::PS P,P1;
   linkList <item>* list = FindList(indicatorType,aName);
   ...
   else
   {
      ...
      item * Pi=new item(aName,aUnit,aAmount,true);
...
#ifndef __ECOSYSTEM__
	timeConsumption->endProfiling(AddIndicatorToOutput);
#endif
}

/****************************************************************************\
\****************************************************************************/
void IndicatorOutput::AddStateIndicator(int indicatorType, char * aName, char * aUnit, nitrogen aAmount)
{
...
/****************************************************************************\
\****************************************************************************/
void IndicatorOutput::AddIndicator(int indicatorType, char * aName, char * aUnit, double aAmount)
{
#ifndef __ECOSYSTEM__
   timeConsumption->startProfiling(AddIndicatorToOutput);
#endif
   ...
   else
   {
   	if (makingList)
      {
         item * Pi=new item(aName,aUnit,aAmount,false);
         list->FirstThat(P,BiggerThan);
         if (P!=NULL)
            list->InsertBefore(P,P1,Pi);
         else
...
/****************************************************************************\
\****************************************************************************/
void IndicatorOutput::AddIndicator(int indicatorType, char * aName, char * aUnit, nitrogen aAmount)
{
...
//	double minSoilN = GetAmount(environmentalIndicator,"32.61 Mineral N in soil");
 ...
 //  double orgSoilN = GetAmount(environmentalIndicator,"32.60 Organic N in soil");
  ...
  // double houseToStorageN = GetAmount(environmentalIndicator,"35.80 N transf. from stable to store");
   ...
   double lossHouseEtcN = storevolat + housevolat + deNHouse + deNStorage + extractedStorageN + exportedManureN;
   ...
   double plantresN      = GetAmount(environmentalIndicator,"32.21 N from straw incorporated into soil");
   ...
   double incorpTopN  = GetAmount(environmentalIndicator,"32.21 N from straw incorporated into soil");
   ...
   if (theTime.GetDay()==15 && theTime.GetMonth()==11 && theTime.GetYear()==1996)
      cout << ""; // TEST - DELETE LATER !!!
...
//   double houseToStorageC = GetAmount(environmentalIndicator,"45.80 C transf. from stable to store");
   ...
   double exportedManureC =GetAmount(environmentalIndicator,"45.22 C in exported manure");
   double lossHouseEtcC = storeCO2 + houseCO2 + houseCH4 + storeCH4 + extractedStorageC + exportedManureC;
   ...
   double plantresC      = GetAmount(environmentalIndicator,"40.81 C from straw incorporated into soil");
   ...
   double incropTopC  = GetAmount(environmentalIndicator,"40.81 C from straw incorporated into soil");
