...
soil::~soil()
{
   ...
   delete Surface;
   delete Profile;
   if (theOutputControl->GetSoilDetails())
   	f.close();
}

/****************************************************************************\
\****************************************************************************/
void soil::Initialize(char * soilFileName, int aIndex)
{
   evaporationDepth = 200.0;
   SetCritical();
   cout << "Initialising soil no " << aIndex << " with the file " << soilFileName << endl;
   ...
   if (theOutputControl->GetSoilDetails())
   {
      ...
      sprintf(filename,"soil%d.xls",Index);
      strcat(leader,filename);
      f.open(leader,ios::out);
      f.precision(4);
      f << "date";
      int i=0;
      while (theOutputControl->GetWaterEndDepth(i)>0.0)
      {
         f << "\twater"<< theOutputControl->GetWaterStartDepth(i) << "-"<< theOutputControl->GetWaterEndDepth(i);
         i++;
      }
      i=0;
      while (theOutputControl->GetNminEnd(i)>0.0)
      {
         f << "\tNmin"<< theOutputControl->GetNminStart(i) << "-"<< theOutputControl->GetNminEnd(i);
         f << "\tNO3-"<< theOutputControl->GetNminStart(i) << "-"<< theOutputControl->GetNminEnd(i);
         f << "\tNH4-"<< theOutputControl->GetNminStart(i) << "-"<< theOutputControl->GetNminEnd(i);
         f << "\tTotal15N-"<< theOutputControl->GetNminStart(i) << "-"<< theOutputControl->GetNminEnd(i);
         i++;
      }
      i=0;
      while (theOutputControl->GetSoilCEnd(i)>0.0)
      {
         f << "\tSMB-C-"<< theOutputControl->GetSoilCStart(i) << "-"<< theOutputControl->GetSoilCEnd(i)<<"[�g/g]";
         i++;
      }
      i=0;
      while (theOutputControl->PoreWaterDepth(i)>0.0)
      {
         f << "\t" << theOutputControl->PoreWaterDepth(i)/10 <<"cmNit-N"
         << "\t" << theOutputControl->PoreWaterDepth(i)/10 <<"cmAm-N"
         << "\t" << theOutputControl->PoreWaterDepth(i)/10 <<"cmN"
       	<< "\t" << theOutputControl->PoreWaterDepth(i)/10 <<"cmCL"
         << "\t" << theOutputControl->PoreWaterDepth(i)/10 <<"cmN15";
         i++;
      }
      i=0;
      while (theOutputControl->GetTemperatureDepth(i)>=0.0)
      {
         f << "\tt" << theOutputControl->GetTemperatureDepth(i);
         i++;
      }
      ...
      f << "\tnetMineralisation\tN2O-N\tN2O-D\tN2O\tN2Oratio\tDrainage\tDrainage-N\tSoilEvap\tTotalEvap";
...
\****************************************************************************/
void soil::Incorporate(double depth, double frac)
{
   if (depth>0 && frac>0)
   {
      ...
      for (int i=0; i<n; i++)
      {
         ...
         LengthList[i] = 0.;
         if (CurrentDepth<=depth) LengthList[i] = LayerThickness;
         else if (CurrentDepth-LayerThickness<depth) LengthList[i] = depth-(CurrentDepth-LayerThickness);
      }
   ...
   int depth = 1000;
 	if (theOutputControl->GetSoilDetails())
	{
	   ...
	   int i=0;
	 	while (theOutputControl->GetWaterEndDepth(i)>0.0)
	 	{
	 		f << "\t" << GetWater(theOutputControl->GetWaterStartDepth(i), theOutputControl->GetWaterEndDepth(i)-theOutputControl->GetWaterStartDepth(i));
	   	i++;
	 	}
      i=0;
	 	while (theOutputControl->GetNminEnd(i)>0.0)
	 	{
         double StartDepth = theOutputControl->GetNminStart(i);
         double Thickness = theOutputControl->GetNminEnd(i)-StartDepth;
      ...
      i=0;
	 	while (theOutputControl->GetSoilCEnd(i)>0.0)
	 	{
         double StartDepth = theOutputControl->GetSoilCStart(i);
         double Thickness = theOutputControl->GetSoilCEnd(i)-StartDepth;
      ...
      i=0;
  		while (theOutputControl->PoreWaterDepth(i)>0.0)
  		{
         double depth = theOutputControl->PoreWaterDepth(i);
      ...
      i=0;
  		while (theOutputControl->GetTemperatureDepth(i)>=0.0)
  		{
         double depth = theOutputControl->GetTemperatureDepth(i);
