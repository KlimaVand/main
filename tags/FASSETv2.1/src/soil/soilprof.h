...
class soilProfile : public base
{
 ...
 soilLayer * first; // VÆK SENERE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 soilLayer * last;  // VÆK SENERE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
 ...
 // Get functions
 virtual double  GetDiscreteDepth(double depth); // Returns a number >= depth, så that the returned depth does not "divide" two layers
 ...
 double          GetSMB(int numOfLayers); // Gets soil microbial biomass in the specified number of layers in microgram C per gram (Katia)
 double          GetSoilMass(double startDep, double thick);

 double          GetDrainage(double depth);
 double          GetDrainageBottom(){return soilLayerArray[NumOfSoilLayers + ExtraLayers-1]->GetWaterFlux();};
