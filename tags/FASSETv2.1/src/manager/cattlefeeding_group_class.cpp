...
/****************************************************************************\
\****************************************************************************/
cattlefeeding_group_class::cattlefeeding_group_class ()
{
...
\****************************************************************************/
cattlefeeding_group_class::cattlefeeding_group_class(const char * aName, const int aIndex, const base * aOwner)
...
/*******************************************************************
********************************************************************/
void cattlefeeding_group_class::ReadParameters(fstream *infile, bool autoFeed)
{
	Setfile(infile);
   SetCritical();
...
/*******************************************************************
Use when in stand-alone mode
********************************************************************/
void cattlefeeding_group_class::ReadParameters(char *feedFileName, bool autoFeed)
{

   if(!OpenInputFile(feedFileName))
		   {theMessage->FatalError("cattlefeeding manager::ReceivePlan - error in opening feedFileName");}
   else
   {
		ReadParameters(file, autoFeed);
   }
}
...
********************************************************************/
void cattlefeeding_group_class::makePregnant(int daySinceParturition)
{
...
Stop lactation
********************************************************************/
void cattlefeeding_group_class::DryOff(int daySinceParturition)
{
   for(int i=0;i<theGroup->NumOfNodes();i++)
   {
	   cattle * tempcattle = theGroup->ElementAtNumber(i);
...
void cattlefeeding_group_class::ShowGroupDiets(fstream *afile, bool header)
 {
	if (theGroupDiets)
   {
   	if (!header)
...
********************************************************************/
void cattlefeeding_group_class::CheckDiets(bool startup)
{
   if (startup)
   	theCurrentDiet=NULL;
...
********************************************************************/
diet * cattlefeeding_group_class::GetHousedDiet()
{
	diet *aDiet=NULL;
   for(int j=0;j<theGroupDiets->NumOfNodes();j++)
   {
...
********************************************************************/
void cattlefeeding_group_class::FeedCattle(int status)
{
  for(int i=0;i<theGroup->NumOfNodes();i++)
   {
	   cattle * tempcattle = theGroup->ElementAtNumber(i);
...
********************************************************************/
double cattlefeeding_group_class::GetHerbageRequired(bool zeroGrazed)
{
...
********************************************************************/
void cattlefeeding_group_class::ClearGroup()
{
	theGroup->ForgetAll();
}
...
/*******************************************************************
********************************************************************/
void cattlefeeding_group_class::AllocateFieldsForGrazing(linkList <field> *fields_avail_for_grazing,
						 int start, int stop)
{
	...
	if (theCurrentDiet)	//could be NULL if there are beef cattle but not dairy cattle present on farm
   {
      ...
      {
         field *aField = fields_avail_for_grazing->ElementAtNumber(i);
...
********************************************************************/
bsTime * cattlefeeding_group_class::GetTurnoutDate(bool zeroGrazed)
{
	diet *aDiet=NULL;
...
********************************************************************/
bsTime * cattlefeeding_group_class::GetHousingDate(bool zeroGrazed)
{
	diet *aDiet=NULL;
...
/*******************************************************************
********************************************************************/
double cattlefeeding_group_class::GetCattleIntake()
{
//	int feedingGroup;
   double ret_val=0.0;
   double DM, ME, FE, DM_herb, ME_herb, FE_herb;
   for(int i=0;i<theGroup->NumOfNodes();i++)
   {
	   dk_cattle * tempcattle = (dk_cattle*)theGroup->ElementAtNumber(i);
      tempcattle->Getintake(&DM, &ME, &FE, &DM_herb, &ME_herb, &FE_herb);
      DM*=tempcattle->GetNumber();
      DM_herb*=tempcattle->GetNumber();
      ret_val+=DM_herb;
   }
   return ret_val;
}
...
/*******************************************************************
********************************************************************/
void cattlefeeding_group_class::GetStatus(fstream * aFile)
{
...
/*******************************************************************
********************************************************************/
double cattlefeeding_group_class::GetNumber()
{
   double ret_val=0.0;
   for(int i=0;i<theGroup->NumOfNodes();i++)
   {
...
/****************************************************************************\
Get amount of conserved roughage of particular types required)
Only works for all year calving, since population distribution is assumed to be static
\****************************************************************************/
void cattlefeeding_group_class::GetRoughageRequired(linkList <product> *aList)
{
   double number = 0;
   ...
   if (number>0.0)
   {
      for(int i=0;i<theGroupDiets->NumOfNodes();i++)
      {
         diet * aDiet = theGroupDiets->ElementAtNumber(i);
         aDiet->GetRoughageRequired(aList,number);
      }
   }
};
...
/****************************************************************************\
\****************************************************************************/
double cattlefeeding_group_class::GetGrassSilageRequired()
{
	double ret_val=0.0;
   double number =  GetNumber();
   if (number>0.0)
   {
      int startSeason = GetTurnoutDate(true)->GetJulianDay();
      int endSeason = GetHousingDate(true)->GetJulianDay();
      int winter = 0;
      ...
      ret_val=aDiet->GetGrassSilageRequired(number);
      ret_val*=winter;
   }
   return ret_val;
};

double cattlefeeding_group_class::GetSilageRequired(int startCode, int endCode)
{
	double ret_val=0.0;
   double number =  GetNumber();
   if (number>0.0)
...
double cattlefeeding_group_class::GetPotentialMilkYield(int durationOfLactation)
{
   double ret_val= 0;
   for(int j=0;j<theGroup->NumOfNodes();j++)
   {
...
double cattlefeeding_group_class::GetEnergyRequirement()
{
   double ret_val= 0;
   for(int j=0;j<theGroup->NumOfNodes();j++)
   {
		   cattle * tempcattle = theGroup->ElementAtNumber(j);
         ret_val+=tempcattle->GetEnergyRequirement();
   }
   return ret_val;
}

void cattlefeeding_group_class::CalcDefaultDiets(double totalEnergyRequired, double proteinRequired,double differentiation, bool defSurp, fstream *aFile)
{
