...
// Functions for class cattlefeeding_manager_class
 ...
 cattlefeeding_manager_class::cattlefeeding_manager_class ()

{
   feedingGroups= new linkList<cattlefeeding_group_class>;
   dairyPresent=false;
   beefPresent=false;
   firstLateLimit=0.0;
   matureLateLimit=0.0;
   ...
   turnoutDate = NULL;
   housingDate =  NULL;
   fHerdPerformance=NULL;
   fCattleFeeding = NULL;
	theLivestock=NULL;
   dairyLo=0;
   dairyHi=0;				//boundaries of dairy animals in feeding group list
   dairyFeedAdjustmentFactor=0.0;       //used to adjust concentrate feeding in dairy cattle
}


cattlefeeding_manager_class::cattlefeeding_manager_class (const char *aName,const int aIndex,const base * aOwner)
{
   feedingGroups= new linkList<cattlefeeding_group_class>;
   dairyPresent=false;
   beefPresent=false;
...
/*******************************************************************
********************************************************************/
void cattlefeeding_manager_class::Initialize(livestock * theLivestockptr)
{
...
/*******************************************************************
********************************************************************/
void cattlefeeding_manager_class::ReceivePlan()
{
   dairyLo = 3;
	dairyHi=6;
   Setfile(NULL);
   ...
   char buf[200];
   cout << _getcwd(buf,200);
   char fileName[30];
 	strcpy(fileName,"cattlefeedplan.dat");
   if(!OpenInputFile(fileName))
		   {theMessage->FatalError("cattlefeeding manager::ReceivePlan - error in opening cattlefeedplan.dat");}
   else
   {
      GetParameter("startLateLactation",&startLateLactation);
      GetParameter("firstLateLimit",&firstLateLimit);
      GetParameter("matureLateLimit",&matureLateLimit);
      GetParameter("differentiation",&differentiation);
      GetParameter("autoFeed",&autoFeed);

   	int start, stop;
      feedingGroups->ForgetAll();

      string namestr;
		cattlefeeding_group_class * aFeedingGroup;
      for(int inx=0;inx<=18;inx++)
      {
         SetCritical();
         namestr=(string) "Group";
         aFeedingGroup = new cattlefeeding_group_class((char *)namestr.c_str(),inx,this); // Possible LEAK, not sure (bmp) !!!
...
//         ReadFeedingGroupParameters(start, stop);
         for(int inx=start;inx<=stop;inx++)
         {
            ...
            aFeedingGroup->ReadParameters(file, autoFeed);                           //get the diet info for each period
...
/*	   if (beefPresent && dairyPresent)
  ...
  //	CalcTurnoutandHousingDates(true);
   if ((theOutputControl->GetHerdDetails())&& (!fHerdPerformance))
...
/*******************************************************************
********************************************************************/
void cattlefeeding_manager_class::ReadFeedingGroupParameters(int start, int stop)
{
   string namestr;
	cattlefeeding_group_class * aFeedingGroup;
   for(int inx=start;inx<=stop;inx++)
   {
      SetCritical();
//      namestr=(string) "Group";
//      cattlefeeding_group_class * aFeedingGroup = new cattlefeeding_group_class((char *)namestr.c_str(),inx,this); // Possible LEAK, not sure (bmp) !!!
//      feedingGroups->InsertLast(aFeedingGroup);
      aFeedingGroup->ReadParameters(file, autoFeed);                           //get the diet info for each period
		aFeedingGroup->Setactive(true);
   }
...
/*******************************************************************
********************************************************************/
void cattlefeeding_manager_class::SortAnimals(bool Cull)
{
	ClearGroups();
...
/*******************************************************************
********************************************************************/
void cattlefeeding_manager_class::daily_update(int status)
{
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
...
********************************************************************/
double cattlefeeding_manager_class::GetHerbageRequired(bool zeroGrazed =false)
{
...
********************************************************************/
double cattlefeeding_manager_class::GetHerbageRequired(int start, int end, bool zeroGrazed)
{
	double herbageRequired=0.0;
...
********************************************************************/
void cattlefeeding_manager_class::weanCalves(int herdNo, double daysSinceBirth)
{
...
********************************************************************/
dairyStock*  cattlefeeding_manager_class::GetHerd(int whichHerd)
{
   dairyStock* theCattle = NULL;
   char herdName[20];
   if (whichHerd==1)
...
********************************************************************/
void cattlefeeding_manager_class::ClearGroups()
{
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
...
********************************************************************/
void cattlefeeding_manager_class::AllocateFieldsForGrazing(linkList <field> *fields_to_graze,
										int numOthers)
{
...
********************************************************************/
void cattlefeeding_manager_class::CalcTurnoutandHousingDates(bool zeroGrazed)
...
double cattlefeeding_manager_class::GetCattleIntake()
{
	double ret_val=0.0;
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
      ret_val+=aFeedingGroup->GetCattleIntake();
   }
   return ret_val;
}


void cattlefeeding_manager_class::WriteGroupOutput()
{
...
void cattlefeeding_manager_class::InitialiseGroupOutput()
{
   char filename[20];
   char leader[100];
...
//         *fHerdPerformance << "Grazed_FE" << "\t";
   ...
   *fHerdPerformance << " "<< "\t";
   ...
   cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(0);
   for(int i=0;i<=feedingGroups->NumOfNodes();i++)
   {
      *fHerdPerformance << " "<< "\t";
      *fHerdPerformance << " "<< "\t";
...
//         *fHerdPerformance << "Grazed_FE" << "\t";
   ...
   *fHerdPerformance << endl;
   ...
   for(int i=0;i<=feedingGroups->NumOfNodes();i++)
   {
   	cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(i);
      if (aFeedingGroup)
      {
      	if (i==0)
         {
...
void cattlefeeding_manager_class::CheckDiets(bool startup)
{
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
...
void cattlefeeding_manager_class::GetRoughageRequired(linkList <product> *aList)
{
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
   	aFeedingGroup->GetRoughageRequired(aList);
   }
}

double cattlefeeding_manager_class::GetGrassSilageRequired()
{
	double ret_val=0.0;
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
   	ret_val+=aFeedingGroup->GetGrassSilageRequired();
   }
   return ret_val;
}

double cattlefeeding_manager_class::GetSilageRequired()
{
	double ret_val=0.0;
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
...
double cattlefeeding_manager_class::GetPotentialMilkYield(int durationOfLactation)
{
	double ret_val=0.0;
...
double cattlefeeding_manager_class::GetEnergyRequirement()
{
	double ret_val=0.0;
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
   	ret_val+=aFeedingGroup->GetEnergyRequirement();
   }
   return ret_val;
};
...
************************************************************************************/
void cattlefeeding_manager_class::AutoFeed(int conceptionTime, int lengthOfLactation)
{
   char fileName[15];
   char *str = "animals.dat";
   strcpy(fileName,str);
   if(!OpenInputFile(fileName))
      theMessage->FatalError("cattleFeedingManager::cattle database file not found");
   else
   {
	   int numOfFeedingGroups;
   	numOfFeedingGroups=feedingGroups->NumOfNodes();
      for (int i=1; i<=2; i++)
      {
...
/*                        if (k==limitLo+5)  //cast animals
      ...
      } //end loop through herds
   } // end else found file
}

void cattlefeeding_manager_class::GetNumber()
{
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattlefeeding_group_class * aFeedingGroup = feedingGroups->ElementAtNumber(j);
   	aFeedingGroup->GetNumber();
   }
}
