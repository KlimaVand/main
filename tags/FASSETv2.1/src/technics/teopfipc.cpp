      ...
      case varCostTotalNotFert:
         ...
         cost = -1.0 *
               (aGang->GetgangFuel()*theProducts->GetExpectedBuyPriceYear1("DIESEL") +
                ...
                area*theChemical1->GetAmount()*(theChemical1->GetaPrice())->GetExpectedBuyPriceYear(1));
         if (theChemical2)
            cost+= -1.0 *
                area*theChemical2->GetAmount()*(theChemical1->GetaPrice())->GetExpectedBuyPriceYear(1);
         if (theChemical3)
            cost+= -1.0 *
                area*theChemical3->GetAmount()*(theChemical1->GetaPrice())->GetExpectedBuyPriceYear(1);
         break;
      default: cost=0.0;
   }
}
