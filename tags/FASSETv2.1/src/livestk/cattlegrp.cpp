...
\****************************************************************************/
void cattleGroup::SeedGroup(double startAge, double endAge, int aggregation, int *IDnumber)//, fstream * CattlefileStream)
{
    ...
    if(numberOfCohorts>0)
      {
         ...
         for(int i=0;i<numberOfCohorts;i++)
         {
            ...
            if (aggregation==1)
                  tempcow->SetdaysFromParturition(175);   // equates to mean milk yield if lactation lasts whole year
...
/*   if (animalModel==1)
   ...
   if (animalModel==2)
      tempcow= new dk_cattle(GetAlias().c_str(),*IDnumber,this);
   ...
   if (tempcow->GetisFemale())
   {
     ...
     else if ((GetAlias()=="DAIRY") || (GetAlias()=="SUCKLER"))    //only FBEEF3 fall out from the if tree
     {
        ...
        tempcow->SetdaysFromParturition(partOfYear);
		  double conceptionTime; //days between parturition and conception
        double lengthOfLactation;
        if (GetAlias()=="DAIRY")
        {
        	conceptionTime = 49;
