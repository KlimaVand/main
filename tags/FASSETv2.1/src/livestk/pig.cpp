...
/****************************************************************************\
\****************************************************************************/
void pig::GetExtraFeed(double NRequired)
{
   ...
   theProducts->addDemand(feedItemClone);
   ...
   *currentfeed  =  *currentfeed + *feedItemClone;
   delete feedItemClone;
	theMessage->Warning("Pig:: ", GetName()," N deficiency - extra feed fed");
}
   ...
   else
   {
      ...
      for(int index=first;index<(first+num);index++)
      {
         ...
         if (feedAmount>0.0)
         {
            ...
            theProducts->addDemand(feedItemClone);  //register demand
...
double pig::ProduceManure(manure* fluidManure, manure* solidManure)
{
   ...
   double surplus_digestibleN,Nconc;
...
Purpose:
Parameters:  fstream* fileStream
Comments:
\****************************************************************************/
void pig::ManureProduction(fstream* fileStream)
{
	theMessage->FatalError("Function in pig not implemented");
   *fileStream << ".LIQM    0.0\n"; //dummy line which is never reached - stops a warning

/*
   if (Stable->GetSlurrySystem())
   {
   	*fileStream << Name << ".SLUR    " << (amountFluid+amountSolid)*feedingDays()/theTime.daysInYear() << "\n";
	   *fileStream << Name << ".DUNG    0.0\n";
	   *fileStream << Name << ".LIQM    0.0\n";
   }
   else
   {
   	*fileStream << Name << ".SLUR    0.0\n";
...
Comments:
\****************************************************************************/
void pig::FeedMax( fstream* fileStream)
{
   *fileStream << Name << ".FE       " << 2*FE_need << "\n";     // amount just approximately
   *fileStream << Name << ".LYSIN    " << 10*lysin_need*1000 << "\n";
...
Comments:
\****************************************************************************/
void pig::FeedMin( fstream* fileStream)
{
   *fileStream << Name << ".FE       " << FE_need << "\n";
...
void pig::ReadParameters(fstream * file)
{
   ...
   GetParameter("lysin_need",&lysin_need);
   GetParameter("meth_need",&meth_need);
   GetParameter("treo_need",&treo_need);
   UnsetCritical();
   Setfile(NULL);
}
...
\****************************************************************************/
bool pig::TruncateName(string * ret_string)
{
...
/****************************************************************************\
\****************************************************************************/
feedItem * pig::GetFeedItem(product * menuPtr)
{
	feedItem * afeedItem;
...
/****************************************************************************\
\****************************************************************************/
void pig::GetDiet(feedItem * theDiet)
{
   feedItem *afeedItem;
   for (int i=0; i<feedItemList->NumOfNodes(); i++)
   {
      afeedItem = (feedItem*) feedItemList->ElementAtNumber(i);
      *theDiet = *theDiet + *afeedItem;
   }
}
