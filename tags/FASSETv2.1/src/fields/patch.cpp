...
//NJH May 2009, cropName allows variety to be passed via seed name for specific operations
         theEcosystem->Sow((char*)S->GetSpecificCropId(i),(char*)S->GetCropName(),aSeed->GetdryMatter()*aSeed->GetAmount()/10.0,RelativeDensity,aSeed->GetAmount()*aSeed->GetN_content()/10.0); // Convert to g/mï¿½
