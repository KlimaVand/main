                               ...
                               double * greenEvap)
{
   ...
   if (theOutputControl->GetWaterDetails())
   {
   	theOutputControl->ScenarieLog(snowEvap+evap);
   }
...
void ecosystem::TransferRootMatter(decomposable * DeadRoot,double * RootLengthList)
{
   ...
   theOutput->AddIndicator(environmentalIndicator,"32.20 N from roots incorporated into soil","kg N",DeadRoot->GetorgN_content()*DeadRoot->GetAmount()*area*1000.0);
...
void ecosystem::CropSoilExchange()
{
   ...
   if (theOutputControl->GetWaterDetails())
   {
   	theOutputControl->ScenarieLogNewLine();
   	theOutputControl->ScenarieLog(theTime.GetString2());
		theOutputControl->ScenarieLog(precip+irrigation);
		theOutputControl->ScenarieLog(EPot);
   }
   ...
   // 6) Update soil
	aSoil->Update(surplus,temp,soilEpot,snowContent,roots);
  	aSoil->Incorporate(10,0.02);                               // Daily transfer of org. matter to top soil layer (10 mm)
...
//          const crop* aCrop = theCrops->ElementAtNumber(j);  //Eclipse complains about the 'const' NJH, feb 2009
   ...
   */
   ...
   if (theOutputControl->GetWaterDetails())
   {
	  	theOutputControl->ScenarieLog(soilEvaporation);
      theOutputControl->ScenarieLog(totalEvaporation-soilEvaporation);
		theOutputControl->ScenarieLog(aSoil->GetDrainage(aSoil->GetMaximumDepth()));
   }
   EndBudget();
}
...
void ecosystem::GiveIndicators()
{
   ...
   theOutput->AddStateIndicator(environmentalIndicator,"40.60 Total C in soil","kg C",aSoil->GetTotalCarbon()*area*10);
#ifdef COOLCROP
	theOutput->AddStateIndicator(environmentalIndicator,"40.62 Total C in soil 0-30 cm","kg C",aSoil->GetTotalCarbon(0,300)*area*10);
	theOutput->AddStateIndicator(environmentalIndicator,"40.63 Total C in soil 30-90 cm","kg C",aSoil->GetTotalCarbon(300,600)*area*10);
	theOutput->AddStateIndicator(environmentalIndicator,"40.64 Total C in soil 90-270 cm","kg C",aSoil->GetTotalCarbon(900,1800)*area*10);
#endif
...
#ifdef COOLCROP
	theOutput->AddStateIndicator(environmentalIndicator,"32.69 Organic N in soil 0-30 cm","kg N",(aSoil->GetTotalNitrogen(0,300)-aSoil->GetMinNitrogen(0,300))*area*10);
	theOutput->AddStateIndicator(environmentalIndicator,"32.70 Mineral N in soil 0-30 cm","kg N",aSoil->GetMinNitrogen(0,300)*area*10);
	theOutput->AddStateIndicator(environmentalIndicator,"32.71 Organic N in soil 30-90 cm","kg N",(aSoil->GetTotalNitrogen(300,900)-aSoil->GetMinNitrogen(300,900))*area*10);
	theOutput->AddStateIndicator(environmentalIndicator,"32.72 Mineral N in soil 30-90 cm","kg N",aSoil->GetMinNitrogen(300,900)*area*10);
	theOutput->AddStateIndicator(environmentalIndicator,"32.73 Organic N in soil 90-270 cm","kg N",(aSoil->GetTotalNitrogen(900,2700)-aSoil->GetMinNitrogen(900,2700))*area*10);
	theOutput->AddStateIndicator(environmentalIndicator,"32.74 Mineral N in soil 90-270 cm","kg N",aSoil->GetMinNitrogen(900,2700)*area*10);
#endif
...
\****************************************************************************/
bool ecosystem::ReadyForHarvestOrLater()
{
...
Demands that the first crop is the main crop
\****************************************************************************/
bool ecosystem::IrrigationDemand()
{
   if (theCrops->NumOfNodes()>0)
...
\****************************************************************************/
crop * ecosystem::AllocateCrop(char * crop_id, char * cropName)
{
//	Setgrazable(false);
   crop * newCrop=NULL;
   ...
   newCrop->SetCropName(cropName);  //NJH May 2009, allows variety to be passed via seed name for specific operations
//   newCrop->ReadParameters("\\FASSET\\CropParameters.dat");  replace by NJH Jan 2009
...
\****************************************************************************/
void ecosystem::GetFeedResource(feedItem * my_resource, double grazing_height, double min_stubble_ht, int animalType)
{
...
void ecosystem::Initialise(char * soilFileName, int aIndex, double anArea)
{
  	aSoil->Initialize(soilFileName,aIndex);
...
void ecosystem::Sow(char * cropID, char * cropName, double SeedDensDryMatt,double RelativeDensity,double NitrogenInSeed)
{
   crop* aCrop=NULL;
   ...
   {  // Remove all bare "crops"
	...
	aCrop = AllocateCrop(cropID,cropName);    //changed NJH May 2009 to allow seed name to determine crop variety
...
*/
   ...
   for (int i=0;i<theCrops->NumOfNodes();i++)
   {
      ...
      if (!aCrop->IsUndersown() && !aCrop->GetTerminated()||(hrv->GetOperationId()==BeetHarvesting))
      {
         ...
         theOutputControl->ScenarieLogNewLine();
         theOutputControl->ScenarieLog(aCrop->CropName);
         theOutputControl->ScenarieLog(theTime.GetString2());
         if (Storage->GetAmount()>0.0)
         	theOutputControl->ScenarieLog(Storage->GetAmount()*(1.0-fractionStorageLeft)*Storage->GetdryMatter());   //MEL 2009: These amounts are in DM
         else
         	theOutputControl->ScenarieLog(0);
         if (Straw->GetAmount() > 0.0)
         	theOutputControl->ScenarieLog(Straw->GetAmount()*(1.0-fractionStrawLeft)*Straw->GetdryMatter());         //MEL 2009: These amounts are in DM
         else
         	theOutputControl->ScenarieLog(0);
         if (Storage->GetTotalN() > 0.0)
         	theOutputControl->ScenarieLog((Storage->GetTotalN()*Storage->GetAmount()*(1-fractionStorageLeft)).n);
         else
         	theOutputControl->ScenarieLog(0);
         if (Straw->GetAllN() > 0.0)
         	theOutputControl->ScenarieLog((Straw->GetAllN()*Straw->GetAmount()*(1.0-fractionStrawLeft)).n);
         else
         	theOutputControl->ScenarieLog(0);
         ...
         theOutput->AddIndicator(environmentalIndicator,"32.21 N from straw incorporated into soil","kg N",StorageWaste->GetAllN()*StorageWaste->GetAmount()*area*10.0);
         theOutput->AddIndicator(environmentalIndicator,"40.81 C from straw incorporated into soil","kg C",StorageWaste->GetAmount()*StorageWaste->GetC_content()*area*10.0);
      ...
      for (int i=0;i<theCrops->NumOfNodes();i++)
      {
         ...
         if (!c->GetTerminated())
         {
            ...
            */
            ...
            theOutputControl->ScenarieLogNewLine();
            theOutputControl->ScenarieLog(c->CropName);
            theOutputControl->ScenarieLog(theTime.GetString2());
            theOutputControl->ScenarieLog(forage->GetAmount()*forage->GetdryMatter()/1000.0);
            theOutputControl->ScenarieLog((forage->GetTotalN()*forage->GetAmount()).n/1000.0);
...
void  ecosystem::Tillage(soilTreatFields *FOP)
{
	if (FOP->GetOperationId()==Ploughing)
   {
      ...
      Incorporation=1.0; // Otherwise error - correct later !!!
		aSoil->Incorporate(Depth,Incorporation);
   }
...
void ecosystem::outputCropDetails(fstream *afile, bool header)
{
	if (header)
         *afile << "\tcrop number\tpClover";
	else
   {
         int cropNumber = theCrops->NumOfNodes();
         *afile << "\t" << cropNumber;
   ...
   if (theCrops)
   {
      ...
      {
         crop * aCrop = theCrops->ElementAtNumber(i);
         aCrop->StreamKeyData(*afile,header);
      }
   }
}
/****************************************************************************\
