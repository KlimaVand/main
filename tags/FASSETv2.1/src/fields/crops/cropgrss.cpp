...
//	TempSumR2        = 500;             // See p. 160 in DAISY-description
   ...
   // Calibrated to Burrehøjvej data
 	MaxRadUseEff     = 6.0; 			
...
{
   theMessage->WarningWithDisplay("cropRyegrass::GrazeOrCut should never be called - use cropRyegrass::Cut instead. Bjørn, things to do !!!");
   CutOrHarvested = true;
	...
	if (!graze)
   {
      ...
      CutDelay = 300.0*DryMatterTotalStandVegTop*(1.0-fractionLeft)/350.0;       // from K. Søegaard p 21.
