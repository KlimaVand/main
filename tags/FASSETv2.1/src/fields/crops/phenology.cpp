...
void phenology::Update(double AirTemp,double SoilTemp,double DayLength)
{
   ...
   else                              						// Emerged
   {
      ...
      if (DS<1.0)                                     // Until flowering
      	...
      	if (DS<2.0)                                  // Until end of grain filling
         	DS += max(AirTemp-TB2,0.0)/TS2;
         else
         	if (TS3>0 && DS<3.0)                                // Until ripe
...
/****************************************************************************\
\****************************************************************************/
void phenology::ReadParameters(char* filename,string CropName)
{
   UnsetCritical();
   if (OpenInputFile(filename))
   {
      ...
      {
      	GetParameter("TS0",&TS0);
			GetParameter("TS1",&TS1);
			GetParameter("TS2",&TS2);
			GetParameter("TS3",&TS3);
			GetParameter("TB0",&TB0);
   		GetParameter("TB1",&TB1);
			GetParameter("TB2",&TB2);
   		GetParameter("TB3",&TB3);
         GetParameter("DS_Flagligule",&DS_Flagligule);
      }
      CloseInputFile();
   }
}
