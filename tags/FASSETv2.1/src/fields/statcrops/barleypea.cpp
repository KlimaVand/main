...
static_barleyPea::static_barleyPea()
{
	strcpy(cropName,"BARLEYPEA");
}

double static_barleyPea::PVstratFactor(int PV_strat)
{
	double f=1.00;
	switch (PV_strat) {
		case 0: f=0.80;break;
...
/****************************************************************************\
\****************************************************************************/
void static_barleyPea::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
...
cloneList<fieldOperationFields> * static_barleyPea::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
	   ...
	   if (PP->GetFluidManureSpring()>0) { // Apply FLUID manure
		...
		{ // Sowing
         ...
         seed * aSeed2  = new seed;
			...
			aSeed2->Setamount(83);
	      FO->DefineSowFieldsOp(Sowing,cropName,cropName,PP->GetStartYear()+1,4,1,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),reduction,aSeed1);
...
int static_barleyPea::NumOfProtectStrat_()
{
	return 2; // No herbicides possible
}

int static_barleyPea::NitrogenLevels_()
{
	return Nitrogen_levels;
}

double static_barleyPea::NitrogenAmount(int JB_NO,double free_N,int N_level)
{
   #ifdef DEVELOPING
   JB_NO=JB_NO;
   free_N=free_N;
...
double static_barleyPea::ExpectedYield_(int JB_NO,int N_level,int PV_strat)
{
   #ifdef DEVELOPING
   N_level=N_level;
   #endif
...
double static_barleyPea::PhosphorAmount_()
{
	return 25;
}

double static_barleyPea::PotassiumAmount_()
{
	return 50;
}

double static_barleyPea::ExtraNRelease_()
{
	return 35; // check ????
}
