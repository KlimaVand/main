...
#include "beet.h"
	...
	#include "contrparm.h"
#endif
static_beet::static_beet()
{
	strcpy(cropName,"BEET");
}

double static_beet::PVstratFactor(int PV_strat)
{
	double f=1.00;
	switch (PV_strat) {
		case 0: f=0.80;break;
...
void static_beet::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
...
cloneList<fieldOperationFields> * static_beet::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(SowingBeets,crop_id,cropName,PP->GetStartYear()+1,4,10,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),reduction,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
...
int static_beet::NumOfProtectStrat_()
{
	return Num_of_pr_str;
}

int static_beet::NitrogenLevels_()
{
	return Nitrogen_levels;
}

double static_beet::NitrogenAmount(int JB_NO,double free_N,int N_level)
{
	double N=Nopt(JB_NO);
	switch (N_level) {
		case 0: N=0;break;
...
double static_beet::Nopt(int JB_NO)
{
...
\****************************************************************************/
double static_beet::Ymin(int JB_NO)
{
	if (JB_NO>4) // Clayish soil
		return 28.0; // 5 t DM
	else {       // Sandy soil
...
Estimates in the right order of magnitude, but needs further verification
\****************************************************************************/
double static_beet::Ymax(int JB_NO)
{
	if (JB_NO>4) // Clayish soil
		return 61.0; // 11 t DM
	else {       // Sandy soil
...
double static_beet::ExpectedYield_(int JB_NO,int N_level,int PV_strat)
{
...
double static_beet::PhosphorAmount_()
{
	return 25; // From: 'Håndbog for plantedyrkning 1995'
}

double static_beet::PotassiumAmount_()
{
	return 100; // From 'håndbog for plantedyrkning 1995'
}

double static_beet::ExtraNRelease_()
{
	return 10; // 10 from 'håndbog for plantedyrkning 1995'
}

int static_beet::SlyrryPossibleThisPeriod(period * p)
{
...
/****************************************************************************\
\****************************************************************************/
void static_beet::SpecifiedPeriods(fieldOrder * PP,int JB_NO,char * crop_id)
{
