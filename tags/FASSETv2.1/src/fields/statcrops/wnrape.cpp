...
static_wnrape::static_wnrape()
{
	strcpy(cropName,"WNRPE");
}

double static_wnrape::PVstratFactor(int PV_strat)
{
	double f=1.00;
	switch (PV_strat) {
		case 0: f=0.80;break;
...
void static_wnrape::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
...
cloneList<fieldOperationFields> * static_wnrape::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),8,15,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),reduction,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
...
int static_wnrape::NumOfProtectStrat_()
{
	return Num_of_pr_str;
}

int static_wnrape::NitrogenLevels_()
{
	return Nitrogen_levels;
}

double static_wnrape::NitrogenAmount(int JB_NO,double free_N,int N_level)
{
	double N=Nopt(JB_NO);
	switch (N_level) {
		case 0: N=0;break;
...
double static_wnrape::Nopt(int JB_NO)
{
...
double static_wnrape::Ymin(int JB_NO)
{
	if (JB_NO>4) // Clayish soil
		return 2.0;
	else {       // Sandy soil
...
double static_wnrape::Ymax(int JB_NO)
{
	if (JB_NO>4) // Clayish soil
		return 4.0;
	else {       // Sandy soil
...
double static_wnrape::ExpectedYield_(int JB_NO,int N_level,int PV_strat)
{
