...
/****************************************************************************\
\****************************************************************************/
void static_grass::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   for (int i=0;i<PP->GetNumOfGrazingStartDates();i++)
	{
      ...
      FO->DefinemakeGrazableOper(MakeGrazable,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),0.0,0,0.0,true);
      AddOpList(FO);
   }
   for (int i=0;i<PP->GetNumOfGrazingStopDates();i++)
	{
      ...
      makeGrazable * FO=new makeGrazable();
      FO->DefinemakeGrazableOper(MakeGrazable,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),0.0,0,0.0,false);
      AddOpList(FO);
   }
}
...
/****************************************************************************\
\****************************************************************************/
cloneList<fieldOperationFields> * static_grass::MakeOpList (fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      ...
      */
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),9,3,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),1.0,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
...
/****************************************************************************\
\****************************************************************************/
int static_grass::NumOfProtectStrat_()
{
	return 1; 
}
...
/****************************************************************************\
\****************************************************************************/
int static_grass::NitrogenLevels_()
{
	return 1;
}
...
/****************************************************************************\
\****************************************************************************/
double static_grass::ExtraNRelease_()
{
	return 0.0;
}
