   ...
   :base(aName,aIndex,aOwner)
{
   ...
   if (theOutputControl->GetPlantDetails())
   {
      char filename[40];
      char leader[100];
      sprintf(leader,theOutputControl->GetoutputDirectory());
...
void field::ReadPatchParameters(patch *p)
{
   ...
   p->Initialise(soilFileName,Index,patchArea);
}
/****************************************************************************\
\****************************************************************************/
void field::ReadParameters(fstream * file)
{
   ...
   if (FindSection(Name,Index))
   {
      ...
      {
	      patch * p =new patch("patch",0,this);
         ReadPatchParameters(p);
   	   thePatches->InsertLast(p);
		}
...
void field::DivideThisInstance(field * &fP,plan * Pl)
{
   ...
   fP->fenced=fenced;
	fP->JBNO=JBNO;
   fstream * fstr=theMessage->GiveHandle();
...
#endif
                  theTechnics->ExecuteFields(FOP);
           			fstream * file=theMessage->GiveHandle();  // write end of line marker
                  *file << endl;
		            theMessage->CloseHandle();
               }
            }
...
/****************************************************************************\
Parametre: dato,N,mængde,operation,type
Nøgle (unik)	Bedrifts-id	Marknummer	Dato	Index (ved flere op. samme dato)	Hovedafgrøde	Areal (ha)	Operation	Produktnavn	Høstet N (kg)	Mængde (kg - ikke slæt)	Høstet FE
...
void field::OutputPlantDetails(bool header)
{
   ...
   while (P!=NULL)
   {
      P->element->outputCropDetails(&f, header);
      thePatches->OneStep(P);
   }
   f << endl;
}
