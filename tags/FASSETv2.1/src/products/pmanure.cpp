...
double manure::SubtractWater(double waterToSubtract)
{
 ...
 if (waterToSubtract!=0.0)
 {
    ...
    if (fabs(water-waterToSubtract)<0.00000001) waterToSubtract=water;   //to avoid false error caused by precision in double
...
/****************************************************************************\
\****************************************************************************/
void manure::AddWater(double waterToAdd)
{
 	double water = GetAmount()*(1-GetdryMatter());
 	double DM = GetAmount()*GetdryMatter();
   water+=waterToAdd;
   ...
   sand 	     	 = Getsand() * GetAmount()/(water+DM);
   degradableOM  = GetdegradableOM() * GetAmount()/(water+DM);
   liveMatter::AddWater(waterToAdd);
}
...
{    //returns evaporation in tonnes
   ...
   if (resistance > 0.0)
   {
      ...
      if (result>GetAmount())  //all water evaporates
      {
...
/****************************************************************************\
  Operator << for output
\****************************************************************************/
ostream& operator<< (ostream& os, const manure& amanure)
{
  	os << (decomposable &) amanure;
  	os << setiosflags(ios::left) << setw(40) << "fromAnimal " << amanure.fromAnimal << endl;
  	os << setiosflags(ios::left) << setw(40) << "Mg_content " << amanure.Mg_content << endl;
  	os << setiosflags(ios::left) << setw(40) << "Cu_content " << amanure.Cu_content << endl;
  	os << setiosflags(ios::left) << setw(40) << "Rt_content " << amanure.Rt_content << endl;
  	os << setiosflags(ios::left) << setw(40) << "sand " << amanure.sand << endl;
	os << setiosflags(ios::left) << setw(40) << "pH " << amanure.pH << endl;
  	return os;
}
...
product& manure::operator+(const product& someManure)
{
	manure* aManure;
	aManure = (manure*)(&someManure);
   if ((fromAnimal == "")&&(aManure->GetfromAnimal()!=""))
	   fromAnimal =  aManure->GetfromAnimal();
   else if ((aManure->GetfromAnimal()!="") && (fromAnimal != aManure->GetfromAnimal()))
	   	fromAnimal="mixed";
	...
	double avolatisation = aManure->Getvolatisation();
   ...
   double adegradableOM = aManure->GetdegradableOM();
	...
	if((amount>1E-6) && (aAmount>1E-6))
      ...
      volatisation   = (volatisation*amount + avolatisation*aAmount)/(amount+aAmount);
...
/****************************************************************************\
\****************************************************************************/
void manure::ReadParameters(fstream * file)
{
	decomposable::ReadParameters(file);
   Setfile(file);
   SetCritical();
//	GetParameter("infiltFactor",&infiltFactor);
   UnsetCritical();
   GetParameter("fromAnimal"  ,&fromAnimal);
   GetParameter("Mg_content"  ,&Mg_content);
	...
	GetParameter("volatisation",&volatisation);
	GetParameter("pH"          ,&pH);
   SetCritical();
   Setfile(NULL);
}
...
/****************************************************************************\
Called by animal housing and storage
\****************************************************************************/
void manure::VolatNitrogen(double temperature, double area, double resistance, double duration,
			double *volatAmmonia, double *N2Emission, double *N2OEmission)
{
	*N2Emission=0.0;
   *N2OEmission=0.0;
   *volatAmmonia=0.0;
 	 *volatAmmonia = InstantaneousVolatAmmonia(temperature, area, resistance, duration);
    doDenitrification(temperature, duration/24.0, N2Emission,N2OEmission);
// denitrification losses are here removed after ammonia volatilisation - this needs to be integrated later??
}

/****************************************************************************\
Calculate the manure-dependent infiltration rate
\****************************************************************************/
double manure::GiveInfiltrationReduction()
{
  double ret_val = 0.0;
  ret_val = exp(-infiltFactor * 100.0 * dryMatter);
  return ret_val;
}

/****************************************************************************\
Calculate the manure-dependent resistance to infiltration
\****************************************************************************/
double manure::GiveInfiltrationResistance()
{
  double ret_val = 0.0;
  ret_val = infiltFactor * dryMatter;
  return ret_val;
}
...
/*void manure::GetCgasEmissions(double temperature, double duration, double *CO2, double *CH4)
{  //returns CO2-C and CH4-C emissions in kg
   *CO2 = GetCarbonDioxideEmission(temperature, duration);
	*CH4 = GetMethaneEmission(*CO2);
    double C = C_content * amount;
    C-= (*CO2 + *CH4);
    SetC_content(C/GetAmount());
    CalcDryMatterConc();
...
double manure::MoistureFunc()
{
   ...
   if ((waterContent<w1) || (waterContent>w4))
	   ...
	   if ((waterContent>w2) && (waterContent<w3))
	   	ret_val= 1.0;
      else
		   if ((waterContent>w1) && (waterContent<w2))
