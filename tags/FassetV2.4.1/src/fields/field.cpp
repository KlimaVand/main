...
void field::AdjustPatchAreas(double adjust,bool balance)
{
   ...
   if (balance)
   {
      ...
      theOutput->AddIndicator(3701,"37.01 N crop difference from area adjustment","kg N",NcropBefore-NcropAfter);
      theOutput->AddIndicator(3702,"37.02 N soil difference from area adjustment","kg N",NsoilBefore-NsoilAfter);
   }
}
...
void field::SplitMe(field * &fP)
{
   if (active)
   {
	   ofstream * fstr;
      fP=nullptr;
      if ((crrPlan==nullptr) || (crrPlan->Finished()))
      {
         cloneList<plan>::PS P;
         Lfutr->PeekHead(P);
         if (P!=nullptr)
...
{
   if (aPlan!=nullptr && !aPlan->Finished()) // Last term added 28.02.2007 by BMP for speed and transparency
	{
...
void field::UpdateField()
{
 ...
 //  cout << " " << GetIndex();
   if (active)
   {
      ...
      {
		  if(theTime.GetMonth()==9&&theTime.GetDay()==15&&theTime.GetYear()==2012)
			  cout<<endl;
         ...
         if (FOP!=nullptr)
         {
            ...
            if (!(FOP->IsIrrigationOp() && crrPlan->AutomaticIrrigation()))
            {
               if (HandleOperation(FOP))
               {
				  ...
				  if ((FOP->GetOperationId()==GatheringBales) || (FOP->GetOperationId()==CombineHarvesting) || (FOP->GetOperationId()==CombineHarvestingShred)  || FOP->GetOperationId()==BeetHarvesting)   // "Mowing" moved down to forage section NJH mar 07 !!!
					{
...
void field::OutputPlantDetails(bool header)
{
	if (header)
	{
      ...
      for (int i=0;i<thePatches->NumOfNodes();i++)
      {
         ...
         {
            f << "\tRootLength-"<< globalSettings->RootData[i].getStartDepth() << "-"<< globalSettings->RootData[i].getEndDepth();
         	f << "\tRootMass-"<< globalSettings->RootData[i].getStartDepth() << "-"<< globalSettings->RootData[i].getEndDepth();
         	f << "\tRootN-"<< globalSettings->RootData[i].getStartDepth() << "-"<< globalSettings->RootData[i].getEndDepth();
      	}
      }
	  f << endl;
	    f << "date";
	  ...
	  for (int i=0;i<thePatches->NumOfNodes();i++)
      {
    		...
    		f << "\tCropName\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2";
    	     	   f << "\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2";
     	   f << "\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2";
     	   f << "\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2";
         f << "\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2\tg C/m2";
         for(unsigned int i=0;i<globalSettings->RootData.size();i++)
         {
            f << "\tg C/m2";
         	f << "\tg C/m2";
...
//needs rewriting for multiple patches
   ...
   theOutput->AddIndicator(3108,"31.08 N in faeces from grazing","kg N",animal_dung->GetAmount()*animal_dung->GetAllN().n*1000.0);
   theOutput->AddIndicator(4109,"41.09 C in faeces from grazing","kg C",animal_dung->GetAmount()*animal_dung->GetC_content()*1000.0);

// new_area_dung+=dung->GetAmount()*110.0;  //OLD METHOD - area covered per tonne fresh weight from During and Weeda 73, NZJAS

   theOutput->AddIndicator(3107,"31.07 N in urine from grazing","kg N",animal_urine->GetAmount()*animal_urine->GetAllN().n *1000.0);
   theOutput->AddIndicator(4108,"41.08 C in urine from grazing","kg C",animal_urine->GetAmount()*animal_urine->GetC_content() *1000.0);
      // Calculate ammonia volatilisation
   theOutput->AddIndicator(3146,"31.46 NH3-N loss during grazing","kg N",prop_lost_volat * animal_urine->GetAmount()
