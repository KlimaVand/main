...
/****************************************************************************\
\****************************************************************************/
cloneList<fieldOperationFields> * staticCloverGrass::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      if (!permanent)
      {
         ...
         { // Sowing
            ...
            FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),9,3,PP->GetArea(),0,PP->GetDistance(),2,aSeed1);
		...
		//first harvest
      {
         ...
         FO->DefineHarvestFieldsOper(ForageHarvesting,crop_id,cropName,PP->GetStartYear()+1,6,1,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      //second harvest
      {
         ...
         aPlantItem->Setamount(ExpectedYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(ForageHarvesting,crop_id,cropName,PP->GetStartYear()+1,7,15,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         FO->SetForageProduct(aPlantItem);
         AddOpList(FO);
         delete aPlantItem;
      }
      //third harvest
      {
         ...
         aPlantItem->Setamount(ExpectedYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(ForageHarvesting,crop_id,cropName,PP->GetStartYear()+1,9,5,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         FO->SetForageProduct(aPlantItem);
         AddOpList(FO);
         delete aPlantItem;
      }
