   ...
   for (int i=0;i<PP->GetNumOfHarvestDates();i++)
	{
      ...
      FO->DefineHarvestFieldsOper(ForageHarvesting,crop_id,cropName,PP->GetYear(date),PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
