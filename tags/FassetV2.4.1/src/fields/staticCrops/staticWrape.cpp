...
void staticWrape::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage->FatalError("static_wnrape::SpecifiedPlan - more than one harvest date specified");
}
...
cloneList<fieldOperationFields> * staticWrape::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),8,15,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
    ...
    if (((FLUID_HUG_spring<=0) && (PP->GetSolidManure()>0)) && ((PP->GetN()>0) || (PP->GetK()>0) || (PP->GetP()>0))) { // Apply mineral fertilizer
      ...
      { // Sk�rl�gning
         ...
         FO->DefineHarvestFieldsOper(Mowing,crop_id,cropName,PP->GetStartYear()+1,7,20,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      { // Using harvester
         ...
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,PP->GetStartYear()+1,8,2,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
