...
{
#ifdef NITROSCAPE
	thePatchInterface				= new patchInterfaceClass("patch",0,this);
#else
...
#ifndef NITROSCAPE
         Sow((char*)S->GetSpecificCropId(i),aSeed->GetdryMatter()*aSeed->GetAmount()/10.0,RelativeDensity,aSeed->GetAmount()*aSeed->GetN_content()/10.0); // Convert to g/m�
#else
			Sow((char*)S->GetSpecificCropId(i),(char*)S->GetCropName(),aSeed->GetdryMatter()*aSeed->GetAmount()/10.0,RelativeDensity,aSeed->GetAmount()*aSeed->GetN_content()/10.0); // Convert to g/m�
#endif
...
void patch::SetGrazed(double bite_depth_factor, double DMGrazed)
{
   ...
   double grazing_height= bite_depth_factor * GetMaxPatchHeight();
	SendGrazed(grazing_height, DMGrazed);
}
