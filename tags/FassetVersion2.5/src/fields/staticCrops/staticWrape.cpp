...
void staticWrape::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
...
cloneList<fieldOperationFields> * staticWrape::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      ...
      if ((FLUID_HUG_fall<=0) && (PP->GetN()>0)) { // Apply mineral fertilizer
         ...
         FO->DefineMineralFertFieldsOp(MineralFertilizing,crop_id,cropName,PP->GetStartYear(),9,1,PP->GetArea(),0,PP->GetDistance(),aFertilizer,nullptr,nullptr);
         N-=Napply;
         AddOpList(FO);
         delete aFertilizer;
      }
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),8,15,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),0,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
      if (PP->GetPVStrat()==2) { // Spraying herbicide
         ...
         FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear(),9,15,PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
         AddOpList(FO);
         delete aChemical1;
         delete aWater;
      }
    ...
    if (((FLUID_HUG_spring<=0) && (PP->GetSolidManure()>0)) && ((PP->GetN()>0) || (PP->GetK()>0) || (PP->GetP()>0))) { // Apply mineral fertilizer
      ...
      if (PP->GetPVStrat()>0) { // Spraying insecticide
         ...
         aWater->Setamount(500); 
         plantCareFields * FO=new plantCareFields;
         FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,6,1,PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
         AddOpList(FO);
         delete aChemical1;
         delete aWater;
      }
      ...
      { // Skårlægning
         ...
         aPlantItem->Setamount(ExpectedYield_(JB_NO,PP->GetNLevel(),PP->GetPVStrat())+ExpectedStrawYield_(JB_NO,PP->GetNLevel(),PP->GetPVStrat()));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(Mowing,crop_id,cropName,PP->GetStartYear()+1,7,20,PP->GetArea(),0,PP->GetDistance(),0,false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      { // Using harvester
         ...
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,PP->GetStartYear()+1,8,2,PP->GetArea(),0,PP->GetDistance(),0,false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
...
int staticWrape::NumOfProtectStrat_()
{
	return Num_of_pr_str;
}

int staticWrape::NitrogenLevels_()
{
	return Nitrogen_levels;
}

double staticWrape::NitrogenAmount(int JB_NO,double free_N,int N_level)
{
	double N=Nopt(JB_NO);
	switch (N_level) {
		case 0: N=0;break;
...
double staticWrape::ExpectedYield_(int JB_NO,int N_level,int PV_strat)
{
...
double staticWrape::PhosphorAmount_()
{
	return 25; // From: 'Håndbog for plantedyrkning 1995'
}

double staticWrape::PotassiumAmount_()
{
	return 100; // From 'håndbog for plantedyrkning 1995'
}

double staticWrape::ExtraNRelease_()
{
	return 25; // 25 - but not achievable with current C turnover model!
              // 25 from 'håndbog for plantedyrkning 1995'
}

int staticWrape::SlyrryPossibleThisPeriod(period * p)
{
   period * p1 =new period(8,19,8,20);
   period * p2 =new period(3,20,4,10);
   bool overlap=(p->Overlap(*p1)>0 || p->Overlap(*p2)>0);
   delete p1;
   delete p2;
   return overlap;
}
