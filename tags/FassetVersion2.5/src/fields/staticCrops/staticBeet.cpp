...
cloneList<fieldOperationFields> * staticBeet::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(SowingBeets,crop_id,cropName,PP->GetStartYear()+1,4,10,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),0,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
      ...
      {
         soilTreatFields * FO=new soilTreatFields;
         FO->DefineOp(RowCultivating,crop_id,cropName,PP->GetStartYear()+1,6,20,PP->GetArea(),0,PP->GetDistance());
         AddOpList(FO);
      }
      ...
      {  // Mangler forskel mellem bånd- og bredsprøjtning. Er det rette kemikalier? !!!
         ...
         FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,5,1,PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
         AddOpList(FO);
         delete aChemical1;
         delete aChemical2;
         delete aWater;
      ...
      {  // Mangler forskel mellem bånd- og bredsprøjtning. Er det rette kemikalier? !!!
         ...
         aWater->Setamount(500);
         plantCareFields * FO=new plantCareFields;
         FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,5,10,PP->GetArea(),0,PP->GetDistance(),aChemical1,aChemical2,nullptr,aWater);
         AddOpList(FO);
         delete aChemical1;
         delete aChemical2;
         delete aWater;
      ...
      {  // Mangler forskel mellem bånd- og bredsprøjtning. Er det rette kemikalier? !!!
         ...
         aWater->Setamount(500);
         plantCareFields * FO=new plantCareFields;
         FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,7,1,PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
         AddOpList(FO);
         delete aChemical1;
         delete aWater;
      }
      ...
      { // Removing top
         ...
         aPlantItem2->Setamount(ExpectedStrawYield_(JB_NO,PP->GetNLevel(),PP->GetPVStrat()));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(BeetTopChopping,crop_id,cropName,PP->GetStartYear()+1,11,1,PP->GetArea(),0,PP->GetDistance(),0,true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      { // Harvesting beets
         ...
         aPlantItem2->Setamount(ExpectedStrawYield_(JB_NO,PP->GetNLevel(),PP->GetPVStrat()));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(BeetHarvesting,crop_id,cropName,PP->GetStartYear()+1,11,2,PP->GetArea(),0,PP->GetDistance(),0,true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
			FO->SetMainProduct(aPlantItem1);
         FO->SetStrawProduct(aPlantItem2);
         AddOpList(FO);
      }
...
int staticBeet::NumOfProtectStrat_()
{
	return Num_of_pr_str;
}

int staticBeet::NitrogenLevels_()
{
...
double staticBeet::PhosphorAmount_()
{
	return 25; // From: 'Håndbog for plantedyrkning 1995'
}

double staticBeet::PotassiumAmount_()
{
	return 100; // From 'håndbog for plantedyrkning 1995'
}

double staticBeet::ExtraNRelease_()
{
	return 10; // 10 from 'håndbog for plantedyrkning 1995'
}

int staticBeet::SlyrryPossibleThisPeriod(period * p)
{
