...
public:
   field(const char * aName, const int aIndex, const base * aOwner = nullptr);
	field(const field& f);
	~field();
	cloneList<patch> * getPatchList(){ return thePatches;};


	void ReadPatchParameters(patch *p, commonData * &data);
