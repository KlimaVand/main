 ...
 * @param cropName
 */
cropMaize2::cropMaize2(const char * aName, const int aIndex, const base * aOwner,
		string cropName) :
...
double cropMaize2::Update(double ActivePar) {
	...
	if (Phenology->Sown() && !Phenology->Ripe() && !terminated) {
		...
		Phenology->Update(temp, soilTemp, DayLength);
...
/*
 *
 * The daily nitrogen increment (dNTOT) is defined as
 * the minimum of the N demand by the crop ( Nd ),
 * and the maximum N amount that the whole root system can absorb
 *
 * Adding Nitrogen uptake to the crop and replace Transpiration Ratio
 */
void cropMaize2::NitrogenAndTranspiration(nitrogen aNitrogenUptake,
		double aTranspirationRatio) {

	TranspirationRatio = aTranspirationRatio;
	// corrects problem with transpiration the day of germination
	if ((TranspirationRatio < 0.5) && (YellowCropAreaIndex + GreenCropAreaIndex < 0.01))
		TranspirationRatio = 1.0;

	if (aNitrogenUptake.n > 1e-15) {
		Nitrogen = Nitrogen + aNitrogenUptake;
		if (Nitrogen.n <= 0)
			theMessage->FatalError("negative nitrogen in NitrogenAndTranspiration");
...
double cropMaize2::LeafAreaFraction(double height) const {
	...
	if (height >= PlantHeight)
		return 1.0;
	return (a + 1) * (a + 2) * (pow(height, a + 1)) / (pow(PlantHeight, a + 2)) * (1 / (a + 1) - height / ((a + 2) * PlantHeight));
}
...
double cropMaize2::RLD_antiderivative(double z) { // Bleken Konor p.19ff
	...
	if ((0 <= z) && (z < z_3))
		...
		v = ((z_3 - (z_3 * z_3 * z_1) / (2 * z_3)) * RLD_0) + ((-1 * (1 - z_1) * exp(-z_2 * (z - z_3)) * RLD_0 / z_2) + ((z_3 - (z_3 * z_3 * z_1) / (2 * z_3)) * RLD_0));
...
double cropMaize2::GiveRootInInterval(double startDep, double thick,
	...
	double res = TotalRootLength / (RootLengthInInterval(startDep, thick) * input);
	return res;
}
...
bool cropMaize2::EndBudget(double * NRemain, double * DMRemain) {
	bool retVal = true;
	*NRemain = Nitrogen.n;
...
double cropMaize2::GreenLeafAreaIndex(double height, double thickness) {
	...
	double UpperArea = (GreenCropAreaIndex + YellowCropAreaIndex) * LeafAreaFraction(height + thickness) - YellowCropAreaIndex;
