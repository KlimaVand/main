...
//   feedName = ""; DKfeedName = ""; plantName = "";
   ...
   /*   cout << "string length " << instring.length() << endl;
      ...
      for (int I=J;I<int(instring.length());I++)    //parse line
      {
         testchar = instring[I];
		 int testint;
         if ((((int (instring[I])==9)==false) && (int (instring[I])==32)==false))    //if not tab
         {
      ...
      ME								= 0.81 * inarray[51] * 1000.0 * dryMatter; //MJ per tonne FW, note feed table contains digestible energy, so use 0.81 to convert (SCA (1990))
