...
cloneList<fieldOperationFields> * staticWbarley::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   {   // Automatic plan
      ...
      { // Use harvester
			...
			if (gatheringStraw)
	   		FO->DefineHarvestFieldsOper(CombineHarvesting,crop_id,cropName,PP->GetStartYear()+1,7,20,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         else
				FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,PP->GetStartYear()+1,7,20,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      if (gatheringStraw)
	   { // Gather straw
	   	...
	   	theProducts->GiveProductInformation(aPlantItem2);
	   	aPlantItem2->Setamount(ExpectedStrawYield_(JB_NO,0,2));
	   	FO->DefineHarvestFieldsOper(GatheringBales,crop_id,cropName,PP->GetStartYear()+1,7,21,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
