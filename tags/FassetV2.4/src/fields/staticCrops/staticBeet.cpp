...
void staticBeet::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   //---------------------------------------------------------------------
	...
	FO->DefineHarvestFieldsOper(BeetHarvesting,crop_id,cropName,year,month,day,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
...
cloneList<fieldOperationFields> * staticBeet::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(SowingBeets,crop_id,cropName,PP->GetStartYear()+1,4,10,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
     ...
     // Spraying herbicide 1. time
      {  // Mangler forskel mellem b�nd- og bredspr�jtning. Er det rette kemikalier? !!!
      ...
      { // Removing top
         ...
         aPlantItem2->Setamount(ExpectedStrawYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(BeetTopChopping,crop_id,cropName,PP->GetStartYear()+1,11,1,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      { // Harvesting beets
         ...
         aPlantItem2->Setamount(ExpectedStrawYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(BeetHarvesting,crop_id,cropName,PP->GetStartYear()+1,11,2,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
			FO->SetMainProduct(aPlantItem1);
         FO->SetStrawProduct(aPlantItem2);
         AddOpList(FO);
      }
