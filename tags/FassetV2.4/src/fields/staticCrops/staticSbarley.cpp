      ...
      for (int i=1;i<PP->GetNumOfHarvestDates();i++)             // cooping with several harvesting dates (i.e. clover/grass)
		{
      	...
      	harvestFields * FO=new harvestFields;
      	FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetYear(date),PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
...
/****************************************************************************\
\****************************************************************************/
cloneList<fieldOperationFields> * staticSbarley::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         */
            ...
            strcpy(input,"B2");
            FO->DefineSowFieldsOp(Sowing,input,cropName,PP->GetStartYear()+1,4,6,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
         }
         else
         	FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear()+1,4,6,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
         AddOpList(FO);
         delete aSeed;
         if (aSeed1)
         {
