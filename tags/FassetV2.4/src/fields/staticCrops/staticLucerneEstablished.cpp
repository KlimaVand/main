...
cloneList<fieldOperationFields> * staticLucerneEstablished::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan - three cuts
...

      {
         ...
         FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetStartYear()+1,6,20,PP->GetArea(),0,PP->GetDistance(),true);
      ...
      }
      {
         ...
         aPlantItem->Setamount(ExpectedYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetStartYear()+1,7,26,PP->GetArea(),0,PP->GetDistance(),true);
         FO->SetMainProduct(aPlantItem);
         AddOpList(FO);
         delete aPlantItem;
      }
      {
         ...
         aPlantItem->Setamount(ExpectedYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetStartYear()+1,9,12,PP->GetArea(),0,PP->GetDistance(),true);
         FO->SetMainProduct(aPlantItem);
         AddOpList(FO);
         delete aPlantItem;
      }
