...
void staticSwheat::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage->FatalError("static_spwht::SpecifiedPlan - more than one harvest date specified");
...
/****************************************************************************\
\****************************************************************************/
void staticSwheat::BasicOpList(fieldOrder * PP,int JB_NO,char * crop_id)
{
	...
	{ // Sowing
		...
		FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear()+1,4,10,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
		AddOpList(FO);
		delete aSeed;
	}
/*	if (2==2) { // Spraying herbicide     ??????       Hvordan spr�jtes v�rhvede?????
   ...
   FO->DefineHarvestFieldsOper(CombineHarvesting,crop_id,cropName,PP->GetStartYear()+1,8,21,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
   ...
   FO1->DefineHarvestFieldsOper(GatheringBales,crop_id,cropName,PP->GetStartYear()+1,8,22,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
