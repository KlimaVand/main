...
\****************************************************************************/
void soil::Incorporate(double MaxDepth, double frac, bool tillage, const char * tillagetype,double MinDepth)
{
   ...
   if ((MaxDepth>0 && frac>0))
   {
      ...
      if (tillage)
      {
         ...
         data.FindItem("TurnoverFactor",&TurnoverFactor);
            	...
            	KillSMB(p1,MaxDepth);
         	if (p2 > 0.0)
            	ModifyAOM(p2,TurnoverFactor,MaxDepth);
      	}
...
void soil::WriteOutput()
{
 	...
 	if(globalSettings->DetailsData.getSoilDetails()==true)
	{
      ...
      if (Profile->GetN2OFromNitrification().n+Profile->GetN2OFromDenitrification().n>1e-10)
         ...
         << "\t" << CO2Evolution;
      f << "\t" << Surface->GetCarbon()
    	<< "\t" << Surface->GetOrganicNitrogenN()
    	<< "\t" <<  Surface->GetTotalNitrogenN();
      f << endl;
   }
}
