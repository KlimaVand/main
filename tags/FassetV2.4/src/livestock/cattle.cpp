...
//   uterusProtein				= anAnimal.uterusProtein;
   ...
   isYoung						= anAnimal.isYoung;
	feedPlanList 				= nullptr;//must be set explicitly later
   feedUseList					= nullptr;
  ...
  // double amountManure = fluidManurePrDay->GetAmount() + solidManurePrDay->GetAmount();
   ...
   if (hoursHoused>0.0)
   {
      ...
      if (theSection)
      {
         ...
         theOutput->AddIndicator(3381,"33.81 Cattle manure N to animal housing","kg N",manureNToHousing*1000.0);
         theOutput->AddIndicator(4381,"43.81 Cattle manure C to animal housing","kg C",manureCToHousing*1000.0);
		   ...
		   for (int i=0;i<fieldList->NumOfNodes(); i++)
         	...
         	if (fieldOfftakeList[i]>0.0)
            {
               ...
               double timeDistribution=(24.0 - hoursHoused)/24.0;
		         ...
		         theOutput->AddIndicator(3380,"33.80 Cattle manure N to pasture","kg N",manureNToPasture*1000.0);
