...
/**
 *  Constructor with arguments. Also call base(const char * aName, const int aIndex = -1, const base * aOwner = nullptr);
 */
               ...
               if (((GetareFemale())&& (aNode->element->GetisFemale()))||       //check if sex of candidate is ok for group
                   ((!GetareFemale())&& (!aNode->element->GetisFemale())))
                {
                  ...
                  *IDnumber=*IDnumber + 1;
                  ...
                  if (theanimalSection) aNode->element->SetanimalSection(theanimalSection);
      ...
      if (tempcattle->GetisFemale())
      {
         ...
         if (aCalf)
         {
         	if (!aCalf->GetisSuckling())
	         {
               ...
               aCalf->SetfeedPlanList(nullptr);  //calf uses same feedplanlist as mother, so set to nullptr to avoid double delete
...
/*		if (indicatorType==environmentalIndicator)
 ...
 /*if (indicatorType==economicIndicator)
 {
   ...
   if (areYoung)
   {
		theOutput->AddStateIndicator(350,"03.50 Value young cattle","Dkr",value);
		theOutput->AddStateIndicator(1150,"11.50 Young cattle present","no",Number);
   }
   else
   {
		theOutput->AddStateIndicator(351,"03.51 Value mature cattle","Dkr",value);
		theOutput->AddStateIndicator(1151,"11.51 Mature cattle present","no",Number);
   }
 }*/
};
...
//if (tempcow->GetIndex()==29)
  //	cout <<"";
   ...
   if (logit)
   {
      ...
      delete anAnimalProduct;
      if (globalSettings->DetailsData.getHerdDetails()==true)
      {
         char outstring[200];
         strcpy(outstring,GetAlias().c_str());
