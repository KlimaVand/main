   ...
   :base(aName,aIndex,aOwner)
{
   ...
   // Root
	RootpF               = theCropData->getRootpF();          // Water potential at wilting point
   RootPentrRate      	= theCropData->getRootPentrRate();        // Root penetration rate (m d-1 deg C-1)
	RootDistrParm        = theCropData->getRootDistrParm();          // Root density distribution parameter
	TempRootMin          = theCropData->getTempRootMin();          // Minimum temperature for root growth  new value by JB 19_3_3
	SpecificRootLength   = theCropData->getSpecificRootLength(); //191;   // Specific root length, m pr. kg.
	RootDensAtBottom     = theCropData->getRootDensAtBottom(); //191;   // Rootdensity at rootdepth (100*191/100)

   MinimumRootDens      = theCropData->getMinimumRootDens(); //500;   BMP: why zero ???!!!
	InitialRootDepth     = theCropData->getInitialRootDepth(); 	       // Root depth by germination
   NitrateUptakeRate  	= theCropData->getNitrateUptakeRate();     // Uptake rate for nitrate-N g/m/d (m root length)
	AmmoniumUptakeRate   = theCropData->getAmmoniumUptakeRate();       // NEW value from DAISY (JB 2002-8) 0.0031;       // Uptake rate for ammonium-N g/m/d (m root length)    0.06*100/191
   MinimumSoilNitrate 	= theCropData->getMinimumSoilNitrate();          // Minimum concentration of nitrate-N left in soil water (g/l)
   MinimumSoilAmmonium	= theCropData->getMinimumSoilAmmonium();          // Minimum concentration of ammonium-N left in soil water (g/l)
   DecayRoot            = theCropData->getDecayRoot();          // Root decay given as a daily fraction of DM at 10 deg. Celsius
   MaxRootDepthCrop     = theCropData->getMaxRootDepth();          // Maximal root depth for this crop

   // DM production and translocation
   TopFraction           = theCropData->getTopFraction();        // Fraction of seed that goes into top
	ConversionCoefficient = theCropData->getConversionCoefficient();         // Coefficent of DM conversion from vegetative to storage
   GerminationConversion = theCropData->getGerminationConversion();         // Conversion factor seed DM -> plant DM
   MinAllocToRoot        =  theCropData->getMinAllocToRoot();;         // Minimal fraction of dry matter production that is allocated to the root
	MaxAllocToRoot        = theCropData->getMaxAllocToRoot();         // Maximal fraction of dry matter production that is allocated to the root
   MaxRadUseEff          = theCropData->getMaxRadUseEff();        // Maximum radiation use efficiency (g/MJ)
	MinDMtemp             = theCropData->getMinDMtemp();           // Minimum temperature for dry matter production (4 in Hansen et al)
	MaxDMtemp             = theCropData->getMaxDMtemp();          // Maximum temperature for dry matter production
   StoreForFilling		 = theCropData->getStoreForFilling();        // Fraction of DM present at initiation of grain filling that is translocated to grain
   ...
   // Nitrogen
	NCurveParam 			= theCropData->getNCurveParam();                    // Parameter giving the N-curve shape

   MinN_Store    			= theCropData->getMinN_Store();               // Minimum content of nitrogen i storage organs (Estimeret fra Elly's data)
	MaxN_Store    			= theCropData->getMaxN_Store();               // Maximum content of nitrogen i storage organs
	MinN_Root     			= theCropData->getMinN_Root();           	 // Minimum content of nitrogen i root organs
	MaxN_Root     			= theCropData->getMaxN_Root();           	 // Maximum content of nitrogen i root organs
	PowDM         			= theCropData->getPowDM();             	 // Power of dry matter in nitrogen status calculation  (Justes et al., 1994)
   NPctMax       			= theCropData->getNPctMax();              	 // Following Justes concept (Justes et al., 1994)
   NPctMin       			= theCropData->getNPctMin();       	 		 // Following Justes concept (Justes et al., 1994)
   ReducedNInYellow  	= theCropData->getReducedNInYellow();             // The min and max N for yellow VegTop is reduced with this parameter

	// LAI and canopy structure
	k                 	= theCropData->getk();            // Extinction coefficient for photosynthetic active radiation
   InterceptCoeff    	= theCropData->getInterceptCoeff();             // Coeff. for interceptioncapacity.
   PhotoSyntActPart  	= theCropData->getPhotoSyntActPart();            // Photosynthetic active part of total global radiation
   CoeffGreenIdx     		= theCropData->getCoeffGreenIdx();          // Coefficient for leaf growth
   LAINitrogenRatio  		=theCropData->getLAINitrogenRatio();             // Maximum ratio between LAI and nitrogen in veg top
   LAIDMRatio        		= theCropData->getLAIDMRatio();           // Maximum ratio between LAI and DM in veg top
   InitialCAI        		= theCropData->getInitialCAI();            // Initial green leaf area index after 200 degree days
   MinimumSenescense 		= theCropData->getMinimumSenescense();            // Used to calculate minimum senescense - value as calibrated for winter wheat
   MaxPlantHeight    		= theCropData->getMaxPlantHeight();             // Max plant height (m)
   LAIDistributionPar		= theCropData->getLAIDistributionPar();             // Distribution of LAI in canopy
   Conversfactor     		= theCropData->getConversfactor();             // Part of green leaf that is converted to yellow leaf by wilting

   ColonialisationRate		= theCropData->getColonialisationRate();           // Expansion of fraction area occupied (per day per degree Celcius)

   WaterDeficitVegGrowth 	= theCropData->getWaterDeficitVegGrowth();
   WaterDeficitLagPhase  	= theCropData->getWaterDeficitLagPhase();
   WaterDeficitGrainFill 	= theCropData->getWaterDeficitGrainFill();

	FractionNminInRoots 		= theCropData->getFractionNminInRoots();        //Fraction of N in roots present as mineral N
   FractionNminInVegTop 	= theCropData->getFractionNminInVegTop();       //Fraction of N in VegTop present as mineral N
   CO2Effect  					= theCropData->getCO2Effect();
	MaxGLAICrop 				= theCropData->getMaxGLAICrop();




   RhizoDepositFraction 	= theCropData->getRhizoDepositFraction();
   FillFactor 					= theCropData->getFillFactor();                  // Fraction of net production after anthesis that goes into grain
...
{
	theMessage->FatalError("crop::Add Function is incomplete and needs update before use");
...
void crop::StreamKeyData(fstream& os)
{
   ...
   os << "\t" << CropName
      << "\t" << DS
      << "\t" << TSum
     << "\t" << theClimate->GetCO2Concentration()
      << "\t" << GreenCropAreaIndex
      << "\t" << YellowCropAreaIndex
      << "\t" << DryMatterRoot;
...
if ((CropName == "Beet") || (CropName == "Potato") || (CropName == "OilRadish"))     //Nitrogen in total top
   ...
   os << "\t" << AccumulatedRootDeposit
      ...
      << "\t" << AccumulatedRootRespiration;
 		...
 		{
 			double StartDepth = theOutputData->RootData[i].getStartDepth()/1000;
 			double Thickness = theOutputData->RootData[i].getEndDepth()/1000-StartDepth;
