             ...
             :crop(aName,aIndex,aOwner, cropName)
{
   ...
   MinDayLength 							= theCropData->getMinDayLength();


	// DM allocation
   // Parameters below adjusted so that the total carbon flow matches data from Paustian et al. 1990. J. Appl. Ecol., 27, 60-84
	RespirationRoot 						= theCropData->getRespirationRoot();         // Root respiration given as a daily fraction of DM at 10 deg. Celsius, Calibrated to Burreh?jvej data
	RespirationTop  						= theCropData->getRespirationTop();          // Top respiration given as a daily fraction of DM at 10 deg. Celsius
   TransferRate    						= theCropData->getTransferRate(); 		  // Fraction (temperature-dependend) of the four age pools that is transferred to subsequent fraction
	BulkDensity     						= theCropData->getBulkDensity();       // g DM/m3 standing crop - guesstimated
   RecycleFraction    					=theCropData->getRecycleFraction();



   // LAI and canopy structure

	InitialLAIIncrease 					=theCropData->getInitialLAIIncrease();
	liveOMD 									= theCropData->getliveOMD();       	// Digestibility of first 3 age classes
	deadOMD 									= theCropData->getdeadOMD();       	// Digestibility of last age class

	RateOfRipeReSeed 						=theCropData->getRateOfRipeReSeed();     		//Rate of storage reseeding
