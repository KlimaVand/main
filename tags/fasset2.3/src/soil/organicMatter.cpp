...
/****************************************************************************\
\****************************************************************************/
bool organicMatter::ExistsPool(const char * poolName)
{
   int j=-1;
   if (strcmp(poolName,"CO2")==0)
      return false;
   for(int i=0;i<numberOfPools;i++)
      if (strcmp(poolName,poolList[i]->GetPoolName())==0)
         j=i;
   if (j==-1)
      return false;
   else
...
/****************************************************************************\
\****************************************************************************/
double organicMatter::GetRatio(const char * poolName)
{
...
/****************************************************************************\
\****************************************************************************/
double organicMatter::GetE(const char * poolName)
{
   bioMatter * b=(bioMatter*)GetPoolPointer(poolName);
...
\****************************************************************************/
matter * organicMatter::GetPoolPointer(const char * poolName)
{
   int j=-1;
   if (strcmp(poolName,"CO2")==0)
      return NULL;
   ...
   if (j==-1)
      Terminate("Pool [",poolName,"] not found");
   return poolList[j];
}

/****************************************************************************\
\****************************************************************************/
cnMatter organicMatter::GetCarbonInPool(const char * s)
{
...
/****************************************************************************\
\****************************************************************************/
void organicMatter::SetCarbonInPool(const char * s,cnMatter c)
{
                               ...
                               char * modelFileName)
{
   ...
   if (FindSection("SoilLayer",index)) // Top of inheritance hierarcy
   {
      ...
      for(int i=0;i<numberOfPools;i++)
      {
         ...
         if (GetParameter(poolList[i]->GetPoolName(),&d))
         {
            cnMatter c;
            c.c=d*TotalC;
            checkSum+=d;
            ...
            {
               char s[250];
               strcpy(s,poolList[i]->GetPoolName());
               strcat(s,".d13C");
               if (GetParameter(s,&d))
               {
                  if (d>=-1000.0)
...
//                Terminate("Can not set N content in '",(char*)poolList[i]->GetPoolName().c_str(),"' as this pool has a fixed C/N ratio");
            ...
            else
            {
               ...
               if (r<=0 && c.c>1E-9)
                  Terminate("C/N ratio must be set for pool '",poolList[i]->GetPoolName());
               if (r>0)
                  c.n=c.c/r;
            }
            strcpy(s,poolList[i]->GetPoolName());
...
void organicMatter::KillSMB(double Fraction)
{
   int count=0;
	for(int i=0;i<numberOfPools;i++)
      if (strcmp("SMB1",poolList[i]->GetPoolName())==0 || strcmp("SMB2",poolList[i]->GetPoolName())==0)
      {
         count++;
...
void organicMatter::ModifyAOM(double Fraction,double TurnoverFactor)
{
   int count=0;
	for(int i=0;i<numberOfPools;i++)
      if (strcmp("AOM1",poolList[i]->GetPoolName())==0 || strcmp("AOM2",poolList[i]->GetPoolName())==0)
      {
         count++;
