   ...
   : base(source)
{
   ...
   strcpy(poolName,source.poolName);
   ...
   for (int i=0;i<source.connections;i++)
   {
      ...
      strcpy(connectionList[i].connectname,source.connectionList[i].connectname);
   }
}
   ...
   if (c.c>0.0)
   {
      ...
      for (int i=0;i<connections;i++)
      {
         if (connectionList[i].Cfraction<0.0 || connectionList[i].Cfraction>1.0000001)
            Terminate("matter::Export - C fraction is illegal for ",poolName);
         if (connectionList[i].Nfraction<0.0 || connectionList[i].Nfraction>1.0000001)
            Terminate("matter::Export - N fraction is illegal for ",poolName);
