...
void bioMatter::Initialize(double clayEff1,double clayEff2,int clayResp)
{
   ...
   if (clayResp==5 && strcmp(poolName,"SMB1")==0) // Principle added march 2002
...
//  {
      if (cnRatio>0)
      {
         ...
         Terminate("C/N ratio not set for the '",poolName,"' pool");
//    if (retVal<-1E-12)
...
void bioMatter::Update(double environmentEffect,int mode)
{
   ...
   switch (mode)
   {
      ...
      case 5: carbon=carbonDerivative[0]+carbonDerivative[1]/6.0+carbonDerivative[2]/3.0
              ...
              break;
      default:
    	// theMessage->FatalError("void bioMatter::Update mode not supported");
    	  break;
   }
