...
bool base::OpenInputFileString(string fileName)
{
	char * tmp=new char[fileName.size()+1];
	strcpy(tmp,fileName.c_str());
	bool returnValue=OpenInputFile(tmp);
	delete [] tmp;
	return returnValue;
}
/****************************************************************************\
...
bool base::OpenInputFile(char * fileName)
{
   ...
   //cout<<"trying to open "<<fileNme<<endl;
   bool succes = false;
#ifdef __BCplusplus__
   char buffer[MAX_TXT];
#ifdef __BCplusplus__
   ...
   strcpy(baseInputDirectory,filesAcceess.getCurrentPath().c_str());
//   cout << baseInputDirectory << endl;
   ...
   {
      if (filesAcceess.changeDir("..")==0)                                 // check if file is in directory above
         file->close();
      else
      {
         if (critical)
   ...
   filesAcceess.changeDir(baseInputDirectory);        // change to original directory
#endif
   if (!succes)
   {
	  error = true;
...
void base::GetSectionNumbers(const char * sectionName, int * firstNum, int * numOfSections)
{
   ...
   {
   	fileAccess hd;
		cout<< " Current path: " << hd.getCurrentPath() <<endl;
...
void base::GetVectorInfo(const char * parameter,int * firstNum,int * vectorSize)
{
   ...
   *firstNum = 32000;
   file->seekg(cur_pos);
   while ((*file) && (file->peek()!=EOF) &&(inputString.empty()|| (inputString[0]!='[')) && searching)
   {
	  getline(*file,inputString,'\n');
...
bool base::GetParameter(const char * parameter, string * aString, int index)
{
   ...
   if (val)
   {
      aString->resize(0);//=""; // Creates a memory leak !!!
