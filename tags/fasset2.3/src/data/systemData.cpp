...
void systemData::checkData()
{
	...
	{
		if(runData[i].getVolatModel()==0)
				{
					cout<<"volat model 0 is not fully supported. Talk with Nic "<<i<<endl;
					cin.get();
					exit(1);
				}
...
cin.get();
		...
		if(runData[i].getEpotCalculationMethodFile()==true)
		{
