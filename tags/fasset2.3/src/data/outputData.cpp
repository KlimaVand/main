...
void outputData::add(){
	for(int i=0;i<3;i++)
	{
		...
		outputSoilN tmpSoilOrgN;
		tmpSoilOrgN.setEndDepth(0);
		tmpSoilOrgN.setEndDepthFile(false);
		tmpSoilOrgN.setStartDepth(0);
		tmpSoilOrgN.setStartDepthFile(false);
		SoilOrgNData.push_back(tmpSoilOrgN);

		outputSoilC tmpSoilOrgC;
		tmpSoilOrgC.setEndDepth(0);
		tmpSoilOrgC.setEndDepthFile(false);
		tmpSoilOrgC.setStartDepth(0);
		tmpSoilOrgC.setStartDepthFile(false);
	...
	SoilOrgCData[0].setStartDepth( 0.0);
	SoilOrgCData[0].setEndDepth(250.0);
	SoilOrgCData[1].setStartDepth( 250.0);
	SoilOrgCData[1].setEndDepth( 500.0);
	SoilOrgCData[2].setStartDepth( 500.0);
	SoilOrgCData[2].setEndDepth(1000.0);

	SoilOrgNData[0].setStartDepth( 0.0);
	SoilOrgNData[0].setEndDepth(250.0);
	SoilOrgNData[1].setStartDepth( 250.0);
	SoilOrgNData[1].setEndDepth( 500.0);
	SoilOrgNData[2].setStartDepth( 500.0);
	SoilOrgNData[2].setEndDepth(1000.0);
...
{
	fileAccess hd;
   	cout << theSystemData->getInputDirectory() << endl;

#ifdef __BCplusplus__
	if (hd.changeDir(theSystemData->getInputDirectory())==-1)
   {
   	cout << theSystemData->getInputDirectory() << endl;
	   theMessage->FatalError ("Directory not found");
   }
#else
	if (!hd.changeDir(theSystemData->getInputDirectory())==0)
   {
		cout << theSystemData->getInputDirectory() << endl;
	   theMessage->FatalError ("Directory not found");
   }
	...
	for(unsigned int i=0;i<items.size();i++)
	{
		...
		else if(loadSection("SoilC",SoilCLevel, nameOfItem))
		{
			if(SoilCLevel==0)
			{
				while(!SoilOrgCData.empty())
					SoilOrgCData.pop_back();
			}
			...
			while(true)
			{
				...
				if(nameOfItem.compare("EndDepth")==0)
				{
					SoilOrgCData[SoilCLevel].setEndDepth(conv.StringToDouble(value)*10);
					SoilOrgCData[SoilCLevel].setEndDepthFile(true);
				}
				else if(nameOfItem.compare("StartDepth")==0)
				{
					SoilOrgCData[SoilCLevel].setStartDepth(conv.StringToDouble(value)*10);
					SoilOrgCData[SoilCLevel].setStartDepthFile(true);
				}
            else if(nameOfItem.substr(0,1)=="[" &&nameOfItem.substr(nameOfItem.size()-1,nameOfItem.size())=="]")
				{
		...
		}else if(loadSection("SoilN",SoilNLevel, nameOfItem))
		{
			if(SoilNLevel==0)
			{
				while(!SoilOrgNData.empty())
					SoilOrgNData.pop_back();
			}
			outputSoilN tmpSoilN;
			tmpSoilN.setEndDepth(0);
			tmpSoilN.setEndDepthFile(false);
			tmpSoilN.setStartDepth(0);
			tmpSoilN.setStartDepthFile(false);
			...
			while(true)
			{
				...
				if(nameOfItem.compare("EndDepth")==0)
				{
					SoilOrgNData[SoilNLevel].setEndDepth(conv.StringToDouble(value)*10);
					SoilOrgNData[SoilNLevel].setEndDepthFile(true);
				}
				else if(nameOfItem.compare("StartDepth")==0)
				{
					SoilOrgNData[SoilNLevel].setStartDepth(conv.StringToDouble(value)*10);
					SoilOrgNData[SoilNLevel].setStartDepthFile(true);
				}
            else if(nameOfItem.substr(0,1)=="[" &&nameOfItem.substr(nameOfItem.size()-1,nameOfItem.size())=="]")
				{
