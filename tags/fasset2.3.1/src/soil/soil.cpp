...
void soil::WriteOutput()
{
 	...
 	if(globalSettings->DetailsData.getSoilDetails()==true)
	{
 		...
 		for(unsigned int i=0;i<globalSettings->SoilOrgNData.size();i++)
 		{
 			...
 			double SoilMass = GetSoilMass(StartDepth,Thickness)/10000000.0; // BMP 29.10.2008 !!!!!!
 			f << "\t" << (GetPoolNitrogen(2,StartDepth,Thickness).n + GetPoolNitrogen(3,StartDepth,Thickness).n)/SoilMass;
 			f << "\t" << (GetPoolNitrogen(2,StartDepth,Thickness).n15 + GetPoolNitrogen(3,StartDepth,Thickness).n15)/SoilMass;
 		   f  << "\t" << GetPoolNitrogen(0,StartDepth,Thickness).n
 		      << "\t" << GetPoolNitrogen(1,StartDepth,Thickness).n
 		      << "\t" << GetPoolNitrogen(2,StartDepth,Thickness).n
 		      << "\t" << GetPoolNitrogen(3,StartDepth,Thickness).n
 		      << "\t" << GetPoolNitrogen(4,StartDepth,Thickness).n;
 	      f.precision(5);
 		   f  << "\t" << GetPoolNitrogen(5,StartDepth,Thickness).n;
 		   f.precision(4);
 		   f  << "\t" << GetPoolNitrogen(6,StartDepth,Thickness).n
 		      << "\t" << GetPoolNitrogen(0,StartDepth,Thickness).n15
 		      << "\t" << GetPoolNitrogen(1,StartDepth,Thickness).n15
 		      << "\t" << GetPoolNitrogen(2,StartDepth,Thickness).n15
 		      << "\t" << GetPoolNitrogen(3,StartDepth,Thickness).n15
 		      << "\t" << GetPoolNitrogen(4,StartDepth,Thickness).n15
 		      << "\t" << GetPoolNitrogen(5,StartDepth,Thickness).n15;
	 	}
