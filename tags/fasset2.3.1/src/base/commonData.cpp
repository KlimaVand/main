...
commonData::~commonData()
{
	for(unsigned int i=0;i<used.size();i++)
	{
		if(used[i]==false)
		{
			if(items[i].compare("NULL2")!=0)
			{
				if(items[i].compare("NULL")!=0)
					if(items[i].compare("[end]")!=0)
					{
	...
	for(unsigned int i=0;i<used2.size();i++)
		{
			if(used2[i]==false)
			{
				if(items2[i].compare("NULL2")!=0)
				{
					if(items2[i].compare("NULL")!=0)
						if(items2[i].compare("[end]")!=0)
						{
