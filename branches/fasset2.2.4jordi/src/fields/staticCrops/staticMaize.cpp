...
void staticMaize::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage->Warning("static_something::SpecifiedPlan - more than one harvest date specified");       // Might not be a problem is barley with undersown clover/grass
}
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         if (underSown)
         {
            ...
            strcpy(tmp, "B2");
            FO->DefineSowFieldsOp(Sowing,tmp,cropName,PP->GetStartYear()+1,5,6,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),reduction,aSeed);
         }
         else
