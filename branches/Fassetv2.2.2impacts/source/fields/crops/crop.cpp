...
void crop::Harvest(decomposable* Storage, decomposable* Straw)
{
  CutOrHarvested = true;
   ...
   //rest of N (negative or positive) is allocated proportionally to the root, straw and storage
...
int crop::IrrigationDemand()
{
   ...
   if (aSoil && (MaxWDeficit<1.0) && RootDepth>0.0)
   {
      ...
      return ((1.0-WaterStatus)>MaxWDeficit);  //Soil water deficit = 1.0-Waterstatus
   }
   else
      return 0;
}
