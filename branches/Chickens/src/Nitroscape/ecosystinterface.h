...
//!output file for ecosystem plant details
   ...
   public:
      // Default Constructor
      ecosystinterfaceClass(int aIndex, char * soilFileName, double anArea = 0.0);
      // Destructor
    ...
    */
   void Tillage(int theType, double theDepth, double theFractionIncorporated);
   //!
   ...
   //plantItem*  createHarvestProduct(char *productName, double amountDM, double amountN, double amountC);
	 ...
	 */
	void Sow(int seedIndex, double amountSeedDM, double NitrogenInSeed, double CarbonInSeed);
    ...
    */
	void AllocateCrop(char * cropID, int cropCode);
	 ...
	 */
	void AddOrganic(int theType, double theamount, double theDepth, double thepercentTotalN, double thepercentNH4N,
   ...
   //!Return the amount of nitrogen in the ecosystem
	double GetTotalEcosystemN(){return TotalEcosystemN;}
	//!Return the amount of Carbon in the ecosystem
	double GetTotalEcosystemC() {return TotalEcosystemC;}
