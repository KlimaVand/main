...
ecosystinterfaceClass::ecosystinterfaceClass(int aIndex, char * soilFileName, double anArea)
{
   ...
   theEcosystem->Initialise(soilFileName, aIndex, anArea);
    ...
    ecoOutput->Initialize(indicatFileName);
...
void ecosystinterfaceClass::OutputPlantDetails(bool initialise)
{
	if (initialise)
	{
      ...
      for (int i=0;i<MaxPlants;i++)
      {
         ...
         {
            plantDetailFile << "\tRootLength-"<< theOutputData->RootData[i].getStartDepth() << "-"<< theOutputData->RootData[i].getEndDepth();
         	plantDetailFile << "\tRootMass-"<< theOutputData->RootData[i].getStartDepth() << "-"<< theOutputData->RootData[i].getEndDepth();
         	plantDetailFile << "\tRootN-"<< theOutputData->RootData[i].getStartDepth() << "-"<< theOutputData->RootData[i].getEndDepth();
      	}
      }
