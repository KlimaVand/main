...
//
 ...
 //following two lines are only needed if simulating ecosystem here
	linkList<ECOMainClass> * theEcosystems;
   theEcosystems = new linkList<ECOMainClass>;
...
//this only works for one farm at the moment - no way here of mapping of ecosystems to farms
      ...
      ECOMainClass *anECOMainClass = new ECOMainClass();
      anECOMainClass->initialise_ECO(numFields,theSystemData->getSystemRunData(farmNo));
		theEcosystems->InsertLast(anECOMainClass);
...
*/
   for (int farmNo=0;farmNo<numOfFarms;farmNo++)
   {
      ...
      ECOMainClass *anECOMainClass = theEcosystems->ElementAtNumber(farmNo);
//		}
