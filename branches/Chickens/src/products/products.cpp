...
void products::InitializeProducts(string productFileName, string baseProductFileName )//,bool cattle)
{
   ...
   SetCritical();
	if(!OpenInputFileString(baseProductFileName))
		cout<<"problems"<<endl;
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      OpenInputFileString(baseProductFileName);
      productInstance->ReadParameters(file,true,productSection);
      CloseInputFile();

      if(OpenInputFileString(productFileName))
    	  productInstance->ReadParameters(file,false,productSection);
      CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      OpenInputFileString(baseProductFileName);
      animalInstance->ReadParameters(file,true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      animalInstance->ReadParameters(file,false,productSection);
      CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      decomposableInstance->ReadParameters(file, true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      decomposableInstance->ReadParameters(file,false,productSection);
      CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      plantInstance->ReadParameters(file,true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      plantInstance->ReadParameters(file,false,productSection);
      CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      seedInstance->ReadParameters(file,true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
     seedInstance->ReadParameters(file,false,productSection);
      CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      chemicalInstance->ReadParameters(file,true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      chemicalInstance->ReadParameters(file,false,productSection);
       CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      manureInstance->ReadParameters(file,true,productSection);

      CloseInputFile();
      if(OpenInputFileString(productFileName))
      manureInstance->ReadParameters(file,false,productSection);
       CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      milkInstance->ReadParameters(file, true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      milkInstance->ReadParameters(file,false,productSection);
       CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      waterInstance->ReadParameters(file,true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      waterInstance->ReadParameters(file,false,productSection);
       CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      energyInstance->ReadParameters(file, true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      energyInstance->ReadParameters(file,false,productSection);
       CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      fertilizerInstance->ReadParameters(file, true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      fertilizerInstance->ReadParameters(file,false,productSection);
       CloseInputFile();
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      premiumInstance->ReadParameters(file,true,productSection);
      CloseInputFile();
      if(OpenInputFileString(productFileName))
      premiumInstance->ReadParameters(file,false,productSection);
       CloseInputFile();
...
void products::SellPlantItem(product *itemToSell)
{
 ...
 //  char* indstr;
   ...
   if (amountStored>0.0)
   {
      ...
      theOutput->AddIndicator(environmentalIndicator,"35.01 N from imported feed","kg",kgTotalN);
      theOutput->AddIndicator(environmentalIndicator,"45.01 C from imported feed","kg",kgTotalC);
#endif
