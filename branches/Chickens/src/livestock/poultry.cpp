...
\****************************************************************************/
poultry::poultry(const char *aName,const int aIndex,const base * aOwner):
...
/****************************************************************************\
Called daily
Will need updating if poultrys are outside animal housing (as for cattle)
\****************************************************************************/
void poultry::DailyUpdate()
{
   animal::DailyUpdate();
   feedItem *afeedItem;
   ...
   if (currentfeed->GetAmount()==0)
         theMessage->FatalError("poultry:: No feed provided!");
...
//   while (surplusN<0) //iteration necessary as feed quality could vary with amount requested from store, if several stores contain the desired feed
   if (surplusN<0)
   {
      theMessage->FatalError("poultry:: Insufficient protein in feed for ", GetName());
...
//   GetStableSection()->AddanimalHeatOp(NumberPrDay*heatProduction);
   ...
   double CinSoldpoultry = NumberPrDay*C_growth;
   ...
   theOutput->AddIndicator(environmentalIndicator,"44.64 C in poultry CH4 production","kg C",CinCH4*1000.0);
   double CinCO2 = CinFeed - (CinSoldpoultry + CinManure + CinCH4);
   if (CinCO2<0.0)
         {theMessage->FatalError("poultry::DailyUpdate - CO2 production cannot be negative");}
   theOutput->AddIndicator(environmentalIndicator,"44.63 C in poultry CO2 production","kg C",CinCO2*1000.0);

/*   Nbudget.AddOutput(manurePrDay->GetAmount()*manurePrDay->GetAllN().n);
   Nbudget.AddOutput(NumberPrDay * N_growth);
   Cbudget.AddOutput(CinManure+CinSoldpoultry+CinCH4+CinCO2);
   Nbudget.Balance(0.0);
   Cbudget.Balance(0.0);  */
...
/****************************************************************************\
\****************************************************************************/
/*void poultry::GetExtraFeed(double NRequired)
{
	feedItem* afeedItem = (feedItem*) theProducts->GetProductElement(499);
   feedItem * storedfeedItem =(feedItem *) theProducts->GetCopyStoredFeedProduct(afeedItem);   //returns a copy of the product if it is found in storage
	feedItem* feedItemClone = NULL;
   if (!storedfeedItem)
   		feedItemClone=(feedItem*)afeedItem->clone();
   else
   	feedItemClone = storedfeedItem;
   double totalNconc		= feedItemClone->GetAllN().n;
   double proteinNconc = feedItemClone->Getprotein_N();
   double digestibleProteinNconc = proteinNconc * feedItemClone->GetproteinN_digestibility();
   double otherNconc       = totalNconc-proteinNconc;
   double amountNeeded = NRequired/(otherNconc+digestibleProteinNconc);
   feedItemClone->Setamount(amountNeeded);

   theProducts->SubtractProduct(feedItemClone);                     // subtract feed from storage
   *currentfeed  =  *currentfeed + *feedItemClone;
   delete feedItemClone;
	theMessage->Warning("poultry:: ", GetName()," N deficiency - extra feed fed");
}
*/
/****************************************************************************\
\****************************************************************************/
void poultry::ReceivePlan(char* fileName)
{
   currentfeed->Setamount(0.0);
   UnsetCritical();
   if(!OpenInputFile(fileName))
   {
      strcpy(fileName,"poultryfeed.fnn");
      if (!OpenInputFile(fileName))
         theMessage->FatalError("poultry::ReceivePlan - error in opening poultryfeed.f??");
   }
   ...
   if (!gotit)
      theMessage->FatalError("poultry::ReceivePlan - cannot find poultry group in poultryfeed.f??");
   else
   {
   	feedItemList->Reset();
...
\****************************************************************************/
double poultry::ProduceManure(manure* fluidManure, manure* solidManure)
{
...
/*
   	double energyInput		= currentfeed->GetAmount() * currentfeed->GetpoultryFeedUnitsPerItemUnit();
      ...
      if (energyInput<0.75*(NumberPrDay*FE_need)) // BMP changed this january 2007
      {
         if (energyInput<0.50*(NumberPrDay*FE_need))
         {
            ...
            theMessage->FatalError("poultry::ProduceManure - ",Alias.c_str()," Very low amount of energy in feed");
         }
         else
            theMessage->Warning("poultry::ProduceManure - ",Alias.c_str()," Insufficient energy in feed");
      }
...
*/
      ...
      */
      if (surplus_digestibleN>=0)
      {
         ...
         if (Nconc<0.0)
            theMessage->FatalError("poultry::protein N digested greater than protein N present");
         solidManure->Setname("poultry-FAECES");
         ...
         fluidManure->Setname("poultry-URINE");
         ...
         if(fabs(balance)>0.0001)
            theMessage->FatalError("poultry::ProduceManure - error in N balance in poultry manure partitioning");
      }
      else
         theMessage->FatalError("poultry::ProduceManure - ",animalName.c_str()," required additional protein in feed"); // BMP modified this !!!
      double Nefficiency =  NumberPrDay * N_growth/totalNInput;
      if (Nefficiency>0.7)
         theMessage->FatalError("poultry::ProduceManure - N efficiency over 70%");
   }
   else
...
/****************************************************************************\
\****************************************************************************/
void poultry::ReadParameters(fstream * file)
{
...
/****************************************************************************\
\****************************************************************************/
void poultry::EndBudget(bool show)
{
	if (show)
   {
	...
	if (NRemaining!=0)
      theMessage->FatalError(GetName(),"poultry::EndBudget - N budget error");
}


/****************************************************************************\
Truncate a name to the first occurance of underscore - used to look for feed for a general class
e.g. SOWS rather than SOWS_PREGNANT
\****************************************************************************/
bool poultry::TruncateName(string * ret_string)
{
   string buffer = this->GetName();
   *ret_string="";
   int count=0;
   while ((count<int(buffer.length()))&& (buffer[count]!='_'))
   {
      *ret_string=*ret_string + buffer[count];
      count++;
   }
   if (ret_string->length()==buffer.length())
      return false;
   return true;
}
...
/****************************************************************************\
\****************************************************************************/
feedItem * poultry::GetFeedItem(product * menuPtr)
{
	feedItem * afeedItem;
   ...
   afeedItem = new feedItem(*(feedItem*) menuPtr);
   if (aStoredProduct)
   {
      if (aStoredProduct->GetAmount()>0.0)
      {
         //this section updates the default variables with the actual characteristics of the feed stored
         //if there is not enough in store, the remainder will later be imported WITH THE SAME QUALITY AS THAT WHICH WAS IN THE STORE            aStoredProduct->Setamount(menuPtr->GetAmount());
         afeedItem->Setamount(0.0);
         *afeedItem+ *aStoredProduct;
         delete aStoredProduct;
         afeedItem->Setamount(menuPtr->GetAmount());
      }
   }
//   afeedItem->SetIsSupplement(true);
   return afeedItem;
}
...
\****************************************************************************/
double poultry::CalcMaintenanceEnergy(double weight)
{
	return 5.09*pow(weight,0.75);
}

void poultry::SeteggN(double aVal)
{
   theMessage->FatalError("poultry::SeteggN - not allowed for this type of bird");
};
void poultry::SeteggC(double aVal)
{
   theMessage->FatalError("poultry::SeteggN - not allowed for this type of bird");
};
