...
//Note that this next part breaks convention that a class should read its own parameters
   for(int i=0;i<grouplist->NumOfNodes();i++)
   {
      ...
      {
         int numCohorts = 0;
         if (numAnimals<=maxNumCohorts)
            numCohorts = int(numAnimals);
         else
            numCohorts = maxNumCohorts;
 ...
 */
void dairyStock::InitCattle(fstream * file, int anAnimalModel, char *filenameCattle)
{
   ...
   SetCritical();
   GetParameter("MaxNumCohorts", &maxNumCohorts);
