...
{
	Nbudget=s.Nbudget;
	aBase=new base(*s.getBase());
	theStore= new storage(*s.gettheStore());
	manureToName=" ";
	nextStore=NULL;
	NH3Emissionfactor=0.0;
	N2Emissionfactor=0.0;
	...
	NMinfactor=0.0;
	OMdegRateConst=0.0;
	Nbudget.SetNames(aBase->GetName(),"N");
	Cbudget.SetNames(aBase->GetName(),"C");
	cumvolat=0;
	strcpy(Alias,"");
	external = false;
	postCleanDepth = 0.0;  ////tonnes manure remaining after cleaning
...
void manurestore::ReadDefaultParameters()
{
	aBase->SetCritical();
	if (aBase->OpenInputFile("StandardBuildings.dat"))
	{
//   verbosity =true;
	   aBase->SetCritical();
	   aBase->FindSection(aBase->GetName());
