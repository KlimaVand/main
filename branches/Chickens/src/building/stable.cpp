...
/**
 * 	ReadParameters() reads information from file.
 */
...
void stable::Volatize()
{
	...
	case 0://!simple emission model, using emission factors
	{
		...
		for(int i=0;i<sections->NumOfNodes();i++)
		{
			double tempvolatAmmonia=0.0, tempN2Emission=0.0, tempN2OEmission=0.0, tempNOEmission=0.0, tempCH4Emission=0.0, tempCO2Emission=0.0;
			animalSection* aSection=sections->ElementAtNumber(i);
	...
	case 1://!dynamic emission model
	{
		if (inUse)   //could be zero if using scaling and no animals for this housing type
		{
			...
			for(int i=0;i<sections->NumOfNodes();i++)
			{
				double tempvolatAmmonia=0.0, tempN2Emission=0.0, tempN2OEmission=0.0, tempNOEmission=0.0, tempCH4Emission=0.0, tempCO2Emission=0.0, tempwaterEvap=0.0;
				animalSection* aSection=sections->ElementAtNumber(i);
	...
	theOutput->AddIndicator(environmentalIndicator,"35.40 NH3-N volatilisation from animal house","kg N",NH3Emission*1000.0);
	...
	//   theOutput->AddIndicator(environmentalIndicator,"35.43 NO-N volatilisation from animal house","kg N",NOEmission*1000.0);
		...
		double windspeed, double outsideAirTemp, double heatOP, double q, double *ventilationRate, double *deltaTemper)
{
	...
	*ventilationRate=0.0;
	...
	for (int j=0; j<solutions; j++)
	{
		...
		//! If the ventilation rate is lower than that resulting from wind-forced ventilation, the thermal ventilation is zero
		if (x[index]<= windforcedVent)
			thermalVent=0.0;
		else
			thermalVent = pow((pow(x[index],2)-pow(windforcedVent,2)),0.5);
