...
{
	if(theTime.GetDay()==17&&theTime.GetMonth()==7&&theTime.GetYear()==2005)
		{
	
			if(poolName=="AOM1")
				cout<<carbon.c<<" poolName "<<endl;
	}
   cnMatter outflux;
   cnMatter out;
   if (numOfSubPools>0)
...
void addedMatter::print(int current,int Last)
{
	static ofstream addedC;
		...
		{
			usedBefore=true;
			addedC.open(theSystemData->getOutputDirectory()+poolName+".xls",ios::out);
			addedC<<theTime<<'\t';

		}
		else
		{
		addedC.open(theSystemData->getOutputDirectory()+poolName+".xls",ios::app);
		}
		if(current==0)
		{
			addedC<<theTime<<'\t';
