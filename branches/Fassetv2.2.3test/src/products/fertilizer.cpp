...
/****************************************************************************\
\****************************************************************************/
void fertilizer::showstats(ostream *os)
{
	product::showstats(os);
	*os << "N_content    " << N_content.n << endl;
	*os << "P_content    " << P_content << endl;
	*os << "K_content    " << K_content << endl;
	*os << "Ca_content   " << Ca_content << endl;
}
...
/****************************************************************************\
\****************************************************************************/
void fertilizer::ReadParameters(fstream * file)
{
...
void fertilizer::Output(fstream * fs)
{
   double quantityN = amount * N_content.n;
   double quantityP = amount * P_content;
   double quantityK = amount * K_content;
//   *fs << setw(10) << setiosflags(ios::right) << setprecision(0) ;
   if (quantityN > 0.0)
	   *fs << " N applied    " << setiosflags(ios::right)  << setw(15) << setprecision(3) << quantityN;
   if (quantityP > 0.0)
	   *fs << " P applied    " << quantityP;
   if (quantityK > 0.0)
	   *fs << " K applied    " << quantityK;
}
