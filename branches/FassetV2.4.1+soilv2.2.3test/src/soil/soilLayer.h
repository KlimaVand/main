...
//public:
 ...
 static bool ammoniumDisolvedUsed=false;
	if(ammoniumDisolvedUsed==false)
	{
		...
		AmmoniumLayer.open(theSystemData->getOutputDirectory()+"AmmoniumLayer.xls",ios::out);
	...
	{
		AmmoniumLayer.open(theSystemData->getOutputDirectory()+"AmmoniumLayer.xls",ios::app);
		if(layer==0)
			AmmoniumLayer<<theTime<<"\t";
	}
 ...
 static bool NitLayer=false;
	if(NitLayer==false)
	{
		...
		NitrateLayer.open(theSystemData->getOutputDirectory()+"NitrateLayer.xls",ios::out);
	...
	{
		NitrateLayer.open(theSystemData->getOutputDirectory()+"NitrateLayer.xls",ios::app);
		if(layer==0)
			NitrateLayer<<theTime<<"\t";
	}
