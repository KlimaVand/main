...
//Calculates number of animals to be imported + herd structure
{
   ...
   cout << inputString << endl;*/
	...
	double daysPregnant = ((pigstysection*) pregnantSwines->GetanimalSection())->GetDaysInSection();
	double daysMating = ((pigstysection*) matingSwines->GetanimalSection())->GetDaysInSection();
	double dayslactating =((pigstysection*) lactatingSwines->GetanimalSection())->GetDaysInSection();
...
/*			cout << aheatOp << endl;
			...
			cout << aheatOp << endl;
			*/
			aPig->GetanimalSection()->SetAnimalParameters(aPig->GetNumberPrDay(),faecesPerYear,urinePerYear,heatOp);
		}
	}
}
...
//Load feed and calculate growth N requirements
{
	...
	if (numSows>0)
	{
		...
		double sucklerDays = (((pigstysection*) lactatingSwines->GetanimalSection())->GetDaysInSection() +    //days taken for suckler to reach end of suckling
...
void swineStock::InitSwines(commonData * &file, buildings* theBuildings)
{
	...
	matingSwines->SetanimalSection(theBuildings->PlaceAnimal(outstring.c_str()));
	outstring = (string) "Pregnant" + Indexstr;
	pregnantSwines->SetanimalSection(theBuildings->PlaceAnimal(outstring.c_str()));
	outstring = (string) "Lactating" + Indexstr;
	lactatingSwines->SetanimalSection(theBuildings->PlaceAnimal(outstring.c_str()));
	outstring = (string) "Weaner" + Indexstr;
	piglets->SetanimalSection(theBuildings->PlaceAnimal(outstring.c_str()));
	outstring = (string) "Finisher" + Indexstr;
	baconers->SetanimalSection(theBuildings->PlaceAnimal(outstring.c_str()));
	outstring = (string) "NewSow" + Indexstr;
	newSows->SetanimalSection(theBuildings->PlaceAnimal(outstring.c_str()));
}
