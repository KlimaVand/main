...
double soilLayer::GetPoolDecompositionRate(const char * Name)
{
...
void soilLayer::EquilibrateNitrogen(double mobileWater)
{
   ...
   //CK if (waterContent>0.0)
   //CK	f = max(0.0,min(1.0,mobileWater/waterContent));
...
\****************************************************************************/
double soilLayer::UpdateOrganicMatter(int layer,int NumOfSoilLayers)
{
   // TEST !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   CO2Evolution = OrganicMatter->Update(NewTemperature,pFValue,&nitrate,&ammonium, layer,NumOfSoilLayers);
#ifdef NEWSLIM
