...
{
	if(theTime.GetDay()==17&&theTime.GetMonth()==4&&theTime.GetYear()==2005)
		cout<<endl;
...
//CK  if(theTime.GetDay()==3&&theTime.GetMonth()==5&&theTime.GetYear()==2005)
   //CK 	   cout<<startMinN.n<<" "<<GetMinNitrogen().n<<endl;
	//  print();
...
void soil::Incorporate(double MaxDepth, double frac, bool tillage, const char * tillagetype,double MinDepth, string plantName)
{
   ...
   if ((MaxDepth>0 && frac>0))
   {
	  ...
	  if(strcmp(tillagetype,"Ploughing") == 0) //CK //CK
	  {
		  startLayer=0;
		  endLayer=0;
	  ...
	  if(strcmp(tillagetype,"Ploughing") == 0) //CK
	  {
...
void soil::ModifyAOM(double Fraction,double TurnoverFactor, double Depth)
{
  for (int i=0; i<Profile->GetLayers(); i++)
  {
	  //CK  cout<<Profile->GetLayer(i)->GetStartDepth()<<endl;
	  //CK  cout<<Profile->GetLayer(i)->GetIndex()<<endl;
  		if (Profile->GetLayer(i)->GetStartDepth()<Depth) // Actually a simplification !!!
      {
...
void soil::WriteOutput()
{
   ...
   int depth = 1000;
	...
	/*  WriteC();
	  ...
	  WriteCarbon("AOM1",usedbefore2);
	  WriteCarbon("AOM2",usedbefore2);
	  WriteCarbon("SMB1",usedbefore2);
	WriteCarbon("SMB2",usedbefore2);

	WriteCarbon("SMR",usedbefore2);
	WriteCarbon("NOM",usedbefore2);

	  WriteN("AOM1",usedbefore2);
	  WriteN("AOM2",usedbefore2);
	  WriteN("SMB1",usedbefore2);
	  WriteN("SMB2",usedbefore2);

	  WriteN("SMR",usedbefore2);
	  WriteN("NOM",usedbefore2);


	  WriteCN("AOM1",usedbefore2);
	  WriteCN("AOM2",usedbefore2);
	  WriteCN("SMB1",usedbefore2);
	  WriteCN("SMB2",usedbefore2);

	  WriteDecompositionRate("SMR",usedbefore2);
	  WriteDecompositionRate("NOM",usedbefore2);
	    WriteDecompositionRate("AOM1",usedbefore2);
	  WriteDecompositionRate("AOM2",usedbefore2);
	  WriteDecompositionRate("SMB1",usedbefore2);
	  WriteDecompositionRate("SMB2",usedbefore2);
