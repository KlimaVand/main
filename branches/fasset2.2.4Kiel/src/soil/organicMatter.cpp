   ...
   : base(Pname,Index,owner)
{
   ...
   NIsotope=0;               // 0 : None; 1 : 15N % atom excess; (2 : 15N tag - LATER!!!)
   ...
   : base (source)
{
   if (&source)
   {
         ...
         first14CYear=source.first14CYear;
         last14CYear=source.last14CYear;
         ...
         totalAddedCarbon = source.totalAddedCarbon;
         initialCarbon = source.initialCarbon;
         todayCO2 = source.todayCO2;
         totalCO2 = source.totalCO2;
         NO3 = source.NO3;
         NH4 = source.NH4;
...
void organicMatter::Add(organicMatter * source, double fraction)
{
	//CK  if (&source)
	 //CK {
      ...
      exchange=exchange*(1.0-fraction)+source->exchange*fraction;
      totalAddedCarbon=totalAddedCarbon*(1.0-fraction)+source->totalAddedCarbon*fraction;
      initialCarbon=initialCarbon*(1.0-fraction)+source->initialCarbon*fraction;
      todayCO2=todayCO2*(1.0-fraction)+source->todayCO2*fraction;
      totalCO2=totalCO2*(1.0-fraction)+source->totalCO2*fraction;
   ...
   //CK }
      //CK else
      //CK     Terminate("organicMatter::Add - function called with NULL pointer");
}
...
/****************************************************************************\
Initiates all organic matter types that can be added later.
filename   input file name
\****************************************************************************/
void organicMatter::InitAddTypes(char * filename)
{
	if (!OpenInputFile(filename))
      Terminate("Error in opening file ",filename);
   int n,first;
   GetSectionNumbers("OrganicProduct",&first,&n);
   for(int i=first;i<(first+n);i++)
	{
		organicProduct * o=new organicProduct("OrganicProduct",i,this);
      o->SetStepFactor(stepFactor);
      o->ReadParameters(file,useNCfractionation);
      prodList[i]=o;
      numberOfProducts++;
	}
   if (numberOfProducts>=maxOrgProds)
   	cout << "organicMatter::~organicMatter - 'numberOfProducts' to high!" << endl;
	CloseInputFile();
   DeleteInputFile(); // ??????!!!!! I just don't get why this works without "DeleteInputFile()" most of the time, but not here - BMP
}
...
/****************************************************************************\
\****************************************************************************/
char* organicMatter::GetIsotopeName()
{
   ...
   {
      case 1:
    	  strcpy(returnValue,"d13C");
    	  break;
      case 2:
    	  if (useDelta14Cvalues)
    		  strcpy(returnValue, "d14C");
    	  else
    		  strcpy(returnValue, "PM");
    	  break;
      case 3:  strcpy(returnValue, "TAG");
		  break;
      case 4:
    	  strcpy(returnValue, "N");
    	  break;
   }
   return returnValue;
}
...
/****************************************************************************\
\****************************************************************************/
double organicMatter::GetIsotopeValue(cnMatter c)
{
   switch (Isotope)
   {
      case 1: return c.Getdelta(Rzs);
      case 2: if (useDelta14Cvalues)
                 return c.Getdelta(Rzs);
              else
                 if (c.c>1E-20)
                    return 100.0*c.c_iso/c.c;
                 else
                    return 0.0;
      case 3: if (c.c>1E-20)
                 return c.c_iso/c.c;
...
/****************************************************************************\
\****************************************************************************/
void organicMatter::SetIsotopeType(int i)
{
   if (i>4)
      Terminate("Illegal C isotope type specified. Legal range 0-4.");
   Isotope=i;
}

/****************************************************************************\
\****************************************************************************/
void organicMatter::SetNitrogenIsotope(int i)
{
	if (i>1)
      Terminate("Illegal N isotope type specified. Legal range 0-1.");
   NIsotope=i;
}
...
/****************************************************************************\
\****************************************************************************/
cnMatter organicMatter::GetAllSMBCarbon()
{
   cnMatter smb;
   string s="SMB";
   if (ExistsPool(&s))
      smb=smb+GetPoolPointer(s)->GetCarbon();
   s="SMB1";
   if (ExistsPool(&s))
      smb=smb+GetPoolPointer(s)->GetCarbon();
   s="SMB2";
...
/****************************************************************************\
\****************************************************************************/
void organicMatter::SetPoolDecompositionRate(string * s,double r)
{
   GetPoolPointer(*s)->SetDecompositionRate(r);
}

/****************************************************************************\
\****************************************************************************/
void organicMatter::SetPoolConnectionFraction(string * s,int nr,double fracC,double fracN)
{
   if (fracN<0.0)
      fracN=fracC;
   GetPoolPointer(*s)->SetConnectionFraction(nr,fracC,fracN);
}

/****************************************************************************\
\****************************************************************************/
void organicMatter::SetPoolMaintenanceRate(string * s,double r)
{
   GetPoolPointer(*s)->SetMaintenanceRate(r);
}

/****************************************************************************\
\****************************************************************************/
void organicMatter::SetPoolDeathRate(string * s,double r)
{
   GetPoolPointer(*s)->SetDeathRate(r);
}

/****************************************************************************\
\****************************************************************************/
void organicMatter::InitRadioCarbonTable(char * filename)
{
/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   fstream f;
   chdir("c:\\c-tool\\");
   if (!OpenInFile(&f,filename))
      Terminate("Radiocarbon file [",filename,"] not found");
   int i=0;
   int year;
   double PM;
   while (f)
   {
      if (i>=max14CYears)
         Terminate("To many years in radiocarbon file. Limit is 500");
      f >> year >> PM;
      if (first14CYear==0)
         first14CYear=year;
      f.getline(buff,slen);
      if (f)
      {
         radioCarbon[i]=PM;
         last14CYear=year;
         if (year!=(first14CYear+i))
            Terminate("Years are not consecutive in radiocarbon file");
      }
      i++;
   }
   f.close();
...
/****************************************************************************\
\****************************************************************************/
double organicMatter::GetPercentModern(int year)
{
   double PM=100.0;
   if (year>=first14CYear && year<(first14CYear+max14CYears))
      PM=radioCarbon[year-first14CYear];
   return PM;
}
...
/****************************************************************************\
Adds organic matter to the pools specified in the organic matter instance.
type   - name of added matter
amount - amount of organic matter
\****************************************************************************/
void organicMatter::AddMatter(string type,cnMatter amount)
{
   int j=-1;
   for(int i=0;i<numberOfProducts;i++)
   {
      if (prodList[i]==NULL)
         Terminate("container::AddMatter - Uninitialised product, numbering in file must start with zero");
      if (type==prodList[i]->GetName())
         j=i;
   }
   if (j==-1)
      Terminate("Product ",(char*)type.c_str()," not found");
   double f=prodList[j]->GetCarbonFraction();
   prodList[j]->SetAmount(amount);
   if (useNCfractionation)
      SetFractionation(prodList[j]);
   totalAddedCarbon=totalAddedCarbon+amount*f;
   //--------------------------------------
   double check=0.0;
   for(int i=0;i<prodList[j]->GetNumOfFractions();i++)
      if (prodList[j]->GetDestPool(i)=="CO2")
      {
         cnMatter CO2directly=prodList[j]->GetAmount()
                             *prodList[j]->GetCFraction(i)*f;
         check+=CO2directly.c;
   ...
   double f0=prodList[j]->GetCFraction(0);
   double f1=prodList[j]->GetCFraction(1);
   double f2=prodList[j]->GetCFraction(2);
   matter * nom=(matter*)GetPoolPointer("NOM");
   if (nom==NULL)
      Terminate("The pool 'NOM' was not found");
   double maxCToNOM=0.0;
   if (prodList[j]->GetCNratio()>0.0)
      maxCToNOM = nom->GetCNratio()/prodList[j]->GetCNratio();
   if (maxCToNOM>f2) // Adjust to avoid program termination
   {
      double diff=f2-maxCToNOM;
      double factor=(diff+f0+f1)/(f0+f1);
      prodList[j]->SetCFraction(0,f0*factor);
      prodList[j]->SetCFraction(1,f1*factor);
      prodList[j]->SetCFraction(2,maxCToNOM);
   }
   // -----------------------------------------------------------
   ...
   {
      num=0;
      poolList[i]->TestAddProduct(prodList[j],&check,&CN,&num,false);
      if (num>0) // Match found
      {
         fsum+=prodList[j]->GetNFraction(num);
         used+=prodList[j]->GetCFraction(num)*prodList[j]->GetCNratio()/CN;
      }
   }
   if (used>(1.0+1E-9))
      Terminate("The added product <",(char*)type.c_str(),"> did not contain enough N to match consistently with pools");
   ...
   for(int i=0;i<numberOfPools;i++) // For pools without fixed C/N ratio
      poolList[i]->TestAddProduct(prodList[j],&check,&CN,&num,true);
   if (fabs(check-amount.c*f)>1E-6)
      Terminate("The added product <",(char*)type.c_str(),"> was not consistently matched with pools for C");
   // Revert fractionation settings
   prodList[j]->SetCFraction(0,f0);
   prodList[j]->SetCFraction(1,f1);
   prodList[j]->SetCFraction(2,f2);
}

/****************************************************************************\
Presently NOT used within FASSET
\****************************************************************************/
void organicMatter::UpdateMineralNitrogen(double factor)
{
   if (NH4.n>NH4allwaysLeft)
   {
      nitrogen nitrified;
      nitrified=NH4*factor*nitrificationRate;
      if (nitrified.n>NH4.n)
         nitrified=NH4;
      NH4near=NH4near*(1.0-nitrified.n/NH4.n);
      NH4=NH4-nitrified;
      NO3=NO3+nitrified;
   }
}
...
{
//CK	if(theTime.GetDay()==19&&theTime.GetMonth()==4&&theTime.GetYear()==2005)
	//CK		cout<<GetPoolCarbon("AOM1")<<" at 19/4-2005"<<endl;
	//CK	if(theTime.GetDay()==15&&theTime.GetMonth()==7&&theTime.GetYear()==2005)
	//CK		cout<<GetPoolCarbon("AOM1")<<" at 26/4-2005"<<endl;
   if (nitrate->n<-1E-9)
...
// Decompose ----------------------------------------------------------------
   ...
   //CK if(theTime.GetDay()==15&&theTime.GetMonth()==7&&theTime.GetYear()==2005)
   //CK 	cout<<GetPoolCarbon("AOM1")<<" at 26/4-2005"<<endl;   // 3. Calculate the total potential immobilisation and the available N
   double Nimob=0.0;
   for(int j=0;j<numberOfPools;j++)
   {
   ...
   for(int j=0;j<numberOfPools;j++)
   {
	   //CK if(tmp==633178)
		   //CK   	if(theTime.GetDay()==15&&theTime.GetMonth()==7&&theTime.GetYear()==2005)
	   		//CK 	cout<<GetPoolCarbon("AOM1")<<" at 26/4-2005 " <<tmp<<endl;
	    double Nwanted=-poolList[j]->PotentialNitrogenMineralisation();
      //CK   if(tmp==633178)
      //CK 	 cout<<Nwanted<<endl;
	      if (Nwanted>0.0)
	      {
...
// Decomposition finished-------------------------------------------------------
  ...
  *ammonium = NH4;
   ...
   //CK if(theTime.GetDay()==15&&theTime.GetMonth()==7&&theTime.GetYear()==2005)
   //CK 	cout<<GetPoolCarbon("AOM1")<<" at 26/4-2005"<<endl;
//	print(layer,NumOfSoilLayers);
   return RetVal.c;
}

void organicMatter::print(int layer,int NumOfSoilLayers)
{
	 //CK double Content = 0;

	static ofstream NO3conent;
	static ofstream NH4conent;
	static ofstream NH4allwaysLeftconent;
	static bool usedBeforeinPrint=false;
	string name1=theSystemData->getOutputDirectory()+"NO3conent.xls";
	string name2=theSystemData->getOutputDirectory()+"NH4conent.xls";
	string name3=theSystemData->getOutputDirectory()+"NH4allwaysLeftconent.xls";
	if(usedBeforeinPrint==false)
	{
		usedBeforeinPrint=true;
		NO3conent.open(name1.c_str(),ios::out);
		NH4conent.open(name2.c_str(),ios::out);
		NH4allwaysLeftconent.open(name3.c_str(),ios::out);
	...
	else
	{
		...
		NO3conent.open(name1.c_str(),ios::app);
		NH4conent.open(name2.c_str(),ios::app);
		NH4allwaysLeftconent.open(name3.c_str(),ios::app);
	}
   ...
   if(layer==0)
   {
	   ...
	   NH4conent<<theTime<<"\t";
	   NH4conent<<NH4.n<<"\t"; 
	   NH4allwaysLeftconent<<theTime<<"\t";
	   NH4allwaysLeftconent<<NH4allwaysLeft<<"\t";
   }
   else if(layer==(NumOfSoilLayers-1))
   {
	   NO3conent<<NO3.n<<endl;
	   NH4conent<<NH4.n<<endl; 
	   NH4allwaysLeftconent<<NH4allwaysLeft<<endl;
   }
   else
   {
	   NO3conent<<NO3.n<<"\t";
 ...
 NH4conent.close();
 NH4allwaysLeftconent<<flush;

 NH4allwaysLeftconent.close();
 for(int i=0;i<numberOfPools;i++)
 {
	 if(poolList[i]->IsAOM())
		 poolList[i]->print(layer,NumOfSoilLayers);
 }
	//NO3.n+max(0.0,NH4.n-NH4allwaysLeft
}
/****************************************************************************\
...
double organicMatter::GetPoolDecompositionRate(const char * name)
{
...
void organicMatter::AddProduct(organicProduct * product)
{
   ...
   */
   ...
   totalAddedCarbon=totalAddedCarbon+product->GetAmount()*f;
...
/****************************************************************************\
Preliminary tillage effect
\****************************************************************************/
void organicMatter::ReleaseReduceTurnover()
{
	for(int i=0;i<numberOfPools;i++) // For pools with fixed C/N ratio
  		poolList[i]->ReleaseReduceTurnover();
}
