...

{
   ...
   //-------------- NEW --------------------
   //CK double halfdesignated[MaxSoilLayers];
    //CK   double realisedFlow[MaxSoilLayers];

    //CK  double porosity[MaxSoilLayers];
   //CK double FC[MaxSoilLayers];
   //CK   double surplusW[MaxSoilLayers];
	...
	for (i=0;i<TotalNumberOfLayers;i++)
   {
      ...
      //CK porosity[i]=soilLayerArray[i]->GetPorosity();
      maxContent[i]=(soilLayerArray[i]->GetPorosity()*soilLayerArray[i]->GetThickness());
      totalFlux[i]=0.0;
      //CK halfdesignated[i]=0.0;
      //CK realisedFlow[i]=0.0;
      //CK FC[i]=soilLayerArray[i]->GetFieldCapacity();
      //CK surplusW[i]=0.0;
      surplusNH4[i].Clear();
      surplusNO3[i].Clear();
   }
                                     ...
                                     nitrogen *AmmoniumLeached, double Chloride)
{
   ...
   while (simulationTime<86400)     //simulates 24 hours
   {
      ...
      for (i=0;i<TotalNumberOfLayers;i++)
      {
    	  //CK double relativeWater = min(1.0,waterContent[i]/soilLayerArray[i]->GetThickness()); 		//volumetric water content
         //CK pressurePotential[i] = -pF[i]->GetPressurePotential(relativeWater);  						// in m WC
      ...
      // Use convection-dispersion equation on mobile domain
      {
         ...
         for (int soluteType=0;soluteType<5;soluteType++)
         {
            if (soluteType==0)        //nitrate N
            {
               ...
               //CK nitrateInOld= nitrateIn;
               //CK soluteInOld=nitrateInOld;
               nitrateIn=soluteIn;
            }
            else if (soluteType==1)    //ammonium N
            {
               ...
               //CK ammoniumInOld= ammoniumIn;
               //CK soluteInOld=ammoniumInOld;
                ammoniumIn=soluteIn;
            }
            else if  (soluteType==2)    //chloride
            {
               ...
               for (i=0;i<TotalNumberOfLayers;i++)
                  if (mobileWaterOld[i]>0.0)
                     ...
                     soluteOld[i]=0.0;
               soluteIn=Chloride*f;
               //CK chlorideInOld= chlorideIn;
               //CK soluteInOld=chlorideInOld;
               chlorideIn=soluteIn;
            }
            else if  (soluteType==3)    //nitrate 15N
            {
               ...
               //CK nitrateN15InOld= nitrateN15In;
               //CK soluteInOld=nitrateN15InOld;
            ...
            else if  (soluteType==4)    //ammonium 15N
            {
               ...
               //CK ammoniumN15InOld= ammoniumN15In;
               //CK soluteInOld=ammoniumN15InOld;
...
void soilProfile::UpdateFinDiffTemperature(double AirTemperature, double snow)
{
   ...
   if (theControlParameters->GetSimpleTemperatureModel() || theControlParameters->GetMeasuredSoilTemperature()||theControlParameters->GetAirToSoilTemperature())
   {
   	if (theControlParameters->GetSimpleTemperatureModel())
      {
   	   ...
   	   for (int i=1;i<NumOfSoilLayers;i++)
      	{
         	soilTemperature=soilLayerArray[i]->GetTemperature();
	         if ((soilTemperature<-50) || (soilTemperature>50))
	            Terminate("soilProfile::UpdateFinDiffTemperature - soil temperature not within realistic range");
	         depth = soilLayerArray[i]->GetStartDepth()+ soilLayerArray[i]->GetThickness()*0.5;
	         UpdateT(&soilTemperature,AirTemperature,depth,8.0);
	      }
	      soilLayerArray[0]->PutTemperature(AirTemperature);
...

   {
      ...
      double waterFlux[MaxSoilLayers], waterFluxMean[MaxSoilLayers], waterFluxSlope[MaxSoilLayers];
   	double heatConductivity[MaxSoilLayers], heatConductivityMean[MaxSoilLayers], heatConductivitySlope[MaxSoilLayers],
   	//CK heatCapacity[MaxSoilLayers],
   	SpecWaterCapacity[MaxSoilLayers];
...
\****************************************************************************/
void soilProfile::Terminate(string c)
{
...
double soilProfile::UpdateOrganicMatter()
{
   ...
   while (current != NULL)
   {
      ...
      CO2Evolution += current->UpdateOrganicMatter(i,NumOfSoilLayers);
      current = current->Next();
      i++;
   }
   return CO2Evolution;
...
string name=theSystemData->getOutputDirectory()+"Ccontent.xls";
	if(usedBefore==false)
	{
		usedBefore=true;
		Cconent.open(name.c_str(),ios::out);
			Cconent<<theTime<<"\t";
		 while (current != NULL)
		 {
			  double Content = 0;
	...
	{
	Cconent.open(name.c_str(),ios::app);
	Cconent<<theTime<<"\t";
	}
...
void soilProfile::WriteCarbon(const char * Name,bool usedBefores)
{

	//CK double Content = 0;
 soilLayer * current = first;
 static ofstream Cconent;
string name=theSystemData->getOutputDirectory()+Name+"_C.xls";
	if(usedBefores==false)
	{
		...
		Cconent.open(name.c_str(),ios::out);
		 ...
		 {
				 //CK   double Content = 0;
				 //CK Content= current->GetStartDepth();
			Cconent<<"g C/m2"<<"\t";
...
void soilProfile::WriteN(const char * Name,bool usedBefores)
{

	//CK  double Content = 0;
 soilLayer * current = first;
 static ofstream Cconent;
string names=theSystemData->getOutputDirectory()+Name+"_N.xls";
	if(usedBefores==false)
	{

		Cconent.open(names.c_str(),ios::out);
			Cconent<<theTime<<"\t";
		 while (current != NULL)
		 {
			  double Content = 0;
	...
	{
	Cconent.open(names.c_str(),ios::app);
	Cconent<<theTime<<"\t";
	}
...
void soilProfile::WriteDecompositionRate(const char * Name,bool usedBefores)
{

	//CK double Content = 0;
 soilLayer * current = first;
 static ofstream Cconent;
string names=theSystemData->getOutputDirectory()+Name+"_DecompositionRate.xls";
	if(usedBefores==false)
	{
...
void soilProfile::WriteCN(const char * Name,bool usedBefores)
{

	//CK  double Content = 0;
 soilLayer * current = first;
 static ofstream Cconent;
string name=theSystemData->getOutputDirectory()+Name+"_CN.xls";
	if(usedBefores==false)
	{
...
void soilProfile::changeLayer(int starLayer,int endLayer)
{
	...
	vector<	soilLayer *> layser;
	 ...
	 for(unsigned  i=1;i<=(layser.size()-1);i++) //CK
	 {
									  ...
									  double * rootLengthList)
{
   ...
   {
	  /* static ofstream addedC;
		static bool usedBefore=false;
		if(usedBefore==false)
		{
			usedBefore=true;
			addedC.open(theSystemData->getOutputDirectory()+"addedC.xls",ios::out);
			addedC<<theTime<<'\t';
			soilLayer * current2 = first;
			while (current2)
			{
				addedC<<current2->GetStartDepth()<<"\t";
		...
		{
		addedC.open(theSystemData->getOutputDirectory()+"addedC.xls",ios::app);
		}
	*/
