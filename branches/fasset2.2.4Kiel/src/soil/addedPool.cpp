...
void addedPool::Decay(double environmentEffect,cnMatter &out,int mode,double cnc)
{
   ...
   if (mode!=5 && carbon.c>1E-45)
   {
      ...
      else // Euler integration
	  {
		...
		{
			  //CK cout<<carbon.c<<endl;
			  //CK intermed=carbon*decompositionRate*environmentEffect*cl;
			  //CK	cout<<intermed.c<<endl;
		  }
		  else
		  {
...
/****************************************************************************\
\****************************************************************************/
void addedPool::CatchInfluxIfHungry(cnMatter &content,int mode)
{
   content.SetAll(0.0,0.0,0.0,0.0);
   if(Hungry())
      CatchInflux(content,mode);
}

/****************************************************************************\
\****************************************************************************/
void addedPool::CatchInfluxIfNotHungry(cnMatter &content,int mode)
{
   content.SetAll(0.0,0.0,0.0,0.0);
   if(!Hungry())
      CatchInflux(content,mode);
...
/****************************************************************************\
\****************************************************************************/
void addedPool::FinalDecayIfHungryAOM(double ratio,cnMatter &out)
{
   if (ratio>1.0 || ratio<0.0)
      Terminate("addedPool::FinalDecayIfHungryAOM - 'ratio' has illegal value");
