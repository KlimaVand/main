...
systemData::systemData(bool notNitroscape)
{
 ...
 void systemData::addRun()
 {
	 //default values for keywords for every run
		...
		newDataset.setGenerateFixedPlans(0);
		newDataset.setNumberOfFixedYears(1);
		newDataset.setNumberOfScenarios(0);
		newDataset.setWaterFlowMethodInt( 0);
		newDataset.setfertiliserUnit(0);
		newDataset.setUseLPModel(0);
		...
		newDataset.setGenerateFixedPlansFile(false);
		newDataset.setNumberOfFixedYearsFile(false);
		newDataset.setNumberOfScenariosFile(false);
		newDataset.setVolatModelFile(false);
		newDataset.setfertiliserUnitFile(false);
		newDataset.setUseLPModelFile(false);
		newDataset.setsingleFieldFile(false);
		newDataset.setStartYearFile(false);
		newDataset.setStopYearFile(false);
		newDataset.setWaterFlowMethodFile(false);
		...
		newDataset.setUseReductionParameters(false);
		newDataset.setGrassWeed(false);
		newDataset.setOrganic(false);
		newDataset.setSkipRun(false);
		...
		newDataset.setUseReductionParametersFile(false);
		newDataset.setGrassWeedFile(false);
		newDataset.setOrganicFile(false);
		newDataset.setSkipRunFile(false);
...
cin.get();
		...
		if(runData[i].getClimateDirectoryFile()==false)
		{
			...
			if (loc>0)
			{
				...
				cout << "systemData::checkData()"<<endl;
...
cin.get();
		...
		if(runData[i].getWaterFlowMethodFile()==true)
		{
			 runData[i].setWaterFlowMethodInt(-1);
			 if(runData[i].getWaterFlowMethodString().compare("DARCY")==0)
				 runData[i].setWaterFlowMethodInt(0);
			 if(runData[i].getWaterFlowMethodString().compare("TIPPING")==0)
				 runData[i].setWaterFlowMethodInt(1);
			 if(runData[i].getWaterFlowMethodString().compare("RICHARDS")==0)
				 runData[i].setWaterFlowMethodInt(2);
			 if(runData[i].getWaterFlowMethodString().compare("COMBINED")==0)
				 runData[i].setWaterFlowMethodInt(3);
			 if (runData[i].getWaterFlowMethodInt()<0)
			 {
			    cout << endl << "Illegal string '" << runData[i].getWaterFlowMethodString() << "' after 'WaterFlowMethod' in 'system.dat'" << endl;
cin.get();
			    exit(99);
			 }
		}
   ...
   RunLevel=1;
	...
	for(unsigned int i=0;i<items.size();i++)
	{
		...
		string value=conv.partOfAString(line,2);
			if(nameOfItem.compare("OperationOffset")==0)
			{
							OperationOffset=conv.StringToInt(value);
							OperationOffsetFile=true;
						}
			else if(nameOfItem.compare("JumpStart")==0)
						{
			...
			else if(nameOfItem.substr(1,3).compare("run")==0)
			{
               ...
               value=conv.partOfAString(line,2);
					...
					else if(nameOfItem.compare("GenerateFixedPlans")==0)
					{
						runData[RunLevel].setGenerateFixedPlans(conv.StringToInt(value));
						runData[RunLevel].setGenerateFixedPlansFile(true);
					}
					else if(nameOfItem.compare("NumberOfFixedYears")==0)
					{
						runData[RunLevel].setNumberOfFixedYears(conv.StringToInt(value));
						runData[RunLevel].setNumberOfFixedYearsFile(true);
					}
					else if(nameOfItem.compare("NumberOfScenarios")==0)
					{
						runData[RunLevel].setNumberOfScenarios(conv.StringToInt(value));
						runData[RunLevel].setNumberOfScenariosFile(true);
					}
					else if(nameOfItem.compare("WaterFlowMethod")==0)
					{
						runData[RunLevel].setWaterFlowMethodString(value);
						runData[RunLevel].setWaterFlowMethodFile(true);
					}
					...
					else if(nameOfItem.compare("UseLPModel")==0)
					{
						runData[RunLevel].setUseLPModel(conv.StringToInt(value));
						runData[RunLevel].setUseLPModelFile(true);
					}
					...
					else if(nameOfItem.compare("UseReductionParameters")==0)
					{
						runData[RunLevel].setUseReductionParameters(conv.StringToBool(value));
						runData[RunLevel].setUseReductionParametersFile(true);
					}
					else if(nameOfItem.compare("UseReductionParameters")==0)
					{
						runData[RunLevel].setUseReductionParameters(conv.StringToBool(value));
					...
					else if(nameOfItem.compare("GrassWeed")==0)
					{
						runData[RunLevel].setGrassWeed(conv.StringToBool(value));
						runData[RunLevel].setGrassWeedFile(true);
					}
					else if(nameOfItem.compare("SkipRun")==0)
					{
						runData[RunLevel].setSkipRun(conv.StringToBool(value));
						runData[RunLevel].setSkipRunFile(true);
					}
