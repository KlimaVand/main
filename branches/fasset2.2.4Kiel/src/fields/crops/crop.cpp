   ...
   :base(aName,aIndex,aOwner)
{
   ...
   DecayRoot            = 0.005;          // Root decay given as a daily fraction of DM at 10 deg. Celsius
   ...
   MinAllocToRoot        =  0.05;         // Minimal fraction of dry matter production that is allocated to the root
   ...
   MinN_Store    			= 0.013;               // Minimum content of nitrogen i storage organs (Estimeret fra Elly's data)
	...
	PowDM         			= -0.44;             	 // Power of dry matter in nitrogen status calculation  (Justes et al., 1994)
   NPctMax       			= 8.3;              	 // Following Justes concept (Justes et al., 1994)
   NPctMin       			= 2.2;       	 		 // Following Justes concept (Justes et al., 1994)
   ...
   WaterDeficitGrainFill 	= 1.0;
	...
	FractionNminInRoots 		= 0.0;        //Fraction of N in roots present as mineral N
   FractionNminInVegTop 	= 0.0;       //Fraction of N in VegTop present as mineral N
   CO2Effect  					= 1.0;
	MaxGLAICrop 				= 7.0;
...
Revider liste !!!
\****************************************************************************/
void crop::ReadParameters(string filename)
{

   commonData data;

   if (data.readFileWithoutExit(filename))
   {
	   int sectionName=data.FindSection(CropName);
	   // int sectionCrop=data.FindSection("Crop"); //CK

	  // int maxs=max(sectionName,sectionCrop);

	   int maxs = sectionName; //CK
		...
		if (maxs!=-1)
      {
         ...
         {
        	 data.FindItem("DMCostPerN",maxs,DMCostPerN);
        	 data.FindItem("MaxFixPerDM",maxs,MaxFixPerDM);
         }
         data.FindItem("ColonialisationRate",maxs,ColonialisationRate);
			data.FindItem("PhotoSyntActPart",maxs,PhotoSyntActPart);
			data.FindItem("RootpF",maxs,RootpF);
			data.FindItem("MinDMtemp",maxs,MinDMtemp);
			data.FindItem("MaxDMtemp",maxs,MaxDMtemp);
			data.FindItem("MaxRadUseEff",maxs,MaxRadUseEff);
			data.FindItem("CoeffGreenIdx",maxs,CoeffGreenIdx);
   		data.FindItem("LAINitrogenRatio",maxs,LAINitrogenRatio);
   		data.FindItem("LAIDMRatio",maxs,LAIDMRatio);
         data.FindItem("MinimumSenescense",maxs,MinimumSenescense);
			data.FindItem("NCurveParam",maxs,NCurveParam);
         data.FindItem("MinN_Store",maxs,MinN_Store);
			data.FindItem("MaxN_Store",maxs,MaxN_Store);
			data.FindItem("MinN_Root",maxs,MinN_Root);
			data.FindItem("MaxN_Root",maxs,MaxN_Root);
      	data.FindItem("NPctMax",maxs,NPctMax);
      	data.FindItem("NPctMin",maxs,NPctMin);
         data.FindItem("PowDM",maxs,PowDM);
         data.FindItem("ReducedNInYellow",maxs,ReducedNInYellow);
			data.FindItem("GerminationConversion",maxs,GerminationConversion);
			data.FindItem("Conversfactor",maxs,Conversfactor);
			data.FindItem("InitialRootDepth",maxs,InitialRootDepth);
			data.FindItem("InitialCAI",maxs,InitialCAI);
			data.FindItem("RootPentrRate",maxs,RootPentrRate);
			data.FindItem("RootDistrParm",maxs,RootDistrParm);
			data.FindItem("RootDensAtBottom",maxs,RootDensAtBottom);
         data.FindItem("DecayRoot",maxs,DecayRoot);
         data.FindItem("RhizoDepositFraction",maxs,RhizoDepositFraction);
         data.FindItem("TempRootMin",maxs,TempRootMin);
			data.FindItem("SpecificRootLength",maxs,SpecificRootLength);
			data.FindItem("MaxAllocToRoot",maxs,MaxAllocToRoot);
			data.FindItem("MinAllocToRoot",maxs,MinAllocToRoot);
			data.FindItem("MaxRootDepthCrop",maxs,MaxRootDepthCrop);
         data.FindItem("WaterDeficitVegGrowth",maxs,WaterDeficitVegGrowth);
   		data.FindItem("WaterDeficitLagPhase",maxs,WaterDeficitLagPhase);
   		data.FindItem("WaterDeficitGrainFill",maxs,WaterDeficitGrainFill);
			data.FindItem("InterceptCoeff",maxs,InterceptCoeff);
			data.FindItem("NitrateUptakeRate",maxs,NitrateUptakeRate);
			data.FindItem("AmmoniumUptakeRate",maxs,AmmoniumUptakeRate);
		   data.FindItem("MinimumSoilNitrate",maxs,MinimumSoilNitrate);
		   data.FindItem("MinimumSoilAmmonium",maxs,MinimumSoilAmmonium);
		   data.FindItem("ConversionCoefficient",maxs,ConversionCoefficient);
         data.FindItem("MaxPlantHeight",maxs,MaxPlantHeight);
         data.FindItem("LAIDistributionPar",maxs,LAIDistributionPar);
			data.FindItem("StoreForFilling",maxs,StoreForFilling);
			data.FindItem("FillFactor",maxs,FillFactor);
			data.FindItem("k",maxs,k);
         data.FindItem("FractionNminInRoots",maxs,FractionNminInRoots);
         data.FindItem("FractionNminInVegTop",maxs,FractionNminInVegTop);
         data.FindItem("TopFraction",maxs,TopFraction);
         data.FindItem("CO2Effect",maxs,CO2Effect);
         data.FindItem("MaxGLAICrop",maxs,MaxGLAICrop);
      }
   }
...
{
	int Place=0;
	if(MaxMeasuredPoints>=MeasuredNumber)
	{
		Place=MaxMeasuredPoints-1;
	}
	else if((MeasuredNumber-1)<0)
	{
		Place=0;
	}
	else
		Place=MeasuredNumber;
	if (theTime.GetDateValue()>=DateMeasured[0] && theTime.GetDateValue()<=DateMeasured[Place])
   {
...
static ofstream NrestFile;
static bool usedBefore=false;
string name=theSystemData->getOutputDirectory()+"nrest.txt";
if(usedBefore==false)
{
	usedBefore=true;
	NrestFile.open(name.c_str(),ios::out);
	   NrestFile<<theTime<<"\t"<<Nrest<<"\t"<<TotalRootN<<endl;
}
else
{
NrestFile.open(name.c_str(),ios::app);
}
   NrestFile<<theTime<<"\t"<<Nrest<<"\t"<<TotalRootN<<endl;
   NrestFile<<flush<<endl;
   NrestFile.close();
   Nrest=0;
...
double crop::Nmax()
{
	double NmaxVal = 0.0;
  	double N = 0.0;
  	//CK double NCO2 = GiveNitrogenResponseCO2();
...
{
   double CO2conc = theClimate->GetCO2Concentration();
 	double retVal,CO2PhotEffect=1; //CK
   if(!C4Photosynthesis)
      CO2PhotEffect = CO2Effect*exp(0.4537-170.97/CO2conc);
   retVal=Rg*PhotoSyntActPart*MaxRadUseEff
          *CO2PhotEffect
...
{
//  double newDeltaRoot = min(OptimalDeltaDryMatter()*FractionToRoot(),*dDryMatt);      //this function created makes no sense at highly stressed plants MEL 2009
...
double crop::MaxTransferable()
{
   ...
   return Nsurplus/CorrectedMinN_Store();
}
...
void crop::CalcRootDistribution()
{
 	...
 	for (int i=0;i<NumOfLayers;i++)
   {
      ...
      if (i>0)
      {
    	  //CK  if (((RootLengthList[i-1]/LayerThickness[i-1])-(RootLengthList[i]/LayerThickness[i]))<-1e-10)
    	  //CK    theMessage->FatalError("crop::CalcRootDistribution - root density is increasing with depth");

    	  //CK  cout << "crop::CalcRootDistribution - root density is increasing with depth " ;
    	  //CK  cout << i << ": " << RootLengthList[i-1] << i-1 << ": " << RootLengthList[i]  << endl;

      }
      if (RootLengthList[i]<0.0)
      {
    	  //CK if (RootLengthList[i]<(-1E-10))
         //CK   theMessage->WarningWithDisplay("crop::CalcRootDistribution - root length can not be negative");
    	  //CK if (i == 10)
    	     cout << "crop::CalcRootDistribution - root length can not be negative" << endl;
    	   RootLengthList[i]=0.0;
      }
