...
/****************************************************************************\
\****************************************************************************/
bool plan::Irrigation()
{
   return irrigationIncluded;
}
...
/****************************************************************************\
\****************************************************************************/
void plan::ShowContent(ostream * fs)
{
	cloneList<fieldOperationFields>::PS P;
	OpListP->PeekHead(P);
	while (P!=NULL)
...
/****************************************************************************\
NJH Aug 2002
Used by manure manager to set the adjustment factor for manure applications
\****************************************************************************/
void plan::DoManureAdjustment(double aValue)
{
   for(int i=0;i<OpListP->NumOfNodes();i++)
   {
      fieldOperationFields *FOP;
      FOP = OpListP->ElementAtNumber(i);
      if ((FOP->GetOperationId()==SpreadingLiquidManure)||(FOP->GetOperationId()==ClosedSlotInjectingLiquidManure)
      										||(FOP->GetOperationId()==OpenSlotInjectingLiquidManure)
                                    ||(FOP->GetOperationId()==TrailingShoeSpreadingLiquidManure)
                                    ||(FOP->GetOperationId()==TrailingHoseSpreadingLiquidManure)
                                    ||(FOP->GetOperationId()==SpreadingSolidManure))
         ((animalFertFields *) FOP)->AdjustManure(aValue);
   }
}
