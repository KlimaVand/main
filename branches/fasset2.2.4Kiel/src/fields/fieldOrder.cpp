...
void fieldOrder::ReadParameters(fstream * f)
{
   ...
   if (Automatic==0)
   {
      ...
      for (int i=first;i<(first+size);i++)
      {
         ...
         double * a = new double(23.0);
         GetParameter("TillageDepth",a,i);
TillageDepth.InsertLast(a);
         double * minDepth=new double(0);
			
         GetParameter("TillageMinDepth",minDepth,i);
		
         TillageMinDepth.InsertLast(minDepth);
		 string * PlantItemS=new string("all");
		 GetParameter("PlantItem",PlantItemS,i);
		 PlantItem.InsertLast(PlantItemS);
         a = new double(0.95);
 ...
 double * minDepth=new double(0);
         ...
         if (*type=="MINERAL")
         {
            ...
            *a=*a*factor;
               ...
               || (*type=="LIQUID-MANURE") || (*type=="SOLID-MANURE")||type->find("PIG-SLURRY-STORED")!=std::string::npos)
            {
               ...
               double orgN=0.0;
               if (!GetParameter("OrganicNitrogen",&on.n,i))
               {
                  if (GetParameter("OrganicN",&orgN,i))                        // FarmN project, minor screw-up
                  {
                     if (unitType==2)                                          // FarmN, minor screw-up
                     {
               ...
               if (GetParameter("FertilizerN",&amount,i))
               {
                  if (unitType!=0)
      ...
      for (int i=first;i<(first+size);i++)
      {
         ...
         if (0==i) 
            GetParameter("RemoveStraw",a);
         GetParameter("RemoveStraw",a,i);
         GetParameter("HarvestRemoveStraw",a,i);
         bool *Remove = new bool(*a>0 || removeRecorded);
      ...
      for (int i=first;i<(first+size);i++)
      {
         ...
         double *a = new double(0.0);
         if (0==i)
            GetParameter("CuttingRemoveStraw",a);
   ...
   else
   {
      ...
      fluid_hug_fall=fluid_hug_fall*factor;
   	if(GetParameter("DUNG.PA",&solid_hug))  //CK ;
...
void fieldOrder::ReadPeriods()
{
   ...
   for (int i=0;i<periodcount;i++)
   {
      ...
      TillageDepth.InsertLast(a);
 double * minDepth=new double(0);
         GetParameter("TillageDepth",minDepth,i);
         TillageMinDepth.InsertLast(minDepth);
      a = new double(0.95);
      GetParameter("TillageIncorporation",a,i);
      TillageIncorporation.InsertLast(a);
