...
//   InitialiseGrowthEstimates();
   currentPeriod=0;
   if ((theOutputData->DetailsData.getPastureDetails())&& (!grazmanfile))
...
********************************************************************/
void grazingManager::checkSilageHolding()
{
...
********************************************************************/
void grazingManager::checkPasture()
{
   field *aField;
   double herbageMass;
...
double grazingManager::GetHerbageAvailability(bool restricted)
{
   ...
   {
	   //CK if (restricted)
	   //CK	aField = (field*) fields_to_graze->ElementAtNumber(i);
	   //CKelse
	   //CK	aField = (field*) fields_avail_to_graze->ElementAtNumber(i);
...
********************************************************************/
double grazingManager::GetGrazingAreaAvailable()
{
   field *aField;
   double area=0.0;
   for (int i=0;i<fields_avail_to_graze->NumOfNodes();i++)
...
********************************************************************/
void grazingManager::SetFieldsGrazed(bsTime * aDate, int duration, double herbageRequired)
{
}
...
********************************************************************/
void grazingManager::SetFieldsGrazed(double herbageRequired)
{
	fields_to_graze->ForgetAll();
...
//   int numFieldsGrazed = fields_avail_to_graze->NumOfNodes();
   if (theOutputData->DetailsData.getPastureDetails())
   {
...
//   cout << theTime.GetDay() << " " << theTime.GetMonth() << " " << numOfFields << endl;
 ...
 //        double area = aField->GetArea();
         if (massDensity>minHarvestable)    //avoid daft values that occur just after sowing
         {
            ...
            //CK   FEperKgDM = N_to_FE(proteinConc/(N_to_protein()));
...
int grazingManager::GetNumGrazableFields()
{
...
*****************************************************/
void grazingManager::ClearTemporaryVariables()
{
	for (int i=0;i<theCropRotation ->GetFields()->NumOfNodes();i++)
   {
   	field *aField = theCropRotation ->GetFields()->ElementAtNumber(i);
