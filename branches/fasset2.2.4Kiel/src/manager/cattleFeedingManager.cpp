...
/*	   if (beefPresent && dairyPresent)
  ...
  //	CalcTurnoutandHousingDates(true);
   if ((theOutputData->DetailsData.getHerdDetails()==true)&& (!fHerdPerformance))
...
/*******************************************************************
********************************************************************/
void cattleFeedingManager::ReadFeedingGroupParameters(int start, int stop)
{
   string namestr;
	cattleFeedingGroup * aFeedingGroup=new cattleFeedingGroup();
   for(int inx=start;inx<=stop;inx++)
   {
      SetCritical();
//      namestr=(string) "Group";
//      cattleFeedingGroup * aFeedingGroup = new cattleFeedingGroup((char *)namestr.c_str(),inx,this); // Possible LEAK, not sure (bmp) !!!
//      feedingGroups->InsertLast(aFeedingGroup);
      aFeedingGroup->ReadParameters(file, autoFeed);                           //get the diet info for each period
		aFeedingGroup->Setactive(true);
   }
...
double cattleFeedingManager::GetCattleIntake()
{
	double ret_val=0.0;
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
...
void cattleFeedingManager::GetRoughageRequired(linkList <product> *aList)
{
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattleFeedingGroup * aFeedingGroup = feedingGroups->ElementAtNumber(j);
...
double cattleFeedingManager::GetGrassSilageRequired()
{
	double ret_val=0.0;
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
...
double cattleFeedingManager::GetEnergyRequirement()
{
	double ret_val=0.0;
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
...
void cattleFeedingManager::AutoFeed(int conceptionTime, int lengthOfLactation)
{
   ...
   {
	  //CK int numOfFeedingGroups;
	   //CK numOfFeedingGroups=feedingGroups->NumOfNodes();
      for (int i=1; i<=2; i++)
      {
...
void cattleFeedingManager::GetNumber()
{
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
	   cattleFeedingGroup * aFeedingGroup = feedingGroups->ElementAtNumber(j);
