...
/**
 * returning the Capacity of a specified area in barrenStorage
 */
double barnStorage::GetCapacity(const int storenum)
{
...
/**
 * Return a list of stores in this building
 */

linkList<storage> * barnStorage::Getstorage()
{
   return stores;
}
...
int barnStorage::SendToStore(product * p)
{
   ...
   for(int i=0;i<NrOfStores;i++)
   {
      ...
      if (!aStore->ProductCompare(p)==0)
      {
         ...
         if (aStore->GetCapacity() < AmountStored) { //CK
        	 if (scalable)
        	 {
...
/****************************************************************************\
\****************************************************************************/
ostream& operator<< (ostream& os,barnStorage s)
{
  os <<  (building*) &s << endl;
...
/**
 * Calculate N and C for plantItemObj and feedItemObj that is stored.
 * The input describe witch kind of output is written to the indicat file(only environmentalIndicator and economicIndicatoris implemented)
 */
void barnStorage::GiveIndicator(int indicatorType)
{
   double N_content = 0.0, C_content = 0.0;
