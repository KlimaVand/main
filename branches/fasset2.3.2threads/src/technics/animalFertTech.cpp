...
/*
   if(pumpSize==0)
      theMessage[id]->FatalError("animalFertTech::CalcGangHours - pumpSize = 0 in operation ",
                            (char*)operationName.c_str());
*/
   if(spreadingCapacity==0)
      theMessage[id]->FatalError("animalFertTech::CalcGangHours - spreadingCapacity = 0 in operation ",
                            (char*)operationName.c_str());
   if(size==0)
      theMessage[id]->FatalError("animalFertTech::CalcGangHours - size = 0 in operation ",
                            (char*)operationName.c_str());
   if(carryCapacity==0)
      theMessage[id]->FatalError("animalFertTech::CalcGangHours - carryCapacity = 0 in operation ",
                            (char*)operationName.c_str());
   if(workingSpeed==0)
      theMessage[id]->FatalError("animalFertTech::CalcGangHours - fieldSpeed = 0 in operation ",
                            (char*)operationName.c_str());
   if(roadSpeed==0)
      theMessage[id]->FatalError("animalFertTech::CalcGangHours - roadSpeed = 0 in operation ",
                             (char*)operationName.c_str());
...
{
	linkList <techEquip> * techFarmList = theTechnics[id]->GetFarmList();
...
//   cout << aTechEquipInFarmList->element->GettechEquipName() << endl;
   ...
   theMessage[id]->WarningWithDisplay("Equipment missing: ",tech.c_str());
	theMessage[id]->FatalError("animalFertTech::GetApplicationTech () - Unable to find manure application equipment amongst farm's equipment");
	return NULL;
}
