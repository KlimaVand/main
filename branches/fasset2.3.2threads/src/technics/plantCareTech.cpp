...
void plantCareTech::CalcGangHours()
{
   ...
   if(area==0)
      theMessage[id]->FatalError("area = 0 in operation ",
                            (char*)operationName.c_str());
   if(tankSize==0)
      theMessage[id]->FatalError("tankSize = 0 in operation ",
                            (char*)operationName.c_str());
   if(roadSpeed==0)
      theMessage[id]->FatalError("roadSpeed = 0 in operation ",
                            (char*)operationName.c_str());
   if(specificArea==0)
      theMessage[id]->FatalError("specificArea = 0 in operation ",
                            (char*)operationName.c_str());
...
{
	(*aChemical)*area;
   theProducts[id]->SubtractProduct(aChemical);
   ...
   switch (flag) {
		case herbicide:
	      theOutput[id]->AddIndicator(8001,"80.01 Herbicide consumption","TFI",aAmount);
			break;
      case fungicide:
      	theOutput[id]->AddIndicator(8002,"80.02 Fungicide consumption","TFI",aAmount);
			break;
      case insecticide:
      	theOutput[id]->AddIndicator(8003,"80.03 Insecticide consumption","TFI",aAmount);
         break;
		default:
			break;
   }
