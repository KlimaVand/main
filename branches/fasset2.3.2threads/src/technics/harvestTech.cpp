...
void harvestTech::ReadParameters(commonData * &file)
{
   ...
   file->FindItem("baleType",&baleType);
...
void harvestTech::CalcGangHours()
{
   ...
   if(size==0.0)
      theMessage[id]->FatalError("harvestTech::CalcGangHours - size = 0 in operation ",
                            (char*)operationName.c_str());

   if(operationID==CombineHarvesting || operationID==CombineHarvestingShred)
   	if(netCapacityWheat==0.0)
	      theMessage[id]->FatalError("harvestTech::CalcGangHours - netCapacityWheat = 0 in operation ",
                            (char*)operationName.c_str());
   if(area==0.0)
      theMessage[id]->FatalError("harvestTech::CalcGangHours - area = 0 in operation ",
                            (char*)operationName.c_str());
   if(carryCapacity==0.0)
      theMessage[id]->FatalError("harvestTech::CalcGangHours - carryCapacity = 0 in operation ",
                            (char*)operationName.c_str());
   if(workingSpeed==0.0)
      theMessage[id]->FatalError("harvestTech::CalcGangHours - fieldSpeed = 0 in operation ",
                            (char*)operationName.c_str());
   if(roadSpeed==0.0)
      theMessage[id]->FatalError("harvestTech::CalcGangHours - roadSpeed = 0 in operation ",
                            (char*)operationName.c_str());
   if(balesLoad==0.0 && operationID==GatheringBales)
      theMessage[id]->FatalError("harvestTech::CalcGangHours - balesLoad = 0 in operation ",
                            (char*)operationName.c_str());
	...
	if(operationID==CombineHarvesting || operationID==CombineHarvestingShred)
   {
      ...
      if (MainProduct[0]==NULL)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - MainProduct[0] should not be NULL here");
      ...
      delete [] tmp;
      if (calcAmount<=0)
    		theMessage[id]->WarningWithDisplay("harvestTech::CalcGangHours - amount harvested/gathered should be greater than zero in operation ",
                                (char*)operationName.c_str());
      A = (
   ...
   {
      if (baleType==-1)
    		theMessage[id]->FatalError("harvestTech::CalcGangHours - baleType not initialized");
      if (strawBalerCapacity<1E-10)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - strawBalerCapacity extremly low or zero");
      A = (
      ...
      {
         if (bindTime<1E-10)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - bindTime extremely low or zero");
         if (balesLoad<1E-10)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - balesLoad extremely low or zero");
      	A1 = ((calcAmount*1000.0)/balesLoad)*bindTime;
  			if(A1<=0.0)
      		theMessage[id]->WarningWithDisplay("harvestTech::CalcGangHours - A1 should be greater than zero in operation ",
                                (char*)operationName.c_str());
      }
	}
   if(A<=0.0)
      theMessage[id]->FatalError("harvestTech::CalcGangHours - A should be greater than zero in operation ",
                            (char*)operationName.c_str());
   ...
   switch(operationID)
   {
      ...
      case CombineHarvestingShred:
         if (prepReloading==0.0)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - prepReloading = 0");
         if (prepReloadingHarvester==0.0)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - prepReloadingHarvester = 0");
      ...
      case BeetTopHarvesting:
         if (reloadingCapacity==0.0)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - reloadingCapacity = 0");
         if (prepReloading==0.0)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - prepReloading = 0");
         if (prepReloadingHarvester==0.0)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - prepReloadingHarvester = 0");
      ...
      case GatheringBales:
         ...
         {
            if (balesLost==0.0)
               theMessage[id]->FatalError("harvestTech::CalcGangHours - balesLost = 0");
            if (prepReloading==0.0)
               theMessage[id]->FatalError("harvestTech::CalcGangHours - prepReloading = 0");
            if (prepReloadingHarvester==0.0)
               theMessage[id]->FatalError("harvestTech::CalcGangHours - prepReloadingHarvester = 0");
         ...
         {
            if (loadCapacity==0.0)
               theMessage[id]->FatalError("harvestTech::CalcGangHours - loadCapacity = 0");
            if (reloading==0.0)
               theMessage[id]->FatalError("harvestTech::CalcGangHours - reloading = 0");
            if (moweCarriage==0.0)
               theMessage[id]->FatalError("harvestTech::CalcGangHours - moweCarriage = 0");
   ...
   {
      if (stops==0.0)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - stops = 0");
   ...
   if(operationID==BeetHarvesting)
   {
      if (loadClampCapacity==0.0)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - loadClampCapacity = 0");
      if (unloadRoothouseCapacity==0.0)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - unloadRoothouseCapacity = 0");
      if (prepUnloadingRoothouse==0.0)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - prepUnloadingRoothouse = 0");
      if (prepClamp==0.0)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - prepClamp = 0");
      if (uncoverClamp==0.0)
         theMessage[id]->FatalError("harvestTech::CalcGangHours - uncoverClamp = 0");
   ...
   switch(operationID)
   {
      ...
      case BeetTopHarvesting:
         if (depositCapacity==0.0)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - depositCapacity = 0");
      ...
      case BeetHarvesting:
         ...
         if (depositRoothouseCapacity==0.0)
            theMessage[id]->FatalError("harvestTech::CalcGangHours - depositRoothouseCapacity = 0");
         I1 = calcAmount*depositCapacity;
...
void harvestTech::UpdateProducts(bool contractor)
{
   ...
   if ((operationID!=CutSetAside)&&(operationID!=Mowing)&&(operationID!=BeetTopChopping))
   {
      ...
      {
	      theProducts[id]->AddHarvestGrainProducts(aList);
      	aList->ForgetAll();
      }
      for (int i=0;i<MaxPlants;i++)
      {
      ...
      if (aList->NumOfNodes()>0)
      {
	      theProducts[id]->AddHarvestStrawProducts(aList);
   	   aList->ForgetAll();
      }
      for (int i=0;i<MaxPlants;i++)
      {
      ...
      if (aList->NumOfNodes()>0)
      {
	      theProducts[id]->AddForageProducts(aList);
   	   aList->ForgetAll();
      }
      delete aList;
   }
