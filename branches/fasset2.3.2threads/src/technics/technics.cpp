...
void technics::CreateOperationList(string aFileName)
{
   ...
   data->setCritical(false); // Presently no manual labour operations used
   ...
   for(index=first;index<(first+num);index++)
   {
      ...
      manualLabourTechInstance->ReadParameters(data);
...
{
   int actualPeriod = thePeriods->GetActualPeriodNumber(theTime[id].GetMonth(),theTime[id].GetDay());
   char periodName[40];
   ...
   if (hiredHours>0.0)
   {
      product * hiredLabour = new product;
	   ...
	   theProducts[id]->GiveProductInformation(hiredLabour);
   	hiredLabour->Setamount(hiredHours);
      theProducts[id]->SubtractProduct(hiredLabour);
      delete hiredLabour;
   }
   ...
   theOutput[id]->AddIndicator(1310,"13.10 Labour, field","hours",hours);

   string LabourPeriod = "13."+ (string) IndicatIndexString + " Field-labour " + (string) periodName;

   theOutput[id]->AddIndicator(1300+IndicatIndex,(char*) LabourPeriod.c_str(),"hours",hours);
   theOutput[id]->AddIndicator(1350,"13.50 Labour, maintance and management","hours",0.15*hours);  // 15%  maintance and management
	theOutput[id]->AddIndicator(1301,"13.01 Total labour","hours",hours*1.15);
}
