...
{
   (*aFertilizer)*area;
   theProducts[id]->SubtractProduct(aFertilizer);
   ...
   nitrogen N_content = aFertilizer->GetN_content()*aFertilizer->GetAmount();
	...
	theOutput[id]->AddIndicator(3101,"31.01 N from mineral fertilizer","kg N",N_content.n);
	theOutput[id]->AddIndicator(5001,"50.01 Phosphorus from mineral fertilizer","kg P",P_content);
	theOutput[id]->AddIndicator(6001,"60.01 Potassium from mineral fertilizer","kg K",K_content);
}
