...
void irrigateTech::CalcGangHours()
{
   ...
   if(pumpCapacity==0)
      theMessage[id]->FatalError("pumpCapacity = 0 in operation ",
...
void irrigateTech::UpdateProducts(bool contractor)
{
   ...
   theProducts[id]->GiveProductInformation(aEnergyProduct);
   aEnergyProduct->Setamount(aGang->GetgangElectricity());
   theOutput[id]->AddIndicator(2002,"20.02 Electricity consumption","KWh",aEnergyProduct->GetAmount());
   theProducts[id]->SubtractProduct(aEnergyProduct);
   delete aEnergyProduct;
   (*theWater)*area;
   theOutput[id]->AddIndicator(7002,"70.02 Irrigation","cubic metre",theWater->GetAmount());
   theProducts[id]->SubtractProduct(theWater);
   #ifdef DEVELOPING
