									...
									commonData * &data,commonData * &soilData, string * &prevoisFileName)
{
   ...
   while (!foundLayer && (i<(first+n)))
   {
      ...
      if (totalThickness<=0.0)
    	  theMessage[id]->FatalError("soilLayer::Initialize - Thickness must be above zero");
   ...
   if (!foundLayer)
	   theMessage[id]->FatalError("soilLayer::Initialize - no appropriate layer found");
...
void soilLayer::SetIceContent(double aIceContent)
{
   if (aIceContent>waterContent)
      theMessage[id]->WarningWithDisplay("soilLayer::SetIceContent - requsted ice content exceeds total amount of water");
/*   if (aIceContent<0)
      theMessage[id]->WarningWithDisplay("soilLayer::SetIceContent - requsted ice content is negative");
!!!!!!!!!!!!!!!!!!!!!!!!!! PUT BACK !!!!!!!!!!!!!! */
...
double soilLayer::GetCenterDepth()
{
   ...
   if (retVal<0)
      theMessage[id]->FatalError("soilLayer::GetCenterDepth() - center depth of a soil layer can not be negative");
   return retVal;
}
...
/****************************************************************************\
\****************************************************************************/
nitrogen soilLayer::GetPoolNitrogen(int Name,double startDepth,double thickness)
{
...
void soilLayer::AddNutrient(nitrogen addNitrate, nitrogen addAmmonium)
{
   ...
   if(ammonium_mob.n<-1E-6 || ammonium_imob.n<-1E-6 || nitrate_mob.n<-1E-6 || nitrate_imob.n<-1E-6)
   	theMessage[id]->WarningWithDisplay("soilLayer::AddNutrient - negative n concentrations");
...
double soilLayer::HeatConductivity()
{
   ...
   if (waterConcentration<=0)
      theMessage[id]->FatalError("soilLayer::HeatConductivity - water content zero or below");
   ...
   if (result>10)
   	theMessage[id]->WarningWithDisplay("soilLayer::HeatConductivity - conductivity to high");
	return result;
}
...
{
   if(ammonium_mob.n<-1E-6 || ammonium_imob.n<-1E-6 || nitrate_mob.n<-1E-6 || nitrate_imob.n<-1E-6)
   	theMessage[id]->WarningWithDisplay("soilLayer::AddWater - negative n concentrations");
	if (surplus<0)
   	theMessage[id]->WarningWithDisplay("soilLayer::AddWater - amount of water to add can not be negative");
   ...
   if((waterContent+0.001)<iceContent)
   	theMessage[id]->WarningWithDisplay("soilLayer::AddWater - ice exceeds total water at the end of the function");
}
                         ...
                         double chlorideAdd)
{
   ...
   if (waterContent<fieldCapacity)  					//all solutes go into imobile domain
   	fractionOfSolutesToMobile = 0.0;
   else if (waterContent-waterInFlow<fieldCapacity) 	//parts go into mobile
   	fractionOfSolutesToMobile = max(0.0,min(1.0,(waterContent-fieldCapacity)/waterInFlow));
   else  														//all go into mobile
   	fractionOfSolutesToMobile = 1.0;

      // Nitrate
      nitrate_mob = nitrate_mob+nitrateAdd*fractionOfSolutesToMobile;
      nitrate_imob = nitrate_imob+nitrateAdd*(1-fractionOfSolutesToMobile);

      // Ammonium
      ammonium_mob = ammonium_mob+ammoniumAdd*fractionOfSolutesToMobile;
      ammonium_imob = ammonium_imob+ammoniumAdd*(1-fractionOfSolutesToMobile);

      // Chloride
      Chloride_mob = Chloride_mob+chlorideAdd*fractionOfSolutesToMobile;
      Chloride_imob = Chloride_imob+chlorideAdd*(1-fractionOfSolutesToMobile);
                            		 ...
                            		 double ChlorideRemove)
{
   ...
   if (waterContent<0)
   	theMessage[id]->FatalError("soilLayer::RemoveWaterAndSolutes - water content below zero");
   if((nitrate_mob.n+nitrate_imob.n+ammonium_mob.n+ammonium_imob.n)<-1E-6)
   	theMessage[id]->FatalError("soilLayer::RemoveWaterAndSolutes - mineral nitrogen content below zero");
   if((waterContent+0.001)<iceContent)
   	theMessage[id]->WarningWithDisplay("soilLayer::RemoveWaterAndSolutes - ice exceeds total water");
   if(ammonium_mob.n<-1E-6 || ammonium_imob.n<-1E-6 || nitrate_mob.n<-1E-6 || nitrate_imob.n<-1E-6)
   	theMessage[id]->WarningWithDisplay("soilLayer::RemoveWaterAndSolutes - negative n concentrations");
}
...
void soilLayer::EquilibrateNitrogen(double mobileWater)
{
   ...
   if (waterContent<0.0)
   	theMessage[id]->FatalError("soilLayer::EquilibrateNitrogen - water content below zero");
...
/*   // Chloride
   ...
   if(Chloride_mob<-1E-6 || Chloride_imob<-1E-6)
   	theMessage[id]->WarningWithDisplay("soilLayer::Equilibrate - negative cloride concentrations");
   if(ammonium_mob.n<-1E-6 || ammonium_imob.n<-1E-6 || nitrate_mob.n<-1E-6 || nitrate_imob.n<-1E-6)
   {

   	theMessage[id]->WarningWithDisplay("soilLayer::Equilibrate - negative n concentrations");
   }
...
{
   if(ammonium_mob.n<-1E-6 || ammonium_imob.n<-1E-6 || nitrate_mob.n<-1E-6 || nitrate_imob.n<-1E-6)
   	theMessage[id]->WarningWithDisplay("soilLayer::RemoveWater - negative n concentrations");
   ...
   if (moveWater>0)
   {
      ...
      *ChlorideLeached += Chloride_mob*alpha*(1.0-FirstMoveFraction);
      ...
      WaterBudget.AddOutput(moveWater);
   	...
   	if (moveWater<0)
   		theMessage[id]->WarningWithDisplay("soilLayer::RemoveWater - amount of water to remove can not be negative");
   if (downwardMovement)
   {
   ...
   if(ammoniumLeaching.n<-1E-6 || nitrateLeaching.n<-1E-6 )
   	theMessage[id]->WarningWithDisplay("soilLayer::RemoveWater - negative leaching");
   if (waterContent<0)
   	theMessage[id]->FatalError("soilLayer::RemoveWater - water content below zero");
   if((nitrate_mob.n+nitrate_imob.n+ammonium_mob.n+ammonium_imob.n)<-1E-6)
   	theMessage[id]->FatalError("soilLayer::RemoveWater - mineral nitrogen content below zero");
   if((waterContent+0.001)<iceContent)
   	theMessage[id]->WarningWithDisplay("soilLayer::RemoveWater - ice exceeds total water");
}
   ...
   if (nitrateUptake>0 && nitrate.n>0)
 	{
      ...
      if (nitrate_mob.n<-1e-6 || nitrate_mob.n15<-1e-6 || nitrate_imob.n<-1e-6 || nitrate_imob.n15<-1e-6)
      	theMessage[id]->WarningWithDisplay("SoilLayer:: SubtractNitrate nitrate less than zero");
...
nitrogen soilLayer::SubtractAmmoniumUptake(double &ammoniumUptake)
{
   ...
   if (ammonium_mob.n15<-1e-10 || ammonium_imob.n15<-1e-10)
   	theMessage[id]->WarningWithDisplay("SoilLayer:: SubtractAmmonium ammonium-N15 less than zero");
   ...
   if (ammoniumUptake>0 && ammonium.n>0)
 	{
      ...
      if (ammonium_mob.n<0 ||  ammonium_imob.n<0)
      	theMessage[id]->WarningWithDisplay("SoilLayer:: SubtractAmmonium ammonium less than zero");
...
double soilLayer::UpdateOrganicMatter()
{
   ...
   if(ammonium_mob.n<-1E-6 || ammonium_imob.n<-1E-6 || nitrate_mob.n<-1E-6 || nitrate_imob.n<-1E-6 || OrganicMatter->GetNitrogen().n15<-1e-10)
   	theMessage[id]->WarningWithDisplay("soilLayer::UpdateOrganicMatter - negative n concentrations");
   return CO2Evolution;
}
...
nitrogen soilLayer::UpdateNitrification()
{
   if (ammonium_imob.n<-1e-10 || ammonium_mob.n<-1e-10 || ammonium_imob.n15<-1e-10 || ammonium_mob.n15<-1e-10)
   	theMessage[id]->WarningWithDisplay("SoilLayer::UpdateNitrification NH4 less than zero");
	nitrogen AddN;
   double ratio = 1.;
...
void soilLayer::UpdateN2OFromDenitrification(double TotalCO2Emission)
{
   ...
   if (Denitrification>0)
   {
      ...
      if(nitrate_mob.n<-0.0 || nitrate_imob.n<-0.0)
   		theMessage[id]->WarningWithDisplay("soilLayer::Denitrification - negative nitrate concentrations");
