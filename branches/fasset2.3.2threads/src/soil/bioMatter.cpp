   ...
   : matter(Pname,Index,owner)
{
   ...
   lackOfN=0.0;
   if (stepFactor<1E-60)
	   theMessage[id]->FatalError("bioMatter::bioMatter - 'stepFactor' unrealisticly small");
}
...
{
	   if (!source.IsBiomass())
	          theMessage[id]->FatalError("bioMatter::bioMatter - source is not biomatter");
...
void bioMatter::Initialize(double clayEff1,double clayEff2,int clayResp)
{
   ...
   if (clayResp==3) // New principle (added august 2001)
   {
      ...
      if (maintenance<=0)
          theMessage[id]->FatalError("bioMatter::Initialize - maintenance can not be zero or below with present clayresponse");
   }
...
void bioMatter::SetCarbon(cnMatter c)
{
   if (c.n>0.0)
   {
      if (c.c/c.n>(cnRatio+1E-6) || c.c/c.n<(minCNratio-1E-6))
          theMessage[id]->FatalError("bioMatter::SetCarbon - C/N ratio beyond possible borders");
   }
   else
      if (c.c>0.0)
          theMessage[id]->FatalError("bioMatter::SetCarbon - C present in pool but no N");
   carbon=c;
}
...
void bioMatter::ReadParameters(commonData * &file)
{
   ...
   if (stepFactor<1E-60)
       theMessage[id]->FatalError("bioMatter::ReadParameters - 'stepFactor' unrealisticly small");
   ...
   if (minCNratio>cnRatio)
       theMessage[id]->FatalError("bioMatter::ReadParameters - max cn ratio must be higher than min cn ratio");
   ...
   {
      if (tt<=0.0 && tr<=0.0)
          theMessage[id]->FatalError("bioMatter::ReadParameters - both 'TurnoverRatio' and 'TotalTurnover' must be specified and above zero");
      if (maintenance>0.0 || deathRate>0.0)
          theMessage[id]->FatalError("bioMatter::ReadParameters - inconsistent reading of parameters");
      deathRate=tt*tr;
      maintenance=tt-deathRate;
   }
   if (deathRate<=0.0)
       theMessage[id]->FatalError("bioMatter::ReadParameters - 'deathRate' must be above zero");
...
//  {
      if (cnRatio>0)
      {
         ...
         if (intermed.c>0.0)
          ...
          theMessage[id]->FatalError("C/N ratio not set for the '",poolNameChar,"' pool");
//    if (retVal<-1E-12)
//          theMessage[id]->FatalError("bioMatter::PotentialNitrogenMineralisation - returns negative value");
//   }
   return retVal;
}
...
double bioMatter::PotentialNitrogenImmobilisation()
{
   ...
   if (retVal>1E-12)
       theMessage[id]->FatalError("bioMatter::PotentialNitrogenImmobilisation - returns positive value");
   return retVal;
}
...
void bioMatter::FinalExport(double ratio)
{
   if (ratio>1.0 || ratio<0.0)
        theMessage[id]->FatalError("matter::FinalExport - 'ratio' has illegal value");
   if(intermed.c>0.0)
   {
...
void bioMatter::Update(double environmentEffect,int mode)
{
   if (carbon.c>0 && carbon.n<=0)
   {
      ...
      cout << theTime << endl;
       theMessage[id]->FatalError("bioMatter::Update - C present but no N (1)");
   }
   else
   {
      double cnrat=carbon.c/carbon.n;
      if (fabs(cnrat-cnRatio)>0.01)
          theMessage[id]->FatalError("bioMatter::Update - illegal C/N ratio");
   }
   if (maintenance<=0.0)
       theMessage[id]->FatalError("bioMatter::Update - 'maintenance' must be above zero");
   TestState();
   if (carbon.n<-1E-10)
       theMessage[id]->FatalError("bioMatter::Update - negative N content");
   if (intermed.c>0.0 || intermedCO2.c>0.0)
       theMessage[id]->FatalError("bioMatter::Update - programming inconsistency");
   switch (mode)
   {
      ...
      case 5: carbon=carbonDerivative[0]+carbonDerivative[1]/6.0+carbonDerivative[2]/3.0
              ...
              break;
      default:
    	// theMessage[id]->FatalError("void bioMatter::Update mode not supported");
    	  break;
   }
   ...
   if (carbon.n<-1E-10)
       theMessage[id]->FatalError("bioMatter::Update - negative N content");
   if (carbon.c>0 && carbon.n<=0)
       theMessage[id]->FatalError("bioMatter::Update - C present but no N (2)");
   else
   {
      double cnrat=carbon.c/carbon.n;
      if (fabs(cnrat-cnRatio)>0.01)
...
void bioMatter::CatchInflux(cnMatter &content,int mode)
{
   ...
   {
      if (cnRatio<=0.0)
          theMessage[id]->FatalError("bioMatter::CatchInflux - 'cnRatio' has illegal value for this instance");
      nitrogen nit;
...
{
   if (r<0.0)
       theMessage[id]->FatalError("bioMatter::GiveMicrobialPoolExtraN - ratio can not be below zero");
...
bool bioMatter::SetParameterByName(string parmName,double val)
{
   ...
   if (parmName=="TotalTurnover")
   {
      ...
      if (tot<=0.0)
          theMessage[id]->FatalError("bioMatter::SetParameterByName - sum of rates must be above zero");
...
bool bioMatter::ExtraTurnover(double Fraction)
{
   ...
   if (fabs(tr-Fraction)>1E-10)
       theMessage[id]->FatalError("bioMatter::ExtraTurnover - coding error!");
   tr=Fraction;
