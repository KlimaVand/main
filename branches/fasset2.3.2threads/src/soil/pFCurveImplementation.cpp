...
double pF_Curve_Implementation::CalcpF(double RelativeWater)
{
 ...
 while (p)
 {
  ...
  if (n>=maxSplinePoints)
   theMessage[id]->FatalError("pF_Curve_Implementation::CalcpF - the maximum of pF-curve points exceeded");
  n++;
 };
...
double pF_Curve_Implementation::CalcRelativeWater(double pFValue)
{
 ...
 while (p)
 {
  ...
  p = p->next;
  if (n>=maxSplinePoints)
   theMessage[id]->FatalError("pF_Curve_Implementation::CalcRelativeWater - the maximum of pF-curve points exceeded");
  n++;
 };
 ...
 if (yp<=0)
  yp = 1e-50;
 if (yp>1)
  theMessage[id]->FatalError("pF_Curve_Implementation::CalcRelativeWater - relative water content can not be above 1");
 return min(1.0,yp);
}
 ...
 */
   ...
   if (Mualem || vanGenuchten)
   {
      ...
      else
      {
	      ...
	      if (orgmat<=0)
	      	theMessage[id]->FatalError("pF-Curve:: algoritm does not work for OM = 0");
   ...
   else
   {
      ...
      if ((first!=f1) || (num!=n1))
    	  theMessage[id]->FatalError("pF_Curve_Implementation::ReadParameters-error in numbering of pF curve vector elements");
            ...
            data->FindItem("PFValue",&pFValue,i);
         	if (relWater>0 && relWater<1 && pFValue>=-1 && pFValue<7)
         		...
         		if (pFValue>7 || relWater<0)                                         // pF = 7 and low relative water are ignored but no warning are given (point is inserted anyhow)
         			theMessage[id]->WarningWithDisplay("soilLayerDescriptor::Initialize-pF point outside boundaries ignored");
         }
      }
