   ...
   : matter(source)
{
	   if (!source.IsAOM())
	      if (cnRatio<=0)
	    	  theMessage[id]->FatalError("addedMatter::addedMatter - illegal state of cnRatio variable");
...
void addedMatter::Add(matter * source,double fraction)
{
   ...
   if (numOfSubPools>maxSubPools)
	   theMessage[id]->FatalError("addedMatter::Add - to many pools");
...
{
   if (cnRatio>0)
	   theMessage[id]->FatalError("The added matter pool '",poolNameChar,"' can not have a fixed C/N ratio");
         ...
         if (poolName==prod->GetDestPool(k))
         {
            *num=k;
            ...
            if (!found)
            {
               ...
               numOfSubPools++;
               if (numOfSubPools>maxSubPools)
            	   theMessage[id]->FatalError("addedMatter::TestAddProduct - to many pools");
