   ...
   : base(source)
{
   ...
   if (!source.IsAOM())
      if (cnRatio<=0)
      {
          ...
          theMessage[id]->FatalError("matter::matter - illegal state of cnRatio variable");
      }
...
void matter::Add(matter * source,double fraction)
{
   if (poolName!=source->poolName)
       theMessage[id]->FatalError("matter::Add - pool names must be identical");
...
void matter::ReadParameters(commonData * &file)
{
   ...
   if (file->FindSection(Name,Index)) // Top of inheritance hierarcy
   {
      ...
      if (!(TAG==-1 || TAG==0 || TAG==1))
          theMessage[id]->FatalError("'TAG' must be either 0 or 1 in structurefile");
      file->setCritical(true);
      // Read connections
      double checkN=0.0;
      double checkC=0.0;
...
double matter::PotentialNitrogenImmobilisation()
{
   ...
   if (retVal>1E-12)
       theMessage[id]->FatalError("matter::PotentialNitrogenImmobilisation - returns positive value");
   return retVal;
}
...
{
   if (c.c<0)
       theMessage[id]->FatalError("C import to pool negative");
   if (c.n<0)
       theMessage[id]->FatalError("N import to pool negative");
   influx=influx+c;
}
...
void matter::AddOrganicProduct(organicProduct * prod,int fractionNumber,double * check)
{
 ...
 //  double nf=prod->GetNFraction(fractionNumber);
   ...
   {  // Adjust to match C/N ratio
      if (cnRatio<=0.0)
          theMessage[id]->FatalError("matter::AddOrganicProduct - C/N ratio should not be zero or negative here");
   ...
   *check+=add.c;
   ...
   {
	   cout<<add.n<<endl;
       theMessage[id]->FatalError("matter::AddOrganicProduct - negative amount of N added");
   }
   if (add.c<0.0)
       theMessage[id]->FatalError("matter::AddOrganicProduct - negative amount of C added");
}
   ...
   if (CNC<=0)
   {
      for (int i=0;i<connections;i++)
      {
         if (connectionList[i].connectPt)
         {
            ...
            if (cn<=0)
                theMessage[id]->FatalError("addedMatter::CalcCNcrit - a pool has undefined CNcrit");
            if (cn<CNC || CNC<0.0)
               CNC=cn;
         }
         else
...
void matter::TestState()
{
   ...
   if (carbon.n15<0.0)
      carbon.n15=0.0;
   if (carbon.c<-1E-10)
       theMessage[id]->FatalError("matter::TestState - negative amount of C");
   if (carbon.n<-1E-10)
       theMessage[id]->FatalError("matter::TestState - negative amount of N");
   if (!IsAOM())
      if (cnRatio<=0)
          theMessage[id]->FatalError("matter::TestState - illegal state of cnRatio variable");
}
...
void matter::Update(double environmentEffect,int mode)
{
   TestState();
   if (intermed.c>0.0)
       theMessage[id]->FatalError("matter::Update - programming inconsistency");
...
{
   if (influx.c<0.0)
       theMessage[id]->FatalError("Influx to pool negative");
   ...
   if ((in.c)>0.0)
   {
      ...
      {
         if (cnRatio<=0.0)
             theMessage[id]->FatalError("matter::CatchInflux - 'cnRatio' has illegal value for this instance");
         nitrogen nit;
         ...
         in.n15-=nit.n15;
          ...
          theMessage[id]->FatalError("matter::CatchInflux - C with no associated N");
   }
   if (mode!=5)
      carbon=carbon+in;
   ...
   if (c.c>0.0)
   {
      ...
      for (int i=0;i<connections;i++)
      {
         if (connectionList[i].Cfraction<0.0 || connectionList[i].Cfraction>1.0000001)
             theMessage[id]->FatalError("matter::Export - C fraction is illegal for ",poolNameChar);
         if (connectionList[i].Nfraction<0.0 || connectionList[i].Nfraction>1.0000001)
             theMessage[id]->FatalError("matter::Export - N fraction is illegal for ",poolNameChar);
...
bool matter::SetParameterByName(string parmName,double val)
{
   ...
   {
      if (connections!=2)
          theMessage[id]->FatalError("matter::SetParameterByName - can only set fraction by two connections");
      if (val>1.0)
         val=1.0;
      double f1=val;
      double f2=1.0-f1;
