...
{
   if (influx.c<0.0)
	   theMessage[id]->FatalError("addedPool::CatchInflux - C influx to pool negative");
   if (influx.n<0.0)
	   theMessage[id]->FatalError("addedPool::CatchInflux - N influx to pool negative");
   if (influx.c>0.0 && influx.n<=0)
	   theMessage[id]->FatalError("addedPool::CatchInflux - Influx to pool has C but no N");
   if (mode!=5)
   {
...
void addedPool::FinalExportAOM(double ratio,cnMatter &out)
{
   if (ratio>1.0 || ratio<0.0)
	   theMessage[id]->FatalError("addedPool::FinalExportAOM - 'ratio' has illegal value");
   if(intermed.c>0.0)
   {
