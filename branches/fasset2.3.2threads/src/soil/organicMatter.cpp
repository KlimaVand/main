   ...
   : base (source)
{
   if (&source)
   {
       ...
       theMessage[id]->FatalError("Attempt to copy 'organicMatter' with NULL pointer");
}
...
void organicMatter::Add(organicMatter * source, double fraction)
{
   if (&source)
   {
      ...
      for(int i=0;i<maxPools;i++)
       ...
       theMessage[id]->FatalError("organicMatter::Add - function called with NULL pointer");
}
...
organicMatter::~organicMatter()
{
   ...
   if (numberOfProducts>=maxOrgProds) // Previous problems, caused by incomplete copy constructors - should be solved now
       theMessage[id]->FatalError("organicMatter::~organicMatter - 'numberOfProducts' to high!");
   for(int i=0;i<maxPools;i++)
      if (poolList[i])
         delete poolList[i];
...
void organicMatter::InitStructure(commonData * &data)
{
   ...
   if (numberOfPools<1)
       theMessage[id]->FatalError("Pools are not properly defined in structurefile.");
   if (data->FindSection("Parameters"))
   {
      ...
      if (data->FindItem("NCslope",&ncSlope))
      {
         ...
         if (maxAOM2>1.0 || maxAOM2<0.0)
             theMessage[id]->FatalError("Model parameter 'MaxAOM2' has illegal value");
      }
   ...
   switch (responseType)
   {
      ...
      case 4:
         ...
         if (clayParameter5<=0.0)
             theMessage[id]->FatalError("'ClayParameter5 must be above zero");
      ...
      case 8:
         ...
         if (rCheck>0.02)
            theMessage[id]->WarningWithDisplay("Turnover rate for 'SMB1' is inconsistent with clay function");
         ...
         if (fSMB2>1E-5)
         {
            ...
            if (s<0.0)
                theMessage[id]->FatalError("organicMatter::ClayEffect - illegal numerical operation attempted");
            s=sqrt(s);
            ...
            clayEffect2=h/(E*r1);
         if (clayEffect2<0.0)
             theMessage[id]->FatalError("organicMatter::ClayEffect - 'clayEffect2' is negative");
         break;
      case 6:
         ...
         clayEffect2=h/(E*r);
         if (clayEffect2<0.0)
             theMessage[id]->FatalError("container::ClayEffect - 'clayEffect2' is negative");
      break;
      case 7:
      ...
      case 9:
         ...
         if (fSMB2>1E-5)
         {
            s=r1*(1.0-fSMB2)*r1*(1.0-fSMB2)+4.0*h*r1*r2*fSMB2;
            if (s<0.0)
                theMessage[id]->FatalError("container::ClayEffect - illegal numerical operation attempted");
            s=sqrt(s);
            clayEffect2=(s-r1*(1.0-fSMB2))/(2.0*E*r1*r2*fSMB2);
         }
         else
      ...
      break;
      default:
          theMessage[id]->FatalError("Illegal state of clay effect type");
         break;
   }
   return retVal;
}
...
double organicMatter::TemperatureEffect(double temp,int responseType)
{
   ...
   switch (responseType)
   {
      ...
      case 6: // Value used directly
         ...
         break;
      default:
          theMessage[id]->FatalError("Illegal state of temperature effect type");
         break;
   }
   if (retVal<0.0)
       theMessage[id]->FatalError("Temperature effect negative");
   return retVal;
}
...
double organicMatter::SoilWaterEffect(double water,int responseType)
{
   ...
   switch (responseType)
   {
      ...
      {
         if (W2Offset<0)
             theMessage[id]->FatalError("W2Offset can not be negative (soil water function)");
         if (W3Offset>0)
             theMessage[id]->FatalError("W3Offset can not be positive (soil water function)");
         double W2=W1+W2Offset;
         double W3=W4+W3Offset;
         if (W2>W3)
             theMessage[id]->FatalError("W2 is larger than W3 (soil water function)");
         f = Y1;
         if (water>W1)
         {
            f = 1.0;
      ...
      default:
          theMessage[id]->FatalError("Illegal state of water effect type");
         break;
   }
   if (f<0.0)
       theMessage[id]->FatalError("Water effect negative");
   return f;
}
...
matter * organicMatter::GetPoolPointer(int poolName)
{
   ...
   if (poolList[poolName]==NULL)
   {
       ...
       theMessage[id]->FatalError("Pool [",conv.IntToString(poolName).c_str(),"] not found");
   }
   else
...
void organicMatter::SetMineralNitrogen(nitrogen nitrate,nitrogen ammonium)
{
   if (NH4.n<-1E-12 || NO3.n<-1E-12)
       theMessage[id]->FatalError("organicMatter::SetMineralNitrogen - negative value for mineral nitrogen");
   NH4=ammonium;
   NO3=nitrate;
}
...
void organicMatter::ExchangeNitrogen(nitrogen * nit)
{
	if ((NO3.n+NH4.n)<-nit->n)
       theMessage[id]->FatalError("organicMatter::ExchangeNitrogen - total mineral N content below zero");
   if (NH4.n<-1E-9)
       theMessage[id]->FatalError("organicMatter::ExchangeNitrogen - NH4 content below zero");
   if (NO3.n<-1E-9)
       theMessage[id]->FatalError("organicMatter::ExchangeNitrogen - NO3 content below zero");
   if (nit->n<0)
   {  // Imobilisation
      double f=1.0;
      ...
      if (f<0.0 || f >1.0)
      {
         cout << f << " " << NH4.n << " " << nit->n << endl;
          theMessage[id]->FatalError("organicMatter::ExchangeNitrogen - f must be in the range [0;1]");
      }
                               ...
                               commonData *soilData)
{
   ...
   if (data->FindSection("SoilLayer",index)) // Top of inheritance hierarcy
   {
				   ...
				   if (data->FindItem(s,&d))
				   {
					  if (d>=-1000.0)
						 c.c_iso=GetFractionFromdelta(d)*c.c;
						  ...
						  theMessage[id]->FatalError("delta value for carbon-13 in file is below -1000");
				   }
				   else
					  c.c_iso=c.c;
				}
				   ...
				   if (data->FindItem(s,&d))
				   {
					  if (d>=0.0)
						 c.c_iso=c.c*d/100.0;
						  ...
						  theMessage[id]->FatalError("Percent modern for carbon-14 in file is below zero");
				   }
				   else
				   {
	;
					  ...
					  if (data->FindItem(s,&d))
					  {
						 if (d>=-1000.0)
							c.c_iso=GetFractionFromdelta(d)*c.c;
							 ...
							 theMessage[id]->FatalError("delta value for carbon-14 in file is below -1000");
					  }
					  else
						 c.c_iso=c.c;
				   }
				   ...
				   if (data->FindItem(s,&d))
				   {
					  if (d>=0.0 && d<=1.0)
						 c.c_iso=d*c.c;
						  ...
						  theMessage[id]->FatalError("TAG value in file must be between zero and one");
				   }
				   else
					  c.c_iso=0.0;
				}
	...
	//                 theMessage[id]->FatalError("Can not set N content in '",(char*)poolList[i]->GetPoolName().c_str(),"' as this pool has a fixed C/N ratio");
				   c.n=d*TotalC;
				}
				else
				{
				   ...
				   if (r<=0 && c.c>1E-9)
					   theMessage[id]->FatalError("C/N ratio must be set for pool '",poolList[i]->GetPoolNameChar());
				   if (r>0)
					  c.n=c.c/r;
				}
				strcpy(s,poolList[i]->GetPoolNameChar());
      ...
      if (fabs(1.0-checkSum)>1E-3)
          theMessage[id]->FatalError("organicMatter::Initialise - Pool carbon fractions do not add to one.");
      if (data->FindItem("ClayContent",&clayContent))
      {
         if (clayContent>1.0)
             theMessage[id]->FatalError("Clay fraction can not be bigger than one.");
         clayEffect1=ClayEffect(clayContent,ClayResponseType);
      }
      if (data->FindItem("ClayEffect",&clayEffect1)) // Overwrites clay content
         if (clayEffect1<0.0)
             theMessage[id]->FatalError("The parameter ClayEffect can not be below zero");
      if (data->FindItem("TemperatureEffect",&temperatureEffect)) // "Fixed" temperature effect
         if (temperatureEffect<0.0)
             theMessage[id]->FatalError("The parameter TemperatureEffect can not be below zero");
      if (data->FindItem("WaterEffect",&waterEffect))             // "Fixed" water effect
         if (waterEffect<0.0)
             theMessage[id]->FatalError("The parameter WaterEffect can not be below zero");
      for(int i=0;i<maxPools;i++)
    	  if(poolList[i]!=NULL)
    		  poolList[i]->Initialize(clayEffect1,clayEffect2,ClayResponseType);
      if (useSpringob) // Adjust size of "IOM" according to C/N ratio - not working properly!!!
      {
         ...
         iomf=min(1.0,iomf);
         bool IOMnamebool=false;
         int IOMname=-1;
         cnMatter iom;

         if (ExistsPool(6))
         {
             iom=GetCarbonInPool(6);
             IOMnamebool=true;
             IOMname=6;
         }
         else
         ...
         if (before.c>0)
         {
            ...
            if (IOMnamebool==false)
                theMessage[id]->FatalError("container::SetLocalParameterByName - could not find relevant inert pool for setting 'IOMFactor'");
            int humName=-1;
            bool humNamebool=false;
            if (ExistsPool(11))
            {
               humName=11;
               humNamebool=true;
            ...
            if (humNamebool==false)
                theMessage[id]->FatalError("Could not find relevant 'humus' pool for setting 'IOMFactor'");
            cnMatter nom=GetCarbonInPool(humName);
            ...
            nom.c+=move_carbon;
            nom.c_iso+=move_14c;
            SetCarbonInPool(IOMname,iom);
            SetCarbonInPool(humName,nom);
									  ...
									  nitrogen * ammonium)
{
   ...
   if (nitrate->n<-1E-9)
       theMessage[id]->FatalError("organicMatter::Update - function called with nitrate below zero");
   if (ammonium->n<-1E-9)
       theMessage[id]->FatalError("organicMatter::Update - function called with anmmnium below zero");
   carbon_avalibility=0.0;
...
// Decompose ----------------------------------------------------------------
   ...
   for(int j=0;j<maxPools;j++)
   {
	   if(poolList[j]!=NULL)
	   {
		  ...
		  {
			  cout<<theTime[id]<<endl;
			  cout<<Ni<<endl;
			  theMessage[id]->FatalError("organicMatter::UpdatePools - programming inconsistency ");
		  }
	   }
   }
...
// Decomposition finished-------------------------------------------------------
  ...
  {
     if (NO3.n>1E-15)
        theMessage[id]->Warning("organicMatter::Update - NO3 below zero");
     NO3.Clear();
  }
  if (NH4.n<0.0)
  {
     if (NH4.n>1E-15)
        theMessage[id]->Warning("organicMatter::Update - NH4 below zero");
     NH4.Clear();
  }
  ...
  *ammonium = NH4;
   ...
   if (fabs(tot-tot1)>1E-5)
       theMessage[id]->FatalError("organicMatter::Update - imbalance in N");
...
double organicMatter::GetCarbon()
{
   ...
   if (cn<0.0)
       theMessage[id]->FatalError("organicMatter::GetCarbon - returns negative value");
   return cn;
}
...
nitrogen organicMatter::GetNitrogen()
{
   ...
   if (TotalNitrogen.n<0.0)
       theMessage[id]->FatalError("organicMatter::GetNitrogen - returns negative value");
...
void organicMatter::AddProduct(organicProduct * product)
{
   ...
   if (product->GetAmount().c>0 && product->GetAmount().n<=0)
       theMessage[id]->FatalError("organicMatter::AddProduct - added matter contains C but no N");
   */
   double tot=GetTotalSystemN()+product->GetTotalNitrogen().n;
   if (product->GetOrganicNitrogen().n<-1E-36)
       theMessage[id]->FatalError("organicMatter::AddProduct - The added product has negative N content");
   ...
   if ((product->GetAmount()*product->GetCarbonFraction()).n<0.0)
       theMessage[id]->FatalError("organicMatter::AddProduct - negative amount of N added");
   if ((product->GetAmount()*product->GetCarbonFraction()).c<0.0)
       theMessage[id]->FatalError("organicMatter::AddProduct - negative amount of C added");
   double check=0.0;
   ...
   if (nom==NULL)
       theMessage[id]->FatalError("The pool 'NOM' was not found");
   ...
   {
       theMessage[id]->FatalError("organicMatter::AddProduct - The added product did not contain enough N to match consistently with pools");
   }
   ...
   if (fabs(check-amount.c*f)>1E-6)
       theMessage[id]->FatalError("organicMatter::AddProduct - The added product was not consistently matched with pools for C");
		 ...
		 if(poolList[i]!=NULL)
		 {
		  if (2==poolList[i]->GetPoolName()|| 3==poolList[i]->GetPoolName())
		  {
			 ...
			 if (!poolList[i]->ExtraTurnover(Fraction))
				 theMessage[id]->FatalError("organicMatter::KillSMB - illegal call for this type of pool");
		  }
		 }
	}
   if (count!=2)
       theMessage[id]->FatalError("organicMatter::KillSMB - SMB pool(s) not found");
}
		...
		if(poolList[i]!=NULL)
		{
		  if (0==poolList[i]->GetPoolName() || 1==poolList[i]->GetPoolName())
		  {
			 ...
			 if (!poolList[i]->CloneAndModify(Fraction,TurnoverFactor))
				 theMessage[id]->FatalError("organicMatter::ModifyAOM - illegal call for this type of pool");
		  }
		}
	}
   if (count!=2)
       theMessage[id]->FatalError("organicMatter::ModifyAOM - AOM pool(s) not found");
}
