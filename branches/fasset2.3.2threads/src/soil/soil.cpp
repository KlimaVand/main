...
soil::~soil()
{
   ...
   delete Surface;
   delete Profile;
   if ( globalSettings[id]->DetailsData.getSoilDetails()==true)
   { 
   	f.close();
   }
}
...
void soil::Initialize(char * soilFileName)
{
   ...
   if (globalSettings[id]->DetailsData.getSoilDetails()==true)
   {
      ...
      string fileName=globalSettings[id]->getOutputDirectory()+"soil"+GetLongNameInNumbers()+".xls";

     	f.open(fileName.c_str(),ios::out);
      f.precision(4);
      f << "date";

      for(unsigned int i=0;i<globalSettings[id]->WaterData.size();i++)
      {
         f << "\twater"<< globalSettings[id]->WaterData[i].getStartDepth() << "-"<<globalSettings[id]->WaterData[i].getEndDepth() ;
      }

      for(unsigned int i=0;i<globalSettings[id]->NminData.size();i++)
      {
         f << "\tNmin"<< globalSettings[id]->NminData[i].getStartDepth() << "-"<< globalSettings[id]->NminData[i].getEndDepth();
         f << "\tNO3-"<< globalSettings[id]->NminData[i].getStartDepth() << "-"<< globalSettings[id]->NminData[i].getEndDepth();
         f << "\tNH4-"<< globalSettings[id]->NminData[i].getStartDepth()  << "-"<< globalSettings[id]->NminData[i].getEndDepth();
         f << "\tTotal15N-"<< globalSettings[id]->NminData[i].getStartDepth() << "-"<< globalSettings[id]->NminData[i].getEndDepth();
      }

      for(unsigned int i=0;i<globalSettings[id]->SoilOrgCData.size();i++)
           {

              f << "\tSMB-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();

               f <<"\tNOM-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();
               f <<"\tIOM-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();
               f <<"\tAOM1-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();
               f <<"\tAOM2-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();
               f <<"\tSMB1-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();
               f <<"\tSMB2-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();
               f <<"\tSMR-C-"<< globalSettings[id]->SoilOrgCData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgCData[i].getEndDepth();

           }
           for(unsigned int i=0;i<globalSettings[id]->SoilOrgNData.size();i++)
           {

         	  f << "\tSMB-N-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]";
         	  f << "\tSMB-N15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tAOM1 N-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tAOM2 N-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tSMB1 N-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tSMB2 N-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tSMR N-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tNOM n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tIOM n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tAOM1 n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tAOM2 n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tSMB1 n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tSMB2 n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    << "\tSMR n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]"
         	    <<"\tMOM n15-"<< globalSettings[id]->SoilOrgNData[i].getStartDepth() << "-"<<  globalSettings[id]->SoilOrgNData[i].getEndDepth()<<"[m/g]";
           }

      for(unsigned int i=0;i<globalSettings[id]->PoreWaterData.size();i++)
      {

         f << "\t" << globalSettings[id]->PoreWaterData[i].getDepth()/10 <<"cmNit-N"
         << "\t" << globalSettings[id]->PoreWaterData[i].getDepth()/10 <<"cmAm-N"
         << "\t" << globalSettings[id]->PoreWaterData[i].getDepth()/10 <<"cmN"
       	<< "\t" << globalSettings[id]->PoreWaterData[i].getDepth()/10 <<"cmCL"
         << "\t" << globalSettings[id]->PoreWaterData[i].getDepth()/10 <<"cmN15";


      }

      for(unsigned int i=0;i<globalSettings[id]->TemperatureData.size();i++)
      {
    	  f << "\tt" <<globalSettings[id]->TemperatureData[i].getDepth();
                  ...
                  linkList<rootStructure>* rootInfo)
{
   ...
   if (theTime[id].GetMonth()==4 && theTime[id].GetDay()==1)
   ...
   if ((depth>0 && frac>0))
   {
      ...
      if (tillage)
      {
         ...
         data.FindItem("TurnoverFactor",&TurnoverFactor);
            	...
            	theMessage[id]->FatalError("Illegal parameter values for soil tillage effect");
        		if (p1 > 0.0)
            	KillSMB(p1,depth);
         	if (p2 > 0.0)
...
void soil::WriteOutput()
{
 	...
 	if(globalSettings[id]->DetailsData.getSoilDetails()==true)
	{
 		...
 		for(unsigned int i=0;i<globalSettings[id]->WaterData.size();i++)
	 	{
	 		f << "\t" << GetWater(globalSettings[id]->WaterData[i].getStartDepth(), globalSettings[id]->WaterData[i].getEndDepth()-globalSettings[id]->WaterData[i].getStartDepth());

	 	}

 		for(unsigned int i=0;i<globalSettings[id]->NminData.size();i++)
 		{
 			double StartDepth =globalSettings[id]->NminData[i].getStartDepth();
 			double Thickness = globalSettings[id]->NminData[i].getEndDepth()-StartDepth;
 		...
 		for(unsigned int i=0;i<globalSettings[id]->SoilOrgCData.size();i++)
 		{
 			double StartDepth =globalSettings[id]->SoilOrgCData[i].getStartDepth();
 			double Thickness = globalSettings[id]->SoilOrgCData[i].getEndDepth()-StartDepth;
 		...
 		for(unsigned int i=0;i<globalSettings[id]->SoilOrgNData.size();i++)
 		{
 			double StartDepth =globalSettings[id]->SoilOrgNData[i].getStartDepth();
 			double Thickness = globalSettings[id]->SoilOrgNData[i].getEndDepth()-StartDepth;
 			double SoilMass = GetSoilMass(StartDepth,Thickness)/10000000.0; // BMP 29.10.2008 !!!!!!
 		...
 		for(unsigned int i=0;i<globalSettings[id]->PoreWaterData.size();i++)
 		{
 			double depth = globalSettings[id]->PoreWaterData[i].getDepth();
      ...
      for(unsigned int i=0;i<globalSettings[id]->TemperatureData.size();i++)
 		{
 			...
 			double depth = globalSettings[id]->TemperatureData[i].getDepth();
...
void soil::AddDecomposable(decomposable* produc,double * rootLengthList)
{
   produc->RemoveWater();                                          // To cope with dry matters less than 1
  	if (produc->GetAmount()<-1E-10)
  		theMessage[id]->FatalError("soil::AddDecomposable - negative amount added");
 	if (produc->GetAmount()>0)
 	{
      if (produc->GetorgN_content().n<0.0)
      {
    	  cout<<produc->GetorgN_content().n<<endl;
   		theMessage[id]->FatalError("soil::AddDecomposable - negative amount of organic N added");
      }
  		//if ((rootLengthList) && (*rootLengthList<=0))
   	//	theMessage[id]->FatalError("soil::AddDecomposable - rootLengthList can not contain zero in first element");
      ...
      // N allocations
  	   ...
  	   if ((produc->GetAOM1_Nfraction()>0.0) || (produc->GetAOM2_Nfraction()>0.0))
       {
        ...
        if (sum<0 || sum>1)
        	theMessage[id]->FatalError("soil::AddDecomposable - inconsistency in values for fAOM1 and/or fAOM2");
       }
