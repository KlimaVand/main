...
\****************************************************************************/
barnStorage::barnStorage(char *aName,const int aIndex,const base* aOwner,commonData *  &runData,commonData * &orignal)
	:building(aName,aIndex,aOwner,runData,orignal)
{
	...
	orignal->FindItem("scalableBstore",&scalableBstore);
	runData->FindItem("scalableBstore",&scalableBstore);
	storage tmp("Store",0,this,0,runData, orignal);
	defaultStorage=tmp;
   ...
   for(int i=0;i<NrOfStores;i++)
   {
      ...
      if (!aStore->ProductCompare(p)==0)
      {
         ...
         if (aStore->GetCapacity() < AmountStored)
         {
        	 ...
        	 {
        		 theMessage[id]->FatalError("barnStorage::SendToStore: not enoug room in store for product: ",p->GetName());
        	 }
         }
      }
   }
...
//      theMessage[id]->Warning("default creation of product store by B_storage2"); No longer relevant
...
#ifndef _STANDALONE
	theOutput[id]->AddStateIndicator(305,"03.05 Value of stored products","Dkr",result);
   theOutput[id]->AddStateIndicator(3562,"35.62 N in stored plant products","kg N",N_content);
   theOutput[id]->AddStateIndicator(4562,"45.62 C in stored plant products","kg C",C_content);
#endif
}
