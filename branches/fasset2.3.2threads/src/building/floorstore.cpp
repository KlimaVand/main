    		 ...
    		 base(aName,aIndex,aOwner)
{
	...
	switch (theControlParameters[id]->GetvolatModel())
	{
		...
		if (manuretype=="SEPARATED")	// need separate as liquid and solid stored separately
		{
			...
			theMessage[id]->FatalError("FloorStore: attempt to use Separated manure system - is not supported");
		}
...
void floorstore::RcvManure(manure *aManure)
{
	...
	{
		theMessage[id]->FatalError("floorstore::RcvManure: SEPARATED not supported. Talk to nick");
		for(int i=0;i<theStores->NumOfNodes();i++)
		{
			instore * ainstore=(instore *) theStores->ElementAtNumber(i);
	...
	else
	{
		...
		if (!ainstore->GetnextStore())//must be solid manure stored on floor - record amount produced here
			theOutput[id]->AddIndicator(1973,"19.73 Solid manure produced","t",aManure->GetAmount());
	}
}
...
manure * floorstore::CleanFloor(double numberOfAnimals)
{
	...
	for(int i=0;i<theStores->NumOfNodes();i++)
	{
		...
		if (ainstore->GetnextStore())
		{
			...
			theOutput[id]->AddIndicator(1973,"19.73 Solid manure produced","t",water->GetAmount());
...
{
	if (AreaPrAnimal==0.0)
		theMessage[id]->FatalError("floorstore:: AreaPrAnimal should not be zero here!");
...
string floorstore::getmanureName()
{
	if (!theStores->NumOfNodes()==1)
		theMessage[id]->FatalError("floorstore::getmanureName - more than one manure type stored");
