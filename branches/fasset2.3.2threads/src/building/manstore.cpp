...
void manurestore::RcvManure(const manure* someManure)
{
	...
	if (strcmp(theStore->GetProductPointer()->GetName(),"SLURRY")==0)
	{
		...
		{
			int id =threadID->getID(std::this_thread::get_id());
			theMessage[id]->FatalError(GetName()," ","manurestore: attempt to add wrong manure type");
		}
	}
	else
	{
...
#else
   		theMessage[id]->FatalError(GetName()," ","manurestore: ammonia volat exceeds NH4 N");
#endif
...
#else
   		theMessage[id]->FatalError(GetName()," ","manurestore: ammonia volat exceeds organic N");
#endif
...
#else
   		theMessage[id]->FatalError(GetName()," ","manurestore: ammonia volat exceeds nitrate N");
#endif
...
#else
		int id =threadID->getID(std::this_thread::get_id());
		theMessage[id]->FatalError(GetName()," ","manurestore: pH error");
#endif
	}
...
void manurestore::ShowManureStore(fstream * afile, bool header)
{
	...
	{
		int id =threadID->getID(std::this_thread::get_id());
		*afile << theTime[id].GetJulianDay() << "\t"<< GetAlias() << "\t"
