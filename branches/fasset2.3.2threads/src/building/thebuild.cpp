...
{  //Note - this allows more than one animal type to be placed in a given section
	...
	if (!aSection)
	{
		theMessage[id]->FatalError("buildings: animal section not found for ",name);
...
product * buildings::ExtractProduct(product* p)
{
		...
		while ((s!=NULL)&&(gottenamount<requestedamount))
		{
			...
			if (tempProduct)
			{
				if (p->GetmainUnitName()==3)
					...
					gottenamount+=tempProduct->GetAmount();
				theOutput[id]->AddIndicator(3580,"35.80 N transf. from stable to store","kg N",(((manure *)tempProduct)->GetAllN().n*tempProduct->GetAmount())*1000.0);
				theOutput[id]->AddIndicator(4580,"45.80 C transf. from stable to store","kg C",(((manure *)tempProduct)->GetC_content()*tempProduct->GetAmount())*1000.0);
				*productFound + *tempProduct;
				delete tempProduct;
			}
	...
	if ((gottenamount<requestedamount) && (gottenamount>0))
		theMessage[id]->Warning("building::not enough ",p->GetName()," stored, the rest bought");
...
void buildings::InitializeAll(string InputFilename, bool pigs, bool dairy, bool beef)
{
	...
	if (runData->readFile(InputFilename))
	{
		...
		if (pigs || dairy || beef)
		{
			...
			for(int inx=small;inx<=large;inx++)
			{
				theMessage[id]->FatalError("buildings::InitializeAll - biogas not currently implemented");
			...
			for(int inx=small;inx<=large;inx++)
			{
				theMessage[id]->FatalError("buildings::InitializeAll - digestatestore not currently implemented");
				runData->setCritical(true);
...
//				cattlehouse *stableInstance=new cattlehouse(InstanceName,InstanceIndex,this,0);
	...
	else
	{
		...
		theMessage[id]->Warning(error.c_str());
	}
...
{
	if (theControlParameters[id]->GetvolatModel()>1)
	{
		for(int i=0;i<theManurestores->NumOfNodes();i++)
		{
			manurestore* asltore=theManurestores->ElementAtNumber(i);
