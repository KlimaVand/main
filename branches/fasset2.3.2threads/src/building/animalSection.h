...
public:
   ...
   animalSection(const char* aName,const int aIndex,const base* aOwner,commonData * &runData,commonData * &common);
   //! Destructor
   virtual ~animalSection();
	   animalSection(const animalSection& aAnimalSection);
	   linkList<floorstore> *getFloorClone();
   ...
   virtual double GetDailyElectricityUse() {return NrOfAnimals * ElectricityUse/theTime[id].daysInYear(0);}
   ...
   virtual void SetAnimalParameters(double numberOfAnimals,double faecesPerYear, double urinePerYear, double heatOP=0.0);
	...
	virtual string GetManureType();
	virtual void CheckFloorIntegrity();
	virtual void SetName(char *aName){strcpy(Name,aName);}
