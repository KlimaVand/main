...
\****************************************************************************/
dungheap::dungheap(const char *aName,const int aIndex,const base* aOwner,commonData *  &runData,commonData * &orignal)
 : manurestore(aName,aIndex,aOwner, runData, orignal), building(aName,aIndex,aOwner, runData, orignal)
{
	ventilated = false;
   surfaceLayer = new manurestore("surface", 0, this,runData, orignal);
   ...
   surfaceLayer->Setdepth(0.02);  //depth 20 cm
	...
	int indexInOld;
   	...
   	string isVentilated;
	orignal->FindItem("Ventilated",&isVentilated);
    ...
    if (globalSettings[id]->DetailsData.getManureManageDetails())
 		OpenDetailsFile();
    else
    	detailsFile=NULL;
}
...
dungheap::dungheap(dungheap& s): manurestore(s)
{
	...
	//theMessage[id]->FatalError("dungheap:: copy constructor not implemented");
}
...
void dungheap::DailyUpdate()
{
   ...
   theOutput[id]->AddIndicator(3543,"35.43 NH3-N volatilisation from manure storage","kg N",volatAmmonia*1000.0);
   theOutput[id]->AddIndicator(3544,"35.44 N2-N volatilisation from manure storage","kg N",N2Emission*1000.0);
   theOutput[id]->AddIndicator(3545,"35.45 N2O-N volatilisation from manure storage","kg N",N2OEmission*1000.0);
   theOutput[id]->AddIndicator(4543,"45.43 CO2-C volatilisation from manure storage","kg C",CO2Emission*1000.0);
   theOutput[id]->AddIndicator(4544,"45.44 CH4-C volatilisation from manure storage","kg C",CH4Emission*1000.0);
...
void dungheap::GiveIndicator(int indicatorType)
{
 ...
 //  cout << " V " << Vt << " N " << N << " C " << C << endl;
	if (external)
   {
    ...
    {
       theOutput[id]->AddStateIndicator(3560,"35.60 N bound in manure storage","kg N",N*1000.0);
       theOutput[id]->AddStateIndicator(4560,"45.60 C bound in manure storage","kg C",C*1000.0);
    }
    theOutput[id]->AddStateIndicator(1171,"11.71 Dung in dungheap","t",Vt/1000);
   }
}
...
{
	theOutput[id]->AddIndicator(1973,"19.73 Solid manure produced","t",someManure->GetAmount());
	manure *tmpManure=(manure*) someManure->clone();
   if (strcmp(theStore->GetProductPointer()->GetName(),"SLURRY")==0)
		theMessage[id]->FatalError(manurestore::GetName()," ","dungheap: attempt to add wrong manure type");
   ...
   {
      theMessage[id]->FatalError("dungheap::RcvManure - trying to add cattle FYM to pig FYM");
   }
   if ((strcmp(tmpManure->GetName(),"PIG-FYM")==0) && (strcmp(theManureName,"CATTLE-FYM")==0))
   {
      theMessage[id]->FatalError("dungheap::RcvManure - trying to add pig FYM to cattle FYM");
   }
...
//                  theStore->AddToStore(tmpManure);  //send to main store
		...
		theMessage[id]->FatalError(manurestore::GetName()," ","manurestore: attempt to add wrong manure type");
