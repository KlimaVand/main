...
;
			if (controlledVent>0)
			{
				...
				if ((minVentilation>maxVentilation)||(maxVentilation<=0.0))
					theMessage[id]->FatalError("Stable:: minimum ventilation rate > maximum or maximum <= 0");
...
void stable::DailyUpdate()
{
	...
	if (theControlParameters[id]->GetvolatModel()==1||theControlParameters[id]->GetvolatModel()==0)
	{
		Volatize();
	}
	else
	...
	theMessage[id]->FatalError("stable::DailyUpdate: volat model ",conv.IntToString(theControlParameters[id]->GetvolatModel()).c_str(), " not supported");
	}

	CleanManure();
#ifndef _STANDALONE
	theOutput[id]->AddStateIndicator(3561,"35.61 N bound in stable manure","kg N",GetTotalN()*1000.0);
	theOutput[id]->AddStateIndicator(4561,"45.61 C bound in stable manure","kg C",GetTotalC()*1000.0);

#endif
	double electricity = GetDailyElectricity();
	//   electricity = electricity/theTime[id].daysInYear();                  // converts to days
	product* Energy = new product;
	Energy->Setname("ELECTRICITY");
	theProducts[id]->GiveProductInformation(Energy);
	Energy->Setamount(electricity);
	theProducts[id]->SubtractProduct(Energy);
	delete Energy;

#ifndef _STANDALONE
	theOutput[id]->AddIndicator(2002,"20.02 Electricity consumption","KWh",electricity);
#endif
	EndBudget(true);
...
void stable::Volatize()
{
	...
	case 1://!dynamic emission model
	{
		if (inUse)   //could be zero if using scaling and no animals for this housing type
		{
			...
			theClimate[id]->GetClimate(Ameantemp, Amin_temp,Amax_temp, Aprecip, Aradiation,AEPot, Awindspeed, Ahumidity);
			//Ameantemp+=5.0;
			double heatOP = 0.0;
	...
	theOutput[id]->AddIndicator(3540,"35.40 NH3-N volatilisation from animal house","kg N",NH3Emission*1000.0);
	theOutput[id]->AddIndicator(3541,"35.41 N2-N volatilisation from animal house","kg N",N2Emission*1000.0);
	theOutput[id]->AddIndicator(3542,"35.42 N2O-N volatilisation from animal house","kg N",N2OEmission*1000.0);
	//   theOutput[id]->AddIndicator("35.43 NO-N volatilisation from animal house","kg N",NOEmission*1000.0);
	theOutput[id]->AddIndicator(4540,"45.40 CO2-C volatilisation from animal house","kg C",CO2Emission*1000.0);
	theOutput[id]->AddIndicator(4541,"45.41 CH4-C volatilisation from animal house","kg C",CH4Emission*1000.0);
...
#ifndef _STANDALONE
	theOutput[id]->AddIndicator(3580,"35.80 N transf. from stable to store","kg N",(aManure->GetAllN().n*aManure->GetAmount())*1000.0);
	theOutput[id]->AddIndicator(4580,"45.80 C transf. from stable to store","kg C",(aManure->GetC_content()*aManure->GetAmount())*1000.0);
#endif
	delete aManure;
}
...
void stable::CheckLinks()
{
	for(int i=0;i<inhouseStores->NumOfNodes();i++)
	{
		...
		if ((aStore->GetnextStore()==NULL) && (aStore->GetManureToName()!="NONE"))
		{
			...
			theMessage[id]->FatalError("Stable:: an inhouse manure store is not linked to an external store");
		}
	}
...
{
	if (theControlParameters[id]->GetvolatModel()> 0)
	{
...
void stable::checkArea()
{
	if (planArea<GetTotalAreaSections())
		theMessage[id]->FatalError("Stable:: total area of animal sections > plan area of housing");
}
...
void stable::OpenDetailsFile()
{
	...
	hd.changeDir(globalSettings[id]->getOutputDirectory());
#ifdef  __BCplusplus__
	char buffer[MAX_TXT];
	getcwd(buffer, MAX_TXT);

	chdir(globalSettings[id]->getOutputDirectory().c_str());
#endif
...
void stable::ShowStable(bool header)
{
	...
	{
		*detailsFile << theTime[id].GetJulianDay() << "\t"<< GetAlias() << "\t" << ventilationRate << "\t"<<  insideTemperature << "\t"
				<< CH4Emission << "\t" << CO2Emission << "\t" << NH3Emission << "\t" << N2Emission << "\t" << N2OEmission<< "\t" ;
		for(int i=0;i<sections->NumOfNodes();i++)
		{
			animalSection* aSection=sections->ElementAtNumber(i);
