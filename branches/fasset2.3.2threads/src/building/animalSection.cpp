	...
	for(int i=0;i<flooring->NumOfNodes();i++)
	{
		...
		for(int j=0;j<floorInst->GetStoreNumber();j++)
		{
			...
			if ((aStore->GetnextStore()==NULL) && (aStore->GetManureToName().compare("NONE")==0))
			{
				...
				theMessage[id]->FatalError(outString.c_str());
				//   			theMessage[id]->FatalError("animalSection:: animal section floor is not linked to a manure store");
			}
		}
	}
}
...
void animalSection::DailyUpdate()
{
	if (NrOfAnimals>0.0)
	{
		...
		theProducts[id]->GiveProductInformation(someStraw);    // sets the default values for wheat straw
		for(int i=0;i<flooring->NumOfNodes();i++)
		{
			floorstore * floorInst=flooring->ElementAtNumber(i);
...
#ifndef _STANDALONE
			theOutput[id]->AddIndicator(3563,"35.63 N in bedding","kg N",someStraw->GetAmount()*someStraw->GetAllN().n*1000.0);
			theOutput[id]->AddIndicator(4563,"45.63 C in bedding","kg C",someStraw->GetAmount()*someStraw->GetC_content()*1000.0);
#endif
			theProducts[id]->SubtractProduct(someStraw);
		}
		delete someStraw;
		//       EndBudget(false);
	}
...
string animalSection::GetManureType()
{
	...
	if (flooring->NumOfNodes()>1)
		theMessage[id]->FatalError("animalSection::GetManureType() - does not currently work for sections with more than one floor type");
...
void animalSection::CheckFloorIntegrity()
{
	...
	if (cumulativePartition!=1.0)
		theMessage[id]->FatalError("Stable:: cumulative excreta partitioning to flooring not equal to 1.0");
}
