...
product& product::operator+(const product& aProduct)
{
	...
	else
	{
		if (mainUnitName!=aMainUnitName)
		{
			...
			theMessage[id]->FatalError("product::operator+ - products with different units added: ",WarningString.c_str());
		}
...
product& product::operator-(const product& aProduct)
{
	...
	if (mainUnitName!=someProduct->GetmainUnitName())
		theMessage[id]->FatalError("product::operator- - products with different units subtracted");
	amount	   -= aAmount;
	return *this;
}
...
int product::GetUnitType(string aMainUnitType)
{
	...
	if(!found)
	{
		theMessage[id]->WarningWithDisplay("product::GetUnitType - the unit type <", (char*)aMainUnitType.c_str(), "> does not exist");
		return -1;
	}
	else
		return typeRes;
...
/****************************************************************************\
\****************************************************************************/
void product::ReadParameters(commonData * &file,const char *  sectionName)
{
