...
product& energy::operator-(const product& someEnergy)
{
   ...
   if (kWhPerUnit!=aEnergy->GetkWhPerUnit())
   	theMessage[id]->WarningWithDisplay("energy::operator- - energy with different composition subtracted");
	product::operator-(someEnergy);
	return *this;
}
 ...
 */
void energy::ReadParameters(commonData * data,const char *  sectionName)
{
