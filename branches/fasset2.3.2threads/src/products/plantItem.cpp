...
product& plantItem::operator-(const product& somePlantItem)
{
	...
	if (tradeName!=aPlantItem->GetTradeName())
		theMessage[id]->WarningWithDisplay("plantItem::operator- - plants with different tradenames subtracted");
	decomposable::operator-(somePlantItem);
	return *this;
	}
...
/****************************************************************************\
\****************************************************************************/
void plantItem::ReadParameters(commonData * data,const char *  sectionName)
{
