...
void organicProduct::SetAOM1_fraction(double f)
{
   ...
   if (fabs(check-1.0)>1E-10)
      theMessage[id]->FatalError("organicProduct::SetAOM1_fraction - fractions do not add to one");
}
...
void organicProduct::SetAOM2_fraction(double f)
{
   ...
   for (int i=0;i<3;i++)
      check+=allocationList[i].cFraction;
   if (fabs(check-1.0)>1E-10)
	   theMessage[id]->FatalError("organicProduct::SetAOM2_fraction - fractions do not add to one");
}
...
nitrogen organicProduct::GetOrganicNitrogen()
{
   ...
   if(amount.n<0.0||amount.n15<0.0)
	   theMessage[id]->FatalError("organicProduct::GetOrganicNitrogen - negative amount of organic N");
	return OrganicN;
}
