...
product& animalProduct::operator-(const product& someanimalProduct)
{
   ...
   if (strcmp(Name,someanimalProduct.GetName())!=0)
	   theMessage[id]->WarningWithDisplay("animalProduct::operator-: different animal products should not be subtracted");
	return *this;
}
 ...
 */
void animalProduct::ReadParameters(commonData * data,const char * sectionName)
{
