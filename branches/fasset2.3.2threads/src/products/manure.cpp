...
*/
   if (!(toSoil)&& (infiltration >0.0))
         theMessage[id]->FatalError("manure:: Infiltration of manure >0 without receiptor object defined");
   if (toSoil)
   {
   ...
   {
   	if ((pH<1.0)||(pH>12.0))
      theMessage[id]->FatalError("pmanure: volatilisation error in pH values");
	   double loss=0.0;
      ...
      *volEvap= evap * duration/1000.0;  //convert from mm to tonnes per sq metre
      ...
      if (resistance==0)
         theMessage[id]->FatalError("manure:: The transfer resistance for ammonia loss cannot be zero");
      ...
      {
	         //theMessage[id]->FatalError("manure:: volatilisation - do not use this without checking if it works");
...
/*            double fact = evap - precipitation;
    ...
    if ((volat<0)||(TANtoSoil<0))
         theMessage[id]->FatalError("manure:: Volatilisation less than zero");
    ...
    *volEvap*=area;
    ...
    if (fabs(balance)>0.01)
         theMessage[id]->FatalError("manure:: Balance error in volatilisation");
...
{    //returns evaporation in tonnes
   ...
   if (resistance > 0.0)
   {
      ...
      delete aClimate;
   	...
   	theMessage[id]->FatalError("manure::GetWaterEvap - resistance to water vapour transport must be greater than zero");
...
product& manure::operator+(const product& someManure)
{
   ...
   double adegradableOM = aManure->GetdegradableOM();
 	if ((apH<1.0)||(apH>12.0) || (pH<1.0)||(pH>12.0))
   		theMessage[id]->FatalError("pmanure: error in pH values in + routine");
   ...
   else if (aAmount>1E-6)
   {
	...
	decomposable::operator+(someManure);
 	if ((NH4_content.n<0.0)||(orgN_content.n<0.0))
   		theMessage[id]->FatalError("pmanure: error in N values in + routine");
	return *this;
}
...
product& manure::operator-(const product& someManure)
{
	...
	if(Mg_content   != aManure->GetMg_content() ||
      ...
      maxDeg       != aManure->GetmaxDeg() ||
      degradableOM != aManure->GetdegradableOM())
         theMessage[id]->Warning("manure::operator- - manure with different composition subtracted");
	decomposable::operator-(someManure);
	NH3emissionFactor=aManure->NH3emissionFactor;
	return *this;
}
...
/****************************************************************************\
\****************************************************************************/
void manure::ReadParameters(commonData * data,const char *  sectionName)
{
