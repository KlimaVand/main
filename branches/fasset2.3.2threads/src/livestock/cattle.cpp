...
void cattle::Initialise()
{
   ...
   growth 								= new feedItem();
#ifndef _STANDALONE
   if (globalSettings[id]->DetailsData.getCowDetails()==true)
...
void cattle::DailyUpdate()
{
	if ((feedPlanList->NumOfNodes()==0)&&(fieldList->NumOfNodes()==0))
   	theMessage[id]->FatalError("cattle: DailyUpdate - no feed plan or fields");
...
// from here on until the end of the function, manure is scaled for animal number
   ...
   *solidManurePrDay * Number;
   ...
   if (isYoung)
   {
	   theOutput[id]->AddIndicator(3341,"33.41 N in young cattle manure","kg N",manureN*1000.0);
	   theOutput[id]->AddIndicator(4341,"43.41 4 in young cattle manure","kg C",manureC*1000.0);
	   theOutput[id]->AddIndicator(1963,"19.63 Manure produced by young cattle","t",amountManure);
   }
   else
   {
   	theOutput[id]->AddIndicator(3342,"33.42 N in dairy cattle manure","kg N",manureN*1000.0);
   	theOutput[id]->AddIndicator(4342,"43.42 C in dairy cattle manure","kg C",manureC*1000.0);
	   theOutput[id]->AddIndicator(1964,"19.64 Manure produced by dairy cattle","t",amountManure);
   }
   ...
   if (hoursHoused>0.0)
   {
      ...
      if (theSection)
      {
         ...
         theOutput[id]->AddIndicator(3381,"33.81 Cattle manure N to animal housing","kg N",manureNToHousing*1000.0);
         theOutput[id]->AddIndicator(4381,"43.81 Cattle manure C to animal housing","kg C",manureCToHousing*1000.0);
...
//         GetStableSection()->AddanimalHeatOp(Number*weight*heatProduction/SRW);
      ...
      else theMessage[id]->FatalError("cattle: no housing designated for receiving manure from ",GetAlias().c_str());
   }
   if (hoursHoused<24.0)
   {
      if (fieldList->NumOfNodes()>0)
      {
         ...
         {
         	theMessage[id]->Warning("Cattle: No DM offtake from fields - dung and urine evenly distributed between grazed fields");
            DMFromFields=10.0;
            for (int i=0;i<fieldList->NumOfNodes(); i++)
            {
		   ...
		   for (int i=0;i<fieldList->NumOfNodes(); i++)
         	...
         	if (fieldOfftakeList[i]>0.0)
            {
               ...
               double timeDistribution=(24.0 - hoursHoused)/24.0;
		         ...
		         theOutput[id]->AddIndicator(3380,"33.80 Cattle manure N to pasture","kg N",manureNToPasture*1000.0);
	   ...
	   else  theMessage[id]->FatalError("cattle: no fields grazed so nowhere for manure to go!!");
   }
#endif
...
//   Nbudget.AddOutput((urineN + dungN)/1000.0);
   ...
   if (urineN>=0)
   {
	...
	// sort out urine first
      ...
      theProducts[id]->GiveProductInformation(fluidManurePrDay);
...
//solid manure
      ...
      if (totalAmountPrDay<0.0)
      	theMessage[id]->FatalError(" ");
      solidManurePrDay->Setname("CATTLE-FAECES");
      theProducts[id]->GiveProductInformation(solidManurePrDay);          // sets the default values for solid-manure
...
#ifndef _STANDALONE
      theOutput[id]->AddIndicator(4101,"41.01 C input into cattle","kg C",Number * carbonIntake);
      theOutput[id]->AddIndicator(4104,"41.04 C in cattle faeces","kg C",Number * faecalCarbon);
      theOutput[id]->AddIndicator(4103,"41.03 C in cattle urine","kg C",Number * urineCarbon);
      theOutput[id]->AddIndicator(4107,"41.07 C retained in cattle body","kg C",Number * CRetained);
      theOutput[id]->AddIndicator(4105,"41.05 C in cattle CO2 production","kg C",Number * CO2_C);
      theOutput[id]->AddIndicator(4102,"41.02 C in cattle CH4 production","kg C",Number * methaneCarbon);
      theOutput[id]->AddIndicator(4106,"41.06 C in milk production","kg C",Number * milkCarbon);
#endif
...
//         theMessage[id]->Warning(animalName+" required additional feed");
    		cout<<urineN<<" is the urine"<< endl;
    		cout<<urineN<<" is the urine "<<theTime <<endl;
         theMessage[id]->FatalError("cattle::Production - urine gone negative");
         deficit=urineN;
         }
...
{
	theMessage[id]->FatalError("Function in cattle not implemented");
...
/*      double milkProtein = milk->GetAmount() * 1000000.0 * milk->GetProteinContent();
   ...
   if (dungN<0.0)
   	theMessage[id]->FatalError("cattle::ProteinDigestion - dung N gone negative");
}
...
{
   product * aStoredProduct =theProducts[id]->GetCopyStoredFeedProduct(menuPtr);   //returns a copy of the product if it is found in storage
   //next line will get a feed of a particular type, rather than a particular feed. Obsolete?
   //product * aStoredProduct = ((feed_resource *) theProducts[id]->GetProductType(menuPtr));
...
{
   theMessage[id]->FatalError("cattle: Get_norm_weight() - this should be called from an overloaded function");
   return 0.0;
}
...
//   factor = 1.0-exp(-5.0*herbage_mass); // earlier value 5 !!!
 ...
 if ((factor<0.0)||(factor>1.0))
 {
 	...
 	theMessage[id]->FatalError("uk_cattle: Availability factor less than zero og bigger than one!");
  }
 return factor;
}
...
void cattle::DoSelection(linkList <feed_resource> * alist, int * count)
{
   ...
   if(!feedPlanList)
		theMessage[id]->FatalError("cattle::", GetName()," feedPlanList not present");
   	...
   	if (hoursHoused<24.0)
      {
       ...
       // request the fields/pastures to generate grazing resource objects
         ...
         if (totGrazableFields==0)
	         theMessage[id]->FatalError("cattle: animals at pasture but no fields selected for grazing");
        ...
        //calculated grazedArea
         ...
         for (int i=0;i<totGrazableFields; i++)
         {
          ...
          for (int j=0;j<numOfPatches; j++)
          {
            ...
            // update my_resource
            aPatch->GetFeedResource(0.0, 0.0, 1); //parameters - stubble height as proportion of patch height, minimum stubble ht (m) and animal type (cattle = 1)
            ...
            if (aFeed_resource->GetAmount()>1000.0)
               theMessage[id]->FatalError("cattle: error in feed resource diet");
            alist->InsertLast(aFeed_resource);
...
#endif
	if (Hi==0) theMessage[id]->FatalError("cattle: no feed available!!");
...
void cattle::WriteCowOutput(bool header)
{
   ...
   {
      *detailsFile << theTime[id].GetYear() << "-" << theTime[id].GetMonth() << "-" << theTime[id].GetDay() << "\t";
...
void cattle::CreateDetailFile(int Num)
{
   ...
   sprintf(detailsFileName,globalSettings[id]->getOutputDirectory().c_str());
...
{
	if (!isFoetus)
   	theMessage[id]->FatalError("cattle:GrowFoetus - this animal is not a foetus!");
   else
   {
...
double cattle::GetSensibleHeatProduction()
{
   ...
   if (ret_val==0.0)
   	theMessage[id]->FatalError("cattle:GetSensibleHeatProduction - could not find animal category");
   double temperature = theClimate[id]->GetMeanTemperature();         //assumes animals experience outside temperature - could be wrong in extreme climates
