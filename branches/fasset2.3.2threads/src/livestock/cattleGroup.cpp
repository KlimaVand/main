...
void cattleGroup::CheckAge(linkList<cattle> * oldies)
{
   ...
   while (aNode)
   {
//   	if (((aNode->element->Getage()/theTime[id].daysInYear(0))>maxAge)&&(!aNode->element->isLactating()))
   	if ((aNode->element->Getage()/theTime[id].daysInYear(0)>maxAge))
      {
...
// open database file of cattle parameters
    ...
    if(numberOfCohorts>0)
      {
         double increment = (endAge-startAge)* theTime[id].daysInYear(0)/numberOfCohorts;
         double currentAge = ((int) (startAge * theTime[id].daysInYear(0)))+1;
         cattle* tempcow;
...
void cattleGroup::CullAnimals(int numberToCull)
{
 if (group->NumOfNodes()<numberToCull)
      theMessage[id]->FatalError("cattlegroup: more animals to cull than are currently present");
 int i=numberToCull;
// linkList <cattle>::PS aNode;
 linkList <cattle>::PS aNode;
 while (i>0)
...
void cattleGroup::AutoRecruit(linkList<cattle> * recruitmentList, int *IDnumber)//, fstream *CattlefileStream)
{
	...
	while (numToGet>0)
   {
               ...
               if (((GetareFemale())&& (aNode->element->GetisFemale()))||       //check if sex of candidate is ok for group
                   ((!GetareFemale())&& (!aNode->element->GetisFemale())))
                {
                  ...
                  *IDnumber=*IDnumber + 1;
                  ...
                  else {theMessage[id]->FatalError("cattle::No current animal section for recruited animals");}
                  ...
                  theMessage[id]->LogEventWithTime(outstring);
	   ...
	   if (numToGet>0)  //cannot recruit enough - buy some
	   {
         ...
         else {theMessage[id]->FatalError("cattlegroup::No current animal section for bought animals");}
		   group->InsertLast(tempcow);
         numToGet--;
  		}
   }
...
{
 double Number=0.0;
 if(theTime[id].GetMonth()==1&&theTime[id].GetDay()==1&&theTime[id].GetYear()==1988)
  cout<<group->NumOfNodes()<<" sice"<<endl;
   for(int i=0;i<group->NumOfNodes();i++)
   {
	   cattle * tempcattle = group->ElementAtNumber(i);
/*		if (indicatorType==environmentalIndicator)
 ...
 {
   animalProduct *anAnimalProduct = new animalProduct(*(animalProduct*) theProducts[id]->GetProductElement(Alias.c_str()));
   double value = anAnimalProduct->GetaPrice()->GetactualSellPrice() * Number;
   delete anAnimalProduct;
   if (areYoung)
   {
		theOutput[id]->AddStateIndicator(350,"03.50 Value young cattle","Dkr",value);
		theOutput[id]->AddStateIndicator(1150,"11.50 Young cattle present","no",Number);
   }
   else
   {
		theOutput[id]->AddStateIndicator(351,"03.51 Value mature cattle","Dkr",value);
		theOutput[id]->AddStateIndicator(1151,"11.51 Mature cattle present","no",Number);
   }
 }
};
...
/*   if (animalModel==1)
   ...
   if (animalModel!=2)
      theMessage[id]->FatalError("current animal Model is not supported at this point");
...
//if (tempcow->GetIndex()==29)
  //	cout <<"";
     ...
     else if ((GetAlias()=="DAIRY") || (GetAlias()=="SUCKLER"))    //only FBEEF3 fall out from the if tree
     {
        ...
        else if (GetAlias()=="SUCKLER")
        {
         ...
         lengthOfLactation = 225;
        	...
        	theMessage[id]->FatalError("cattleGroup::buyNewAnimal - ");
   ...
   if (tempcow->isPregnant() && !tempcow->GetFoetus())
   	theMessage[id]->FatalError("cattleGroup::buyNewAnimal - logic error as there is a pregnant animal without a foetus");
   if (logit)
   {
      animalProduct *anAnimalProduct = new animalProduct(*(animalProduct*) theProducts[id]->GetProductElement(Alias.c_str()));
      tempcow->loadAnimalProduct(anAnimalProduct);
      theProducts[id]->SubtractProduct(anAnimalProduct);
      delete anAnimalProduct;
      if (globalSettings[id]->DetailsData.getHerdDetails()==true)
      {
         char outstring[200];
         strcpy(outstring,GetAlias().c_str());
...
void cattleGroup::CullAnimal(cattle *theVictim, bool deleteAnimal)
{
   ...
   anAnimalProduct = new animalProduct(*(animalProduct*) theProducts[id]->GetProductElement(theVictim->GetAlias().c_str()));
...
//   anAnimalProduct->Setamount(anAnimalProduct->GetAmount()*anAnimalProduct->GetEndWeight());  //convert from number to mass in kg
   theProducts[id]->AddProduct(anAnimalProduct);
   delete anAnimalProduct;
   if (deleteAnimal)
	   delete theVictim;
}
