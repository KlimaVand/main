 ...
 */
 ...
 dairyStock::~dairyStock()
 {
	 ...
	 {
		 theMessage[id]->FatalError("dairyStock::pointy calves should be empty.");
	 }
	 ...
	 {
		 theMessage[id]->FatalError("dairyStock::oldies should be empty.");
	 }
	 delete oldies;
 ...
 //Calculates number of animals to be imported + herd structure
 {
	 ...
	 else
	 {
		 for(int i=0;i<grouplist->NumOfNodes();i++)
		 {
			 cattleGroup * agroup = grouplist->ElementAtNumber(i);
			 string name = agroup->GetAlias();
			 data.FindItem(name.c_str(),&numAnimals);
			 int numberToCull;
  ...
  */
 void dairyStock::InitCattle(commonData * &file, int anAnimalModel)
 {
 ...
 void dairyStock::Cull(linkList<cattle> * victims)
 {
	 ...
	 while (aNode)
	 {
		 ...
		 animalProduct *anAnimalProduct = new animalProduct(*(animalProduct*) theProducts[id]->GetProductElement("DAIRY"));
 ...
 void dairyStock::GiveIndicator(int indicatorType)
 {
	 if (indicatorType == environmentalIndicator)
	 {
		 double val=0.0;
		 val = GetHerdN();
		 theOutput[id]->AddStateIndicator(3360,"33.60 N bound in cattle","kg N",val);
		 val = GetHerdC();
		 theOutput[id]->AddStateIndicator(4360,"43.60 C bound in cattle","kg C",val);
		 for(int i=0;i<grouplist->NumOfNodes();i++)
		 {
