...
:animalStock(aName,aIndex,aOwner)
{
	...
	matingSwines->SetthisProduct((animalProduct*) theProducts[id]->GetProductElement("SOWS"));
	lactatingSwines = new swine("SOWS",aIndex,this);
	lactatingSwines->ReadParameters(data);
	lactatingSwines->SetthisProduct((animalProduct*) theProducts[id]->GetProductElement("SOWS"));
	pregnantSwines = new swine("SOWS",aIndex,this);
	pregnantSwines->ReadParameters(data);
	pregnantSwines->SetthisProduct((animalProduct*) theProducts[id]->GetProductElement("SOWS"));
	piglets= new slaughterSwine("PIGLETS",aIndex,this);
	piglets->ReadParameters(data);
	piglets->SetthisProduct((animalProduct*) theProducts[id]->GetProductElement("PIGLETS"));
	baconers = new slaughterSwine("BACONERS",aIndex,this);
	baconers->ReadParameters(data);
	baconers->SetthisProduct((animalProduct*) theProducts[id]->GetProductElement("BACONERS"));
	newSows = new slaughterSwine("NEWSOWS",aIndex,this);
	newSows->ReadParameters(data);
	newSows->SetthisProduct((animalProduct*) theProducts[id]->GetProductElement("NEWSOWS"));
	animalProduct *temp = new animalProduct(*(animalProduct*) theProducts[id]->GetProductElement("SUCKLINGPIG"));
	if (temp->GetEndWeight()!=piglets->GetthisProduct()->GetStartWeight())
		theMessage[id]->FatalError("swineStock::piglet start weight differs from weaner end weight");
	delete temp;
	if (piglets->GetthisProduct()->GetEndWeight()!=baconers->GetthisProduct()->GetStartWeight())
		theMessage[id]->FatalError("swineStock::baconer start weight differs from piglet end weight");
	if (baconers->GetthisProduct()->GetEndWeight()!=newSows->GetthisProduct()->GetStartWeight())
		theMessage[id]->FatalError("swineStock::new sow start weight differs from baconer end weight");
	delete data;
...
//Calculates number of animals to be imported + herd structure
{
   ...
   cout << inputString << endl;*/
	...
	double daysInYear = (double)theTime[id].daysInYear(1);
	...
	*weanersInOutPrDay = *theProducts[id]->GetProductElement("SUCKLINGPIG");
	weanersInOutPrDay->Setamount(0.0);
	*pigletsInOutPrDay = *piglets->GetthisProduct(); //*theProducts[id]->GetProductElement("PIGLETS");
	...
	*pigletsPrDay = *piglets->GetthisProduct(); //*theProducts[id]->GetProductElement("PIGLETS");
	pigletsPrDay->Setamount(numPiglets * pigletsPrDay->GetEndWeight()/daysInYear);
	*baconersPrDay = *baconers->GetthisProduct();//*theProducts[id]->GetProductElement("BACONERS");
	baconersPrDay->Setamount(numBaconers * baconersPrDay->GetEndWeight()/daysInYear);
	*newsowsPrDay = *newSows->GetthisProduct();//*theProducts[id]->GetProductElement("NEWSOWS");
	newsowsPrDay->Setamount(numNewSows * newsowsPrDay->GetEndWeight()/daysInYear);
	*castsowsPrDay = *matingSwines->GetthisProduct();//*theProducts[id]->GetProductElement("SOWS");
...
//Load feed and calculate growth N requirements
{
	...
	if (numSows>0)
	{
		...
		startNContent = ((animalProduct*) theProducts[id]->GetProductElement("NEWSOWS"))->GetN_content();
		startWeight =   ((animalProduct*) theProducts[id]->GetProductElement("NEWSOWS"))->GetEndWeight();
		endNContent = ((animalProduct*) theProducts[id]->GetProductElement("SOWS"))->GetN_content();
		endWeight =   ((animalProduct*) theProducts[id]->GetProductElement("SOWS"))->GetEndWeight();
		...
		startCContent = ((animalProduct*) theProducts[id]->GetProductElement("NEWSOWS"))->GetC_content();
		endCContent = ((animalProduct*) theProducts[id]->GetProductElement("SOWS"))->GetC_content();
		...
		double NContent = ((animalProduct*) theProducts[id]->GetProductElement("SUCKLINGPIG"))->GetN_content();   //suckler pig final N content
		double Weight =   ((animalProduct*) theProducts[id]->GetProductElement("SUCKLINGPIG"))->GetEndWeight();   //suckler pig final weight
		...
		endCContent = ((animalProduct*) theProducts[id]->GetProductElement("SUCKLINGPIG"))->GetC_content();
	...
	if (numPiglets>0)
	{
		...
		double pigletProductionCycle = ((pigstysection*) piglets->GetStableSection())->GetDaysInSection();
		startNContent = ((animalProduct*) theProducts[id]->GetProductElement("SUCKLINGPIG"))->GetN_content();
		...
		startCContent = ((animalProduct*) theProducts[id]->GetProductElement("SUCKLINGPIG"))->GetC_content();
	...
	if (strcmp(fileExtension,"lp3")!=0)
	{
		...
		double sell = theProducts[id]->GetActualSellPrice("SUCKLINGPIG");
		double buy  = theProducts[id]->GetActualBuyPrice("SUCKLINGPIG");
		livestockPrice += ((sell+buy)/2)*weanersInOutPrDay->GetAmount();
		sell = theProducts[id]->GetActualSellPrice("PIGLETS");
		buy  = theProducts[id]->GetActualBuyPrice("PIGLETS");
		livestockPrice += ((sell+buy)/2)*pigletsInOutPrDay->GetAmount();
		sell = theProducts[id]->GetActualSellPrice("BACONERS");
		buy  = theProducts[id]->GetActualBuyPrice("BACONERS");
		livestockPrice += ((sell+buy)/2)*baconersInOutPrDay->GetAmount();
		sell = theProducts[id]->GetActualSellPrice("NEWSOWS");
		buy  = theProducts[id]->GetActualBuyPrice("NEWSOWS");
		livestockPrice += ((sell+buy)/2)*newsowsInOutPrDay->GetAmount();
		sell = theProducts[id]->GetActualSellPrice("SOWS");
		buy  = theProducts[id]->GetActualBuyPrice("SOWS");
		livestockPrice += (sell+buy)/2*(numNewSows/theTime[id].daysInYear(1));
		theOutput[id]->AddIndicator(306,"03.06 Value of livestock","Dkr",livestockPrice);
	}
}
...
void swineStock::DailyUpdate(void)
{
	...
	theOutput[id]->AddStateIndicator(1130,"11.30 Sows present","no",NumberPrDay);
	NumberPrDay = baconers->GetNumberPrDay();
	theOutput[id]->AddStateIndicator(1131,"11.31 Baconers present","no",NumberPrDay);
	NumberPrDay = piglets->GetNumberPrDay();
	theOutput[id]->AddStateIndicator(1132,"11.32 Piglets present","no",NumberPrDay);
	NumberPrDay = newSows->GetNumberPrDay();
	theOutput[id]->AddStateIndicator(1133,"11.33 New sows present","no",NumberPrDay);
	...
	ExportStock();
	theOutput[id]->AddStateIndicator(3460,"34.60 N bound in pigs","kg N",0.0);
}
...
\****************************************************************************/
void swineStock::InitSwines(commonData * &file, buildings* theBuildings)
{
	for(int i=0;i<pointyPigs->NumOfNodes();i++)
...
void swineStock::CheckNbalance()
{
	...
	double totalImportN=aProduct->GetAmount() * aProduct->GetN_content() * theTime[id].daysInYear(1) * 1000.0; //Nbudget.GetInput()*1000.0;
	delete aProduct;
	//growth first
	for(int i=0;i<pointyPigs->NumOfNodes();i++)
	{
		...
		if (tempPig->GetNumberPrDay()>0.0)
			totalGrowthN+= tempPig->GetNumberPrDay() * tempPig->GetN_growth() * theTime[id].daysInYear(1);
	}
	// convert from tonnes to kg
	totalGrowthN*=1000.0;
	//export
	aProduct = GetStockSold();
	totalExportN=aProduct->GetAmount() * aProduct->GetN_content() * theTime[id].daysInYear(1) * 1000.0;    //in kg
	delete aProduct;
	double balance=totalExportN-(totalGrowthN+totalImportN);
	if (fabs(balance)>0.001)
		theMessage[id]->FatalError("Swinestock: N budget error - exported N and growth N are unequal");
	...
	for(int i=0;i<pointyPigs->NumOfNodes();i++)
	{
		...
		if (tempPig->GetNumberPrDay()>0.0)
		{
			...
			feedN = currentfeed->GetAmount() * currentfeed->GetAllN().n * theTime[id].daysInYear(1) * 1000.0;
			growthN = tempPig->GetNumberPrDay() * tempPig-> GetN_growth() * theTime[id].daysInYear(1) * 1000.0;
			tempPig->ProduceManure( fluidManure, solidManure);
			manureN = theTime[id].daysInYear(1) * (fluidManure->GetAmount() * fluidManure->GetAllN().n +
					solidManure->GetAmount() * solidManure->GetAllN().n) * 1000.0;
			balance = feedN-(growthN + manureN);
			if (fabs(balance)>1.0)
				theMessage[id]->FatalError("Swinestock: ", tempPig->GetName()," error - N in feed > (growth N + manure N)");
		}
	}
...
// Selling cast sows means that at the end of the year, all sows are sold
{
	/*   if (castsowsPrDay->GetAmount()>0.0)  //buy and sell sows
   {
	...
	if (newsowsInOutPrDay->GetAmount()<0.0)  //sell animals
	{
		...
		theProducts[id]->AddProduct(newsowsInOutPrDay);
		newsowsInOutPrDay->Setamount(newsowsInOutPrDay->GetAmount()*-1.0);
	}
	//   double Nout=0.0;
}
