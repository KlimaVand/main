...
void pig::DailyUpdate()
{
   ...
   for (int i=0; i<feedItemList->NumOfNodes(); i++)
   {
      ...
      feedItem * storedfeedItem =(feedItem *) theProducts[id]->GetCopyStoredFeedProduct(afeedItem);   //returns a copy of the product if it is found in storage
      if ((storedfeedItem)&&(storedfeedItem->GetAmount()>0.0))
      {
         storedfeedItem->Setamount(afeedItem->GetAmount());
         theProducts[id]->SubtractProduct(storedfeedItem);                     // subtract feed from storage
      ...
      else
      {
            ...
            theMessage[id]->FatalError("Pig:: ");
			}
         theProducts[id]->SubtractProduct(afeedItem);                     // subtract feed from storage
         *currentfeed = *currentfeed + *afeedItem;
      }
   }
   if (currentfeed->GetAmount()==0)
         theMessage[id]->FatalError("Pig:: No feed provided!");
...
//   while (surplusN<0) //iteration necessary as feed quality could vary with amount requested from store, if several stores contain the desired feed
   if (surplusN<0)
   {
      theMessage[id]->FatalError("Pig:: Insufficient protein in feed for ", GetName());
...
//   GetStableSection()->AddanimalHeatOp(NumberPrDay*heatProduction);
   ...
   theOutput[id]->AddIndicator(4464,"44.64 C in pig CH4 production","kg C",CinCH4*1000.0);
   double CinCO2 = CinFeed - (CinSoldPig + CinManure + CinCH4);
   if (CinCO2<0.0)
         {theMessage[id]->FatalError("pig::DailyUpdate - CO2 production cannot be negative");}
   theOutput[id]->AddIndicator(4463,"44.63 C in pig CO2 production","kg C",CinCO2*1000.0);
...
void pig::ReceivePlan(char* fileName)
{
   ...
   theMessage[id]->LogEvent((char*) logString.c_str());
   ...
   if (!gotit)
      theMessage[id]->FatalError("pig::ReceivePlan - cannot find pig group in pigfeed.f??");
   else
   {
   	feedItemList->Reset();
      ...
      for(int index=first;index<=(first+num);index++)
      {
         ...
         afeedItem=((product *) theProducts[id]->GetProductElement(feedCode));
         double feedAmount;
...
//          feedItemClone->Setamount(feedAmount/(theTime[id].daysInYear()*NumberPrDay)); //set amount to daily requirement per animal
...
// Fluid manure
      ...
      if (energyInput<0.75*(NumberPrDay*FE_need)) // BMP changed this january 2007
      {
         if (energyInput<0.50*(NumberPrDay*FE_need))
         {
            ...
            theMessage[id]->FatalError("pig::ProduceManure - ",Alias.c_str()," Very low amount of energy in feed");
         }
         else
            theMessage[id]->Warning("pig::ProduceManure - ",Alias.c_str()," Insufficient energy in feed");
      }

      if (energyInput>1.6*(NumberPrDay*FE_need))
      {
      	double energyPercent = 100.0*energyInput/(NumberPrDay*FE_need);
         theMessage[id]->WarningWithDisplay(Alias.c_str(),energyPercent,"% excess energy in feed ");
      }
      ...
      */
      if (surplus_digestibleN>=0)
      {
         ...
         if (Nconc<0.0)
            theMessage[id]->FatalError("pig::protein N digested greater than protein N present");
         solidManure->Setname("PIG-FAECES");
         theProducts[id]->GiveProductInformation(solidManure);          // sets the default values for solid-manure
//         solidManure->SetfromAnimal(animalName);
         ...
         theProducts[id]->GiveProductInformation(fluidManure);
 ...
 //        fluidManure->SetfromAnimal(animalName);
         ...
         if(fabs(balance)>0.0001)
            theMessage[id]->FatalError("pig::ProduceManure - error in N balance in pig manure partitioning");
      }
      else
         theMessage[id]->FatalError("pig::ProduceManure - ",animalName.c_str()," required additional protein in feed"); // BMP modified this !!!
      double Nefficiency =  NumberPrDay * N_growth/totalNInput;
      if (Nefficiency>0.7)
         theMessage[id]->FatalError("pig::ProduceManure - N efficiency over 70%");
   }
   else
   {                                                             // sets the default values for manure
      ...
      theProducts[id]->GiveProductInformation(solidManure);
...
void pig::EndBudget(bool show)
{
	if (show)
   {
	...
	if (NRemaining!=0)
      theMessage[id]->FatalError(GetName(),"pig::EndBudget - N budget error");
}
