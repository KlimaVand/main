	...
	: base(aName, aIndex, aOwner)
{
   ...
   solidManurePrDay = new manure("SOLID-MANURE",0,this);
	...
	theProducts[id]->GiveProductInformation(solidManurePrDay);
...
/*   if (anAnimal.thePartitioning)
	...
	theProducts[id]->GiveProductInformation(solidManurePrDay);
   vetExpensesPrDay = new product("VET",0,this);
	theProducts[id]->GiveProductInformation(vetExpensesPrDay);
 ...
 *vetExpensesPrDay * Number;
 theProducts[id]->SubtractProduct(vetExpensesPrDay);                 // cost of vetenarian
}
...
void animal::checkBalances(bool show)
{
	...
	theMessage[id]->FatalError(GetName()," animal: checkBalances - base class function should not be called");
}
