...
void dk_cattle::ReadParameters(commonData * &data)
{
	   ...
	   string namestr;
		...
		data->FindSection(namestr.c_str(),code);  // find the correct place in the input file
...
/*      if (weightChange!=0.0)
   ...
   {
      cout << theTime << endl;
   	theMessage[id]->FatalError("dk_cattle: urine N has gone negative!");
   }
		if (weight<0.0)
      theMessage[id]->FatalError(GetAlias().c_str()," dk_cattle: weight has fallen below zero");
}
...
double dk_cattle::DoPregnancy(double *avail_FE, double *availableN)
{
  ...
  // 	   theMessage[id]->FatalError("dk_cattle: insufficient protein available for pregnancy");
...
{
	if (pregnant)
   	   theMessage[id]->Warning("dk_cattle: animal is already pregnant");
   else
   {
...
void dk_cattle::DoLactation(double *avail_FE, double *availableN, double *milkN)
{
   *milkN = 0.0;
   ...
   milk = new feedItem(*(feedItem*)theProducts[id]->GetProductElement(308));   //deleted later in this routine (if no calf), otherwise deleted by calf
   ...
   *availableN-=lactationN;
   ...
   if (!calf)
   {
//         theProducts[id]->AddProduct(milk);
#ifndef _STANDALONE
      milk->SetaPrice(theProducts[id]->GetPricePtr("MILK"));
      double value = milk->GetaPrice()->GetactualSellPrice();
      theOutput[id]->AddIndicator(3320,"33.20 N in milk","kg N",Number * *milkN/1000.0);
      theOutput[id]->AddIndicator(555,"05.55 Milk sold","Dkr",Number * milk->GetAmount() * 1000.0 * value);    //proce is in litrs
      theOutput[id]->AddIndicator(1940,"19.40 Milk sold","litres",Number * milk->GetAmount() * 1000.0);
#endif
   }
...
//return size;
	 ...
	 if(MBW==0)
		 ...
		 k = 0.47 / pow(MBW, 0.27);
	 ret_val =  MBW - (MBW - birth_w) * exp(-k *  age * 12/theTime[id].daysInYear(0));
    return ret_val;
};
...
//   for (int counter=0;counter<=Hi;counter++)           taken out until BPE sorts out Comsort
   ...
   while ((counter>=0) && (!animalIsFull))
   {
      ...
      if (!animalIsFull)
      {
         ...
         if (aFeed_resource->GetIsSupplement())
         {
            ...
            if (DM_intake_this_resource>0.0)
            {
               *aFeed_resource * GetNumber();     //adjust for animal number prior to output of demand
               theProducts[id]->SubtractProduct(aFeed_resource);                     // subtract feed from storage
         ...
         {
            if (hoursHoused==24.0)
               {theMessage[id]->WarningWithDisplay("dk_cattle: intake - grazed feed in diet but animal housed all day");}
...
//                                                aFeed_resource->Getarea_occupied();
            ...
            // calc possible intake from this resource
               ...
               if (aFeed_resource->GetAmount()>1000.0)
                  theMessage[id]->FatalError("dk_cattle: error in feed resource diet");
	            *aFeed_resource * GetNumber();     //adjust for animal number prior to output of demand
               ...
               ((patch *) aFeed_resource ->GetOwner())->SetGrazed(0.0,GetNumber() * DM_intake_this_resource, NGrazed);
               theOutput[id]->AddIndicator(3304,"33.04 N grazed by cattle","kg N",NGrazed);
               theOutput[id]->AddIndicator(4304,"43.04 C grazed by cattle","kg C",CGrazed);
	            *aFeed_resource * (1.0/GetNumber());     //adjust back
   	...
   	theMessage[id]->FatalError("dk_cattle: om");
#ifndef _STANDALONE
...
/*if (age>1063)
            cout << endl;*/
   if (FE_intake>1000.0)
   	theMessage[id]->FatalError("dk_cattle: error in current diet - too much energy");
   if (FE_intake<0.00001)
   {
   	cout << endl << theTime << endl;
   	theMessage[id]->FatalError("dk_cattle: error in current diet - too little energy for ",GetAlias().c_str());
   }
   ...
   {
      theOutput[id]->AddIndicator(3301,"33.01 N in feed for young cattle","kg N",totalNinput);
      theOutput[id]->AddIndicator(4301,"43.01 C in feed for young cattle","kg C",totalCinput);
   }
   else
   {
   	theOutput[id]->AddIndicator(3302,"33.02 N in feed for adult cattle","kg N",totalNinput);
   	theOutput[id]->AddIndicator(4302,"43.02 C in feed for adult cattle","kg C",totalCinput);
   }
#endif
...
//      feedPlanList=NULL;  //otherwise, if calf is culled, feedPlanList will be deleted and mother will have no diet
   ...
   if ((feedNPerc<1.0)&&(!isSuckling))
   	theMessage[id]->WarningWithDisplay("dk_cattle: N in diet <1.0%");
      ...
      else
      {
         ...
         else
         {
          ...
          if (!SolveQuadratic(false, quadraticTerm,linearTerm,constantTerm,&amountNewFeed))
            theMessage[id]->FatalError("NixCbits - SolveQuadratic: solution cannot be found");
...
{
	if (isFoetus)
   	theMessage[id]->FatalError("cattle:GrowFoetus - this animal is a foetus!");
   else
   {
