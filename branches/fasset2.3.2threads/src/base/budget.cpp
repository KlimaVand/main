...
void budget::SetInput (double aInput)
{
	...
	input = aInput;
   if (aInput<0)
   	theMessage[id]->FatalError("budget::SetInput - Input gone negative");
}
...
void budget::SetOutput (double aOutput)
{
	...
	output = aOutput;
   if (output<0)
   	theMessage[id]->FatalError("budget::SetInput - Output gone negative");
}
...
bool budget::Balance(double remain)
{
	...
	if ((fabs(diff)>maxRelError) && (fabs(diff-lastError)>maxRelError))
   {
      ...
      if (theMessage)
	      theMessage[id]->FatalError(outputString.c_str());
      else
