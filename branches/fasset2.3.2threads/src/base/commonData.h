...
public:
  commonData();
  commonData(const commonData& p);
  ~commonData();
  void print();
  void setOutputWarning(bool aOutputWarning){ outputWarning=aOutputWarning;}
  void setCritical(bool aCritical);
  int findSectionThatContains(string common,string item, string section);
  bool FindSection(string aSectionName, int sectionNumber=-1);
