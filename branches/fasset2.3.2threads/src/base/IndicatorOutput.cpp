...
//--------------Static data member initialization-----------------------------
 fstream* IndicatorOutput::fileStreamCSV[100] ={ NULL};
 static int indicatIndex[100] ={ -1};
 int IndicatorOutput::yearNumber[100] = { 0};
//----------------------------------------------------------------------------
   ...
   if(fileStreamCSV[id])
	   {
	   	   ...
	   	   fileStreamCSV[id]->close();

	   	   delete fileStreamCSV[id];
	   }
   	for (int j=environmentalIndicatorStart; j<SubIndex;j++)
   		if (indicatorList[j])
...
IndicatorOutput::IndicatorOutput(const IndicatorOutput& o)
{
	cout<<"here"<<endl;
}
/****************************************************************************\
...
double IndicatorOutput::GetAmount(int index, string aName)
{
   ...
   indicatIndex[id]=index;

   if ( indicatorList[index]!=NULL)
      return indicatorList[index]->GetAmount();
   else
   {
      theMessage[id]->WarningWithDisplay("output::GetAmount - '",aName,"' not found.");
      return 0.0;
   }
...
void IndicatorOutput::AddStateIndicator(int index, const char * aName, const char * aUnit, double aAmount)
{
   ...
   else
   {
      ...
      if (!makingList)
      {
      	theMessage[id]->WarningWithDisplay("Indicator - ",aName);
         theMessage[id]->WarningWithDisplay("IndicatorOutput::AddIndicator - no new indicators should be added at this point");
      }
...
void IndicatorOutput::Initialize(char * filenameCSV)
{
   InitializeIndicators();
	fileStreamCSV[id] = new fstream;

 	fileStreamCSV[id]->open(filenameCSV,ios::out);
}
...
void IndicatorOutput::OutCSV(item * aItem)
{
  ...
  aItem->Out(*fileStreamCSV[id],*aItem,yearNumber[id],"\t","");          // tabulated to ease handling by excel
}
...
int IndicatorOutput::FindString(item * aItem)
{
   ...
   int id =threadID->getID(std::this_thread::get_id());
   return (aItem->GetNameInt()==indicatIndex[id]);
}
...
void IndicatorOutput::AddIndicator(int index,const char * aName, const char * aUnit, double aAmount)
{
   ...
   else
   {
   	if (makingList==true)
      {
			...
			theMessage[id]->FatalError("indicat::AddIndicator - attempting to add an indicator when MakeList is false");
   }
...
void IndicatorOutput::FlushIndicator(int indicatorType, int aYearNumber)
{
   ...
   for (int j=start; j<stop;j++)
   {
	   if(indicatorList[j]!=NULL)
		   {
			   	  indicatorList[j]->Out(*fileStreamCSV[id],* indicatorList[j],yearNumber[id],"\t","");;
...
//      double balanceW=storeW-oldstoreW+lossW-gainW-lastErrorW;
      ...
      if (fabs(balanceW)>1)
      {
         theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in water balance",balanceW);
...
//	double minSoilN = GetAmount("32.61 Mineral N in soil");
 ...
 //  double orgSoilN = GetAmount("32.60 Organic N in soil");
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in soil N balance",balanceSoilN);

      lastErrorSoilN=balanceSoilN+lastErrorSoilN; // Avoid getting the "same" error more than once
   }
   if (fabs(balanceCropN)>maxErrorN)
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in crop N balance",balanceCropN);
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in livestock N balance",balanceLivestockN);
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in housing and storage N balance",balanceHouseEtcN);
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in stored product N balance",balanceProductStore);
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in total N balance",balanceN);
...
//   double balanceC=storeC-oldstoreC+lossC-gainC-lastErrorC;
   ...
   if (areaAdjustmentN<1E-20 && areaAdjustmentN>-1E-20) // Add code for this later !!!
   {
      ...
      {
         theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in soil C balance",balanceSoilC);
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in crop C balance",balanceCropC);
      error=true;
   ...
   */
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in livestock C balance",balanceLivestockC);
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in housing and storage C balance",balanceHouseEtcC);
   ...
   {
      theMessage[id]->WarningWithDisplay("IndicatorOutput::CheckBalance - error in stored product C balance",balanceProductStore);
