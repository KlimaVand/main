...
void bsTime::SetTime(int d, int m, int y)
{
   ...
   if ((y<0) || (y>2300))
	...
	{
      theMessage[id]->FatalError("Time::Illegal year");
      char ch=getch();
      if (ch=='y' || ch=='Y' )
         exit(99);
   }
...
void bsTime::SetTime(string day,string month, string year)
{
   ...
   if (year=="R1")                                                                             // facilitates relative years (usefull in field plans)
   	SetTime(atoi((char*) day.c_str()), atoi((char*) month.c_str()), theTime[id].GetYear()+1);
   else
   	if (year=="" || year=="R0")
      	SetTime(atoi((char*) day.c_str()), atoi((char*) month.c_str()), theTime[id].GetYear());
  		else
...
void bsTime::SetTime(string aTimeString)
{
   ...
   else
   {
	  int id =threadID->getID(std::this_thread::get_id());
   	...
   	SetTime(1, 8, theTime[id].GetYear()); // Set to default date
   }
}
