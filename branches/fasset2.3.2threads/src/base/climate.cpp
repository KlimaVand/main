...
//			f1 >> date >> tmin >> tmax >> tmean >> precip >> rad  >> humidity >> vel;
   ...
   if (rad<0.0)
   {
   	theMessage[id]->WarningWithDisplay("climate::Readline - incoming radiation cannot be negative");
      rad=0.0;
   }
   if (precip<0.0)
   {
   	theMessage[id]->WarningWithDisplay("climate::Readline - precipitation is negative");
      precip=0.0;
   }
   if ((d==1)&&(mnth==1))
      ResetdegreeDays();
...
void climate::UpdateMeasuredSoilTemperature()
{
   ...
   while (TemperatureFile && MeasureDate<theTime[id])
   {
...
void climate::InitClimate(char * filename)
{
   ...
   if (TemperatureFile)
   	TemperatureFile.close();
#ifndef _STANDALONE
   if (theControlParameters[id]->GetMeasuredSoilTemperature())
...
double climate::GetCO2Concentration()
{
   if (CO2Concentration > 2000 || CO2Concentration < 100)
      theMessage[id]->FatalError("climate::GetCO2Concentration - CO2 concentration beyond reasonable borders");
...
{
   int DayNo=theTime[id].GetDayInYear();
	if (((theTime[id].GetYear() % 4)==0) && (DayNo>59))
      DayNo--;
	return 2*3.1415926*(DayNo-1)/theTime[id].daysInYear(0);
}
						 ...
						 double &EPot, double &windspeed, double &relHumidity)
{
  ...
  else
  {
   ...
   relHumidity = 0.0;
   windspeed = 0.0;
   theMessage[id]->WarningWithDisplay("climate::GetClimate - today's weather has not been found yet");
   cout << "";
  }
}
...
void climate::Update()
{
   ...
   if (theTime>LastDate)
   {
   	if (!Circular) theMessage[id]->WarningWithDisplay("Climate::Current date greather than last date - using circular dates!");
      Circular = true;
   ...
   if (!found)
   	theMessage[id]->Warning("Climate::DailyUpdate - date not in climate-database: ");

#ifndef _STANDALONE
   if (theControlParameters[id]->GetMeasuredSoilTemperature())
   	UpdateMeasuredSoilTemperature();
#endif
   if ((theTime[id].GetDay()==1)&&(theTime[id].GetMonth()==1))
   	degreeDays=0.0;
   if (tmean>0.0)
	   degreeDays+=tmean;
}
...
void climate::Update()
{
   ...
   if (theTime[id]<FirstDate)
   	year = FirstDate.GetYear()+1;
   if (theTime[id]>LastDate)
   {
   	if (!Circular) theMessage[id]->WarningWithDisplay("Climate::Current date greather than last date - using circular dates!");
      Circular = true;
   	year = FirstDate.GetYear() + ((year-FirstDate.GetYear()) % (LastDate.GetYear()-FirstDate.GetYear()));                   // ensures cirkular referencing if you run
   ...
   if (co2>100)
   {
      if (co2<2000)
         ...
         theMessage[id]->FatalError("climate::Update - CO2 concentration at an unrealistic high level");
   }
   ...
   if (!found && Circular && month==2 && day>=29)
   	found = true;                 // a fix to ensure correct circular referencing
   if (!found)
   	theMessage[id]->WarningWithDisplay("Climate::DailyUpdate - date not in climate-database: ");
...
void climate::CalcAverageClimate()
{
   ...
   if (numYears==0)
	   theMessage[id]->FatalError("Climate::Less than one year of data in climate database");
   if (numYears<5)
   	theMessage[id]->Warning("Climate::average climate based on <5 years of data");
   while (*theDate<=*endDate)
   {
      ...
      {
         theClimate[id]->Update(theDate);
         sumTemp+=theClimate[id]->tmean;
         sumRad+=theClimate[id]->rad;
         theDate->AddOneDay();
         count++;
      }
  ...
  theClimate[id]->Update();  //reset climate to current date
  hd.changeDir(buffer);   //revert to previous directory
}
...
void climate::CalcAverageClimateForPeriod(bsTime *startDate, int numOfDays, double *aveTemperature, double *aveWindspeed)
{
   ...
   while (*theDate<*endDate)
   {
         theClimate[id]->Update(theDate);
         sumTemp+=theClimate[id]->tmean;
         sumWind+=theClimate[id]->windVelocity;
         theDate->AddOneDay();
         count++;
	}
