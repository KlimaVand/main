...
{
   if (numOfPlants==0)
      theMessage[id]->FatalError("sowFields::AddSeed - function can not be called yet");
   if(numOfPlants>MaxPlants)
	   theMessage[id]->FatalError("sowFields::AddSeed - To many plants");
...
{
   if (numOfPlants>0)
      theMessage[id]->FatalError("sowFields::DefineSowFieldsOp - no plants should be added at this stage");
   if(numOfPlants>MaxPlants)
  	   theMessage[id]->FatalError("sowFields::AddSeed - To many plants");
                            ...
                            bool                  update)
{
   ...
   {
      theMessage[id]->WarningWithDisplay("sowFields::CalcCost - Operation ",operationNameList[op_id].c_str()," not found in list");
      return false;
   }
   else
      if(area<=0.0)
      {
         theMessage[id]->WarningWithDisplay("sowFields::CalcCost - area is less than zero");
         return false;
      }
      else
      {
         ...
         if (sowTechInstance->CreateGang(aTechFarmList))
         {
            // All neccesary machines present at farm
	         ...
	         if(update) {
               ...
               theOutput[id]->AddIndicator(708,"07.08 Cost of machine contractors","Dkr",-sowTechInstance->GetMachinePoolPrice()*area);
				   delete hiredService;
            }
