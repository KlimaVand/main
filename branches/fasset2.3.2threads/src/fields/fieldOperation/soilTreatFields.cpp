                                  ...
                                  bool                  update)
{
   ...
   {
      theMessage[id]->WarningWithDisplay("soilTreatFields::CalcCost - Operation ",operationNameList[op_id].c_str()," not found in list");
      return false;
   }
   else
      if(area<=0.0)
      {
         theMessage[id]->WarningWithDisplay("soilTreatFields::CalcCost - area is less than zero");
         return false;
      }
      else
      {
         ...
         if (soilTreatTechInstance->CreateGang(aTechFarmList))
         {
	         ...
	         if(update)
            {
               ...
               theOutput[id]->AddIndicator(708,"07.08 Cost of machine contractors","Dkr",-soilTreatTechInstance->GetMachinePoolPrice()*area);
				   delete hiredService;
            }
