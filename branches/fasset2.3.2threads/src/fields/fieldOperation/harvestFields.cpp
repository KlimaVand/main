...
{
   if (i>=MaxPlants || i<0)
      theMessage[id]->FatalError("harvestFields::SetMainProduct - function called with illegal index");
   if (aMainProduct==NULL)
      theMessage[id]->FatalError("harvestFields::SetMainProduct - function called with NULL");
   if (MainProduct[i])
   	delete MainProduct[i];
...
{
   if (i>=MaxPlants || i<0)
      theMessage[id]->FatalError("harvestFields::AddMainProduct - function called with illegal index");
   if (aMainProduct==NULL)
      theMessage[id]->FatalError("harvestFields::AddMainProduct - function called with NULL");
   if (MainProduct[i]==NULL)
      theMessage[id]->FatalError("harvestFields::AddMainProduct - can not add product to NULL product pointer");
...
{
   if (i>=MaxPlants || i<0)
      theMessage[id]->FatalError("harvestFields::SetStrawProduct - function called with illegal index");
   if (aStrawProduct==NULL)
      theMessage[id]->FatalError("harvestFields::SetStrawProduct - function called with NULL");
	if (StrawProduct[i])
   	delete StrawProduct[i];
...
{
   if (i>=MaxPlants || i<0)
      theMessage[id]->FatalError("harvestFields::AddStrawProduct - function called with illegal index");
   if (aStrawProduct==NULL)
      theMessage[id]->FatalError("harvestFields::AddStrawProduct - function called with NULL");
   if (StrawProduct[i]==NULL)
      theMessage[id]->FatalError("harvestFields::AddStrawProduct - can not add product to NULL product pointer");
...
{
   if (i>=MaxPlants || i<0)
      theMessage[id]->FatalError("harvestFields::SetForageProduct - function called with illegal index");
   if (aForageProduct==NULL)
      theMessage[id]->FatalError("harvestFields::SetForageProduct - function called with NULL");
	if (ForageProduct[i])
   	delete ForageProduct[i];
                                ...
                                bool                  update)
{
   ...
   {
      theMessage[id]->FatalError("harvestFields::CalcCost - Operation ",operationNameList[op_id].c_str()," not found in list");
      return false;
   }
   else
      if(area<=0.0)
      {
         theMessage[id]->FatalError("harvestFields::CalcCost - area is less than zero");
         return false;
      }
      else
      {
         ...
         if (harvestTechInstance->CreateGang(aTechFarmList))
         {
	         ...
	         if(update)
            {
               product * hiredService = new product;
					...
					theProducts[id]->GiveProductInformation(hiredService);
					hiredService->Setamount(harvestTechInstance->GetMachinePoolPrice()*area);
               // theProducts[id]->AddProduct(hiredService); Removed BMP 12.10.2000
		         harvestTechInstance->UpdateProducts(true);

               theOutput[id]->AddIndicator(708,"07.08 Cost of machine contractors","Dkr",-harvestTechInstance->GetMachinePoolPrice()*area);
				   delete hiredService;
            }
	                  }
         ...
         //NJH, feb07
         ofstream * file=theMessage[id]->GiveHandle();
		   Output(file);
