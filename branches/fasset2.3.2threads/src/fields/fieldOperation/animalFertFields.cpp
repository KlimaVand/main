                                   ...
                                   bool                  update)
{
   ...
   {
      theMessage[id]->WarningWithDisplay("animalFertFields::CalcCost - Operation ",operationNameList[op_id].c_str()," not found in list");
      return false;
   }
   else
      if(area<=0.0)
      {
         theMessage[id]->WarningWithDisplay("animalFertFields::CalcCost - area is less than zero");
         return false;
      }
      else
      {
         ...
         if (animalFertTechInstance->CreateGang(aTechFarmList))
         {
	         ...
	         if(update)
            {
               ...
               // theProducts[id]->SubtractProduct(hiredService); Removed BMP 12.10.2000
		         animalFertTechInstance->UpdateProducts(true);
               theOutput[id]->AddIndicator(708,"07.08 Cost of machine contractors","Dkr",-animalFertTechInstance->GetMachinePoolPrice()*aManure->GetAmount());
				   delete hiredService;
            }
	                 }
...
fieldOperationTech* animalFertFields::GetfieldOperationTech(linkList<operation>    * aOperationList)
{
   ...
   if(aOperationNode==NULL)
   {
      theMessage[id]->FatalError("animalFertFields::GetfieldOperationTech - Operation ",operationNameList[op_id].c_str()," not found in list");
      return NULL;
   }
   else
   {
