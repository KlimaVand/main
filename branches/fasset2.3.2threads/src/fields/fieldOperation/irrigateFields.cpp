...
// Calculate cost of operation
   ...
   {
      theMessage[id]->WarningWithDisplay("irrigateFields::CalcCost - Operation ",operationNameList[op_id].c_str()," not found in list");
      return false;
   }
   else
      if(area<=0.0)
      {
         theMessage[id]->WarningWithDisplay("irrigateFields::CalcCost - area is less than zero");
         return false;
      }
      else
      {
