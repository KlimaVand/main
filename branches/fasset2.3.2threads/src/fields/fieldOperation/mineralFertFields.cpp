...
// Calculate cost of operation
   ...
   {
      theMessage[id]->WarningWithDisplay("mineralFertFields::CalcCost - Operation ",operationNameList[op_id].c_str()," not found in list");
      return false;
   }
   else
      if(area<=0.0)
      {
         theMessage[id]->WarningWithDisplay("mineralFertFields::CalcCost - area is less than zero");
         return false;
      }
      else
      {
         ...
         if (mineralFertTechInstance->CreateGang(aTechFarmList))
         {
            // All neccesary machines present at farm
	         ...
	         if(update) {
               ...
               theOutput[id]->AddIndicator(708,"07.08 Cost of machine contractors","Dkr",-mineralFertTechInstance->GetMachinePoolPrice()*area);
				   delete hiredService;
            }
         }
