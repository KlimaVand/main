...
{
	if ((opEnd)||(opStart))
   	theMessage[id]->FatalError("fieldOperationFields::DefineOp - date for this operation has already been defined");
   else
   {
		opStart = new bsTime();
		opEnd = new bsTime();
   ...
   if (*opStart > *opEnd)
   {
   	theMessage[id]->WarningWithDisplay("Field no: ", fieldno);
   	theMessage[id]->WarningWithDisplay("Crop: ", crop);
      theMessage[id]->WarningWithDisplay("fieldOperationFields::DefineOp - field no and op name ",
							GetFieldNumber(),operationNameList[GetOperationId()].c_str());
   	theMessage[id]->FatalError("fieldOperationFields::DefineOp - start date for operation is after end date");
   }
   TSum=aTSum;
