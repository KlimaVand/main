...
{
	int run=globalSettings[id]->getCurrentRun();
	if(scenarielogNumber!=run)
	{
		string currentDir=scenarielog.getCurrentPath();

		scenarielog.changeDir(globalSettings[id]->getOutputDirectory());
		waterDetails.changeDir(globalSettings[id]->getOutputDirectory());
                               ...
                               double * greenEvap)
{
   ...
   if (ePot>0)
   {
      ...
      theOutput[id]->AddIndicator(7012,"70.12 Evapotranspiration","cubic metre",snowEvap*area*10);
      ...
      *ePotGreenLeaves = *ePotGreenLeaves*(1.0-snowEvap/ePot);
      ...
      if (evap>0)
      {
         ...
         *ePotGreenLeaves-=*greenEvap;
         ...
         theOutput[id]->AddIndicator(7012 ,"70.12 Evapotranspiration","cubic metre",evap*area*10);
      }
   }
   if (globalSettings[id]->DetailsData.getWaterDetails()==true)
   {
...
void ecosystem::TransferRootMatter(decomposable * DeadRoot,double * RootLengthList)
{
   ...
   theOutput[id]->AddIndicator(3220,"32.20 N from roots incorporated into soil","kg N",(DeadRoot->GetorgN_content().n+DeadRoot->GetNO3_content().n+DeadRoot->GetNH4_content().n)*DeadRoot->GetAmount()*area*1000.0);
	theOutput[id]->AddIndicator(4080,"40.80 C from roots incorporated into soil","kg C",DeadRoot->GetAmount()*DeadRoot->GetC_content()*area*1000.0);
...
void ecosystem::CheckNfromSoil(nitrogen &NFromSoil,double NitrogenDemand)
{
   ...
   {
      theMessage[id]->WarningWithDisplay("ecosystem::CheckNfromSoil - nitrogen from soil should not exceed request");
      NFromSoil.n=NitrogenDemand;
   }
   if (NFromSoil.n<0)
   {
    	if (NFromSoil.n<-0.000000001)
      	theMessage[id]->WarningWithDisplay("ecosystem::CheckNfromSoil - nitrogen from soil can not be less than zero");
	   NFromSoil.n=0.0;
   }
}
...
void ecosystem::CheckTranspirationRatio(double &TranspirationRatio)
{
   if (TranspirationRatio<0)
   {
      if (TranspirationRatio<-1E-10)
         theMessage[id]->WarningWithDisplay("ecosystem::CheckTranspirationRatio - transpirationratio can not be negative");
      TranspirationRatio=0.0;
   }
   if (TranspirationRatio>1.0)
   {
      if (TranspirationRatio>(1+1E-10))
         theMessage[id]->WarningWithDisplay("ecosystem::CheckTranspirationRatio - transpirationratio can not be greater than one");
...
{
   double precip = theClimate[id]->precip;
   double EPot = theClimate[id]->epot;
   double temp = theClimate[id]->tmean;
   double radiation = theClimate[id]->rad;
   ...
   double NitrogenInRain = theClimate[id]->GetNitrogenInRain();
   ...
   theOutput[id]->AddIndicator(7001,"70.01 Rainwater","cubic metre",precip*area*10.0);
   theOutput[id]->AddIndicator(3102,"31.02 N from deposition","kg N",NitrogenInRain*precip*area*10.0);
   theOutput[id]->AddIndicator(3105,"31.05 N from irrigation water","kg N",irrigation*irrigationN*area*10.0);

   if (globalSettings[id]->DetailsData.getWaterDetails()==true)
   {

	   waterDetails.WriteWithTabS(theTime[id].GetString2());
   ...
   if (theTime[id].GetMonth()==6 && theTime[id].GetDay()==15 && (totalgLAI+totalyLAI)<1E-10)
   {
      cout << endl << " Object path : " << GetLongName();
      theMessage[id]->FatalError("ecosystem::CropSoilExchange - no leaf area present 15th June");
   }
#endif
   ...
   for (int i=0;i<roots->NumOfNodes();i++)
   {
      ...
      NFromSoil.n15=0.0;
   	if (NitrogenUptake)
	   {
      	...
      	theOutput[id]->AddIndicator(3210,"32.10 N uptake from soil","kg N",NFromSoil.n*area*10);
      }
   ...
   for (int j=0;j<theCrops->NumOfNodes();j++)
   {
      ...
      if (topMatter)
      {
         aSoil->AddDecomposable(topMatter);
   	   theOutput[id]->AddIndicator(3222,"32.22 N from leaf litterfall incorporated into soil","kg N",topMatter->GetAllN().n*topMatter->GetAmount()*area*1000.0);
		   theOutput[id]->AddIndicator(4082,"40.82 C from leaf litterfall incorporated into soil","kg C",topMatter->GetAmount()*topMatter->GetC_content()*area*1000.0);
      }
      theOutput[id]->AddIndicator(3104,"31.04 N from fixation","kg N",aCrop->GetNFixationThisDay()*area*10.0);
      nFixThisDay+=aCrop->GetNFixationThisDay();
   }
   theOutput[id]->AddIndicator(4001,"40.01 Net crop carbon fixation","kg C",TotalCFixation*area*10);
...
void ecosystem::AddHarvestedAreaToIndicators(const char * cid, double a)
{
   if (strcmp(cid,"W1")==0 || strcmp(cid,"W2")==0)
      theOutput[id]->AddIndicator(1810,"18.10 Exp. W.wheat yield (harv. bef. 1/8)","ha",a);
	if (strcmp(cid,"W5")==0 || strcmp(cid,"W6")==0)
      theOutput[id]->AddIndicator(1811,"18.11 Exp. S.wheat yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"RW")==0)
      theOutput[id]->AddIndicator(1812,"18.12 Exp. W.rape yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"RS")==0)
      theOutput[id]->AddIndicator(1813,"18.13 Exp. S.rape yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"B5")==0 || strcmp(cid,"B6")==0)
      theOutput[id]->AddIndicator(1814,"18.14 Exp. W.barley yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"B1")==0 || strcmp(cid,"B2")==0)
      theOutput[id]->AddIndicator(1815,"18.15 Exp. S.barley yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"RY")==0)
      theOutput[id]->AddIndicator(1816,"18.16 Exp. Rye yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"OA")==0)
      theOutput[id]->AddIndicator(1817,"18.17 Exp. Oat yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"PE")==0)
      theOutput[id]->AddIndicator(1818,"18.18 Exp. Pea yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"BP")==0)
		theOutput[id]->AddIndicator(1826,"18.26 Barley/Pea area (harv. bef. 1/9)","ha",a);
   if (strcmp(cid,"PO")==0)
      theOutput[id]->AddIndicator(1819,"18.19 Exp. Potato yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"SG")==0)
      theOutput[id]->AddIndicator(1820,"18.20 Exp. Grass for seed yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"MA")==0)
      theOutput[id]->AddIndicator(1821,"18.21 Exp. Maize yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"BP")==0)
      theOutput[id]->AddIndicator(1823,"18.23 Exp. Barley/Pea yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"C1")==0 || strcmp(cid,"C2")==0)
      theOutput[id]->AddIndicator(1824,"18.24 Exp. Grass/clover yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"BE")==0)
      theOutput[id]->AddIndicator(1822,"18.22 Exp. Beet yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"SB")==0)
      theOutput[id]->AddIndicator(1821,"18.21 Exp. Soy bean yield (harv. bef. 1/8)","ha",a);
   if (strcmp(cid,"SF")==0)
      theOutput[id]->AddIndicator(1823,"18.23 Exp. Sunflower yield (harv. bef. 1/8)","ha",a);
}
/**
...
void ecosystem::GiveIndicators()
{
   if (balanceDepth<1E-10)
      theMessage[id]->FatalError("ecosystem::GiveIndicators - soil depth not initialised");

   theOutput[id]->AddStateIndicator(4060,"40.60 Total C in soil","kg C",aSoil->GetTotalCarbon()*area*10);  //GetPoolCarbon(const char * Name, double startDep, double thick)

	theOutput[id]->AddStateIndicator(4062,"40.62 Total C in soil 0-25 cm","kg C",aSoil->GetTotalCarbon(0,250)*area*10);
	theOutput[id]->AddStateIndicator(4063,"40.63 Total C in soil 25-50 cm","kg C",aSoil->GetTotalCarbon(250,250)*area*10);
	theOutput[id]->AddStateIndicator(4064,"40.64 Total C in soil 50-75 cm","kg C",aSoil->GetTotalCarbon(500,250)*area*10);
	theOutput[id]->AddStateIndicator(4065,"40.65 Total C in soil 75-250 cm","kg C",aSoil->GetTotalCarbon(750,1750)*area*10);
	theOutput[id]->AddStateIndicator(4066,"40.66 C in AOM+SMB 0-25 cm","kg C",
		...
		theOutput[id]->AddStateIndicator(4067,"40.67 C in AOM+SMB 25-50 cm","kg C",
		...
		theOutput[id]->AddStateIndicator(4068,"40.68 C in AOM+SMB 50-75 cm","kg C",
		...
		theOutput[id]->AddStateIndicator(4069,"40.69 C in AOM+SMB 75-250 cm","kg C",
		...
		theOutput[id]->AddStateIndicator(4070,"40.70 C in NOM 0-25 cm","kg C",aSoil->GetPoolCarbon(5,0,250)*area*10);
		theOutput[id]->AddStateIndicator(4071,"40.71 C in NOM 25-50 cm","kg C",aSoil->GetPoolCarbon(5,250,250)*area*10);
		theOutput[id]->AddStateIndicator(4072,"40.72 C in NOM 50-75 cm","kg C",aSoil->GetPoolCarbon(5,500,250)*area*10);
		theOutput[id]->AddStateIndicator(4073,"40.73 C in NOM 75-250 cm","kg C",aSoil->GetPoolCarbon(5,750,1750)*area*10);
		theOutput[id]->AddStateIndicator(4074,"40.74 C in IOM 0-25 cm","kg C",aSoil->GetPoolCarbon(6,0,250)*area*10);
		theOutput[id]->AddStateIndicator(4075,"40.75 C in IOM 25-50 cm","kg C",aSoil->GetPoolCarbon(6,250,250)*area*10);
		theOutput[id]->AddStateIndicator(4076,"40.76 C in IOM 50-75 cm","kg C",aSoil->GetPoolCarbon(6,500,250)*area*10);
		theOutput[id]->AddStateIndicator(4077,"40.77 C in IOM 75-250 cm","kg C",aSoil->GetPoolCarbon(6,750,1750)*area*10);
   theOutput[id]->AddStateIndicator(4090,"40.90 AOM1 C in soil","kg C",aSoil->GetPoolCarbon(0,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(4091,"40.91 AOM2 C in soil","kg C",aSoil->GetPoolCarbon(1,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(4092,"40.92 SMB1 C in soil","kg C",aSoil->GetPoolCarbon(2,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(4093,"40.93 SMB2 C in soil","kg C",aSoil->GetPoolCarbon(3,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(4094,"40.94 SMR C in soil","kg C",aSoil->GetPoolCarbon(4,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(4095,"40.95 NOM C in soil","kg C",aSoil->GetPoolCarbon(5,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(4096,"40.96 IOM C in soil","kg C",aSoil->GetPoolCarbon(6,0,balanceDepth)*area*10);

   theOutput[id]->AddIndicator(4040,"40.40 CO2 soil respiration","kg C",aSoil->GetCO2Evolution()*area*10);

   theOutput[id]->AddStateIndicator(3262,"32.62 AOM1 N in soil","kg N",aSoil->GetPoolNitrogenN(0,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(3263,"32.63 AOM2 N in soil","kg N",aSoil->GetPoolNitrogenN(1,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(3264,"32.64 SMB1 N in soil","kg N",aSoil->GetPoolNitrogenN(2,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(3265,"32.65 SMB2 N in soil","kg N",aSoil->GetPoolNitrogenN(3,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(3266,"32.66 SMR N in soil","kg N",aSoil->GetPoolNitrogenN(4,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(3267,"32.67 NOM N in soil","kg N",aSoil->GetPoolNitrogenN(5,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(3268,"32.68 IOM N in soil","kg N",aSoil->GetPoolNitrogenN(6,0,balanceDepth)*area*10);
   theOutput[id]->AddStateIndicator(3260,"32.60 Organic N in soil","kg N",(aSoil->GetTotalNitrogenN(0,balanceDepth)-aSoil->GetMinNitrogenN(0,balanceDepth))*area*10);

	theOutput[id]->AddStateIndicator(3269,"32.69 Organic N in soil 0-25 cm","kg N",(aSoil->GetTotalNitrogenN(0,250)-aSoil->GetMinNitrogenN(0,250))*area*10);
	theOutput[id]->AddStateIndicator(3270,"32.70 Mineral N in soil 0-25 cm","kg N",aSoil->GetMinNitrogenN(0,250)*area*10);
	theOutput[id]->AddStateIndicator(3271,"32.71 Organic N in soil 25-50 cm","kg N",(aSoil->GetTotalNitrogenN(250,250)-aSoil->GetMinNitrogenN(250,250))*area*10);
	theOutput[id]->AddStateIndicator(3272,"32.72 Mineral N in soil 25-50 cm","kg N",aSoil->GetMinNitrogenN(250,250)*area*10);
	theOutput[id]->AddStateIndicator(3273,"32.73 Organic N in soil 50-75 cm","kg N",(aSoil->GetTotalNitrogenN(500,250)-aSoil->GetMinNitrogenN(500,250))*area*10);
	theOutput[id]->AddStateIndicator(3274,"32.74 Mineral N in soil 50-75 cm","kg N",aSoil->GetMinNitrogenN(500,250)*area*10);
	theOutput[id]->AddStateIndicator(3275,"32.75 Organic N in soil 75-250 cm","kg N",(aSoil->GetTotalNitrogenN(750,1750)-aSoil->GetMinNitrogenN(750,1750))*area*10);
	theOutput[id]->AddStateIndicator(3276,"32.76 Mineral N in soil 75-250 cm","kg N",aSoil->GetMinNitrogenN(750,1750)*area*10);
	theOutput[id]->AddStateIndicator(3277,"32.77 N in AOM+SMB 0-25 cm","kg N",
		...
		theOutput[id]->AddStateIndicator(3278,"32.78 N in AOM+SMB 25-50 cm","kg N",
		...
		theOutput[id]->AddStateIndicator(3279,"32.79 N in AOM+SMB 50-75 cm","kg N",
		...
		theOutput[id]->AddStateIndicator(3280,"32.80 N in AOM+SMB 75-250 cm","kg N",
		...
		theOutput[id]->AddStateIndicator(3281,"32.81 N in NOM 0-25 cm","kg N",aSoil->GetPoolNitrogenN(5,0,250)*area*10);
		theOutput[id]->AddStateIndicator(3282,"32.82 N in NOM 25-50 cm","kg N",aSoil->GetPoolNitrogenN(5,250,250)*area*10);
		theOutput[id]->AddStateIndicator(3290,"32.90 N in NOM 50-75 cm","kg N",aSoil->GetPoolNitrogenN(5,500,250)*area*10);
		theOutput[id]->AddStateIndicator(3291,"32.91 N in NOM 75-250 cm","kg N",aSoil->GetPoolNitrogenN(5,750,1750)*area*10);
		theOutput[id]->AddStateIndicator(3292,"32.92 N in IOM 0-25 cm","kg N",aSoil->GetPoolNitrogenN(6,0,250)*area*10);
		theOutput[id]->AddStateIndicator(3293,"32.93 N in IOM 25-50 cm","kg N",aSoil->GetPoolNitrogenN(6,250,250)*area*10);
		theOutput[id]->AddStateIndicator(3294,"32.94 N in IOM 50-75 cm","kg N",aSoil->GetPoolNitrogenN(6,500,250)*area*10);
		theOutput[id]->AddStateIndicator(3295,"32.95 N in IOM 75-250 cm","kg N",aSoil->GetPoolNitrogenN(6,750,1750)*area*10);

   theOutput[id]->AddStateIndicator(3283,"32.83 Nmin 0-500","kg N",aSoil->GetMinNitrogenN(0,500)*area*10);
	theOutput[id]->AddStateIndicator(3284,"32.84 Nmin 0-750","kg N",aSoil->GetMinNitrogenN(0,750)*area*10);
	theOutput[id]->AddStateIndicator(3285,"32.85 Nmin 0-1000","kg N",aSoil->GetMinNitrogenN(0,1000)*area*10);

   theOutput[id]->AddStateIndicator(101,"01.01 Total area","ha",area);

   theOutput[id]->AddIndicator(7040,"70.40 Percolation from leaching depth","cubic meter",aSoil->GetDrainage(leachingDepth)*area*10);
   theOutput[id]->AddIndicator(7010,"70.10 Percolation","cubic meter",aSoil->GetDrainage(balanceDepth)*area*10);
   theOutput[id]->AddIndicator(7011,"70.11 Surface run-off","cubic meter",aSoil->GetRunOff()*area*10);
	theOutput[id]->AddIndicator(7012,"70.12 Evapotranspiration","cubic meter",aSoil->GetEvapotranspiration()*area*10);
	theOutput[id]->AddIndicator(7042,"70.42 Bare soil evaporation","cubic meter",aSoil->GetSoilEvaporation()*area*10);
	theOutput[id]->AddStateIndicator(7015,"70.15 Water in field to balancedepth","cubic meter",(snowContent+waterContent+aSoil->GetWater(0,balanceDepth)+aSoil->GetSurfaceWater())*area*10);
	theOutput[id]->AddStateIndicator(7041,"70.41 Water in field to leaching depth","cubic meter",(snowContent+waterContent+aSoil->GetWater(0,leachingDepth)+aSoil->GetSurfaceWater())*area*10);
	theOutput[id]->AddStateIndicator(7020,"70.20 Water in soil","cubic meter",waterContent*area*10);

   theOutput[id]->AddIndicator(3240,"32.40 Nitrate N leached from leaching depth","kg N",aSoil->GetNitrateLeaching(leachingDepth).n*area*10);
   theOutput[id]->AddIndicator(3241,"32.41 Ammonium N leached from leaching depth","kg N",aSoil->GetAmmoniumLeaching(leachingDepth).n*area*10);
   theOutput[id]->AddIndicator(3140,"31.40 Nitrate N leached","kg N",aSoil->GetNitrateLeaching(balanceDepth).n*area*10);
	theOutput[id]->AddIndicator(3141,"31.41 Ammonium N leached","kg N",aSoil->GetAmmoniumLeaching(balanceDepth).n*area*10);
	theOutput[id]->AddIndicator(3147,"31.47 N in surface run-off","kg N",aSoil->GetRunOff_N().n*area*10);
	theOutput[id]->AddIndicator(3142,"31.42 N2 from denitrification","kg N",aSoil->GetDenitrification().n*area*10);
   theOutput[id]->AddIndicator(3143,"31.43 N2O from denitrification","kg N",aSoil->GetN2OFromDenitrification().n*area*10);
   theOutput[id]->AddIndicator(3144,"31.44 N2O from nitrification","kg N",aSoil->GetN2OFromNitrification().n*area*10);
	theOutput[id]->AddIndicator(3230,"32.30 Gross N-mineralisation from soil org. matter","kg",aSoil->GetNitrogenNetMineralisation().n*area*10.0);
   theOutput[id]->AddStateIndicator(3261,"32.61 Mineral N in soil","kg N",aSoil->GetMinNitrogen().n*area*10);

   theOutput[id]->AddIndicator(3812,"38.12 N15 leached from leaching depth","kg N",aSoil->GetNitrateLeaching(leachingDepth).n15*area*10+aSoil->GetAmmoniumLeaching(leachingDepth).n15*area*10);
	theOutput[id]->AddIndicator(3821,"38.21 N15-N2O from denitrification","kg N",aSoil->GetN2OFromDenitrification().n15*area*10);
   theOutput[id]->AddIndicator(3822,"38.22 N15-N2O from nitrification","kg N",aSoil->GetN2OFromNitrification().n15*area*10);
   theOutput[id]->AddIndicator(3823,"38.23 N15-Denitrification","kg N",aSoil->GetDenitrification().n15*area*10);
 	theOutput[id]->AddStateIndicator(3824,"38.24 Total soil/soilsurface 15N","kg N",aSoil->GetTotalNitrogen(1000).n15*area*10);
   ...
   theOutput[id]->AddStateIndicator(3160,"31.60 N in standing crop/straw","kg N",NCrop.n*area*10);
   theOutput[id]->AddStateIndicator(3161,"31.61 Total soil/soilsurface N","kg N",aSoil->GetTotalNitrogenN(balanceDepth)*area*10);
	theOutput[id]->AddStateIndicator(3199,"31.99 Total soil/soilsurface nitrogen to 1 m depth","kg N",aSoil->GetTotalNitrogenN(1000)*area*10);
   theOutput[id]->AddStateIndicator(4061,"40.61 C in standing crop/straw","kg C",CCrop*area*10);

      if (theTime[id].GetDay()==1 && (theTime[id].GetMonth()==9 || theTime[id].GetMonth()==11))
   {
      double CatchDM = 0;
      double CatchN = 0;
      cloneList<crop>::PS aCropElement;
   	...
   	while (aCropElement!=NULL)
      {
   	...
   	if (theTime[id].GetMonth()==9 && theTime[id].GetDay()==1)
      {
   		theOutput[id]->AddIndicator(3901,"39.01 Catch crop DM (1/9)","kg DM",CatchDM);
			theOutput[id]->AddIndicator(3903,"39.03 Catch crop N (1/9)","kg N",CatchN);
      }
      if (theTime[id].GetMonth()==11 && theTime[id].GetDay()==1)
      {
			theOutput[id]->AddIndicator(3902,"39.02 Catch crop DM (1/11)","kg DM",CatchDM);
			theOutput[id]->AddIndicator(3904,"39.04 Catch crop N (1/11)","kg N",CatchN);
      }
   }
	...
	if (cutPlantProducts)
   {
      if (cutPlantProducts->NumOfNodes()>0)
      {
         ...
         theOutput[id]->AddStateIndicator(3160,"31.60 N in standing crop/straw","kg N",NInStraw.n*area);
      }
   }
}
/**
...
void ecosystem::AddYieldToIndicators(const char * cid, double y,double n)
{
   ...
   {
		theOutput[id]->AddIndicator(1710,"17.10 W.wheat yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3600,"36.00 W.wheat yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"SPWHT")==0)
	{
   	theOutput[id]->AddIndicator(1711,"17.11 S.wheat yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3601,"36.01 S.wheat yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"WNRPE")==0)
	{
   	theOutput[id]->AddIndicator(1712,"17.12 W.rape yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3602,"36.02 W.rape yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"SPRAPE")==0)
	{
   	theOutput[id]->AddIndicator(1713,"17.13 S.rape yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3603,"36.03 S.rape yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"WNBRL")==0)
	{
   	theOutput[id]->AddIndicator(1714,"17.14 W.barley yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3604,"36.04 W.barley yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"BARLEY")==0)
	{
   	theOutput[id]->AddIndicator(1715,"17.15 S.barley yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3605,"36.05 S.barley yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"RYE")==0)
	{
      theOutput[id]->AddIndicator(1716,"17.16 Rye yield (harv. bef. 1/8)","t",y);
	   theOutput[id]->AddIndicator(3606,"36.06 Rye yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"OAT")==0)
   {
   	theOutput[id]->AddIndicator(1717,"17.17 Oat yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3607,"36.07 Oat yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"PEA")==0)
	{
   	theOutput[id]->AddIndicator(1718,"17.18 Pea yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3608,"36.08 Pea yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"POTATO")==0)
	{
   	theOutput[id]->AddIndicator(1719,"17.19 Potato yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3609,"36.09 Potato yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"SEEDGRASS")==0)
	{
   	theOutput[id]->AddIndicator(1720,"17.20 Grass for seed yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3610,"36.10 Grass for seed yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"MAIZE")==0)
	{
   	theOutput[id]->AddIndicator(1721,"17.21 Grain maize yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3611,"36.11 Grain maize yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"BEET")==0)
	{
   	theOutput[id]->AddIndicator(1722,"17.22 Beet yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3612,"36.12 Beet yield (harv. bef. 1/8)","kg N",n);
   }
   if ((strcmp(cid,"RYEGRASS")==0)||(strcmp(cid,"RYEGRASSWHOLE")==0))
	{
   	theOutput[id]->AddIndicator(1723,"17.23 Ryegrass yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3613,"36.13 Ryegrass yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"CLOVER")==0)
	{
   	theOutput[id]->AddIndicator(1724,"17.24 Clover yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3614,"36.14 Clover yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"BP")==0)
	{
   	theOutput[id]->AddIndicator(1725,"17.25 Barley/Pea yield (harv. bef. 1/9)","t",y);
   	theOutput[id]->AddIndicator(3615,"36.15 Barley/Pea yield (harv. bef. 1/9)","kg N",n);
   }
   if (strcmp(cid,"MAIZEWHOLE")==0)
	{
   	theOutput[id]->AddIndicator(1726,"17.26 Maize wholecrop yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3616,"36.16 Maize wholecrop yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"ITALRYEGWHOLE")==0)
	{
   	theOutput[id]->AddIndicator(1727,"17.27 Italian ryegrass yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3617,"36.17 Italian ryegrass yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"SOYBEAN")==0)
	{
   	theOutput[id]->AddIndicator(1728,"17.28 Soybean yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3618,"36.18 Soybean yield (harv. bef. 1/8)","kg N",n);
   }
   if (strcmp(cid,"SUNFLOWER")==0)
	{
   	theOutput[id]->AddIndicator(1729,"17.29 Sunflower yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3619,"36.19 Sunflower yield (harv. bef. 1/8)","kg N",n);
   }
   if ((strcmp(cid,"WNWHTWHOLE")==0)||(strcmp(cid,"SPWHTWHOLE")==0))
   {
		theOutput[id]->AddIndicator(1730,"17.30 Wheat silage yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3620,"36.20 Wheat silage yield (harv. bef. 1/8)","kg N",n);
   }
   if ((strcmp(cid,"LUCERNE")==0)||(strcmp(cid,"LUCERNEWHOLE")==0))
	{
   	theOutput[id]->AddIndicator(1731,"17.31 Lucerne yield (harv. bef. 1/8)","t",y);
   	theOutput[id]->AddIndicator(3621,"36.21 Lucerne yield (harv. bef. 1/8)","kg N",n);
   }
}
...
{
   if (strcmp(cid,"W1")==0 || strcmp(cid,"W2")==0)	theOutput[id]->AddIndicator(1810,"18.10 Exp. W.wheat yield (harv. bef. 1/8)","t",y);
	if (strcmp(cid,"W5")==0 || strcmp(cid,"W6")==0) theOutput[id]->AddIndicator(1811,"18.11 Exp. S.wheat yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"RW")==0)                        theOutput[id]->AddIndicator(1812,"18.12 Exp. W.rape yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"RS")==0)                        theOutput[id]->AddIndicator(1813,"18.13 Exp. S.rape yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"B5")==0 || strcmp(cid,"B6")==0) theOutput[id]->AddIndicator(1814,"18.14 Exp. W.barley yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"B1")==0 || strcmp(cid,"B2")==0) theOutput[id]->AddIndicator(1815,"18.15 Exp. S.barley yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"RY")==0)                        theOutput[id]->AddIndicator(1816,"18.16 Exp. Rye yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"OA")==0)                        theOutput[id]->AddIndicator(1817,"18.17 Exp. Oat yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"PE")==0)                        theOutput[id]->AddIndicator(1818,"18.18 Exp. Pea yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"PO")==0)                        theOutput[id]->AddIndicator(1819,"18.19 Exp. Potato yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"SG")==0)                        theOutput[id]->AddIndicator(1820,"18.20 Exp. Grass for seed yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"MA")==0)                        theOutput[id]->AddIndicator(1821,"18.21 Exp. Maize yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"BE")==0)                        theOutput[id]->AddIndicator(1822,"18.22 Exp. Beet yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"BP")==0)                        theOutput[id]->AddIndicator(1823,"18.23 Exp. Barley/Pea yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"C1")==0 || strcmp(cid,"C2")==0) theOutput[id]->AddIndicator(1824,"18.24 Exp. Grass/clover yield (harv. bef. 1/8)","t",y);
   if (strcmp(cid,"G1")==0 || strcmp(cid,"G2")==0) theOutput[id]->AddIndicator(1825,"18.25 Exp. Grass ley yield (harv. bef. 1/8)","ha",y);
}
/****************************************************************************\
...
bool ecosystem::GetReadyForHarvestOrLater()
{
   ...
   theMessage[id]->WarningWithDisplay("ecosystem::ReadyForHarvestOrLater - function called but no crops are defined. May hamper the rest of the simulation.");
   return false;
}
  ...
  double * RootLengthList=NULL;
   ...
   while (aCropElement!=NULL)
   {
      ...
      if (aCrop)
      {
         ...
         {
            if (cutPlantProducts->NumOfNodes()>0)
               theMessage[id]->WarningWithDisplay("ecosystem-KillAllCrops:: Cut plant products should not be present");
            ...
            if (Straw)
            {
               ...
               theOutput[id]->AddIndicator(3221,"32.21 N from straw incorporated into soil","kg N",Straw->GetAllN().n*Straw->GetAmount()*area*1000.0);
               double C_Amount = Straw->GetAmount()*Straw->GetC_content()*area*1000.0;
               theOutput[id]->AddIndicator(4081,"40.81 C from straw incorporated into soil","kg C",C_Amount);
//               theOutput[id]->AddIndicator("40.99 Plant top res. C incorp. in org. matt.","kg C",C_Amount);
               delete Straw;
               Straw = NULL;
            }
            if (Root->GetAmount()>0)
   ...
   if (theCrops->NumOfNodes()!=1)
      theMessage[id]->FatalError("ecosystem::KillAllCrops - there should only be one (bare) crop here");
}
...
//	Setgrazable(false);
   ...
   if (strcmp(crop_id,"F2")==0 || strcmp(crop_id,"BP")==0 || strcmp(crop_id,"C2")==0
      ...
      theMessage[id]->FatalError("ecosystem::AllocateCrop - crop ",crop_id," can not be sown directly");
   if (newCrop==NULL)
      theMessage[id]->FatalError("ecosystem::AllocateCrop - crop ",crop_id," not found");
...
bool ecosystem::EndBudget()
{
   ...
   if (!WaterBudget.Balance(WaterRemain+waterContent+snowContent))
   	ret_val=false;
   if (!ret_val)
      theMessage[id]->FatalError("ecosystem::EndBudget error");
   return ret_val;
}
...
//   double grazing_height= bite_depth_factor * GetMaxCropHeight();
   ...
   */
   ...
   if (!gotCrop)
         theMessage[id]->FatalError("ecosystem:GetFeedResource - no grazable crop found");
...
void ecosystem::SetGrazed(double grazing_height, double DMGrazed)
{
   ...
   theOutput[id]->AddIndicator(3123,"31.23 N removed by cattle","kg N",grazedN*area*10.0);
   theOutput[id]->AddIndicator(4023,"40.23 C removed by cattle","kg C",grazedC*area*10.0);
}
...
/*void ecosystem::Add(ecosystem* addPatch, double fraction)
{
   ...
   if (cutPlantProducts || addPatch->cutPlantProducts)
   	theMessage[id]->Warning("Patch::Add - there should not be straw on ecosystem");
   if (cutPlantProducts)
      for (int i=0;i<cutPlantProducts->NumOfNodes();i++)
   ...
   birthday->SetTime(theTime[id].GetDay(),theTime[id].GetMonth(),theTime[id].GetYear());
}
*/
...
void ecosystem::SimpleGraze()
{
   crop* aCrop=NULL;
   if (theCrops->NumOfNodes()==0)
      theMessage[id]->FatalError("ecosystem::Graze - attempt to graze on field without plants");
   double totalDM=0.0;
   double density=0.0;
   cloneList<crop>::PS aCropElement;
   theCrops->PeekHead(aCropElement);
...
//   double totHay=0.0;
   ...
   while (aCropElement!=NULL)
   {
      ...
      if (Hay->GetAmount()>0.0)
      {
         ...
         theOutput[id]->AddIndicator(3123,"31.23 N removed by cattle","kg N",Hay->GetAllN().n*Hay->GetAmount()*1000.0);
         theOutput[id]->AddIndicator(4023,"40.23 C removed by cattle","kg C",Hay->GetC_content()*Hay->GetAmount()*1000.0);

         Waste->Setamount(Waste->GetAmount()*WasteFraction*0.01);        // Convert t/ha
         aSoil->AddDecomposable(Waste);
   	   theOutput[id]->AddIndicator(2053,"20.53 Plant top res. N incorp. in org. matt.","kg N",Waste->GetAllN().n*Waste->GetAmount()*area*1000.0);
//		   theOutput[id]->AddIndicator("40.99 Plant top res. C incorp. in org. matt.","kg C",Waste->GetAmount()*Waste->GetC_content()*area*1000.0);
      }
   	delete Hay;
      theCrops->OneStep(aCropElement);
   }
   ...
   if (NitrogenLoad>0.0)
   {
      ...
      theProducts[id]->GiveProductInformation(&cowManure);
      cowManure.Setamount(area*NitrogenLoad/cowManure.GetAllN().n/100.0);                         // Convert g/m2 to t
      theProducts[id]->SubtractProduct(&cowManure);
      ...
      theOutput[id]->AddIndicator(3146,"31.46 Ammonia-N loss during grazing","kg N",LostNH4.n*1000.0);
      theOutput[id]->AddIndicator(3109,"31.09 Nitrogen from grazing animals","kg N",
                              cowManure.GetAllN().n*cowManure.GetAmount()*1000.0+LostNH4.n*1000.0);
      theOutput[id]->AddIndicator(4110,"41.10 C from grazing animals","kg C",
                              cowManure.GetC_content()*cowManure.GetAmount()*1000.0);
      theOutput[id]->AddIndicator(1021,"10.21 Manure spread on field","t",cowManure.GetAmount());
...
void ecosystem::Sow(char * cropID, double SeedDensDryMatt,double RelativeDensity,double NitrogenInSeed, double CarbonInSeed)
{
   ...
   if ((theCrops->NumOfNodes()+1)>MaxPlants)
   {
      ...
      theMessage[id]->FatalError("ecosystem::Sow - Too many crops planted");
   }
   cloneList<crop>::PS aCropElement;
   theCrops->PeekHead(aCropElement);
   while (aCropElement!=NULL)
   {  // Remove all bare "crops"
	...
	theCrops->InsertLast(aCrop);
      ...
      theOutput[id]->AddIndicator("31.03 N from seed","kg N",N);
      theOutput[id]->AddIndicator("40.03 C from seed","kg C",CarbonInSeed*area*10.0);
#endif
};
...
void ecosystem::HarvestOperations(harvestFields * hrv)
{
	switch (hrv->GetOperationId())
   {
      ...
      case  GatheringCereal:
      			...
      			break;
      default:
      theMessage[id]->FatalError("ecosystem::HarvestOperations - Operation not found");
   }
};
...
/*   if (theCrops->NumOfNodes()>0)
   ...
   if (cutPlantProducts->NumOfNodes()>0)              // Check if straw has not been gathered or ploughed
      theMessage[id]->WarningWithDisplay("patch::HandleOperation - There should not be straw on field at this point.");
/*
   // Expected yields
   for (int i=0;i<theCrops->NumOfNodes();i++)
...
*/
   ...
   for (int i=0;i<theCrops->NumOfNodes();i++)
   {
      ...
      {
         double fractionStrawLeft   = theControlParameters[id]->GetStrawLeft(); // Fraction of straw biomass left by combine harvesting
         double fractionStorageLeft = theControlParameters[id]->GetHarvestWaste(); // Covers waste by harvest and higher N-content in residues of reproductional plant parts
         if (fractionStorageLeft>0.25)
            theMessage[id]->WarningWithDisplay("patch::patch - abnormal high waste by harvest");
         if (theControlParameters[id]->GetOrganicFarm())
         ...
         double MaxRootDepth = aCrop->GiveRootDepth();
                  theOutput[id]->AddIndicator(1704,"17.04 Maximum root depth","m",MaxRootDepth);

         aCrop->Harvest(Storage,Straw);
         operationPerformed=true;

         scenarielog.WriteWithTabS(aCrop->CropName);
         scenarielog.WriteWithTabS(theTime[id].GetString2());
         ...
         theOutput[id]->AddIndicator(1702,"17.02 Grain yield at economic indicator date","t",Storage->GetAmount()*(1.0-fractionStorageLeft)*area/100.0);
         theOutput[id]->AddIndicator(1703,"17.03 Straw yield at economic indicator date","t",Straw->GetAmount()*(1.0-fractionStrawLeft)*area/100.0);
         theOutput[id]->AddIndicator(3120,"31.20 N removed in grain","kg N",N_removed);;
         theOutput[id]->AddIndicator(4020,"40.20 C removed in grain","kg C",Storage->GetAmount()*Storage->GetC_content()*(1.0-fractionStorageLeft)*area*10.0);
         theOutput[id]->AddIndicator(3810,"38.10 N15 harvest in grain","kg N",StorageN.n15*(1.0-fractionStorageLeft)*area*10.0);
         theOutput[id]->AddIndicator(3811,"38.11 Total N15 harvest","kg N",StorageN.n15*(1.0-fractionStorageLeft)*area*10.0);
			theOutput[id]->AddIndicator(3910,"39.10 Grain yield at environmental indicator date","t",Storage->GetAmount()*(1.0-fractionStorageLeft)*area/100.0);
			theOutput[id]->AddIndicator(3911,"39.11 Straw yield at environmental indicator date","t",Straw->GetAmount()*(1.0-fractionStrawLeft)*area/100.0);
			if (i==0)
			      	{
			      		bsTime aDate;
			   			aDate = aCrop->GiveDateOfEmergence();
			      		int DOY = aDate.GetDayInYear();
			      		theOutput[id]->AddIndicator(3920,	"39.20 Date of emergence of main crop","day of year",double(DOY));
			   			aDate = aCrop->GiveDateOfFlowering();
			      		DOY = aDate.GetDayInYear();
							theOutput[id]->AddIndicator(	3921,"39.21 Date of flowering of main crop","day of year",double(DOY));
			   			aDate = aCrop->GiveDateOfEndGrainFill();
			      		DOY = aDate.GetDayInYear();
							theOutput[id]->AddIndicator(3922,	"39.22 Date of end of grain filling of main crop","day of year",double(DOY));
							aDate = aCrop->GiveDateOfRipeness();
			      		DOY = aDate.GetDayInYear();
			         	theOutput[id]->AddIndicator(	3923,"39.23 Date of ripeness of main crop","day of year",double(DOY));
			      	}

#ifdef TUPLE
         if (((field*)Owner)->GetWriteTuples());
            ((field*)Owner)->AddTuple(theTime[id].GetString2(),-N_removed,Storage->GetAmount()*area*0.1,"HARVEST","MAIN_PRODUCT",0);
#endif
         ...
         theOutput[id]->AddIndicator(3223,"32.23 N from storage incorporated into soil","kg N",StorageWaste->GetAllN().n*StorageWaste->GetAmount()*area*10.0);
         theOutput[id]->AddIndicator(4083,"40.83 C from storage incorporated into soil","kg C",StorageWaste->GetAmount()*StorageWaste->GetC_content()*area*10.0);
         ...
         theOutput[id]->AddIndicator(3221,"32.21 N from straw incorporated into soil","kg N",StrawWaste->GetAllN().n*StrawWaste->GetAmount()*area*10.0);
         theOutput[id]->AddIndicator(4081,"40.81 C from straw incorporated into soil","kg C",StrawWaste->GetAmount()*StrawWaste->GetC_content()*area*10.0);
         ...
         if (Storage)
         {
            ...
            theMessage[id]->Warning("patch::HandleOperation - Harvested amount should not be zero at this point.");

         if ((hrv->GetOperationId()==CombineHarvestingShred  || hrv->GetOperationId()==BeetHarvesting))
         {
            theOutput[id]->AddIndicator(3221,"32.21 N from straw incorporated into soil","kg N",Straw->GetAllN().n*Straw->GetAmount()*area*10.0);
         ...
         if(!aCrop->ContinueGrowth())
                 {
                 	...
                 	if (ExtraStraw->GetAmount()>0.0)
                 		theMessage[id]->WarningWithDisplay("ecosystem::Harvest crop not harvested correctly");
                 	delete ExtraStraw;
                 	TransferRootMatter(Root,RootLengthList);
                 	delete Root;
      ...
      {  // Fraction of undersown crop that is cut by ordinary harvest
         ...
         {
            theOutput[id]->AddIndicator(3221,"32.21 N from straw incorporated into soil","kg N",Waste->GetAllN().n*Waste->GetAmount()*area*1000.0);
            theOutput[id]->AddIndicator(4081,"40.81 C from straw incorporated into soil","kg C",Waste->GetAmount()*Waste->GetC_content()*area*1000.0);
            aSoil->AddDecomposable(Waste);
            delete Waste;
         }
   ...
   if (!operationPerformed)
      theMessage[id]->FatalError("ecosystem::HarvestOp - no crops present for harvest in harvest operation");
}
...
void ecosystem::ForageHarvestOp(harvestFields * hrv)
{
   ...
   {
      theMessage[id]->WarningWithDisplay("patch::HandleOp - cutting height in forage harvesting is above 0.5 m, cm input assumed!");
   ...
   if (GetMaxCropHeight()>0.0)               // This is returned in m
   {
	...
	//straw->GetAllN()*straw->GetAmount();
         ...
         if (!c->GetTerminated())
         {
            ...
            else // This is not a crop that can be cut more than once
            {
            	...
            	if (plantHeight>cuttingHeight)
            	{
            		...
            		if (ExtraStraw->GetAmount()>0.0)
            			theMessage[id]->WarningWithDisplay("patch::Harvest crop not harvested correctly");
            		delete ExtraStraw;
            		TransferRootMatter(Root,RootLengthList);
            		delete Root;
            	}
            	...
            	{
            		theMessage[id]->FatalError("patch::Harvest plantHeight is lower that cuttingHeight");
            	}
            ...
            if (area<=0)
               theMessage[id]->WarningWithDisplay("Patch::HandleOp - area shold be above zero");
            ...
            if (hrv->GetOperationId()==CutSetAside)
            {
            	if (forage->GetAmount()>0.0)
                {
                    ...
                    theOutput[id]->AddIndicator(3221,"32.21 N from straw incorporated into soil","kg N",forage->GetAllN().n*forage->GetAmount()*area*1000.0);
                    theOutput[id]->AddIndicator(4081,"40.81 C from straw incorporated into soil","kg C",forage->GetAmount()*forage->GetC_content()*area*1000.0);
                    delete forage;
                  }
            }
            ...
            nitrogen forageN = forage->GetTotalN()*forage->GetAmount();
                  ...
                  if (hrv->GetOperationId()==ForageHarvesting)
                  {
                    ...
                    theOutput[id]->AddIndicator(3122,"31.22 N removed in cut","kg N",n);
                    theOutput[id]->AddIndicator(4022,"40.22 C removed in cut","kg C",forage->GetAmount()*forage->GetC_content()*1000.0);
            		theOutput[id]->AddIndicator(3912,"39.12 Cut at environmental indicator date","t",forage->GetAmount());
            #ifdef TUPLE
                              if (((field*)Owner)->GetWriteTuples());
                                 ((field*)Owner)->AddTuple(theTime[id].GetString2(),-n,forage->GetAmount()*10.0,"HARVEST","FORAGE/SILAGE",0);
            #endif

                     if (j>10)
                     	theMessage[id]->FatalError("ecosystem::HarvestGrasslandOp - too many harvested products");
                      forage ->Setamount(forage ->GetAmount()*area);
      ...
      theMessage[id]->WarningWithDisplay("patch::HandleOp - crop has zero height at a cutting date");
...
//         plantItem* aCutProduct = new plantItem(*cutPlantProducts->ElementAtNumber(i));
         if (i>3)
            theMessage[id]->FatalError("ecosystem::Gathering - too many harvested products");
         ...
         {
            theOutput[id]->AddIndicator(3121,"31.21 N removed in straw","kg N",Noutput);
            theOutput[id]->AddIndicator(4021,"40.21 C removed in straw","kg C",Coutput);
#ifdef TUPLE
            if (((field*)Owner)->GetWriteTuples());
               ((field*)Owner)->AddTuple(theTime[id].GetString2(),-Noutput,aCutProduct->GetAmount()*10.0,"HARVEST","SECONDARY_PRODUCT",0);
#endif
         ...
         if (hrv->GetOperationId()==GatheringCereal)
         {
            if (i==0)   //is grain
            {
         		...
         		theOutput[id]->AddIndicator(3120,"31.20 N removed in grain","kg N",Noutput);
               theOutput[id]->AddIndicator(4020,"40.20 C removed in grain","kg C",Coutput);
#ifdef TUPLE
               if (((field*)Owner)->GetWriteTuples());
                  ((field*)Owner)->AddTuple(theTime[id].GetString2(),-Noutput,aCutProduct->GetAmount()*10.0,"HARVEST","MAIN_PRODUCT",0);
#endif
            ...
            if (i>1)   //is error
               theMessage[id]->FatalError("Patch:: HandleOp - too many products in GatheringCereal");
         }
         if (hrv->GetOperationId()==GatheringSilage || hrv->GetOperationId()==GatheringHay)
         {
            theOutput[id]->AddIndicator(3122,"31.22 N removed in cut","kg N",Noutput);
            theOutput[id]->AddIndicator(4022,"40.22 C removed in cut","kg C",Coutput);
#ifdef TUPLE
            if (((field*)Owner)->GetWriteTuples());
               ((field*)Owner)->AddTuple(theTime[id].GetString2(),-Noutput,aCutProduct->GetAmount()*10.0,"HARVEST","FORAGE/SILAGE",0);
#endif
      ...
      theMessage[id]->Warning("patch::HandleOperation - Straw already gathered.");
...
void  ecosystem::Tillage(soilTreatFields *FOP)
{
	if (FOP->GetOperationId()==Ploughing)
   {
      ...
      Incorporation=1.0; // Otherwise error - correct later !!!
		aSoil->Incorporate(Depth,Incorporation,true,"Ploughing");
   }
...
/*void ecosystem::Add(patch* addPatch, double fraction)
{
   ...
   if (theCrops)
   	for (int i=0;i<theCrops->NumOfNodes();i++)
	   	theCrops->ElementAtNumber(i)->Add(addPatch->GetCrops()->ElementAtNumber(i),fraction);
   if (cutPlantProducts || addPatch->cutPlantProducts)
   	theMessage[id]->Warning("Patch::Add - there should not be straw on patch");
   if (cutPlantProducts)
      for (int i=0;i<cutPlantProducts->NumOfNodes();i++)
      	*(cutPlantProducts->ElementAtNumber(i))=*(cutPlantProducts->ElementAtNumber(i))*(1.0-fraction);
   if (addPatch->cutPlantProducts)
...
void ecosystem::Volatilise(double duration, bool kill)
{
   ...
   if (soilManure)
   {
      ...
      theClimate[id]->GetClimate(temperature, min_temp, max_temp, ppt, radiation, evap, windspeed, relHumidity);
      infiltration = 0.0;
      ...
      if (fabs(test)>0.1)
            theMessage[id]->FatalError("ecosystem::Volatilise - balance error");
   ...
   theOutput[id]->AddIndicator("31.45 Ammonia-N loss by spreading","kg N",actualNH4_loss * area *1000.0);
}
