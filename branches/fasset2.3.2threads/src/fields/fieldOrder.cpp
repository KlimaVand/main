...
double fieldOrder::GetFractionStrawHarvested(int nr)
{
	if (FractionStrawHarvested.NumOfNodes()>nr)
   	...
   	return 1.0-theControlParameters[id]->GetStrawLeft();
}
/****************************************************************************\
\****************************************************************************/
double fieldOrder::GetFractionGrainHarvested(int nr)
{
	if (FractionStrawHarvested.NumOfNodes()>nr)
   	...
   	return 1.0-theControlParameters[id]->GetHarvestWaste();
}
...
{
   if (*DateString=="")
      theMessage[id]->FatalError("fieldOrder::CheckDateStringForEmpty - date not specified");
}
...
void fieldOrder::ReadParameters(commonData * &data)
{
   ...
   if (!(area>0.0))
	   theMessage[id]->FatalError("fieldOrder::ReadParameters - area must be bigger than zero");
   ...
   int unitType=theControlParameters[id]->GetFertiliserUnit(); // Default value for unitType = 0 -> fertiliser given per ha in file
   if (unitType<0 || unitType>2)
      theMessage[id]->FatalError("fieldOrder::ReadParameters - 'unitType' has illegal value");
   ...
   if (Automatic==0)
   {
      ...
      for (int i=first;i<=(size);i++)
      {
         ...
         if(s->compare(PLOUGH)!=0&&s->compare(HARROW)!=0)
         {
        	 ...
        	 cout<<*s<<" has been read as TillageType"<<endl;
        	theMessage[id]->FatalError("fieldOder::ReadParameters Fasset supports only PLOUGH and HARROW as TillageType");
         }
      ...
      data->FindVectorSize("PloughDate",&first,&size);
      for (int i=first;i<=(size);i++)
      {
         string * s = new string;
         data->FindItem("PloughDate",s,i);
         CheckDateStringForEmpty(s);
         TillageDate.InsertLast(s);
      ...
      // Added for backward compatibility (november 2006 BMP)
      data->FindVectorSize("HarrowDate",&first,&size);
      for (int i=first;i<=(size);i++)
      {
         string * s = new string;
         data->FindItem("HarrowDate",s,i);
         CheckDateStringForEmpty(s);
         TillageDate.InsertLast(s);
      ...
      for (int i=first;i<=(size);i++)
      {
         ...
         s=new string;
      	if (data->FindItem("SowStartDate",s,i))
               theMessage[id]->FatalError("fieldOrder::ReadParameters - not using the MEASCOPE compiler option, you pillock");
         delete s;
      }
      ...
      for (int i=first;i<=(size);i++)
      {
         ...
         if (*type=="")
            theMessage[id]->FatalError("fieldOrder::ReadParameters - must specify type of fertilizer");
         ...
         if (*type=="MINERAL")
         {
            ...
            *MineralFormString = "SOLID";
            ...
            if (aNitrogen->n<0.0)
            {
               theMessage[id]->FatalError("fieldOrder::ReadParameters - negative amount of fertiliser");
               aNitrogen->n=0;
            }
            ...
            if (unitType!=1 && aNitrogen->n<10.0)
            {
               ...
               theMessage[id]->WarningWithDisplay("fieldOrder::ReadParameters - N from mineral fertiliser below 10 kg per ha");
            }
            if (unitType!=1 && aNitrogen->n>400.0)
            {
               cout << " N: " << aNitrogen->n << endl;
               theMessage[id]->WarningWithDisplay("fieldOrder::ReadParameters - N from mineral fertiliser above 400 kg per ha");
            }
#ifdef TUPLE
            if (area>0.001)
            ...
            *a=*a*factor;
            ...
            if (found==0)
               theMessage[id]->FatalError("fieldOrder::ReadParameters - must specify N, P or K content of mineral fertilizer");
         }
         else
               ...
               || (*type=="LIQUID-MANURE") || (*type=="SOLID-MANURE"))
            {
               ...
               //!call to check if using manureParameters.dat
	      		...
	      		theProducts[id]->GiveProductInformation(aManure);
...
//!amounts could be per hectare or per field (will be adjusted by 'factor' later)
               ...
               double amountTotalN = 0.0;
            	data->FindItem("FertilizerN",&amountTotalN,i);
               data->FindItem("ManureAmount",&amount,i);
               double NH3emissionFactor=0.15;
               data->FindItem("NH3emissionFactor",&NH3emissionFactor,i);

	            if ((amountTotalN==0.0)&& (amount==0.0))
	               theMessage[id]->FatalError("fieldOrder::ReadParameters - amount of manure must be input in tonnes fresh weight (ManureAmount) or kilograms N (FertilizerN)");
	            if ((amountTotalN>0.0)&& (amount>0.0))
	               theMessage[id]->FatalError("fieldOrder::ReadParameters - amount of manure cannot be input in both tonnes fresh weight (ManureAmount) and kilograms N (FertilizerN)");
...
//!do some primitive quality control
               if (amount<5.0)
               {
                  ...
                  theMessage[id]->WarningWithDisplay("fieldOrder::ReadParameters - manure amount below 5 tonnes per ha");
               }
               if (amount>100.0)
               {
                  cout << endl << "Manure amount " << amount << " t/ha, containing " << (1000.0*amount*aManure->GetTotalN().n) << " kg N/ha" << endl;
                  theMessage[id]->WarningWithDisplay("fieldOrder::ReadParameters - manure amount above 100 tonnes per ha");
               }
#ifdef TUPLE
               if (area>0.001)
            ...
            else
            {
               ...
               theMessage[id]->FatalError("fieldOrder::ReadParameters - unknown fertilizer type");
            }
      }
      ...
      for (int i=first;i<=(size);i++)
      {
         ...
         if (*a==0.0)
            theMessage[id]->FatalError("fieldOrder::ReadParameters - irrigation amount zero or unspecified");
      ...
      for (int i=first;i<=(size);i++)
      {
         ...
         CuttingRemovesStraw.push_back(CuttingRemoveStraw);
         ...
         {
	         theMessage[id]->WarningWithDisplay("fieldOrder::ReadParameters - Cutting height converted from cm to m");
      ...
      for (int i=first;i<=(size);i++)
      {
         ...
         if (*a<0.0)
            theMessage[id]->FatalError("fieldOrder::ReadParameters - Nitrogen below zero or unspecified");
   ...
   else
   {
      ...
      fluid_hug_fall=fluid_hug_fall*factor;
   	if(data->FindItem("DUNG.PA",&solid_hug))
		if(solidHugUsed==true)
			theMessage[id]->FatalError("fieldOrder::ReadParameters: Cannot use both DUNG.PA and DUNG.PS");
      solid_hug=solid_hug*factor;
      a=0.0;
      ...
      // Rigtige perioder til nedenst. For alle afledte af static_crop undtagen raps skal der ikke kunne gives gylle om efter�ret (warning). �rt m� ikke f� N-g�dn. overhovedet
   	...
   	if(data->FindItem("SLUR.P00",&a)==true)
   		theMessage[id]->FatalError("fieldOrder::ReadParameters: this part of Fasset is not implemented");
...
void fieldOrder::OpenTupleFile(string s)
{
   ...
   if (theControlParameters[id]->GetObsTupleHasBeenOpened())
   ...
   else
   {
      ...
      theControlParameters[id]->SetObsTupleHasBeenOpened(true);
      theControlParameters[id]->SetWriteTuples(true);
   }
...
void fieldOrder::ReadPeriods()
{
   ...
   for (int i=0;i<periodcount;i++)
   {
      theMessage[id]->FatalError("fieldOrder::ReadParameters - irrigation periods not implemented");
   ...
   {
      theMessage[id]->FatalError("fieldOrder::ReadParameters - pesticide periods not implemented");
   ...
   }
   {
      ...
      if (GetParameter("PesticideDate",s,i)||
         ...
         GetParameter("TillageDate",s,i))
               theMessage[id]->FatalError("fieldOrder::ReadParameters - field operations input with fixed date format");
   }
