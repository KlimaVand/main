...
void staticSeedGrass::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->FatalError("static_seedgrass::SpecifiedPlan - more than one harvest date specified");
   delete aPlantItem1;
   delete aPlantItem2;
}
...
cloneList<fieldOperationFields> * staticSeedGrass::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   if (!PP->GetAutomatic())
      SpecifiedPlan(PP,JB_NO,PP->GetCropId());
   	...
   	theMessage[id]->FatalError("static_seedgrass::MakeOpList - automatic plan not implemented");
   return LP;
}
