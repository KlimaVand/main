...
void staticPotato::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->FatalError("static_potato::SpecifiedPlan - more than one harvest date specified");
...
cloneList<fieldOperationFields> * staticPotato::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      if (fluid_HUG_fall>0)
         theMessage[id]->Warning("static_potato::MakeOpList - Fluid manure in fall not allowed for potatos");
