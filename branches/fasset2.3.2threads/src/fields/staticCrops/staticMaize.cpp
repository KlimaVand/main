...
void staticMaize::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->Warning("static_maize::SpecifiedPlan - more than one harvest date specified");       // Might not be a problem is barley with undersown clover/grass
}
...
cloneList<fieldOperationFields> * staticMaize::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      theMessage[id]->FatalError("static_maize::MakeOpList - Automatic plan is not implemented");
