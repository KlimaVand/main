...
void staticItalianRyeGrass::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->FatalError("static_italryeg::SpecifiedPlan - more than one harvest date specified");
}
...
cloneList<fieldOperationFields> * staticItalianRyeGrass::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      theMessage[id]->FatalError("static_italryeg::MakeOpList - automatic plan not implemented");
   }
   delete aPlantItem1;
   delete aPlantItem2;
 	return LP;
