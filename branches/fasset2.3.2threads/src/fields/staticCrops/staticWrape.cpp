...
void staticWrape::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->FatalError("static_wnrape::SpecifiedPlan - more than one harvest date specified");
}
...
cloneList<fieldOperationFields> * staticWrape::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      ...
      if ((FLUID_HUG_fall<=0) && (PP->GetN()>0)) { // Apply mineral fertilizer
         ...
         theProducts[id]->GiveProductInformation(aFertilizer);
    ...
    if (((FLUID_HUG_spring<=0) && (PP->GetSolidManure()>0)) && ((PP->GetN()>0) || (PP->GetK()>0) || (PP->GetP()>0))) { // Apply mineral fertilizer
      ...
      if ((N<vsm) || (N>vs) || (P<vsm) || (P>vs) || (K<vsm) || (K>vs) || (fHUG<vsm) || (fHUG>vs))
         theMessage[id]->WarningWithDisplay("static_wnrape::BasicOpList - Error in fertilizer handling.");
      ...
      { // Sk�rl�gning
         ...
         theProducts[id]->GiveProductInformation(aPlantItem);
