...
void staticSwheat::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->FatalError("static_spwht::SpecifiedPlan - more than one harvest date specified");
...
void staticSwheat::BasicOpList(fieldOrder * PP,int JB_NO,double reduction,char * crop_id)
{
	...
	if (PP->GetFluidManureFall()>0)
		theMessage[id]->Warning("static_spwht::BasicOpList - Fluid manure not allowed in fall.");
	...
	{ // Sowing
		...
		theProducts[id]->GiveProductInformation(aSeed);
		aSeed->Setamount(aSeed->GetStandardDensity());     // 		180
		sowFields * FO=new sowFields;
		FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear()+1,4,10,PP->GetArea(),0,PP->GetDistance(),2,reduction,aSeed);
		AddOpList(FO);
		delete aSeed;
	}
/*	if (2==2) { // Spraying herbicide     ??????       Hvordan spr�jtes v�rhvede?????
		...
		theProducts[id]->GiveProductInformation(aChemical1);
		aChemical1->SetByTFI(1);
		chemical * aChemical2=new chemical;
		aChemical2->Setname("Arelon");
		theProducts[id]->GiveProductInformation(aChemical2);
	...
	if ((PP->GetN()>0) || (PP->GetP()>0) || (PP->GetK()>0)) { // Apply mineral fertilizer       dato korrekt?????
		...
		theProducts[id]->GiveProductInformation(aFertilizer1);
		...
		theProducts[id]->GiveProductInformation(aFertilizer2);
		...
		theProducts[id]->GiveProductInformation(aFertilizer3);
	...
	if ((N<vsm) || (N>vs) || (P<vsm) || (P>vs) || (K<vsm) || (K>vs) || (fHUG<vsm) || (fHUG>vs))
		theMessage[id]->WarningWithDisplay("static_spwht::BasicOpList - Error in fertilizer handling.");
   ...
   theProducts[id]->GiveProductInformation(aPlantItem2);
