...
void staticWwheat::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->FatalError("static_wnwht::SpecifiedPlan - more than one harvest date specified");
}
...
cloneList<fieldOperationFields> * staticWwheat::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   { // Automatic plan
      ...
      if (PP->GetFluidManureFall()>0)
         theMessage[id]->Warning("static_wnwht::BasicOpList - Fluid manure not allowed in fall.");
      ...
      if ((N<vsm) || (N>vs) || (P<vsm) || (P>vs) || (K<vsm) || (K>vs) || (fHUG<vsm) || (fHUG>vs))
         theMessage[id]->WarningWithDisplay("static_wnwht::BasicOpList - Error in fertilizer handling.");
