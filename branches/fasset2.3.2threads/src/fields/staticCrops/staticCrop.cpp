...
char * staticCrop::GetSeedName(char * crop_id)
{
   ...
   if (strcmp(crop_id,"W1")==0 || strcmp(crop_id,"W2")==0)
      ...
      strcpy(output, "SEEDLUCERNE");  //added NJH May 2009
       ...
       || strcmp(crop_id,"B9")==0)
      theMessage[id]->FatalError("staticCrop::GetSeedName - crop ",crop_id," can not be sown directly");
   else
	   theMessage[id]->FatalError("staticCrop::GetSeedName - crop ",crop_id," not found");
   return output;
}
	...
	{ // Sow
      ...
      */
      ...
      else
      {
         ...
         if (!(t1<=t2))
            theMessage[id]->FatalError("staticCrop::SpecifiedPlan - sowing dates must be given in non-decreasing order");
      ...
      else
      {
         switch (*PP->GetCuttingProcessProducts(i))
         {
            ...
            case 2:
               ...
               break;
            default:
                  theMessage[id]->FatalError("static crop: process product code invalid");
         }
      }
      // - to here
...
//		         aPlantItem1->Setamount(ExpectedYield_(JB_NO,0,2));
            ...
            else
            {
            	...
            	if (s=="F1" || s=="F2")
            	{
               	theMessage[id]->WarningWithDisplay("staticCrop::SpecifiedPlan - combine harvesting on fallow. Changed to cutting");
               	FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),1.0,true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
            	}
               else
            	{
               	theMessage[id]->WarningWithDisplay("staticCrop::SpecifiedPlan - combine harvesting on grass, grass clover or clover. Changed to forage harvesting");
               	FO->DefineHarvestFieldsOper(ForageHarvesting,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),1.0,true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
            	}
            	FO->SetCuttingHeight(CuttingHeight);
            	FO->SetForageProduct(aPlantItem1);
      ...
      else
      {
         ...
         day=PP->GetDay(date);
         if (aPlantItem1==NULL)
            theMessage[id]->FatalError("staticCrop::SpecifiedPlan - about to call a method with a NULL pointer");
//         aPlantItem1->Setamount(ExpectedYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         if (gatheringStraw)                                                          
...
cloneList<fieldOperationFields> * staticCrop::MakeOpList(fieldOrder * PP,int JB_NO,double reduction)
{
   ...
   if (LP->NumOfNodes()==0)
   {
      ...
      yearOffset=theControlParameters[id]->GetoperationOffset();
   #endif
   	soilTreatFields * FO=new(soilTreatFields);
...
{
theMessage[id]->FatalError("staticCrop::SpecifiedPeriods - needs to be modified and tested for new cutting process");
   int yearOffset=theControlParameters[id]->GetoperationOffset();
