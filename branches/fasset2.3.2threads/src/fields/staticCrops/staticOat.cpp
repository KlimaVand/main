...
void staticOat::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage[id]->Warning("static_oat::SpecifiedPlan - more than one harvest date specified");       // Might not be a problem, is oat with undersown clover/grass
}
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         if (underSown)
         {
            ...
            theProducts[id]->GiveProductInformation(aSeed1);
            aSeed1->Setamount(15);
            aSeed2 = new seed;
            aSeed2->Setname("SEEDCLOVER");
            theProducts[id]->GiveProductInformation(aSeed2);
            aSeed2->Setamount(5);
