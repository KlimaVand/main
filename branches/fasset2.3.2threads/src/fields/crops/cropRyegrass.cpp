             ...
             :crop(aName,aIndex,aOwner, cropName)
{
   ...
   data->FindItem("RespirationRoot",&RespirationRoot); // Root respiration given as a daily fraction of DM at 10 deg. Celsius, Calibrated to Burreh?jvej data
   data->FindItem("RespirationTop",&RespirationTop);   // Top respiration given as a daily fraction of DM at 10 deg. Celsius
   data->FindItem("TransferRate",&TransferRate); // Fraction (temperature-dependend) of the four age pools that is transferred to subsequent fraction
...
{
   theMessage[id]->WarningWithDisplay("cropRyegrass::GrazeOrCut should never be called - use cropRyegrass::Cut instead. Bj�rn, things to do !!!");
   CutOrHarvested = true;
   PlantHeight = PlantHeight*fractionLeft;
   if (GiveDryMatterVegTop()<=0.0)
      theMessage[id]->WarningWithDisplay("cropRyegrass::Cut/Graze - vegetative top must be bigger than zero here");

   Hay->Setname(StrawItemName);
   theProducts[id]->GiveProductInformation(Hay);
   Hay->RemoveWater();
   ...
   if (aDryMatterInVegTop > 0)
   {
      ...
      Hay->SetorgN_content(aNitrogenInVegTop/(aDryMatterInVegTop*(1-fractionLeft)));    //MEL 2009
   	...
   	theMessage[id]->Warning("RyeGrass:: Nothing to cut/graze");
...
void cropRyegrass::RootDecay(double RootTranslocation)
{
   ...
   if ((DryMatterStorage + DryMatterRoot + DryMatterVegTop > 0.0) && (Nitrogen.n > 0.5))
   {
      ...
      {
      	cout << GetLongName() << endl;
   		theMessage[id]->WarningWithDisplay("crop::RootDecay - not enough nitrogen in plant");
   	}
   	if (maximumN < Nitrogen.n * 0.75)
   	{
      	cout << GetLongName() << endl;
	   	theMessage[id]->WarningWithDisplay("crop::RootDecay - too much nitrogen in plant");
      }
   }
...
double cropRyegrass::TopRespiration()
{
   ...
   if ((DryMatterStorage + DryMatterRoot + DryMatterVegTop > 0.0) && (Nitrogen.n > 0.5))
   {
   	...
   	{
   		cout << GetLongName() << endl;
      	theMessage[id]->WarningWithDisplay("crop::TopRespiration - not enough nitrogen in plant");
   	}
   	if (maximumN < Nitrogen.n * 0.75)
   	{
   		cout << GetLongName() << endl;
	   	theMessage[id]->WarningWithDisplay("crop::TopRespiration - too much nitrogen in plant");
   	}
   }
...
//		double TotalLostN = NConcentrationInYellow*(1.0-RF)*senescenceDM;
      ...
      if (senescenceN.n<0.0)
   	{
      	theMessage[id]->Warning("cropRyeGrass:: negative N loss");
         senescenceN.Clear();
      }
      ...
      theProducts[id]->GiveProductInformation(topMatter);
   ...
   {
      theMessage[id]->Warning("cropRyeGrass::UpdateAgeClasses - grazing (or transfer) has resultet in negative DM in age class 0");
      AgeClassTop[0]=1.0; // Hack to get movin' !!!
   }
   AgeClassTop[1]+=transfer[1]-transfer[2] - grazedDM[1];
   if (AgeClassTop[1]<0.0)
   {
      theMessage[id]->Warning("cropRyeGrass::UpdateAgeClasses - grazing (or transfer) has resultet in negative DM in age class 1");
      AgeClassTop[1]=1.0; // Hack to get movin' !!!
   }
   AgeClassTop[2]+=transfer[2]-transfer[3] - grazedDM[2];
   if (AgeClassTop[2]<0.0)
   {
      theMessage[id]->Warning("cropRyeGrass::UpdateAgeClasses - grazing (or transfer) has resultet in negative DM in age class 2");
   ...
   {
      theMessage[id]->WarningWithDisplay("cropRyeGrass::UpdateAgeClasses - grazing (or transfer) has resultet in negative DM in age class 3");
   ...
   if (Nitrogen.n<0.0)
   {
      theMessage[id]->Warning("cropRyeGrass::UpdateAgeClasses - grazing has resultet in negative N");
...
double cropRyegrass::GiveDryMatterVegTop()
{
   ...
   if (retVal<0.0)
      theMessage[id]->WarningWithDisplay("cropRyegrass::GiveDryMatterVegTop - negative dry matter");
   return retVal;
}
...
//   rootMatter = NULL;    NJH pinged this out March 2009
   ...
   {
      if (aSoil==NULL)
         theMessage[id]->FatalError("cropRyegrass::Update - Invalid soil pointer");
...
void cropRyegrass::Cut(plantItem * cutPlantMaterial, double cut_height)
{
   if (cut_height<0.0)
      cut_height=0.01;
   if (GiveDryMatterVegTop()<=0.0)
      theMessage[id]->WarningWithDisplay("cropRyegrass::Cut - vegetative top must be bigger than zero here");
/*-------------------------------
   ...
   return grazableDM;
-------------------------------*/
   if (cutPlantMaterial->GetName()[0]=='\0') // This might need expansion in the future. For FarmN purpose this will probably do. !!!
   {
   ...
   {
      theMessage[id]->FatalError("cropRyegrass::Cut - dry matter content is below 1 %");
   ...
   if (GiveDryMatterVegTop()<=0.0)
      theMessage[id]->WarningWithDisplay("cropRyegrass::Cut - zero or negative amount of dry matter in veg. top");
   HasBeenCut = true;
   Phenology->SetDSAfterCut();
}
...
{
	feedItem *myProduct = new feedItem(*(feedItem *)theProducts[id]->GetProductElement(465));  //set product to grass, 100kgN, cut after 2 weeks (N content will be modified)
   ...
   if (liveOMD<deadOMD)
   	liveOMD = deadOMD;
   grazableDM=GetAvailableDM(0.0);
   tempDigest= GetAvailableDigestibleDM(0.0);
   ...
   if (grazableDM>0)
   	{
         ...
         switch (animalType)
         {
            ...
            default:
            {
               theMessage[id]->FatalError("cropRyegrass::GetAvailability - not an allowed animal type");
               break;
            }
         }
      }
...
{
   grazableDM = GetAvailableDM(0.0);
   double proportionCut=DMGrazed/grazableDM;
//   cout << theTime << " DMGrazed " << DMGrazed << " grazing_height " << grazing_height << " Nitrogen " << Nitrogen.n << endl;
   double checkDM=0.0;
   for(int i=0;i<4;i++)
   {
      ...
      if (grazableDM>0.0)
      {
      	grazedDM[i]+=proportionCut * AgeClassTop[i] * uptakeWeight[i];
         checkDM+=grazedDM[i];
//            cout << theTime << " i " << i << " grazedDM "  << grazedDM[i] << endl;
      }
   }
}
...
void cropRyegrass::Terminate(decomposable* Straw,decomposable* DeadRoot,double *& RootLengthList)
{
   for(int i=0;i<4;i++)
      if (grazedDM[i]>0)
         theMessage[id]->FatalError("cropRyegrass::Terminate - terminating a crop on same day as it was grazed");
...
void cropRyegrass::Grazing(double grazedDM)
{
	   ...
	   if (proportionCut>1.0)
		   theMessage[id]->FatalError("cropRyegrass::Grazing - Proportion cut > 1.0");
	   nitrogen  aNitrogenInVegTop = NitrogenInVegTop();
	   for(int i=0;i<4;i++)
	   {
	      tempDM = proportionCut*AgeClassTop[i];
...
double cropRyegrass::GetAvailableDigestibleDM(double residualDM)
{
	double retVal=0.0;
   residualDM/=4.0;
   double tempDM=0.0;
   for(int i=0;i<4;i++)
   {
      ...
      {
         if (i<3)
            retVal += tempDM * liveOMD;
         else
            retVal += tempDM * deadOMD;
      }
   }
   return retVal;
}
...
double cropRyegrass::GetAvailableN(double residualDM)
{
   ...
   double availDM = GetAvailableDM(residualDM);
	retVal = (availDM/GiveDryMatterVegTop())* NitrogenInStorage().n + NitrogenInVegTop().n;
   return retVal;
};
