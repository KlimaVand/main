...
/****************************************************************************\
\****************************************************************************/
cropChickory::cropChickory(const char * aName, const int aIndex, const base * aOwner, string cropName)
...
void cropChickory::Cut(plantItem * cutPlantMaterial, double CutHeight)
{
   ...
   if (DryMatterVegTop>0.0)
   {
      ...
      {
         fraction=0.0;
         theMessage[id]->WarningWithDisplay("cropChickory::Cut - crop to low to cut");
      }
   	else
   	{
      ...
      if (DryMatterVegTop<0 || CutN.n<0 || Nitrogen.n<0 || CutDM<0)
      	theMessage[id]->FatalError("CropChickory:: Values at harvest should not be negative");
