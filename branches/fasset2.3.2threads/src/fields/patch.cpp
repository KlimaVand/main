...
:base(aName,aIndex,aOwner)
{
	...
	soilManure					= NULL;
	cropManure					= NULL;
	birthday 					= new bsTime(theTime[id]);
	dung							= false;
	overlap						= false;
	my_resource					= NULL;
	type                    = 0;
...
void patch::HandleOp(fieldOperationFields * FOP)
{
   ...
   if (theCrops->NumOfNodes()>0)
	...
	if ((FOP->GetOperationId()==SpreadingLiquidManure)||(FOP->GetOperationId()==SpreadingSolidManure))
	{
		...
		double factor = area * theManure_manager[id]->GetadjustmentFactor();
		...
		*theManure*factor;
		theProducts[id]->SubtractProduct(theManure);  //note that manure is removed from store and quality parameters are updated here
		if (area<1E-36)
			theMessage[id]->FatalError("patch::HandleOp - patch has extremely low or zero area");
		else
			*theManure * (1.0/area);     //reconvert to tonnes per ha
		double N_added = theManure->GetAmount()*theManure->GetAllN().n*1000.0;  //kg per ha

		if (N_added<1)
			theMessage[id]->WarningWithDisplay("patch::HandleOp - very little or no N in manure");
		theOutput[id]->AddIndicator(3106,"31.06 N from spread manure","kg N",N_added*area);
		theOutput[id]->AddIndicator(1965,"19.65 Manure spread on field","t",theManure->GetAmount()*area);
		theOutput[id]->AddIndicator(4002,"40.02 C from field-applied manure","kg C",theManure->GetC_content()*theManure->GetAmount()*area*1000.0);
#ifdef TUPLE
		if (((field*)Owner)->GetWriteTuples());
		((field*)Owner)->AddTuple(theTime[id].GetString2(),N_added*area,theManure->GetAmount()*area,"FERTILISATION","MANURE",0);
#endif
		animalFertTech * aTechnique = ((animalFertTech *) theTechnics[id]->GetFieldTechnique(aFF));
      ...
      ------------------------------------------------------------------------*/
		...
		theOutput[id]->AddIndicator(3145,"31.45 NH3-N loss by spreading","kg N",NH3volat*area*1000.0);
		break;
		case 1:
		...
		case 2:
		{
			...
			if (!soilManure)
				...
				theMessage[id]->FatalError("patch::HandleOp - attempt to apply more manure, before volatilisation has ceased from the previous manure");
			...
			if (appRate>0.0)
			{
				...
				if (!tempString.find("PIG")==0)
					typeOfManure=1;
				theClimate[id]->CalcAverageClimateForPeriod(&theTime[id], 7, &aveTemperature, &aveWindspeed);         //get climate average for next 7 days
				anALFAM = new ALFAM();
	...
	if (FOP->GetOperationId()==MineralFertilizing)
	{
		...
		//     	theOutput[id]->AddIndicator("31.01 N from mineral fertilizer","kg N",N1.n*10.0*area);
		...
		theOutput[id]->AddIndicator(3148,"31.48 NH3-N loss from mineral fertiliser","kg N",NH3Volat.n*area*10.0);
#ifdef NITROSCAPE
...
#ifdef TUPLE
		if (((field*)Owner)->GetWriteTuples());
		((field*)Owner)->AddTuple(theTime[id].GetString2(),N1.n*area*10.0,0,"FERTILISATION","MINERAL_N",0);
#endif
	...
	if (FOP->GetOperationId()==Sowing || FOP->GetOperationId()==SowingBeets)
	{
		...
		for (int i=0;i<S->GetSeedNum();i++)
		{
			...
			if (aSeed->GetAmount() < 1E-20)
			{
				...
				theMessage[id]->FatalError("patch::HandleOp - no seed amount in sowing");
			}
			...
			if (RelativeDensity > 3.0 && !name.compare("SEEDRYEGRASS") && !name.compare("SEEDCLOVER"))
			{
				...
				theMessage[id]->WarningWithDisplay("patch::HandleOp - seed amount more than three times higher than standard");
			}

			if (RelativeDensity < 0.3333 && !name.compare("SEEDRYEGRASS") && !name.compare("SEEDCLOVER") && !name.compare("SEEDBEET"))
			{
				cout << aSeed->GetName() << " " << aSeed->GetAmount() << " " << aSeed->GetStandardDensity() << endl;
				theMessage[id]->WarningWithDisplay("patch::HandleOp - seed amount more than three times lower than standard");
			}
			...
			theOutput[id]->AddIndicator(3103,"31.03 N from seed","kg N",N);
			theOutput[id]->AddIndicator(4003,"40.03 C from seed","kg C",aSeed->GetAmount()*aSeed->GetC_content()*area);
#else
...
#ifdef TUPLE
			if (((field*)Owner)->GetWriteTuples());
			((field*)Owner)->AddTuple(theTime[id].GetString2(),N,aSeed->GetAmount()*area,"SOWING","SEED",0);
#endif
		}
		if (theControlParameters[id]->GetUseReductionParameters())
#ifndef NITROSCAPE
			theEcosystem->SetRUEfactor(S->GetReduction()); // Use reductions depending on previous crop and level of pesticides.
#else
		theMessage[id]->WarningWithDisplay("patch::HandleOp - GetUseReductionParameters has been called!!");
#endif
	}
...
{
#ifdef NITROSCAPE
	theMessage[id]->FatalError("function not operative within NITROSCAPE");
	return 0.0;
#else
...
void patch::Volatilise()
{
	...
	case 0:
		...
		theOutput[id]->AddIndicator(3145,"31.45 NH3-N loss by spreading","kg N",lost*area);
		}
		break;
	case 1:
		...
		if (anALFAM)
		{
			...
			if (volat==0.0)
			{
				...
				//                     theMessage[id]->FatalError("patch:Volatilise - premature termination of volatilisation (missing incorporation)");
			}
			else
			{
				soilManure->SetNH4_content((soilManure->GetNH4_content()*soilManure->GetAmount()-volat/1000.0)/soilManure->GetAmount());
				theOutput[id]->AddIndicator(3145,"31.45 NH3-N loss by spreading","kg N",volat*area);
#ifdef NITROSCAPE
...
feedItem * patch::GetFeedResource(double bite_depth_factor, double min_stubble_ht, int animalType)
{
	...
	default:
	{
		theMessage[id]->FatalError("ecosystem:GetFeedResource - not an allowed animal type");
		break;
	}
	}
...
{
	int age = theTime[id].GetDateValue()-birthday->GetDateValue();
	return age;
}
...
#endif*/
	theMessage[id]->FatalError("patch::Add ecosystem not implemented!!");
...
void patch::GiveIndicators()
{
	if (soilManure)
	{
		...
		theOutput[id]->AddStateIndicator(3161,"31.61 Total soil/soilsurface N","kg N",NInsoilManure);
		double CInsoilManure= soilManure->GetC_content()*soilManure->GetAmount() * area *1000.0; // kg C/ha
		theOutput[id]->AddStateIndicator(4060,"40.60 Total C in soil","kg C",CInsoilManure);
	}
	if (cropManure)
	{
		double NIncropManure= cropManure->GetAllN().n*cropManure->GetAmount()*1000.0; // kg N/ha
		theOutput[id]->AddStateIndicator(3161,"31.61 Total soil/soilsurface N","kg N",NIncropManure);
		double CIncropManure= cropManure->GetC_content()*cropManure->GetAmount() * area *1000.0; // kg C/ha
		theOutput[id]->AddStateIndicator(4060,"40.60 Total C in soil","kg C",CIncropManure);
	}
	GiveEcoIndicators();
}
...
void patch::Volatilise(double duration, bool kill)
{
   ...
   if (soilManure)
   {
      ...
      if (fabs(test)>0.1)
            theMessage[id]->FatalError("patch::Volatilise - balance error");
