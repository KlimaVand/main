...
void plan::HandPlanOver(cloneList<fieldOperationFields> * OP,string crop_id,int futrNum)
{
   ...
   if (NextOperation==NULL)
      theMessage[id]->Warning("plan::HandPlanOver - no operations specified for this field");  // JB changed this 7/11-02 as might not be wrong (i.e. permanent grass)
   else
   {
...
fieldOperationFields * plan::ReturnOperation(int year,int month,int day,bool update)
{
   ...
   operatindex=NxtOp->element->GetStartYear()*512
	...
	if (operatindex<=idx)
   {
   	if (NxtOp->element->GetTSum()>=theClimate[id]->GetdegreeDays())
      	doOp=true;
	...
	while (P!=NULL)
   {
  			 ...
  			 P->element->GetOperationId()==Mowing)
      {
      	if (NextOperation)
         {
            ...
            OpListP->Remove(P);
            theMessage[id]->Warning("Herbage harvest operation removed");
         }
      }
		OpListP->OneStep(P);
	}
...
bool plan::CheckTSum()
{
   ...
   if (theClimate[id]->GetdegreeDays()>TSum)
   	return true;
   else
		return false;
}
...
//      	cout << "Operation name " << P->element->GetOperationId() << endl;   //should be a warning
         ...
         theMessage[id]->FatalError("plan: end date overlap");
      }
		OpListP->OneStep(P);
   }
}
