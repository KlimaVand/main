...
/*******************************************************************
********************************************************************/
void cattleFeedingGroup::ReadParameters(commonData * infile, bool autoFeed)
{

	infile->setCritical(true);
	infile->FindSection(GetName(),GetIndex());  // find the correct place in the input file
   infile->FindItem("Alias",&Alias); // Causes a LEAK, probably in base.cpp (hard to understand - BMP) !!!
//   cout << Alias << endl;
   string namestr;
   char Indexstr[10];
...
//   if (theGroup->NumOfNodes()>0)
   {
      ...
      for(int j=0;j<theGroupDiets->NumOfNodes();j++)
      {
         ...
         {
//           if ((aDiet->GetstartDate()->GetDay()<=theTime[id].GetDay())&&
           //    (aDiet->GetstartDate()->GetMonth()<=theTime[id].GetMonth()))

            if (aDiet->GetstartDate()->GetJulianDay(aDiet->GetstartDate()->GetDay(),aDiet->GetstartDate()->GetMonth(), theTime[id].GetYear())
            								<=theTime[id].GetJulianDay())
      ...
      if (!theCurrentDiet)
            {theMessage[id]->FatalError(GetAlias(),": No cattle diet found for this date");}
   }
}
...
diet * cattleFeedingGroup::GetHousedDiet()
{
   ...
   for(int j=0;j<theGroupDiets->NumOfNodes();j++)
   {
	...
	theMessage[id]->FatalError(GetName(),": No housing diet found for this animal");
   return aDiet;
}
...
//         	cout <<
      ...
      if ((tempcattle->GetfeedPlanList()->NumOfNodes()==0) && (theCurrentDiet->GetHoused()==24.0))
      		theMessage[id]->FatalError(GetAlias(),": animals in housing but there are no feed items");
   }
}
