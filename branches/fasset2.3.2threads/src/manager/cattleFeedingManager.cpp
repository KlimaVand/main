...
/*	   if (beefPresent && dairyPresent)
  ...
  //	CalcTurnoutandHousingDates(true);
   if ((globalSettings[id]->DetailsData.getHerdDetails()==true)&& (!fHerdPerformance))
...
{
   *fHerdPerformance << theTime[id].GetYear() << "-" << theTime[id].GetMonth() << "-" << theTime[id].GetDay() << "\t";
   for(int j=0;j<feedingGroups->NumOfNodes();j++)
   {
      cattleFeedingGroup * aFeedingGroup = feedingGroups->ElementAtNumber(j);
      ...
      for (int i=1; i<=2; i++)
      {
         ...
         if (theCattle)
         {
            ...
            if (conceptionTime > startLateLactation)
               theMessage[id]->FatalError("conceptionTime > startLateLactation");
