...
//	manure_manager->Initialize(theCropRotation,theBuildings);
	theManure_manager[id]->Initialize(theCropRotation,theBuildings);
   }
...
//      cattleFeedingManager->checkPasture();
   ...
   if ((theLivestock)&&(theTime[id].GetDay()==1)&&( theTime[id].GetMonth()==3))
      theManure_manager[id]->DoAdjustment();
   if ((theLivestock)&&(theTime[id].GetDay()==1)&&( theTime[id].GetMonth()==6))
      theManure_manager[id]->ExportStoredManure();
}
...
void operationsManager::ReceivePlan(char *fileExtension, char *inputDir)
{
   ...
   data.FindSection("Crops",0);
      ...
      theProducts[id]->SetSellthePlantProducts(SellAllPlantProducts);
   ...
   if (cattleFeedingManagerInstance)  //check to see if cattleFeedingManager is present
   {
      ...
      if (milkQuota>0.0)  //need to regulate feeding to achieve a target annual yield
      {
         ...
         if (potentialMilkYield==0.0)
         	theMessage[id]->FatalError("OperationsManager: milk quota set but no milk will be produced!");
         double factor = milkQuota/potentialMilkYield;
         if (factor>1.0)
         	theMessage[id]->Warning("OperationsManager: milk quota higher than potential milk yield!");

      }
//      cattleFeedingManager->AutoFeed(conceptionTime,lengthOfLactation);
   }

	theManure_manager[id]->ReceivePlan(fileExtension);
}
...
{
   if (cattleFeedingManagerInstance && globalSettings[id]->DetailsData.getHerdDetails()==true)  //check to see if cattleFeedingManager is present
   {
