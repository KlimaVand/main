   ...
   : base (source)
{
   if (&source)
   {
       ...
       theMessage->FatalError("Attempt to copy 'organicMatter' with NULL pointer");
}
...
void organicMatter::Add(organicMatter * source, double fraction)
{
   if (&source)
   {
      ...
      for(int i=0;i<maxPools;i++)
       ...
       theMessage->FatalError("organicMatter::Add - function called with NULL pointer");
}
				   ...
				   if (data->FindItem(s,&d))
				   {
					  if (d>=-1000.0)
						 c.c_iso=GetFractionFromdelta(d)*c.c;
						  ...
						  theMessage->FatalError("delta value for carbon-13 in file is below -1000");
				   }
				   else
					  c.c_iso=c.c;
				}
				   ...
				   if (data->FindItem(s,&d))
				   {
					  if (d>=0.0)
						 c.c_iso=c.c*d/100.0;
						  ...
						  theMessage->FatalError("Percent modern for carbon-14 in file is below zero");
				   }
				   else
				   {
	;
					  ...
					  if (data->FindItem(s,&d))
					  {
						 if (d>=-1000.0)
							c.c_iso=GetFractionFromdelta(d)*c.c;
							 ...
							 theMessage->FatalError("delta value for carbon-14 in file is below -1000");
					  }
					  else
						 c.c_iso=c.c;
				   }
				   ...
				   if (data->FindItem(s,&d))
				   {
					  if (d>=0.0 && d<=1.0)
						 c.c_iso=d*c.c;
						  ...
						  theMessage->FatalError("TAG value in file must be between zero and one");
				   }
				   else
					  c.c_iso=0.0;
				}
