...
cloneList<fieldOperationFields> * staticGrassPermanent::MakeOpList(fieldOrder * PP,int JB_NO)
{
	...
	{ // Sowing
		...
		FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),9,3,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
		AddOpList(FO);
      delete aSeed;
	}
	...
	{ // Afhugning
		...
		FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetStartYear()+1,7,14,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
   ...
   // second year
	...
	{ // Afhugning
		...
		aPlantItem->Setamount(ExpectedYield_(JB_NO,0,2));
		harvestFields * FO=new harvestFields;
		FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetStartYear()+2,7,14,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
		FO->SetForageProduct(aPlantItem);
		AddOpList(FO);
		delete aPlantItem;
	}
