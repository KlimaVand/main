...
char * staticCrop::GetSeedName(char * crop_id)
{
   ...
   else if (strcmp(crop_id,"LU")==0)
         strcpy(output, "SEEDLUCERNE");
...
/****************************************************************************\
\****************************************************************************/
void staticCrop::InitCrop(bool irrigt)
{
...
//     		FO->DefineSowFieldsOp(Sowing,(char*)PP->GetSowCrop(i)->c_str(),(char*)PP->GetSowCrop(i)->c_str(),year,month,day,PP->GetArea(),0,PP->GetDistance(),2,1.0,aSeed);
     		FO->DefineSowFieldsOp(Sowing,(char*)crop_identifier.c_str(),(char*)PP->GetSowCrop(i)->c_str(),year,month,day,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
      }
		delete aSeed;
      oldyear=year;
      oldmonth=month;
   ...
   for (int i=0;i<PP->GetNumOfPesticidesDates();i++)
   {
      ...
      FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
      AddOpList(FO);
      delete aChemical1;
      delete aWater;
   }
...
//      FO->DefineHarvestFieldsOper(ForageHarvesting,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),1.0,true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      {
         FO->DefineHarvestFieldsOper(ForageHarvesting,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      else
      {
         ...
         {
            case 0:
               FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
               break;
            case 1:
               FO->DefineHarvestFieldsOper(Mowing,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
               FO->SetForageProduct(aPlantItem1,0);
               if (aPlantItem2)
                  FO->SetForageProduct(aPlantItem2,1);
               //collect silage
               FO1=new harvestFields;
               FO1->DefineHarvestFieldsOper(GatheringSilage,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date)+1,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
               break;
            case 2:
               FO->DefineHarvestFieldsOper(Mowing,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
               FO->SetForageProduct(aPlantItem1,0);
               if (aPlantItem2)
                  FO->SetForageProduct(aPlantItem2,1);
               FO1=new harvestFields;
               FO1->DefineHarvestFieldsOper(GatheringHay,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date)+1,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
               break;
            default:
...
//		         aPlantItem1->Setamount(ExpectedYield_(JB_NO,0,2));
         		harvestFields * FO=new harvestFields;
         		if (gatheringStraw)
            		FO->DefineHarvestFieldsOper(CombineHarvesting,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         		else
            		FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,PP->GetYear(date)+yearOffset,PP->GetMonth(date),PP->GetDay(date),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         		AddOpList(FO);
            }
            else
            {
      ...
      else
      {
         ...
         day=PP->GetDay(date);
         if (aPlantItem1==nullptr)
            theMessage->FatalError("staticCrop::SpecifiedPlan - about to call a method with a nullptr pointer");
//         aPlantItem1->Setamount(ExpectedYield_(JB_NO,0,2));
         harvestFields * FO=new harvestFields;
         if (gatheringStraw)                                                          
            FO->DefineHarvestFieldsOper(CombineHarvesting,crop_id,cropName,year,month,day,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         else
            FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,year,month,day,PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         ...
         { // Gathering straw
            ...
            FO1->DefineHarvestFieldsOper(GatheringBales,crop_id,cropName,bs.GetYear(),bs.GetMonth(),bs.GetDay(),PP->GetArea(),0,PP->GetDistance(),true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
...
\****************************************************************************/
cloneList<fieldOperationFields> * staticCrop::MakeOpList(fieldOrder * PP,int JB_NO)
{
