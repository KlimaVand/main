...
settings::settings()
{
	...
	version= "Farm ASSEsment Toolt version newtrunk ";
...
void settings::readParameter()
{
	...
	if(data.FindSection("Details"))
	{
		...
		data.FindItem("EcosystemDetails",&DetailsData.EcosystemDetails);
		data.FindItem("WaterIndicatDaily",&DetailsData.WaterIndicatDaily);
