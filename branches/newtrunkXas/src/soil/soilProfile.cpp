...
{
   if (soilLayerArray[NumOfSoilLayers-1]==nullptr)
...
/*
 if (flowMethod==4)
   { // ---------------- TIPPING BUCKET TEST METTE & BJï¿½RN ---------------------
      bool macropore=true;
      bool aliquots=false;
     ...
     if (testwrite)
     {
        ...
        fout << "Inhold fï¿½r gennemlï¿½b" << endl;
        for (i=0;i<TotalNumberOfLayers-1;i++)
     ...
     for (int k=0;k<aliq;k++)
     {
      ...
      if (infiltration>0)
      {
         ...
         if (testwrite)
         {
            cout << endl << "Lag 0 tilfï¿½rt " << infiltration << endl;
            fout << endl << "Lag 0 tilfï¿½rt " << infiltration << endl;
         }
      }
      ...
      // Gem ï¿½ "overskud" og fjern fra lag, hvis det kan for nï¿½ste lag, ellers flyt sï¿½ meget som muligt.
      // W og S fjernes
      ...
      if (realisedFlow[0]>0.0)
      {
         ...
         if (testwrite)
         {
            cout << "Lag 0 frafï¿½rt " << realisedFlow[0] << endl;
            fout << "Lag 0 frafï¿½rt " << realisedFlow[0] << endl;
         }
      }
      if (surplusW[0]>0.0)
      {
      ...
      for (i=1;i<TotalNumberOfLayers;i++) // Phase 1-3;
      {
         if (halfdesignated[i-1]>0.0) // Downward movement
         {
            ...
            // Tilfï¿½j vand fra ovenliggende lag
            ...
            if (testwrite)
            {
               cout << "Lag " << i << " tilfï¿½rt " << realisedFlow[i-1] << endl;
               fout << "Lag " << i << " tilfï¿½rt " << realisedFlow[i-1] << endl;
            }
         }
        // Mobilt vand i lag i - regn "overskud" ud
         ...
         if (soilLayerArray[i]->GetTotalWater()>soilLayerArray[i]->GetFieldCapacity())
         {
            ...
            if (testwrite)
            {
               cout << "Lag " << i << " frafï¿½rt " << realisedFlow[i] << endl;
               fout << "Lag " << i << " frafï¿½rt " << realisedFlow[i] << endl;
            }
            if (surplusW[i]>0.0)
            {
               nitrogen NO3,NH4;
      ...
      // Nu er ï¿½ vand flyttet i alle lag ---------------------------------------
      ...
      // Udfï¿½r 1. halvdel af bypass flow, fï¿½rst fyldes "flasken" op med evt. stï¿½ende vand fra overfladen
      if (macropore)
      {
         ...
         if (remainCap<0.5*bypassCapacity) // Er der "overskydende" vand
            for (i=0;i<TotalNumberOfLayers;i++) // Bevï¿½g nu "flasken" ned gennem alle lag
            {
               totalFlux[i]+=bucketW;
               double move=min(bucketW,maxContent[i]-soilLayerArray[i]->GetTotalWater()); // Start med at hï¿½lde af flasken
               if (move>0.0)
               {
               ...
               if (surplusW[i]>0.0) // Hvis der er overskud, fyld pï¿½ flasken
               {
                  bucketW+=surplusW[i];
                  if (testwrite)
                  {
      ...
      for (i=0;i<TotalNumberOfLayers;i++) // Phase 4 - Ekvilibrer lag i
      {
         if (!eqilibrateOnce || k==(aliq-1))
            soilLayerArray[i]->EquilibrateNitrogen(0.0); // Se pï¿½ hvad dette nul betyder - senere !!!
      }
      for (i=0;i<TotalNumberOfLayers;i++) // Phase 5
      {
         // Flyt direkte ï¿½ overskud, hvis det kan for nï¿½ste lag, ellers flyt sï¿½ meget som muligt, resten bliver i laget
         realisedFlow[i]=halfdesignated[i];
         surplusW[i]=0.0;
         if (i<(TotalNumberOfLayers-1))
         {
         ...
         if (realisedFlow[i]>0.0)
         {
            ...
            if (i<(TotalNumberOfLayers-1))
            {
               ...
               if (testwrite)
               {
                  cout << "Lag " << (i+1) << " tilfï¿½rt " << realisedFlow[i] << endl;
                  fout << "Lag " << (i+1) << " tilfï¿½rt " << realisedFlow[i] << endl;
               }
            }
         }
         if (surplusW[i]>0.0)
      ...
      // Udfï¿½r 2. halvdel af bypass flow, fï¿½rst fyldes "flasken" op med evt. stï¿½ende vand fra overfladen - hvis der er plads
      if (macropore)
      {
         bucketW=0.0;
         extraInfiltration=min(remainCap,standingWater);
...
*/
    ...
    if (macropore && (standingWater>0.0))
    {
      ...
      if (testwrite)
      {
         fout << endl << "Inhold efter gennemlï¿½b" << endl;
         for (i=0;i<TotalNumberOfLayers-1;i++)
            fout << "Lag " << i << "\t" << soilLayerArray[i]->GetTotalWater() << endl;
...
//                        if (thetaForward[i]==0.0)
               ...
               for (i=0;i<TotalNumberOfLayers;i++)
               {
                  if (i==0)
                  {
                     double b_nullptr,d_nullptr;
                     if (soluteIn>0.0)
                     {
                        soluteInFlux = soluteIn/deltaT;
                        b_nullptr=(dispersion[i]*DistPreviousLayerInv[i]-0.5*flowBack[i])/(2*(dispersion[i]*DistPreviousLayerInv[i]+0.5*flowBack[i]));
                        d_nullptr=(soluteInFlux+(dispersion[i]*DistPreviousLayerInv[i]-0.5*flowBack[i]))*soluteOld[i]/(2*(dispersion[i]*DistPreviousLayerInv[i]+0.5*flowBack[i]));
                     }
                     else
                     {
                        b_nullptr=0.0;
...
void soilProfile::SubtractTranspiration(linkList<rootStructure>* rootList)
{
   ...
   while (currentSoilLayer != nullptr)
   {
   ...
   if (rootList!=nullptr) // Perform only if plants has roots
   {
   	soilLayer * currentSoilLayer = first;
      int SoilLayerIndex = 0;
      while (currentSoilLayer != nullptr)
...
void soilProfile::UpdateTemperature(double AirTemperature, double snow)
{
   ...
   if (theControlParameters->GetSimpleTemperatureModel() || theControlParameters->GetMeasuredSoilTemperature()||theControlParameters->GetAirToSoilTemperature())
   {
   	if (theControlParameters->GetSimpleTemperatureModel())
      {
   	   ...
   	   for (int i=1;i<NumOfSoilLayers;i++)
      	{
	         ...
	         if ((soilTemperature<-50) || (soilTemperature>50))
	            theMessage->FatalError("soilProfile::UpdateTemperature - soil temperature not within realistic range");
	         depth = soilLayerArray[i]->GetStartDepth()+ soilLayerArray[i]->GetThickness()*0.5;
	         UpdateT(&soilTemperature,AirTemperature,depth,meanAirTemperature);
	      }
	      soilLayerArray[0]->PutTemperature(AirTemperature);
      }
      else
         if (theControlParameters->GetAirToSoilTemperature())
	 {
...

   {
      ...
      double waterFlux[MaxSoilLayers], waterFluxSlope[MaxSoilLayers];
   ...
   if (theControlParameters->GetSimpleTemperatureModel() || theControlParameters->GetMeasuredSoilTemperature()||theControlParameters->GetAirToSoilTemperature())
   {
   	if (theControlParameters->GetSimpleTemperatureModel())
      {
   	   ...
   	   for (int i=1;i<NumOfSoilLayers;i++)
      	{
         	soilTemperature=soilLayerArray[i]->GetTemperature();
	         if ((soilTemperature<-50) || (soilTemperature>50))
	        	 theMessage->FatalError("soilProfile::UpdateFinDiffTemperature - soil temperature not within realistic range");
	         depth = soilLayerArray[i]->GetStartDepth()+ soilLayerArray[i]->GetThickness()*0.5;
...

   {
      ...
      for (i=0;i<numberOfTotalLayers;i++)
      {
         waterFlux[i] = WaterDensity*soilLayerArray[i]->GetWaterFlux()/86400/1000;            // unit kg m-2 s-1
        	if (fabs(waterFlux[i])>0.01)
         	theMessage->WarningWithDisplay("soilProfile::UpdateFinDiffTemperature waterflux is larger than 0.01 kg m-2 s-1");
        	if (waterFlux[i]<0.0)
      ...
      while (currentTime<86400)
      {
         ...
         for(i=0; i<numberOfTotalLayers; ++i)
            if (frostChange[i] || firstTime)
            {
               ...
               if (MidVector[0]==0.0)
               	theMessage->WarningWithDisplay("soilProfile::UpdateFinDiffTemperature - Error 1 in tridag vectors");
            }
			...
			for (int i=0;i<numberOfTotalLayers;i++)
   		{
                  ...
                  ((TNew[i] > 0.0) && (currentLayer->GetIce()>0.0)))    //thawing
            {
               ...
               currentLayer->SetIceContent(max(0.0,min(currentLayer->GetTotalWater(),ice+ moreFrozen[i])));
            	...
            	moreFrozen[i]=0.0;
            if (TNew[i]>50)
               theMessage->WarningWithDisplay("soilProfile::UpdateFinDiffTemperature - soil layer temperature above realistic temperature");
            if (TNew[i]<-50)
            	theMessage->WarningWithDisplay("soilProfile::UpdateFinDiffTemperature - soil layer temperature below realistic value");
         }
      ...
      for (int i=0;i<numberOfTotalLayers;i++)
      {
         if (i>9 && i<19 && temperature[i]<minTemp[month-1] && year!=lastErrorYear)
         {
            lastErrorYear=year;
            cout << "Layer no " << i << " temperature is " << temperature[i];
            theMessage->WarningWithDisplay("soilProfile::UpdateFinDiffTemperature - soil layer temperature below expert opinion monthly threshold");
         }
         if (temperature[i]>100)
            theMessage->FatalError("soilProfile::UpdateFinDiffTemperature - soil layer temperature above boiling point");
         if (temperature[i]>50)
            theMessage->WarningWithDisplay("soilProfile::UpdateFinDiffTemperature - soil layer temperature above realistic temperature");
         if (temperature[i]<-50)
            theMessage->FatalError("soilProfile::UpdateFinDiffTemperature - soil layer temperature below possible value");
         soilLayerArray[i]->PutTemperature(temperature[i]);
      }
      if (theControlParameters->GetWriteSoilTemperature())
      {
...
double soilProfile::UpdateOrganicMatter()
{
   ...
   while (current != nullptr)
   {
      CO2Evolution += current->UpdateOrganicMatter(layerNumber,numbersOfLayers);
	  layerNumber++;
      current = current->Next();
   }
   return CO2Evolution;
...
double soilProfile::GetAmmoniumN15(double startDep,double thick)
{
   double Content=0;
   soilLayer * current = first;
   while (current != nullptr)
...
double soilProfile::GetNitrateN15(double startDep,double thick)
{
   double Content=0;
   soilLayer * current = first;
   while (current != nullptr)
...
double soilProfile::GetOrganicNitrogenN15(double startDep, double thick)
{
   double Content=0;
   soilLayer * current = first;
   while (current != nullptr)
...
void soilProfile::AddProduct(const organicProduct& product,
									  double * rootLengthList)
{
   if (rootLengthList!=nullptr)     // Perform only if a root distribution is defined
   {
...
void soilProfile::WriteCarbon(int Name,bool usedBefores)
{
 ...
 static ofstream Cconent;
	...
	if(usedBefores==false)
	{

		Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_C.xls",ios::out);
			Cconent<<theTime<<"\t";
		 while (current != nullptr)
		 {
			  double Content = 0;
	...
	{
	Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_C.xls",ios::app);
	Cconent<<theTime<<"\t";
	}
...
void soilProfile::WriteN(int Name,bool usedBefores)
{
 ...
 static ofstream Cconent;
	...
	if(usedBefores==false)
	{
		...
		Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_N.xls",ios::out);
			Cconent<<theTime<<"\t";
		 while (current != nullptr)
		 {
			  double Content = 0;
	...
	{
	Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_N.xls",ios::app);
	Cconent<<theTime<<"\t";
	}
...
void soilProfile::WriteDecompositionRate(int Name,bool usedBefores)
{
 ...
 static ofstream Cconent;
	...
	if(usedBefores==false)
	{
		...
		Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_DecompositionRate.xls",ios::out);
			Cconent<<theTime<<"\t";
		 while (current != nullptr)
		 {
			  double Content = 0;
	...
	{
	Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_DecompositionRate.xls",ios::app);
	Cconent<<theTime<<"\t";
	}
...
void soilProfile::WriteCN(int Name,bool usedBefores)
{
 ...
 static ofstream Cconent;
	...
	if(usedBefores==false)
	{
		...
		Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_CN.xls",ios::out);
			Cconent<<theTime<<"\t";
		 while (current != nullptr)
		 {
			  double Content = 0;
	...
	{
	Cconent.open(globalSettings->getOutputDirectory()+IntToName(Name)+"_CN.xls",ios::app);
	Cconent<<theTime<<"\t";
	}
...
static bool usedBefore=false;
	...
	{
		usedBefore=true;
		Cconent.open(globalSettings->getOutputDirectory()+"Ccontent.xls",ios::out);
			Cconent<<theTime<<"\t";
		 while (current != nullptr)
		 {
			  double Content = 0;
	...
	{
	Cconent.open(globalSettings->getOutputDirectory()+"Ccontent.xls",ios::app);
	Cconent<<theTime<<"\t";
	}
...
void soilProfile::changeLayer(int starLayer,int endLayer)
{
