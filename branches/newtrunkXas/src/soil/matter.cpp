...
void matter::Add(matter * source,double fraction)
{
   ...
   if (poolName!=source->poolName)
       theMessage->FatalError("matter::Add - pool names must be identical");
...
void matter::ReadParameters(commonData * &file)
{
   ...
   if (file->FindSection(Name,Index)) // Top of inheritance hierarcy
   {
      ...
      if (!(TAG==-1 || TAG==0 || TAG==1))
          theMessage->FatalError("'TAG' must be either 0 or 1 in structurefile");
      file->setCritical(true);
      // Read connections
      double checkN=0.0;
      double checkC=0.0;
...
double matter::PotentialNitrogenImmobilisation()
{
   ...
   if (retVal>1E-12)
       theMessage->FatalError("matter::PotentialNitrogenImmobilisation - returns positive value");
   return retVal;
}
...
void matter::print(int current,int Last)
{
	cout<<endl;
}
...
void matter::FinalDecayIfHungry(double ratio)
{
   if (ratio>1.0 || ratio<0.0)
       theMessage->FatalError("matter::FinalDecayIfHungry - 'ratio' has illegal value");
   if(Hungry() && intermed.c>0.0)
   {
      carbon=carbon+intermed*(1.0-ratio);
      Export(intermed*ratio);
      intermed.SetAll(0.0,0.0,0.0,0.0);
...
void matter::Decay14C(double HalfTime)
{
   double f=exp(log(0.5)/HalfTime);
   cnMatter decay;
   decay.c_iso=carbon.c_iso*(1-f);
   carbon.c_iso=carbon.c_iso*f;
...
if (parmName=="PM" && Isotope==2)
   ...
   {
      if (connections!=2)
          theMessage->FatalError("matter::SetParameterByName - can only set fraction by two connections");
      if (val>1.0)
         val=1.0;
      double f1=val;
      double f2=1.0-f1;
...
/****************************************************************************\
Streams detailed information about the current pool
\****************************************************************************/
void matter::WriteSelf(iostream * s)
{
   *s
   ...
   *s
   << endl
   << "Decomposition rate      " << decompositionRate << endl;
   if (useClayEffect)
