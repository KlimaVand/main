...
void addedMatter::Add(matter * source,double fraction)
{
   ...
   while (hi>lo)
   {
      ...
      if (lo<numOfSubPools)
         while (lo<numOfSubPools && addPoolList[lo]->GetCarbonC()>0)
         {
            lo++;
            if (lo>=numOfSubPools)
               break;
      ...
      if (hi>=0)
         while (hi>=0 && addPoolList[hi]->GetCarbonC()<1E-36)
         {
            hi--;
            if (hi<0)
               break;
...
void addedMatter::Decay14C(double HalfTime)
{

   if (numOfSubPools>0)
      for(int i=0;i<numOfSubPools;i++)
         addPoolList[i]->Decay14C(HalfTime);
...
void addedMatter::FinalDecayIfHungry(double ratio)
{
   cnMatter outflux;
   ...
   {
      for(int i=0;i<numOfSubPools;i++)
         addPoolList[i]->FinalDecayIfHungryAOM(ratio,outflux);
      Export(outflux);
...
double addedMatter::GetCarbonC()
{
   ...
   if (numOfSubPools>0)
      for(int i=0;i<numOfSubPools;i++)
         retVal=retVal+addPoolList[i]->GetCarbonC();
   return retVal;
}
...
double addedMatter::GetNitrogenN()
{
   ...
   if (numOfSubPools>0)
      for(int i=0;i<numOfSubPools;i++)
         retVal=retVal+addPoolList[i]->GetNitrogenN();
   return retVal;
}
double addedMatter::GetNitrogenN15()
{
   ...
   if (numOfSubPools>0)
      for(int i=0;i<numOfSubPools;i++)
         retVal=retVal+addPoolList[i]->GetNitrogenN15();
   return retVal;
}
/****************************************************************************\
...
/****************************************************************************\
Writes debug info about subpools to screen
\****************************************************************************/
void addedMatter::WriteSubPoolsToScreen()
{
   for(int i=0;i<numOfSubPools;i++)
      cout << i << " " << addPoolList[i]->GetCarbon().c << " " << addPoolList[i]->IsDerived() << endl;
   cout << endl;
...
\****************************************************************************/
void addedMatter::WriteSelf(iostream * s)
{
   *s
   << "Type of pool            " << Name << endl
   << "Name of pool            " << poolName << endl
   << "Carbon content          " << carbon.c;
   if (Isotope>0)
      *s << " (" << ((organicMatter*)Owner)->GetIsotopeValue(carbon) << " "
         << ((organicMatter*)Owner)->GetIsotopeName() << ")";
   *s << endl;
   if (useClayEffect)
      *s << "Clay effect is          ON" << endl;;
   if (expDecay)
      *s << "Exponential decay is    ON" << endl;
   if (TAG==0)
      *s << "All outgoing carbon is  UNTAGGED" << endl;
   if (TAG==1)
      *s << "All outgoing carbon is  TAGGED" << endl;
   if (connections>0)
   {
      *s
      << "Number of connections " << connections << ":" << endl;
      for(int i=0;i<connections;i++)
      {
         *s << i << ": " <<  connectionList[i].connectname
         << ". C allocation fraction: " << connectionList[i].Cfraction
         << endl
         << i << ": " <<  connectionList[i].connectname
         << ". N allocation fraction: " << connectionList[i].Nfraction
         << endl;
      }
   }
   if (numOfSubPools>0)
   {
      *s
      << "Number of sub-pools " << numOfSubPools << ":" << endl;
   	for(int i=0;i<numOfSubPools;i++)
         addPoolList[i]->WriteSelf(s);
   }
   else
      *s << endl;
}
...
void addedMatter::print(int current,int Last)
{
	static ofstream addedC;
		static bool usedBefore=false;
		string name =globalSettings->getOutputDirectory()+poolNameChar+".xls";
		if(usedBefore==false)
		{
			usedBefore=true;

			addedC.open(name.c_str(),ios::out);
			addedC<<theTime<<'\t';

		}
		else
		{
		addedC.open(name.c_str(),ios::app);
		}
		if(current==0)
		{
			addedC<<theTime<<'\t';
		}
		addedC<<numOfSubPools<<'\t';
		if(current==(Last-1))
			addedC<<endl;
