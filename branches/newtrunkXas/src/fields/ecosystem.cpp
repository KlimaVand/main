...
//header units
   ...
   if(globalSettings->DetailsData.getEcosystemDetails()==true)
   {
...
//          const crop* aCrop = theCrops->ElementAtNumber(j);  //Eclipse complains about the 'const' NJH, feb 2009
   ...
   */
   ...
   WaterBudget.AddOutput(aSoil->GetDrainage(aSoil->GetMaximumDepth())+aSoil->GetSoilEvaporation() +aSoil->GettotalTranspiration()+aSoil->GetRunOff());
...
void ecosystem::GiveIndicators()
{
  ...
  // theOutput->AddStateIndicator(101,"01.01 Total area","ha",area);
	...
	theOutput->AddIndicator(7006,"70.06 Transpiration","cubic meter",aSoil->GettotalTranspiration() * area * 10.0);
	...
	theOutput->AddIndicator(7012,"70.12 Evapotranspiration","cubic meter",(aSoil->GettotalTranspiration() + snowEvap + canopyEvaporation +
					aSoil->GetSoilEvaporation())*area*10);
	theOutput->AddIndicator(7013,"70.13 Drainage from leaching depth","cubic meter",aSoil->GetDrainage(leachingDepth)*area*10);
	theOutput->AddIndicator(7014,"70.14 Drainage from balance depth","cubic meter",aSoil->GetDrainage(balanceDepth)*area*10);
	...
	cloneList<crop>::PS aCropElement;
      ...
      for (int i=0;i<theCrops->NumOfNodes();i++)
      {
		 ...
		 if(name.compare("NoCrop")!=0)
			 found=true;

      }
	  if(found==true)
	  {
		  	theOutput->AddIndicator(7017,"70.17 Evapotranspiration with plant","cubic meter",(aSoil->GettotalTranspiration() + snowEvap + canopyEvaporation +
					aSoil->GetSoilEvaporation())*area*10);
	  }
	//water state variables - status at this time
	theOutput->AddStateIndicator(7015,"70.15 Water in field to balancedepth","cubic meter",(snowContent+canopyWaterAmount+aSoil->GetWater(0,balanceDepth)+aSoil->GetSurfaceWater())*area*10);
	theOutput->AddStateIndicator(7016,"70.16 Water on soil surface","cubic meter",aSoil->GetSurfaceWater()*area*10);  
	theOutput->AddStateIndicator(7041,"70.41 Water in field to leaching depth","cubic meter",(snowContent+canopyWaterAmount+aSoil->GetWater(0,leachingDepth)+aSoil->GetSurfaceWater())*area*10);
	theOutput->AddStateIndicator(7043,"70.43 Snow on surface","cubic metre",snowContent*area*10);
	theOutput->AddStateIndicator(7020,"70.20 Water in soil","cubic meter",canopyWaterAmount*area*10);
  	theOutput->AddStateIndicator(7020,"70.20 Water in total ecosystem","cubic meter",(snowContent+canopyWaterAmount+aSoil->GetWater(0,balanceDepth)+aSoil->GetSurfaceWater())*area*10);
   ...
   theOutput->AddIndicator(3299,"32.99 N leaching","kg N",aSoil->GetNitrateLeaching(leachingDepth).n*area*10);
   ...
   theOutput->AddIndicator(3299,"32.99 N leaching","kg N",aSoil->GetAmmoniumLeaching(leachingDepth).n*area*10);
   theOutput->AddIndicator(3140,"31.40 Nitrate N leached","kg N",aSoil->GetNitrateLeaching(balanceDepth).n*area*10);
   theOutput->AddIndicator(3299,"32.99 N leaching","kg N",aSoil->GetNitrateLeaching(balanceDepth).n*area*10);
	theOutput->AddIndicator(3141,"31.41 Ammonium N leached","kg N",aSoil->GetAmmoniumLeaching(balanceDepth).n*area*10);
	theOutput->AddIndicator(3299,"32.99 N leaching","kg N",aSoil->GetAmmoniumLeaching(balanceDepth).n*area*10);
...
void ecosystem::AddExpectedYieldToIndicators(const char * cid, double y)
{
   ...
   if (strcmp(cid,"G1")==0 || strcmp(cid,"G2")==0) theOutput->AddIndicator(1825,"18.25 Exp. Grass ley yield (harv. bef. 1/8)","ha",y);
    if (strcmp(cid,"HV")==0)
      theOutput->AddIndicator(1826,"18.26 Exp. Dyerswoad yield (harv. bef. 1/8)","ha",y);
   if (strcmp(cid,"TR")==0)
      theOutput->AddIndicator(1827 ,"18.27 Exp. Triticale yield (harv. bef. 1/8)","ha",y);
   if (strcmp(cid,"FB")==0)
      theOutput->AddIndicator(1828,"18.28 Exp. FabaBean yield (harv. bef. 1/8)","ha",y);
   if (strcmp(cid,"CR")==0)
      theOutput->AddIndicator(1829,"18.29 Exp. CrimsonClover yield (harv. bef. 1/8)","ha",y);
}*/
/****************************************************************************\
...
void ecosystem::TerminateCrop(crop * aCrop)
{
   ...
   if (Straw)
   {
      Straw->Setamount(0.01*Straw->GetAmount()); // Convert from g/m2 to t/ha
      aSoil->AddDecomposable(Straw);
	  nitrogen tmp=Straw->GetAllN()*Straw->GetAmount()*area*1000.0;
      theOutput->AddIndicator(3221,"32.21 N from straw incorporated into soil","kg N",tmp.n);

      theOutput->AddIndicator(4081,"40.81 C from straw incorporated into soil","kg C",Straw->GetAmount()*Straw->GetC_content()*area*1000.0);
      delete Straw;
      Straw = nullptr;
...
/*void ecosystem::Add(ecosystem* addPatch, double fraction)
{
   ...
   canopyWaterAmount = (1.0-fraction)*canopyWaterAmount+fraction*addPatch->canopyWaterAmount;
      ...
      for (int i=0;i<theCrops->NumOfNodes();i++)
      {
         ...
         if (!c->GetTerminated())
         {
            ...
            */
            ...
            plantItem* Straw = nullptr;  // used to store information on material harvested in straw or leaves and stem. Will be passed to products.cpp
...
void ecosystem::Volatilise(double duration, bool kill)
{
   ...
   theOutput->AddIndicator(3145"31.45 Ammonia-N loss by spreading","kg N",actualNH4_loss * area *1000.0);
}
