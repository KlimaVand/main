...
//   CurrentNPct          = 10.0;
//   rootMatter = nullptr;                // Transfered to soil
...
//	FractionInStorage    =acrop.FractionInStorage;            // Fraction of dry matter constituting of storage organs.
   ...
   if (acrop.rootMatter==nullptr)
   	rootMatter	= nullptr;
   else
      rootMatter        =	new decomposable(*acrop.rootMatter);
   if (acrop.topMatter==nullptr)
      topMatter         = nullptr;
   else
...
void crop::NitrogenAndTranspiration(nitrogen aNitrogenUptake,double aTranspirationRatio)
{
   ...
   if (aNitrogenUptake.n>1e-15)
   {
      ...
      if(Nitrogen.n<=0)
      	 theMessage->FatalError("crop::NitrogenAndTranspiration - nitrogen in plant is zero or below after uptake");
