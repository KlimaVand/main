...
cloneList<fieldOperationFields> * staticBarleyPea::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
   	{ // Ploughing
		...
		if ((PP->GetK()>0) || (PP->GetP()>0)) { // Spread mineral fertilizer
			...
			FO->DefineMineralFertFieldsOp(MineralFertilizing,cropName,cropName,PP->GetStartYear()+1,3,29,PP->GetArea(),0,PP->GetDistance(),aFertilizer1,aFertilizer2,nullptr);
			AddOpList(FO);
			delete aFertilizer1;
			delete aFertilizer2;
		}
	   ...
	   if (PP->GetFluidManureSpring()>0) { // Apply FLUID manure
		...
		{ // Sowing
         ...
         seed * aSeed2  = new seed;
			...
			aSeed2->Setamount(83);
	      FO->DefineSowFieldsOp(Sowing,cropName,cropName,PP->GetStartYear()+1,4,1,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),0,aSeed1);
		...
		if (PP->GetPVStrat()==0) { // Weed harrowing
			soilTreatFields * FO=new soilTreatFields;
			FO->DefineOp(WeedHarrowing,cropName,cropName,PP->GetStartYear()+1,4,15,PP->GetArea(),0,PP->GetDistance());
			AddOpList(FO);
		}
		if (PP->GetPVStrat()==0) { // Weed harrowing
			soilTreatFields * FO=new soilTreatFields;
			FO->DefineOp(WeedHarrowing,cropName,cropName,PP->GetStartYear()+1,4,29,PP->GetArea(),0,PP->GetDistance());
			AddOpList(FO);
		}
      ...
      { // Irrigating 30 mm on sandy soil
		...
		{ // Combine harvesting
			...
			FO->DefineHarvestFieldsOper(CombineHarvestingShred,cropName,cropName,PP->GetStartYear()+1,8,15,PP->GetArea(),0,PP->GetDistance(),0,false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
...
int staticBarleyPea::NumOfProtectStrat_()
{
	return 2; // No herbicides possible
}

int staticBarleyPea::NitrogenLevels_()
{
...
double staticBarleyPea::PhosphorAmount_()
{
	return 25;
}

double staticBarleyPea::PotassiumAmount_()
{
	return 50;
}

double staticBarleyPea::ExtraNRelease_()
{
	return 35; // check ????
}
