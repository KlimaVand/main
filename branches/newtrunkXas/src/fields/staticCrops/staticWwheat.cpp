...
void staticWwheat::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
...
cloneList<fieldOperationFields> * staticWwheat::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      ...
      if (JB_NO>4)
      { // 2nd seed bed harrowing (only clayish soils)
         soilTreatFields * FO=new soilTreatFields;
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),9,15,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),0,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
      ...
      if ((N>0) || (P>0) || (K>0))
      { // Apply mineral fertilizer for the last time
         fertilizer * aFertilizer1=new fertilizer;
         aFertilizer1->Setname("MINERAL-N");
         theProducts->GiveProductInformation(aFertilizer1);
      ...
      if (PP->GetPVStrat()==2)
      { // Spray insecticide + herbicide 3rd time
         chemical * aChemical1=new chemical;
         aChemical1->Setname("TiltTop");
         theProducts->GiveProductInformation(aChemical1);
         aChemical1->SetByTFI(0.5);
      ...
      { // Use harvester
         ...
         if (gatheringStraw)
            FO->DefineHarvestFieldsOper(CombineHarvesting,crop_id,cropName,PP->GetStartYear()+1,8,21,PP->GetArea(),0,PP->GetDistance(),0,false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         else
            FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,PP->GetStartYear()+1,8,21,PP->GetArea(),0,PP->GetDistance(),0,false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      { // Gather straw
         ...
         theProducts->GiveProductInformation(aPlantItem2);
         aPlantItem2->Setamount(ExpectedStrawYield_(JB_NO,PP->GetNLevel(),PP->GetPVStrat()));
         FO->DefineHarvestFieldsOper(GatheringBales,crop_id,cropName,PP->GetStartYear()+1,8,21,PP->GetArea(),0,PP->GetDistance(),0,false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
...
int staticWwheat::NumOfProtectStrat_()
{
	return Num_of_pr_str;
}

int staticWwheat::NitrogenLevels_()
{
	return Nitrogen_levels;
}

double staticWwheat::NitrogenAmount(int JB_NO,double free_N,int N_level)
{
	double N=Nopt(JB_NO);
	switch (N_level) {
		case 0: N=0;break;
...
double staticWwheat::ExpectedYield_(int JB_NO,int N_level,int PV_strat)
{
...
double staticWwheat::PhosphorAmount_()
{
	return 25;
}

double staticWwheat::PotassiumAmount_()
{
	return 60;
}

double staticWwheat::ExtraNRelease_()
{
	return 0;
}

int staticWwheat::SlyrryPossibleThisPeriod(period * p)
{
...
double staticWwheat::ExpectedStrawYield_(int JB_NO,int N_level,int PV_strat)
{
   if (gatheringStraw)
