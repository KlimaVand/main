...
/****************************************************************************\
\****************************************************************************/
void staticSwheat::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
...
/****************************************************************************\
\****************************************************************************/
void staticSwheat::BasicOpList(fieldOrder * PP,int JB_NO,double reduction,char * crop_id)
{
	...
	{ // Sowing
		...
		FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear()+1,4,10,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),reduction,aSeed);
		AddOpList(FO);
		delete aSeed;
	}
/*	if (PP->GetPVStrat()==2) { // Spraying herbicide     ??????       Hvordan sprøjtes vårhvede?????
	...
	if ((PP->GetN()>0) || (PP->GetP()>0) || (PP->GetK()>0)) { // Apply mineral fertilizer       dato korrekt?????
		...
		FO->DefineMineralFertFieldsOp(MineralFertilizing,crop_id,cropName,PP->GetStartYear()+1,4,30,PP->GetArea(),0,PP->GetDistance(),aFertilizer1,aFertilizer2,aFertilizer3);
		N-=Napply;
		P-=PP->GetP();
		K-=PP->GetK();
		AddOpList(FO);
	...
	if ((fHUG>0) && (PP->GetSolidManure()>0)) { // Apply FLUID manure
      ...
      if (PP->GetFluidHugIsSlurry())
		...
		FO->DefineAnimalFertFieldsOp(SpreadingLiquidManure,crop_id,cropName,PP->GetStartYear()+1,4,30,PP->GetArea(),0,PP->GetDistance(),aManure);
		fHUG=0;
		AddOpList(FO);
		delete aManure;
	}
	...
	if (PP->GetPVStrat()==1) { // Spray herbicide
		...
		theProducts->GiveProductInformation(aWater);
		aWater->Setamount(400); // ???
		FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,4,11,PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
		AddOpList(FO);
		delete aChemical1;
      delete aWater;
	}
	...
	if (PP->GetPVStrat()==2) { // Spray fungicide
		...
		theProducts->GiveProductInformation(aWater);
		aWater->Setamount(400); // ???
		FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,5,15,PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
		AddOpList(FO);
		delete aChemical1;
      delete aWater;
	}
	...
	if (PP->GetPVStrat()>0) { // Spray fungicides - 2nd time
		...
		aWater->Setamount(400); // ???
		plantCareFields * FO=new plantCareFields;
		FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,6,15,PP->GetArea(),0,PP->GetDistance(),aChemical1,nullptr,nullptr,aWater);
		AddOpList(FO);
		delete aChemical1;
		delete aWater;
	}
	if (PP->GetPVStrat()==2) { // Spray insecticide + herbicide 3rd time
		...
		theProducts->GiveProductInformation(aWater);
		aWater->Setamount(400); // ???
		FO->DefinePlantCareFieldsOp(Spraying,crop_id,cropName,PP->GetStartYear()+1,7,5,PP->GetArea(),0,PP->GetDistance(),aChemical1,aChemical2,nullptr,aWater);
		AddOpList(FO);
		delete aChemical1;
		delete aChemical2;
		delete aWater;
	...
	{ // Stubharrow
		soilTreatFields * FO=new soilTreatFields;
		FO->DefineOp(StubHarrowing,crop_id,cropName,PP->GetStartYear()+1,8,23,PP->GetArea(),0,PP->GetDistance());
		AddOpList(FO);
   }
   ...
   FO1->DefineHarvestFieldsOper(GatheringBales,crop_id,cropName,PP->GetStartYear()+1,8,22,PP->GetArea(),0,PP->GetDistance(),reduction,false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
...
/****************************************************************************\
\****************************************************************************/
cloneList<fieldOperationFields> * staticSwheat::MakeOpList(fieldOrder * PP,int JB_NO)
{
...
/****************************************************************************\
\****************************************************************************/
int staticSwheat::NumOfProtectStrat_()
{
	return Num_of_pr_str;
}

/****************************************************************************\
\****************************************************************************/
int staticSwheat::NitrogenLevels_()
{
...
/****************************************************************************\
\****************************************************************************/
double staticSwheat::NitrogenAmount(int JB_NO,double free_N,int N_level)
{
	double N=Nopt(JB_NO);
	switch (N_level) {
		case 0: N=0;break;
...
/****************************************************************************\
\****************************************************************************/
double staticSwheat::ExpectedYield_(int JB_NO,int N_level,int PV_strat)
{
...
/****************************************************************************\
\****************************************************************************/
double staticSwheat::PhosphorAmount_()
{
	return 25;
}
...
/****************************************************************************\
\****************************************************************************/
double staticSwheat::PotassiumAmount_()
{
	return 60;
}
...
/****************************************************************************\
\****************************************************************************/
double staticSwheat::ExtraNRelease_()
{
	return 0;
}
...
/****************************************************************************\
\****************************************************************************/
int staticSwheat::SlyrryPossibleThisPeriod(period * p)
{
   period * p1 =new period(3,20,4,20);
