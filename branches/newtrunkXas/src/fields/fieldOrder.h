...
public:
   fieldOrder(const char * aName, const int, const base * aOwner = nullptr);
   ~fieldOrder()
   {
#ifdef TUPLE
      if (tuple_file_open)
   ...
   void SetPVStrat(int pv){pv_strat=pv;};
   void SetNLevel(int nl){N_level=nl;};
   ...
   int GetPVStrat(){return pv_strat;};
   int GetNLevel(){return N_level;};
   ...
   double * GetTillageMinDepth(int nr){return TillageMinDepth.ElementAtNumber(nr);};
//   string * GetTillagePlantItem(int nr){return PlantItem.ElementAtNumber(nr);};
   ...
   double GetNH4_fraction(int nr) {return *NH4_fractions.ElementAtNumber(nr);};
   ...
   void AddFluidManureSpring(double f){fluid_hug_spring+=f;};
