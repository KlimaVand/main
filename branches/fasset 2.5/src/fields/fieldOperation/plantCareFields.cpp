                                  ...
                                  bool                  update)
{
   if (!update && ((mode==hoursCombine) || (mode==hoursAnimalFert)
      || (mode==hoursMineralFert) || (mode==varCostAnimalFert)
            ...
            // All neccesary machines present at farm
	         ...
	         if(update) {
               ...
               plantCareTechInstance->UpdateProducts(true);
                theOutput->AddIndicator(708,"07.08 Cost of machine contractors","Dkr",-plantCareTechInstance->GetMachinePoolPrice()*area);
				   delete hiredService;
            }
	         else
