...
/****************************************************************************\
\****************************************************************************/
void staticCrop::Show(fieldOperationFields *PP)
{
	cout  << PP->GetDay() << "." << PP->GetMonth() << ".   " << PP->GetOperationId() << " " << " " << "\n";
}
...
char * staticCrop::GetSeedName(char * crop_id)
{
   ...
   else if (strcmp(crop_id,"TR")==0)
      strcpy(output, "SEEDTriticale");  
    else if (strcmp(crop_id,"FB")==0)
      strcpy(output, "SEEDFabaBean");
    else if (strcmp(crop_id,"CR")==0)
      strcpy(output, "SEEDCrimsonClover");  
...
//     		FO->DefineSowFieldsOp(Sowing,(char*)PP->GetSowCrop(i)->c_str(),(char*)PP->GetSowCrop(i)->c_str(),year,month,day,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),1.0,aSeed);
     		FO->DefineSowFieldsOp(Sowing,(char*)crop_identifier.c_str(),(char*)PP->GetSowCrop(i)->c_str(),year,month,day,PP->GetArea(),0,PP->GetDistance(),PP->GetPVStrat(),1.0,aSeed);
      }
		delete aSeed;
      oldyear=year;
      oldmonth=month;
...
Parameters values per ha.
\****************************************************************************/
void staticCrop::ShowOpList(cloneList<fieldOperationFields> * OP)
{
	cout << "List of field operations" << endl;
	OP->ForEach(Show);
}

/****************************************************************************\
\****************************************************************************/
int staticCrop::NumOfProtectStrat_()
{
...
/****************************************************************************\
\****************************************************************************/
int staticCrop::NitrogenLevels_()
{
...
/****************************************************************************\
\****************************************************************************/
double staticCrop::PhosphorAmount_()
{
	return 0;
}
...
/****************************************************************************\
\****************************************************************************/
double staticCrop::PotassiumAmount_()
{
	return 0;
}

/****************************************************************************\
The amount of nitrogen 'left' for the next crop.
Cereals give zero per definition.
\****************************************************************************/
 double staticCrop::ExtraNRelease_()
{
	// Creals 0
	// rape 25
	// clovergrass 30-40
	// beets 10
	// fallow 0
	// After: "H�ndbog for plantedyrkning 1995".
	return 0;
}
