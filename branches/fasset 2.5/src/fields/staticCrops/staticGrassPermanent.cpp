...
cloneList<fieldOperationFields> * staticGrassPermanent::MakeOpList(fieldOrder * PP,int JB_NO)
{
	...
	{ // Apply mineral fertilizer
		...
		FO->DefineMineralFertFieldsOp(MineralFertilizing,crop_id,cropName,PP->GetStartYear()+1,4,3,PP->GetArea(),0,PP->GetDistance(),aFertilizer1,nullptr,nullptr);
		AddOpList(FO);
		delete aFertilizer1;
	}
	{ // Afhugning
		...
		FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetStartYear()+1,7,14,PP->GetArea(),0,PP->GetDistance(),0,true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
   ...
   // second year
	...
	{ // Apply mineral fertilizer
		...
		aFertilizer1->Setamount(PP->GetN());
		mineralFertFields * FO=new mineralFertFields;
		FO->DefineMineralFertFieldsOp(MineralFertilizing,crop_id,cropName,PP->GetStartYear()+2,4,3,PP->GetArea(),0,PP->GetDistance(),aFertilizer1,nullptr,nullptr);
		AddOpList(FO);
		delete aFertilizer1;
	}
	{ // Afhugning
		...
		aPlantItem->Setamount(ExpectedYield_(JB_NO,PP->GetNLevel(),PP->GetPVStrat()));
		harvestFields * FO=new harvestFields;
		FO->DefineHarvestFieldsOper(CutSetAside,crop_id,cropName,PP->GetStartYear()+2,7,14,PP->GetArea(),0,PP->GetDistance(),0,true,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
		FO->SetForageProduct(aPlantItem);
		AddOpList(FO);
		delete aPlantItem;
	}
...
int staticGrassPermanent::NumOfProtectStrat_()
{
	return 1; 
}

int staticGrassPermanent::NitrogenLevels_()
{
	return 1;
}

int staticGrassPermanent::FallowCrop()
{
	return 1;
}

double staticGrassPermanent::ExtraNRelease_()
{
	return 0.0;
}
