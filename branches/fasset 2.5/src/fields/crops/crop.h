...
crop(const char * aName, const int aIndex, const base * aOwner, string cropName);
   ...
   virtual double GetTotalStrawN(){return 0;};
   virtual int GetTotStorageN(){return 0;};

   virtual string GetPlantItemName(){return PlantItemName;};
   ...
   */
   ...
   virtual bool ReadyForRemoval() {return terminated && (!(rootMatter->GetAmount()>0.0)) && (nullptr==topMatter);};
...
//   virtual void CutPlant(double CuttingHeight,double& DM, double& N, double& digest) {};
   ...
   virtual feedItem *  GetAvailability(double cutHeight, int animalType) {return nullptr;}
   ...
   virtual void SetCropName(char * aName){CropName = aName;};
	...
	virtual double GetAvailableN(double residualDM){theMessage->FatalError("Crop:;GetAvailableN - this crop cannot be grazed"); return 0.0;};
	 virtual string GetStrawItemName(){return StrawItemName;};
