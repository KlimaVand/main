...
IndicatorOutput::IndicatorOutput()
{
   ...
   makingList=false;
	 ...
	 {
	   waterBalance.open(globalSettings->getOutputDirectory()+"waterBalance.xlsx",ios::out);
	   waterBalance<<"date"<<"\t";
...
void IndicatorOutput::InitializeIndicators()
{
   makingList=true;
	...
	// Headlines:
   /*AddIndicator(  0,  "00.00	","",0);
   AddIndicator(  100, "01.00    -Farm characteristics-","",0);
   ...
   AddIndicator(   632,  "06.32 Costs feed and bedding","Dkr",0);
	...
	AddIndicator(  654,   "06.54 Beef cattle calves bought","Dkr",0);
	AddIndicator(  655,   "06.55 Mature beef cattle bought","Dkr",0);
...
//   AddIndicator("32.23 Plant top res. N incorp. in org. matt.","kg N",0);
   ...
   // Water
	...
	AddIndicator(7043,"70.23 Snow on surface","cubic meter",0);

	AddIndicator(7040,"70.40 Drainage from leaching depth","cubic meter",0);
...
double IndicatorOutput::GetAmount(int index, string aName)
{
   ...
   if ( indicatorList[index]!=nullptr)
...
double IndicatorOutput::GetDailyAmount(int index, string aName)
{
...
//	double minSoilN = GetAmount("32.61 Mineral N in soil");
 ...
 //  double orgSoilN = GetAmount("32.60 Organic N in soil");
   ...
   {
#ifndef STRICT_BALANCES
      theMessage->WarningWithDisplay("IndicatorOutput::CheckBalance - error in st ored product N balance",balanceProductStore);
#else
