...
void bioMatter::FinalDecayIfHungry(double ratio)
{
   if (ratio>1.0 || ratio<0.0)
...
TEST - remove later !!!!!!!
\****************************************************************************/
void bioMatter::TestCNratio()
{
   if (carbon.c>0 && carbon.n<=0)
   {
      cout << "Name path: " << GetLongName() << endl;
      cout << theTime << endl;
      theMessage->FatalError("bioMatter::TestCNratio() - C present but no N");
   }
   else
   {
      double cnrat=carbon.c/carbon.n;
      if (fabs(cnrat-cnRatio)>0.01)
         theMessage->FatalError("Illegal C/N ratio");
   }
}
...
\****************************************************************************/
void bioMatter::WriteSelf(iostream * s)
...

{
   ...
   *s
   << endl
   << "Maintenance rate        " << maintenance << endl
   << "Death rate              " << deathRate << endl
   << "Utilization efficiency  " << utilizationEfficiency << endl;
   if (useClayEffect)
