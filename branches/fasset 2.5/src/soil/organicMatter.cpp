...
filename   input file name
\****************************************************************************/
void organicMatter::InitAddTypes(string filename)
{
	commonData * data=new commonData();
	if (!data->readFile(filename))
      theMessage->FatalError("Error in opening file ",filename);
   int n,first;
   data->getSectionsNumbers("OrganicProduct",&first,&n);
   for(int i=first;i<(first+n);i++)
	{
		organicProduct * o=new organicProduct("OrganicProduct",i,this);
      o->SetStepFactor(stepFactor);
      o->ReadParameters(data,useNCfractionation);
      prodList[i]=o;
      numberOfProducts++;
	}
...
{
   if (i>4)
      theMessage->FatalError("Illegal C isotope type specified. Legal range 0-4.");
   Isotope=i;
}
...
{
	if (i>1)
      theMessage->FatalError("Illegal N isotope type specified. Legal range 0-1.");
   NIsotope=i;
...
if (useSpringob) // Adjust size of "IOM" according to C/N ratio - not working properly!!!
      {
         ...
         if (before.c>0)
         {
            ...
            if (ExistsPool(10))
			{
				nom=GetCarbonInPool(10);
               IOMname=10;
			}
            if (IOMname==-1)
               theMessage->FatalError("container::SetLocalParameterByName - could not find relevant inert pool for setting 'IOMFactor'");
            int humName=-1;
            name="HUM";
            if (ExistsPool(11))
			{
               humName=11;
			   nom=GetCarbonInPool(11);
			}
            name="NOM";
            if (ExistsPool(5))
			{
               humName=5;
			   nom=GetCarbonInPool(5);
			}
            if (humName=-1)
               theMessage->FatalError("Could not find relevant 'humus' pool for setting 'IOMFactor'");
