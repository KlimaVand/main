...
pF_Curve_Implementation::pF_Curve_Implementation(char * Pname, int Intpol)
  : base(Pname)
{
   InitVars(Intpol);
...
void pF_Curve_Implementation::SetIntpol(int Intpol)
{
 IntpolMethod = Intpol;
}
/****************************************************************************\
...
void pF_Curve_Implementation::AddWaterItem(pF_WaterData data)
{
 ...
 if (Ok)
 {
  if (firstW==nullptr || data.relativeWaterContent<firstW->relativeWaterContent)
  {
	newlink->next = firstW;
	firstW = newlink;
  }
...
void pF_Curve_Implementation::AddConductionItem(pF_ConductionData data)
{
 ...
 if (Ok)
 {
  if (firstC==nullptr || data.pF<firstC->pF)
  {
	newlink->next = firstC;
	firstC = newlink;
  }
  else
  {
	p = firstC;
	l = p;
	while (p->next!=nullptr && data.pF>p->pF)
	{
	 l = p;
	 p = p->next;
	}
...
void pF_Curve_Implementation::AddExternalConductionData(double pFValue, double logCond)
{
 ExternalConduc[numExternalConduc].pF=pFValue;
 ExternalConduc[numExternalConduc].logConduc=logCond;
 numExternalConduc++;
}
...
/****************************************************************************\
\****************************************************************************/
void pF_Curve_Implementation::ReadParameters(commonData * data,int index,double dryBulkDensity,double clayContent,double siltContent,double carbon, bool TopSoil)
{
 ...
 if (num>0)
 {
  for (int i=first;i<(first+num);i++)
   if (FindSection(section,i))
   {
 	 double conduc = 0.0;
	 double pFValue = -2;
	 data->FindItem("Conductivity",&conduc);
	 data->FindItem("PFValue",&pFValue);
 ...
 */
   ...
   else
   {
      ...
      if ((first!=f1) || (num!=n1))
    	  theMessage->FatalError("pF_Curve_Implementation::ReadParameters-error in numbering of pF curve vector elements");
