 ...
 */
floorstore::floorstore(char *aName,const int aIndex,const base* aOwner,commonData *  &runData,commonData * &original, int numberOfFloors):
    		 base(aName,aIndex,aOwner)
{
	...
	switch (theControlParameters->GetvolatModel())
	{
		...
		if(manuretype.compare("SLURRY")==0)
		{
		...
		else if (manuretype.compare("SEPARATED"))	// need separate as liquid and solid stored separately
		{
...
linkList<instore> *floorstore::GetStoreClone()
{
   ...
   {
	   instore *p = new instore(*(theOnFloorStores->ElementAtNumber(i)));
...
void floorstore::Volatize(double airVelocity, double waterEvapThisFloor, double temperature,
...
void floorstore::RcvManure(manure* aUrine,manure* aFaeces)
{
...
//	EndBudget(false);
	...
	manure *water=new manure(nextManureName.c_str(),0,nullptr);   //is this deleted later??
	...
	*manureToAdd + *water;
	...
	for(int i=0;i<theOnFloorStores->NumOfNodes();i++)
	{
		...
		aManure->Setname(nextManureName.c_str());
		*manureToAdd + *aManure;
...
{
	if (show) cout << "Balance for " << GetName() << ".(" << GetIndex() << ")" << endl;
	//N budget
...
{
	if (theNextStores->NumOfNodes()>0) //check to see if the manure is to be sent to another store. If so, scale that store.
	{
...
#ifdef NITROSCAPE
	if ((manureToName.compare(aManureStore->GetName())==0)
				&&(aManureStore->getmanureStoreID()==ManureToID))
			theNextStores->InsertLast(aManureStore);
#endif
#ifndef NITROSCAPE
	if ((manureToName.compare(aManureStore->GetName())==0)
				&&(aManureStore->GetIndex()==ManureToID))
			theNextStores->InsertLast(aManureStore);
#endif 
}
