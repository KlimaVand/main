 ...
 */
stable::stable(const char* aname,const int aIndex,const base* aOwner,commonData *  &runData,commonData * &original)
:building(aname,aIndex,aOwner, runData, original)
{
	...
	for(int inx=small;inx<=large;inx++)
	{
		...
		{
			instore *manureInst=new instore((char *)sectionName.c_str(),inx,this,runData,original);
	...
	case 1://!dynamic emission model
	{
		if (inUse)   //could be zero if using scaling and no animals for this housing type
		{
			...
			for(int i=0;i<sections->NumOfNodes();i++)
			{
				...
				aSection->Volatize(airVelocity,waterEvapThisSection,insideTemperature,
						&tempvolatAmmonia, &tempN2Emission, &tempN2OEmission, &tempCH4Emission, &tempCO2Emission);
				if (!controlledVent)
			...
			for(int i=0;i<inhouseStores->NumOfNodes();i++)
			{
				double tempvolatAmmonia=0.0, tempN2Emission=0.0, tempN2OEmission=0.0, tempCH4Emission=0.0, tempCO2Emission=0.0, tempwaterEvap=0.0;
				instore * aStore=inhouseStores->ElementAtNumber(i);
				aStore->inhouseVolatize(airVelocity,0.0,insideTemperature,1,
						&tempvolatAmmonia, &tempN2Emission, &tempN2OEmission, &tempCH4Emission, &tempCO2Emission);
				NH3Emission+=tempvolatAmmonia;
...
void stable::LinkManureStores(manurestore * aManureStore)
{
	for(int i=0;i<inhouseStores->NumOfNodes();i++)
	{
		...
		else if ((aManureStore->GetName()==aStore->GetManureToName())&& (aManureStore->GetIndex() == aStore->GetManureToIndex()))
			{
			aStore->SetnextStore(aManureStore);
			}
...
void stable::CheckLinks()
{
	...
	{
		instore * aStore=inhouseStores->ElementAtNumber(i);
		if ((aStore->GetnextStore()==nullptr) && (aStore->GetManureToName()!="NONE"))
		{
 ...
 */
void stable::controlled(double heatOP, double outsideAirTemp, double solarRad, double *supplementaryHeat)
{
	...
	*supplementaryHeat=0.0;
	...
	double airDensity = GetdensityAir(StandardAirPressure,targetTemperature,GetsaturatedWaterVapourPressure(targetTemperature));
 ...
 */
void stable::uncontrolled(double heatOP,double outsideAirTemp, double windspeed, double solarRad, double relativeHumidity)
{
	...
	double surfaceTemp = outsideAirTemp; //rough estimate of temperature of roof surface, K
	double longWave = GetNetLongWave(skyTemp, surfaceTemp, emissivity);
	double airDensity = GetdensityAir(StandardAirPressure,outsideAirTemp,GetsaturatedWaterVapourPressure(outsideAirTemp));//density of the air in kg per cubic metre
	//should include a water balance!
	...
	deltaTemper=(heatOP+q)/(GetspecificHeatCapAir()*airDensity* targetventilationRate + meanThermalTrans*surfaceArea);
	...
	if ((outsideAirTemp+deltaTemper>maxTemperature) && (outsideAirTemp<maxTemperature))
	{
		...
		targetventilationRate=((heatOP+q) - heatTransportThroughWalls)/(maxDeltaT * GetspecificHeatCapAir()*airDensity);
		deltaTemper=(heatOP+q)/(GetspecificHeatCapAir()*airDensity* targetventilationRate + meanThermalTrans*surfaceArea);
	}
	...
	if (outsideAirTemp+deltaTemper<minTemperature)
	{
		...
		targetventilationRate=((heatOP+q) - heatTransportThroughWalls)/(tempDeltaT * GetspecificHeatCapAir()*airDensity);
		deltaTemper=(heatOP+q)/(GetspecificHeatCapAir()*airDensity* targetventilationRate + meanThermalTrans*surfaceArea);
	}
...
double stable::GetNetLongWave(double skyTemp, double surfaceTemp, double emissivity)
{
...
{
	double ret_val=0.0;
	ret_val = 0.8 -0.38*pow(temperature,2)/1000.0;
...
void stable::ShowStable(bool header)
{
	...
	{
		*detailsFile  << "Day "<< "\t"<< " DayNo" << "\t"<< "HouseName" << "\t" << "ventilationRate" << "\t"<<  "insideTemperature"  << "\t"<<  "outsideTemperature"<< "\t"
		...
		*detailsFile << endl;
		*detailsFile  << " "<< "\t"<< " " << "\t"<< " " << "\t" << " m**3/" << "\t"<<  "Celsius"  << "\t"<<  "Celsius"<< "\t"
				<< "kgC/day" << "\t" << "kgC/day" << "\t" << "kgN/day" << "\t" << "kgN/day" << "\t" << "kgN/day" << "\t" ;
		for(int i=0;i<sections->NumOfNodes();i++)
		{
			*detailsFile << " " << "\t" << "tonnes" << "\t" << "kg" << "\t"
					<< "kg" << "\t" << "kg" << "\t";
		}
		for(int i=0;i<inhouseStores->NumOfNodes();i++)
		{
	...
	{
		*detailsFile << theTime.GetDateValue() << "\t" << theTime.GetJulianDay() << "\t"<< GetAlias() << "\t" << ventilationRate << "\t"<<  insideTemperature << "\t" 
			<< theClimate->GetMeanTemperature() << "\t" 
				<< CH4Emission << "\t" << CO2Emission << "\t" << NH3Emission << "\t" << N2Emission << "\t" << N2OEmission<< "\t" ;
		for(int i=0;i<sections->NumOfNodes();i++)
		{
			animalSection* aSection=sections->ElementAtNumber(i);
			*detailsFile << aSection->GetName() << "\t" << aSection->GetManureAmount() << "\t" << aSection->GetStoredDM() << "\t"
...
double stable::GetMaxPermissableTemperature(double thermalHeatIndex, double relativeHumidity)
{
   ...
   double retVal=0;
   retVal=(thermalHeatIndex - (46.3 - 0.143 * relativeHumidity))/(0.81 + 0.0099 * relativeHumidity);
   return retVal;
}
