...
public:
	...
	int getNumberOfFarms(){return farmInformation.size();};
	...
	int getNumberOfEco(unsigned int farm,unsigned int field){
	...
	int getEco(unsigned int farm,unsigned int field,unsigned int patch){
			if(farm<=farmInformation.size()&&field<=farmInformation.at(farm).size())
					{
