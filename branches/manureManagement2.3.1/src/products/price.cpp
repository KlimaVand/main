...
void price::ReadParameters(commonData * file,int simulationLength)
{
	...
	if ((file->FindItem("sellYear0",&someyearprice)) || (file->FindItem("buyYear0",&someyearprice)))     // fixed prices
	{
		...
		while(ayear<simulationLength)
		{
			...
			convert conv;
			strcpy(tmp,"sellYear");
	        strcat(tmp,conv.IntToString(ayear).c_str());
