...
double manure::InstantaneousVolatAmmonia(double temperature, double area, double resistance)
{  //returns instantaneous volatilisation of NH3-N
	//temperature in K, infilt, precip and evap in (kg per sq metre per hr (mm/hr), area in sq metres and resistance in s per metre
   double volat=0;
/*
...
*/
	if (amount>0.0)
   {
      ...
      if (resistance==0)
         theMessage->FatalError("manure:: The transfer resistance for ammonia loss cannot be zero");
      double q = KH/((1+pow(10,-pH)/KN)*resistance * volume * 14.0); //rate constant, per hourTemperature in degrees Celsius
      volat = q * TANPresent;
    }
 return volat;
 //volatilisation in tonnes.
