...
int modified()
{
	...
	//      ScenarioFileName,
	 ...
	 //  bool SellPlantProducts=false;
	   ...
	   char the_path[256];
	    getcwd(the_path, 255);
		cout << "Reading system.dat from " << the_path << endl;
	   int firstScenarie=0;
	   int num=0;
	...
	OutputDirectory1=OutputDirectory1+"/";
	            ...
	            if(!data->FindItem("ClimateFileType",&ClimateFileType ))
	            {
	            	int size=ClimateFileName.size();
	            				...
	            				if(ClimateFileName.substr(size-3, size).compare("CLM")==0)
	            					ClimateFileType=0;
	            				else if(ClimateFileName.substr(size-3, size).compare("clm")==0)
	            					ClimateFileType=0;
	            				else if(ClimateFileName.substr(size-3, size).compare("KML")==0)
	            					ClimateFileType=1;
	            				else if(ClimateFileName.substr(size-3, size).compare("kml")==0)
	            					ClimateFileType=1;
								else if(ClimateFileName.substr(size-3, size).compare("dat")==0)
	            					ClimateFileType=5;
