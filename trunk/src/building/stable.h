...
protected:
 ...
 //! ammonia emission factor for use in static emission model
   ...
   double optimumAirVelocity;
	...
	//!standard animal housing sections
	linkList<animalSection> *standardSections;
...
//!Calculates the total wall area of the animal housing (in square metres)
    ...
    */
   virtual void controlled(double heatOP, double outsideAirTemp, double solarRad, double *supplementaryHeat);
    ...
    */
	virtual void uncontrolled(double heatOP, double outsideAirTemp, double windspeed, double solarRad, double relativeHumidity);
...
double GetNetLongWave(double skyTemp, double surfaceTemp, double emissivity);
   ...
   //!Empty the animal house of manure and Creates an instance of product class and returns a pointer
   ...
   void OpenDetailsFile();
	...
	//!Output the state variables for all parts of the animal housing
	/*!
   ...
   double GetMaxPermissableTemperature(double thermalHeatIndex, double relativeHumidity);
