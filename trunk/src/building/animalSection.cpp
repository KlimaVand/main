...
void animalSection::Volatize(double airVelocity, double waterEvapThisSection, double temperature,
		double *volatAmmonia, double *N2Emission, double *N2OEmission, double *CH4Emission,  double *CO2Emission)
{
	for(int i=0;i<flooring->NumOfNodes();i++)
	{
		...
		tempwaterEvap=aFloor->GetAreaPartition()*waterEvapThisSection;
		aFloor-> Volatize(airVelocity, tempwaterEvap, temperature, &tempvolatAmmonia,
...
/*		if(floorInst->GetStoreNumber()!=floorInst->GetNextStoreNumber())
		{
			cout<<floorInst->GetName()<<endl;
			cout<<floorInst->GetStoreNumber()<<endl;
			cout<<floorInst->GetNextStoreNumber()<<endl;
			string outString1 =  "animalSection::CheckLinks ";
				string outString2 =  " next store and store does not match";
				string outString =  outString1  + outString2;
...
/*		for(int j=0;j<floorInst->GetStoreNumber();j++)
		...
		*/
			...
			if ((aStore->GetnextStore()!=nullptr)&&(floorInst->GetmanureToName()!="nullptr"))
			{
				string outString1 =  "animalSection:: ";
				string outString2 =  " is not linked to a manure store";
				string outString =  outString1  + floorInst->GetName() + outString2;
