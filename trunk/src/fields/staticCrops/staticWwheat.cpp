...
void staticWwheat::SpecifiedPlan(fieldOrder * PP,int JB_NO,char * crop_id)
{
   ...
   if (PP->GetNumOfHarvestDates()>1)
      theMessage->FatalError("static_wnwht::SpecifiedPlan - more than one harvest date specified");
}
...
cloneList<fieldOperationFields> * staticWwheat::MakeOpList(fieldOrder * PP,int JB_NO)
{
   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         FO->DefineSowFieldsOp(Sowing,crop_id,cropName,PP->GetStartYear(),9,15,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
         AddOpList(FO);
         delete aSeed;
      }
      ...
      { // Use harvester
         ...
         if (gatheringStraw)
            ...
            FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,PP->GetStartYear()+1,8,21,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
      ...
      { // Gather straw
         ...
         theProducts->GiveProductInformation(aPlantItem2);
         aPlantItem2->Setamount(ExpectedStrawYield_(JB_NO,0,2));
         FO->DefineHarvestFieldsOper(GatheringBales,crop_id,cropName,PP->GetStartYear()+1,8,21,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
