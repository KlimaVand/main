   ...
   { // Automatic plan
	   ...
	   if (PP->GetFluidManureSpring()>0) { // Apply FLUID manure
		...
		{ // Sowing
         ...
         seed * aSeed2  = new seed;
			...
			aSeed2->Setamount(83);
	      FO->DefineSowFieldsOp(Sowing,cropName,cropName,PP->GetStartYear()+1,4,1,PP->GetArea(),0,PP->GetDistance(),2,aSeed1);
      ...
      { // Irrigating 30 mm on sandy soil
		...
		{ // Combine harvesting
			...
			FO->DefineHarvestFieldsOper(CombineHarvestingShred,cropName,cropName,PP->GetStartYear()+1,8,15,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
