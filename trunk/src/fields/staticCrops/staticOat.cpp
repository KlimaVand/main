   ...
   { // Automatic plan
      ...
      { // Sowing
         ...
         if (underSown)
         {
            ...
            strcpy(tmp, "B2");
            FO->DefineSowFieldsOp(Sowing,tmp,cropName,PP->GetStartYear()+1,4,6,PP->GetArea(),0,PP->GetDistance(),2,aSeed);
         }
         else
      ...
      { // Combine harvesting
         ...
         if (gatheringStraw)
            FO->DefineHarvestFieldsOper(CombineHarvesting,crop_id,cropName,PP->GetStartYear()+1,9,1,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         else
            FO->DefineHarvestFieldsOper(CombineHarvestingShred,crop_id,cropName,PP->GetStartYear()+1,9,1,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
         ...
         { // Gathering straw
            harvestFields * FO=new harvestFields;
            FO->DefineHarvestFieldsOper(GatheringBales,crop_id,cropName,PP->GetStartYear()+1,8,16,PP->GetArea(),0,PP->GetDistance(),false,PP->GetFractionStrawHarvested(0),PP->GetFractionGrainHarvested(0));
            FO->SetStrawProduct(aPlantItem2);
         	AddOpList(FO);
         }
      }
