...
*/
    ...
    /**
* get canopyInterception
*/
        public pool getcanopyInterception() { return canopyInterception; }
    /**
* get theEvaporation
*/
...
*/
    ...
    /**
         ...
         */
        public void Initialise2(int zoneNR, int soiltypeNo, double amaxRootingDepth, double rootingDepth, double totalLAI, 
            double [] layerOrgC)
        {
            theLayers = new List<layerClass>();
            ...
            if (soilDepth < maxRootingDepth)
                GlobalVars.Instance.Error("Maximum rooting depth is greater than soil depth");
        ...
        */
            ...
            ref double rel_droughtPlant, ref double relativeDroughtSoil)
        {
            ...
            if ((totalLAI == 0) && (canopyInterception.getvolume() > 0))//this can occur if a crop has just been harvested and there was water in the canopy
            {
                ...
                drainage = theRooting.update(drainage, 0, 0, 0);
            ...
            else  //insufficient water available in surface to enable evaporation at the potential rate
            {
                ...
                if ((maxWaterAvail / maxCapacity) < 0.5)
                    capillary *= 2 * maxWaterAvail / maxCapacity;
                ...
                if (rootAvailWater > 0.0)  //there is water in the rooting pool
                    soilEvap = capillary + surfaceAvailWater;
                else
                    soilEvap = capillary;
            }
            ...
            {
                theEvaporation.update(drainage, capillary, soilEvap, 0.0);    //update the evaporation pool
                if (soilEvap <= (theRooting.getvolume() + drainage))
                {
                    drainage = theRooting.update(drainage, 0.0, soilEvap, 0.0);
                ...
                else
                {
                    ...
                    double fromSubZone = soilEvap - (fromRooting + drainage); //capillary flow provided from below the rooting zone
                    if (fromSubZone > theSubZone.getvolume())
                    {
                        soilEvap -= (fromSubZone - theSubZone.getvolume());
                        fromSubZone = theSubZone.getvolume();
                    }
                    double temp =fromRooting + drainage + fromSubZone - soilEvap;
                    if (temp < 0)//THIS SHOULD NOT HAPPEN. WHY DOES IT?
                        soilEvap += temp;
                    drainage = theRooting.update(drainage, fromSubZone, soilEvap, 0.0);  //update rooting zone
                    drainage = theSubZone.update(drainage, 0, fromSubZone, 0.0);  //update sub rooting zone
                }
            }
            else   //update all the soil pools
            {
                theEvaporation.update(drainage, 0.0, soilEvap, 0.0);
                drainage = theRooting.update(drainage, 0.0, soilEvap, 0.0);
                drainage = theSubZone.update(drainage, 0.0, 0.0, 0.0);
            }
            evap += soilEvap;
                ...
                /*                if (newrootCapacity < oldrootCapacity)   //if the rooting depth is decreasing
                                {
                                    ...
                                    newSubZoneVol = oldSubZonevolume - (theRooting.getvolume() - oldrootVolume);
                                ...
                                else
                                {
                                    ...
                                    {
                                        double waterFromSubzone = oldSubZonevolume * (newrootCapacity - oldrootCapacity) / oldSubZonecapacity;
                                        theRooting.setvolume(oldrootVolume + waterFromSubzone);
                                     //   waterIn += waterFromSubzone;
                                        newSubZoneVol = oldSubZonevolume - waterFromSubzone;
                                    }
                                }*/
...
//                double newSubZoneVol=oldSubZonevolume - (theRooting.getvolume() - oldrootVolume);
                if (newSubZoneVol < 0)
                {
