...
namespace AnimalChange
{
    ...
    {
    /**
         * \param args[0] and args[1] are farm number and scenario number respectively
         * \param if args[2] is 1, the energy demand of livestock must be met (0 is used when reporting current energy balance of livestock to users)
         * \param if args[3] is 1, the crop model requires the modelled production of grazed DM to match the expected production. If =0, grazed DM will be imported if there is a deficit
         * \param if args[4] is -1, the model will spinup for the years in the spinupYearsBaseLine parameter then run a baseline scenario and generate a Ctool data transfer file
         * \param if args[4] is a positive integer and the spinupYearsNonBaseLine parameter is zero, the model will read Ctool data from the Ctool data transfer file. If spinupYearsNonBaseLine is not zero, the model will spin up for spinupYearsNonBaseLine years then run the scenario
         * \param So to just calculate the animal production, set the farm file in system.xml to "farm.xml" and the args to <farm number> <scenario number> "0" "0" "-1"
         */
        static void Main(string[] args)
        {
            ...
            int stoppingtime = settings.getItemInt("stoppingtime", false);
            if (-1 == stoppingtime)
                stoppingtime = 240000;
            stop stopperThreat = new stop(stoppingtime);
            Thread oThread = new Thread(new ThreadStart(stopperThreat.counDown));
            oThread.Start();
            ...
            for (int settingsID = minSetting; settingsID <= maxSetting; settingsID++)
            {
                ...
                if (args.Length == 5)
                {
                    ...
                    {
                        GlobalVars.Instance.Error("4th argument is wrong and cannot be used");
                    }
                ...
                if (settings.doesIDExist(settingsID))
                {
                    ...
                    farmNames[1] = farmName.Split('.')[0] + "Alternative.xml";
                    ...
                    for (int farmNr = min; farmNr <= max; farmNr++)
                    {
                        ...
                        try
                        {
                            if (farmInformation.doesIDExist(farmNr))
                            {
                                ...
                                {
                                    if (args.Count() != 0)
                                        if (args[1].CompareTo(settingsnr.ToString()) != 0)
                                            GlobalVars.Instance.Error("scenarie nr does not match with cmd input and farmfile");
                                    ...
                                    else
                                    {
                                        ...
                                        {
                                            GlobalVars.Instance.Error("something wrong with the first argument");
                                        }
                                    }
                                    if (args.Length == 0)
                                    {
                                    ...
                                    GlobalVars.Instance.openSeyda(settings.getItemString("outputDir"), ScenarioNr.ToString());
                                    ...
                                    if (farmInformation.doesIDExist(settingsnr))
                                    {
                                        ...
                                        if(GlobalVars.Instance.GetstrictGrazing() == true)
                                        {
                                            ...
                                            if (GlobalVars.Instance.reuseCtoolData != -1)
                                            {
                                                ...
                                                {
                                                    int soilNr=rotationList[i].GetsoiltypeNo();
                                                ...
                                                for (int j = 0; j < lines.Length; j++)
                                                {
                                                    ...
                                                    try
                                                    {
                                                        ...
                                                        oldArea[soilNr] += Convert.ToInt32(tmp[9]);
                                                    }
                                                    catch
                                                    {
                                                        GlobalVars.Instance.Error("something is wrong the the ctool file");
                                                    }
                                                }
                                                for (int j = 0; j < 20; j++)
                                                {
                                                    ...
                                                    {
                                                        GlobalVars.Instance.Error("area for soil type "+j.ToString()+" donot match");
                                                    }
                                                }
                                        ...
                                        else
                                        {
                                          ...
                                          //if (((listOfHousing.Count() == 0) && (listOfLivestock.Count > 0))) //there are grazing animals that are not housed
                                            ...
                                            if (GlobalVars.Instance.reuseCtoolData == -1)
                                            {
                                                //Jonas - replace 20 with variable name
                                                ...
                                                {
                                                    CropSequenceClass rotation = rotationList[i];
                                                    int soiltypeNo=rotation.GetsoiltypeNo();
                                        ...
                                        GlobalVars.Instance.closeSeyda();
    ...
    /**
         * find the proper filename for files
         */
        static string[] fileName(string[] args, FileInformation settings, string file)
        {
            ...
            if (args.Length != 0)
            {
                ...
                alternativePath = alternativePath.Split('.')[0] + "Alternative.xml";
                if (File.Exists(alternativePath))
                {
                ...
                {
                    names[1] = constants.Split('.')[0] + "Alternative.xml"; 
                }
            }
            else
            {
                names[0] = settings.getItemString(file);
                names[1] = settings.getItemString(file).Split('.')[0] + "Alternative.xml"; ;
            }
            return names;
