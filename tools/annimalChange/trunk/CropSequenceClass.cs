...
/**
    * get P input in fertiliser
    */
    public double CalcFertPinput()
    {
        double retVal = 0;
        for (int i = 0; i < theCrops.Count - 1; i++)
            retVal += theCrops[i].GetfertiliserPinput() * area;
        return retVal;
    }

/**
    * get P output in crop products
    */
    public double CalcCropProductPoutput()
    {
        double retVal = 0;
        for (int i = 0; i < theCrops.Count - 1; i++)
            retVal += theCrops[i].GetproductPoutput() * area;
        return retVal;
    }
/**
     * return output P value 
     */
    public double getoutputP() { return outputP; }
/**
* set output P value 
*/
    public void SetoutputP(double newValue) { outputP = newValue; }
/**
* calculate P balance 
*/
    public double CalcPbalance() { return 0; }
/**
    * get init soil C
    */
...
*/
  ...
  /*      for (int i = 0; i < theCrops.Count; i++)
        ...
        thesoilWaterModel.Initialise2(zoneNr, soiltypeNo, theCrops[0].GetMaximumRootingDepth(), currentRootingDepth, currentLAI,
            layerOM);
        for (int i = 0; i < theCrops.Count; i++)
...
/**
* get amout of C that is harvested
*/
...
/**
* get amount of CO that is burned
*/
...
/**
* get amount of NOxN that is burned
*/
...
/**
* get Nh3n emission from fertiliser
*/
...
/**
* get amout of water that is available of water 
*/
...
/**
* calculation of input of each plan
*/
...
* do calculation of the crop sequance 
*/
...
*/
    public void Write()
    {
        ...
        //GlobalVars.Instance.writeInformationToFiles("inputP", "inputP", "-", inputP, Parens);
        GlobalVars.Instance.writeInformationToFiles("outputP", "outputP", "-", outputP, Parens);
        for (int i = 0; i < theCrops.Count; i++)
        {
            theCrops[i].Write();
    ...
    public void CalcModelledYield()
    {
        ...
        for (int i = 0; i < theCrops.Count; i++)
        {
            ...
            while ((gotit == false) || (doneOnce == false))//iterate for each crop, until the crop yield stabilises (note special treatment of grazed crops)
            {
                ...
                gotit = theCrops[i].CalcModelledYield(ref surplusMineralN, ref relGrowth, true);
                doneOnce = true;
            }
            count = 0;
...
*/
    public bool CheckRotationCBalance(int maxCrops)
    {
        ...
        if (Math.Abs(diff) > tolerance)
         ...
         Cbalance = ((Cfixed + manureC + faecalC + urineC - (soilCO2_CEmission + Cleached + CdeltaSoil + Charvested + burntC + residueCcarriedOver)))/lengthOfSequence;
...
*/
    public void CheckRotationNBalance(int maxCrops)
    {
        ...
        Nbalance = (Ninput - (NLost + harvestedN + NdeltaSoil + residualMineralN + residueNcarriedOver))/lengthOfSequence;
...
/**
* Get NH3N Fertiliser from Manure
*/
...
*/
 ...
 /**
    * Get fertiliser costs
    */
        public double GetFertiliserCost()
        {
            double retVal = 0;
            for (int i = 0; i < theCrops.Count; i++)
            {
                retVal += theCrops[i].GetfertiliserPurchase() * area;
            }
            return retVal;
        }
   ...
   public void spinup(ref double thestartsoilMineralN, double initialFOM_Cinput, double InitialFOMCtoN, double[] meanTemperature,int CropSeqID)
    {
        ...
        if (spinupYears > 0)
        {
            ...
            for (int j = 0; j < spinupYears; j++)
            {
                ...
                double droughtFactorPlant = GlobalVars.Instance.theZoneData.GetMeanDroughtIndex(1, 1, 1, 31, 12, 1);

                node.Add(aModel.Dynamics(true, 1, startDay + (j + 1) * 365, startDay - 1 + (j + 2) * 365, FOM_Cin, HUM_Cin, fomnIn, cultivation, meanTemperature, droughtFactorPlant,
...
* Run Ctool
*/
   public input RunCropCTool(bool diagnostics, bool writeOutput, int cropNo, double[] meanTemperature, double droughtFactorPlant, double cultivationDepth, ref double CropCinputToSoil, ref double CropNinputToSoil,
...
//        Console.WriteLine(theCrops[cropNo].getStartLongTime().ToString() + " " + theCrops[cropNo].getEndLongTime().ToString());
    ...
    XElement tmp= aModel.Dynamics(writeOutput, (int)JDay, theCrops[cropNo].getStartLongTime(), theCrops[cropNo].getEndLongTime(), FOM_Cin, HUM_Cin, fomnIn, cultivation,
        meanTemperature, droughtFactorPlant, ref Cchange, ref CO2Emission, ref tempCleached, ref Nmin, ref orgNleached,identity);
    if (writeOutput == true)
        node.Add(tmp);
   ...
   public void CalculateWater(int cropNo)
   {
       ...
       thesoilWaterModel.CalcSoilWaterProps(soilC);
        ...
        while (k < cropDuration)
        {
            if ((k == 0)&&(cropNo==2))
                Console.Write("");
            double LAI = theCrops[cropNo].CalculateLAI(k);
            ...
            double propAvailWater = SMD / maxAvailWaterToRootingDepth;
            double droughtFactorPlant = 0;
            double droughtFactorSoil = 0;
            double irrigation = 0;
               
            if ((theCrops[cropNo].GetisIrrigated()) && (propAvailWater >= irrigationThreshold))
            {
                double irrigationAmount=0.9 * SMD;
                if ((irrigationAmount - precip) > irrigationMinimum)
                    irrigation=irrigationAmount;
            }
            thesoilWaterModel.dailyRoutine(potevapotrans, precip, irrigation, airTemp, LAI, rootingDepth, ref droughtFactorPlant,
                ref droughtFactorSoil);
            ...
            theCrops[cropNo].SetdroughtFactorSoil(k, droughtFactorSoil);
            ...
            if (cropNo > theCrops.Count - 5)
            {
                GlobalVars.Instance.WriteDebugFile("day", clockit.getLongTime()+k, '\t');
                GlobalVars.Instance.WriteDebugFile("precip", precip, '\t');
                GlobalVars.Instance.WriteDebugFile("irrigation", irrigation, '\t');
                GlobalVars.Instance.WriteDebugFile("evap", evap, '\t');
                GlobalVars.Instance.WriteDebugFile("transpire", transpire, '\t');
                GlobalVars.Instance.WriteDebugFile("drainage", drainage, '\t');
                GlobalVars.Instance.WriteDebugFile("waterInSoil", waterInSoil, '\t');
                GlobalVars.Instance.WriteDebugFile("plantwaterInSoil", thesoilWaterModel.GetPlantAvailWaterInSoil(), '\t');
                GlobalVars.Instance.WriteDebugFile("droughtFactorPlant", droughtFactorPlant, '\t');
                GlobalVars.Instance.WriteDebugFile("droughtFactorSoil", droughtFactorSoil, '\t');
                GlobalVars.Instance.WriteDebugFile("LAI", LAI, '\t');
