        ...
        static void Main(string[] args)
        {
            ...
            for (int settingsID = minSetting; settingsID <= maxSetting; settingsID++)
            {
                ...
                if (settings.doesIDExist(settingsID))
                {
                    ...
                    {
                        GlobalVars.Instance.Error("constant file not found", false);
                    }
                            ...
                            if (farmInformation.doesIDExist(farmNr))
                            {
                                ...
                                for (int ScenarioNr = minScenario; ScenarioNr <= maxScenario; ScenarioNr++)
                                {
                                    ...
                                    if (farmInformation.doesIDExist(ScenarioNr))
                                    {
                                        ...
                                        outputName = outputName.Split('.')[0];
                                        outputName = outputName+"small" + ".xml";
                                        XmlWriter writerSmall = GlobalVars.Instance.OpenOutputXMLSmall(outputName);
                                        //writer.WriteStartDocument();
                                        GlobalVars.Instance.writeStartTab("Farm",true);
                                        ...
                                        for (int rotationID = minRotation; rotationID <= maxRotation; rotationID++)
                                        {
                                            ...
                                            {
                                                CropSequenceClass anExample = new CropSequenceClass(RotationPath, rotationID, zoneNr, FarmTyp, farmNr.ToString() + "_" + ScenarioNr.ToString() + "_" + rotationID.ToString());
                                        ...
                                        for (int LiveStockID = minLivestock; LiveStockID <= maxLivestock; LiveStockID++)
                                        {
                                            ...
                                            {
                                                livestock anAnimal = new livestock(LivestockPath, LiveStockID, zoneNr, animalNr, farmNr.ToString() + "_" + ScenarioNr.ToString() + "_" + LiveStockID.ToString());
                                        ...
                                        for (int ManureStorageID = minManureStorage; ManureStorageID <= maxManureStorage; ManureStorageID++)
                                        {
                                            ...
                                            {
                                                manureStore amanurestore = new manureStore(ManureStoragePath, ManureStorageID, zoneNr, farmNr.ToString() + "_" + ScenarioNr.ToString() + "_" + ManureStorageID.ToString());
                                        ...
                                        for (int i = 0; i < listOfLivestock.Count(); i++)
                                        {
                                            ...
                                            for (int j = 0; j < anAnimalCategory.GethousingDetails().Count(); j++)
                                            {
                                                ...
                                                housing aHouse = new housing(housingType, anAnimalCategory, j, zoneNr, farmNr.ToString() + "_" + ScenarioNr.ToString() + 
                                        ...
                                        farmBalanceClass theBalances = new farmBalanceClass(farmNr.ToString() + "_" + ScenarioNr.ToString() + "_1");
                                    ...
                                    System.IO.File.WriteAllText(outputDir + "done" + farmNr.ToString() + "ScenarioNr.txt" + ScenarioNr.ToString(), ticks.ToString());
                                }//end of scenario
                            }//end of farm exists
                        }
                        catch (Exception e)
