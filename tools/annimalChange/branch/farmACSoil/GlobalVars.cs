...
public class GlobalVars
{
     ...
     public struct grazedItem
     {
         ...
         public void Write()
         {
             GlobalVars.Instance.writeStartTab("GrazedItem",true);
             GlobalVars.Instance.writeInformationToFiles("name", "Name", "-", name, parens,true);
             GlobalVars.Instance.writeInformationToFiles("fieldDMgrazed", "FieldDMgrazed", "kg", fieldDMgrazed, parens, true);
             GlobalVars.Instance.writeInformationToFiles("ruminantDMgrazed", "ruminantDMgrazed", "kg", ruminantDMgrazed, parens, true);
             GlobalVars.Instance.writeEndTab(true);
         }
     }
    ...
    int versionChoice = 2;
    public int GetversionChoice() { return versionChoice; }
    public void SetversionChoice(int choice) {versionChoice = choice;}
    ...
    public struct zoneSpecificData
    {
        ...
        public struct fertiliserData
        {
            ...
            public double Cconcentration;
            public double TANconcentration;
            public double organicNconcentration;
            public double degCconcentration;
            public double nonDegCconcentration;
            public double humicCconcentration;
        }
        ...
        public fertiliserData GetFertManDetails(string aname)
        {
            int i = 0;
            while (i < theFertManData.Count)
            {
                if (theFertManData[i].name == aname)
                    break;
                i++;
            }
            if (i == theFertManData.Count)
            {
                string messageString = "Cannot find details of fertiliser or manure with name  " + aname;
                GlobalVars.Instance.Error(messageString);
            }
            return theFertManData[i];
        }
        ...
        public double GetPeriodTemperatureSum(int startDay, int startMonth, int startYear, int endDay, int endMonth, int endYear, double baseTemp)
        {
            ...
            for (int i = (startMonth) + 1; i <= endCount - 1; i++)
            {
                ...
                if (i > 12)
                    monthCount -= 12;
                Tsum += GetTemperatureSum(airTemp[monthCount - 1], baseTemp) * time.tabDaysPerMonth[monthCount - 1];
            }
            Tsum += GetTemperatureSum(airTemp[startMonth - 1], baseTemp) * (time.tabDaysPerMonth[startMonth - 1] - startDay);
            Tsum += GetTemperatureSum(airTemp[endMonth - 1], baseTemp) * (time.tabDaysPerMonth[endMonth - 1] - endDay);
            return Tsum;
        }
        ...
        public int GetPeriod(int startDay, int startMonth, int eventDay, int eventMonth, int duration)
        {
            timeClass starttime = new timeClass();
            starttime.SetDate(startDay, startMonth, 0);
            timeClass eventtime = new timeClass();
            eventtime.SetDate(eventDay, eventMonth, 0);

            int numbersOfDays = 0;
            numbersOfDays = ((int) eventtime.getLongTime() - (int)starttime.getLongTime());
    ...
    public double GetPropFieldCapacity(CropClass cropData)
    {
        ...
        public void readZoneSpecificData(int zone_nr, int currentFarmType)
        {
            ...
            Console.WriteLine(GlobalVars.Instance.getParamFilePath());
            ...
            for (int j = minSoil; j <= maxSoil; j++)
            {
                ...
                //soilWaterData anewsoilWaterData = new soilWaterData();
                newsoilData.thesoilWaterData = new soilWaterData();
            ...
            for (int j = minFert; j <= maxFert; j++)
            {
                ...
                basePath ="AgroecologicalZone(" + zone_nr.ToString() + ").Fertiliser(-1).FertiliserType" + '(' + j.ToString() + ")";
                string RecipientPath = basePath;
                ...
                RecipientPath = basePath +".EFNH3(-1)";
                ...
                RecipientPath = basePath + ".Cconcentration(-1)";
                newfertData.Cconcentration=AEZParamFile.getItemDouble("Value", RecipientPath);
                RecipientPath = basePath + ".Nconcentration(-1)";
                newfertData.Nconcentration=AEZParamFile.getItemDouble("Value", RecipientPath);
            ...
            for (int j = minMan; j <= maxMan; j++)
            {
                ...
                basePath = "AgroecologicalZone(" + zone_nr.ToString() + ").Manure(-1).ManureType" + '(' + j.ToString() + ")";
                RecipientPath = basePath + ".TANconcentration(-1)";
                newfertData.TANconcentration = AEZParamFile.getItemDouble("Value", RecipientPath);
                RecipientPath = basePath + ".organicNconcentration(-1)";
                newfertData.organicNconcentration = AEZParamFile.getItemDouble("Value", RecipientPath);
                RecipientPath = basePath + ".degCconcentration(-1)";
                newfertData.degCconcentration = AEZParamFile.getItemDouble("Value", RecipientPath);
                RecipientPath = basePath + ".nonDegCconcentration(-1)";
                newfertData.nonDegCconcentration = AEZParamFile.getItemDouble("Value", RecipientPath);
                RecipientPath = basePath + ".humicCconcentration(-1)";
                newfertData.humicCconcentration = AEZParamFile.getItemDouble("Value", RecipientPath);
                theFertManData.Add(newfertData);
            }         
        }         
    }
    ...
    public class product
    {
        ...
        public void Write(string theParens)
        {
            ...
            parens = theParens + "_" + composition.GetFeedCode().ToString();
 ...
 * FileInformation file = new FileInformation(GlobalVars.Instance.getfertManFilePath());
                    ...
                    int itemNr=0;
                    i = 0;
                    while (i < GlobalVars.Instance.theZoneData.theFertManData.Count)
                    {
                        GlobalVars.zoneSpecificData.fertiliserData theFert = GlobalVars.Instance.theZoneData.theFertManData[i];

                        int StorageTypeFile=theFert.storageType;
                        int SpeciesGroupFile = theFert.speciesGroup;
    ...
    public XmlWriter OpenOutputXMLSmall(string outputName)
    {
        writerSmall = XmlWriter.Create(outputName);
        writerSmall.WriteStartDocument();
        return writerSmall;
    }
    public void CloseOutputXML()
    {
        ...
        writer.Close();
        writerSmall.WriteEndDocument();
        writerSmall.Close();

    }
    public void writeStartTab(string name, bool smallFile=false)
    {
        if(smallFile==true)
            writerSmall.WriteStartElement(name);
        tabFile.Write(name + "\n");
        writer.WriteStartElement(name);
    }
    public void writeEndTab(bool smallFile=false)
    {
        if(smallFile)
            writerSmall.WriteEndElement();
        writer.WriteEndElement();
    }
    public void writeInformationToFiles(string name, string Description, string Units, bool value, string parens, bool smallFile=false)
    {
        writeInformationToFiles(name, Description, Units, Convert.ToString(value), parens, smallFile);
    }
    public void writeInformationToFiles(string name, string Description, string Units, double value,string parens, bool smallFile = false)
    {
        writeInformationToFiles(name, Description, Units, Convert.ToString(Math.Round(value, 4)), parens, smallFile);
    }
    public void writeInformationToFiles(string name, string Description, string Units, int value, string parens, bool smallFile = false)
    {
        writeInformationToFiles(name, Description, Units, Convert.ToString(value), parens, smallFile);
    }
    public void writeSinglItem(string name, double value, bool smallFile = false)
    {
        if (smallFile)
        {
            writerSmall.WriteStartElement(name);
            writerSmall.WriteValue(value);
            writerSmall.WriteEndElement();
        }
        writer.WriteStartElement(name);
    ...
    public void writeInformationToFiles(string name, string Description, string Units, string value,string parens, bool smallFile = false)
    {
        if (smallFile == true)
        {
            writerSmall.WriteStartElement(name);
            writerSmall.WriteStartElement("Description");
            writerSmall.WriteValue(Description);
            writerSmall.WriteEndElement();
            writerSmall.WriteStartElement("Units");
            writerSmall.WriteValue(Units);
            writerSmall.WriteEndElement();
            writerSmall.WriteStartElement("Name");
            writerSmall.WriteValue(name);
            writerSmall.WriteEndElement();
            writerSmall.WriteStartElement("Value");
            writerSmall.WriteValue(value);
            writerSmall.WriteEndElement();
            writerSmall.WriteEndElement();
        }
    ...
    public void OpenOutputTabFile(string outputName)
    {
    ...
    public void Error(string erroMsg, bool withExeption=true)
    {
        try
        {
        ...
        if (erroMsg.CompareTo("farm Fail") != 0)
        {
            ...
            if ((grazedArray[i].fieldDMgrazed > 0)||(grazedArray[i].ruminantDMgrazed>0.0))
            {
                ...
                else
                {
                    ...
                    if (Math.Abs(diff) > tolerance)
                    {
                        ...
                        messageString += " Feed code = " + i.ToString() + " Consumed DM = " + actualDM.ToString() + " Produced DM = " + expectedDM.ToString();
                        GlobalVars.instance.Error(messageString);
                    }
                }
            }
