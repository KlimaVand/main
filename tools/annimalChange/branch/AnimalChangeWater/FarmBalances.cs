...
class farmBalanceClass
{
   ...
   //!nitrous oxide emissions from crop residues
        ...
        List<manureStore> listOfManurestores)
    {
        ...
        for (int rotationID = minRotation; rotationID <= maxRotation; rotationID++)
            farmUnutilisedGrazableDM += rotationList[rotationID-1].GetUnutilisedGrazableDM();
        double numDairy = 0;
        for (int i = 0; i < listOfLivestock.Count; i++)
        {
            livestock anAnimalCategory = listOfLivestock[i];
            ...
            farmConcentrateDM += anAnimalCategory.GetDMintake() * listOfLivestock[i].GetAvgNumberOfAnimal() * GlobalVars.avgNumberOfDays / 1000.0;
            farmLivestockDM += anAnimalCategory.GetConcentrateDM() * listOfLivestock[i].GetAvgNumberOfAnimal() * GlobalVars.avgNumberOfDays / 1000.0;
            farmGrazedDM += anAnimalCategory.GetgrazedDM() * listOfLivestock[i].GetAvgNumberOfAnimal() * GlobalVars.avgNumberOfDays / 1000.0;
    ...
    public void WriteFarmBalances()
        ...
        GlobalVars.Instance.writeInformationToFiles("NGrowth", "N exported in meat", "kg N/yr", NGrowth, parens);
        GlobalVars.Instance.writeInformationToFiles("Nmortalities", "N in mortalities", "kg N/yr", Nmortalities, parens);
        double NeffLivestock = (Nmilk + NGrowth) / (liveFeedImportN + liveGrazedN);
        ...
        GlobalVars.Instance.writeInformationToFiles("MeatProductionPerUnitArea", "Meat production per unit area", "kg/yr", farmMeatProduction / agriculturalArea, parens);
        GlobalVars.Instance.writeInformationToFiles("LivestockDMintake", "LivestockDMintake", "kg/yr", farmLivestockDM, parens);
        GlobalVars.Instance.writeInformationToFiles("farmConcentrateDM", "farmConcentrateDM", "kg/yr", farmConcentrateDM, parens);
        GlobalVars.Instance.writeInformationToFiles("farmGrazedDM", "farmGrazedDM", "kg/yr", farmGrazedDM, parens);
        GlobalVars.Instance.writeEndTab();
    }
}