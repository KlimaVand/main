...
'response.write request.form
	if request("selectFarm")<>"" then Session("farmNumber")=request("selectFarm")
...
if nFarm > 1 then
   Response.Write("<input type=""button"" style=""width: 120px; height: 20px; font-size: 10px"" value="""&str.item("strDeleteFarm")&""" onClick=""if (confirm('Er du sikker på at du vil slette bedriften - '+document.forms.selectFarm.selectFarm.options[document.forms.selectFarm.selectFarm.selectedIndex].text+' - ?\nalle tilhørende data vil blive slettet permanent.')){document.forms.selectFarm.DeleteFarm.value='Slet';submit();}""/>" & vbCr)
end if
   Response.Write("</td>" & vbCr)
   Response.Write("<td>" & vbCr)
