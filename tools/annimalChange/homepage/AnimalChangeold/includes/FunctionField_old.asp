...
'    response.write "<br> MeanUtilization "& MeanUtilization
end if ' MANGLER AT SÆLGE KØBT HG HVIS NØDVENDIG
...
for i=0 to ubound(arrHGdistribute)
   ...
   next
   for j=0 to ubound(arrBoughtManure)
     If Cint(arrHGdistribute(i,dictSalesOrderList.Item("StorageID"))) = CInt(arrBoughtManure(j,dictBoughtManure.Item("BoughtManureType"))) then
...
'    response.write "<br> arrB("&b&") "& arrB(b)
 ...
 '   Response.write "<br> GetUtilizationDegree(" & CLng(Session("farmNumber")) & ","&CInt(Session("scenarioNumber")) & ","&arrNNeed(b,dictNNeed.Item("RotationID")) & ","&arrNNeed(b,dictNNeed.Item("FieldPlanRotationID")) & "," & arrHGdistribute(a,dictSalesOrderList.Item("StorageID")) & ") - " & UBound(arrUtilDegree)
...
'	     if (arrResult(a,b)>0.01 And ObjectCoeff(a,b)>0) Or (CInt(a)=CInt(9) And CInt(arrB(b))=CInt(0)) then 
	     if (arrResult(a,b)>0.01 And ObjectCoeff(a,b)>0) then 
            arrResult(a,b)=arrResult(a,b)*100/arrHGdistribute(a,dictSalesOrderList("N_UtilizationDegree"))
