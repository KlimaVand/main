    ...
    public CropSequenceClass(string aPath, int aID, int zoneNr, int currentFarmType, string aParens)
    {
        ...
        for (int j = 0; j < repeats-1; j++)
        {
            for (int i = 0; i < theCrops.Count; i++)
            {
                ...
                if (theCrops[i].GetEndYear() > theCrops[i].GetStartYear() || (theCrops[i].GetEndYear() > theCrops[i].GetStartYear()) == false && (theCrops[i].getEndLongTime() - theCrops[i].getStartLongTime())== 364)
                    endk++;
