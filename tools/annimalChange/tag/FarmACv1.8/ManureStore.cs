...
public class manureStore
{
    ...
    public double GetrunoffN() { return NRunoffHum + NRunOffOrg + NrunoffTan; }
...
/*            CdegradationRate = b1 * Math.Pow(Math.E, lnArr - GlobalVars.Instance.getEapp() * (1 / (rgas * (meanTemp + GlobalVars.absoluteTemp)))); //1.48
    ...
    public void DoNitrogen()
    {
        ...
        if ((GlobalVars.Instance.getcurrentInventorySystem() == 1)|| (GlobalVars.Instance.getcurrentInventorySystem() == 2))
        {
            NRunOffOrg =NorgInstore* ohmOrg; //assume runoff occurs immediately, before degradation
            theManure.SethumicN(theManure.GethumicN() * (1-NRunOffOrg / NorgInstore));  //adjust humic N for loss in runoff
            totalOrgNdegradation = (Cdegradation / (Cinput-CRunOffOrg)) * (NorgInstore - NRunOffOrg);
            ...
            if (totalOrgNdegradation < newNHUM)  //there will be immobilisation of TAN in humic N
            {
                ...
                {
                    string message1 = "Insufficient mineral N to create new humic N in " + name;
                    GlobalVars.Instance.Error(message1);
                }
                else
                {
            ...
            NorgOutStore = NorgInstore - (NRunOffOrg + totalOrgNdegradation) + newNHUM;
        }
        else
        {
