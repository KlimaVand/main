    ...
    public CropSequenceClass(string aPath, int aID, int zoneNr, int currentFarmType, string aParens)
    {
        ...
        for (int i = min; i <= max; i++)
        {
            if (rotation.doesIDExist(i))
            {
                ...
                aCrop.AddProductsWithResidue(residue);
                residue=aCrop.GetProductsWithResidue();
        ...
        if (FarmType <= 3)
        {
            initialC = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].initialC;
            initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].InitialFOM;
        }
        else
        {
            if ((initialSoilC == 0.0) || (initialFOM_Cinput == 0.0))
            {
                initialFOM_Cinput = 0;
                for (int i = 0; i < theCrops.Count; i++)
                {
                    CropClass aCrop = theCrops[i];
                    for (int j = 0; j < aCrop.GettheProducts().Count; j++)
                    {
                        initialFOM_Cinput += aCrop.GettheProducts()[j].GetPotential_yield() *
                             aCrop.GettheProducts()[j].composition.GetC_conc()
                            * aCrop.GetMaxpropAboveGroundResidues();
                        initialFOM_Cinput += aCrop.GettheProducts()[j].GetPotential_yield()
                            * (aCrop.GetCconcBelowGroundResidues() * aCrop.GetpropBelowGroundResidues());
                    }
                }
            }
        }
        double intensiveFactor = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[0].intensiveFactor;
        Console.WriteLine(intensiveFactor);
        double extensiveFactor = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[0].extensiveFactor;
        switch (FarmType)
        {
            case 1:
            case 2:
            case 3:
                break;
            case 4:
                initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[0].intensiveFactor * initialFOM_Cinput / lengthOfSequence;
                initialC = 25 * initialFOM_Cinput;
                FarmType = 1;
                break;
            case 5:
                initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[0].extensiveFactor * initialFOM_Cinput / lengthOfSequence;
                initialC = 25 * initialFOM_Cinput;
                FarmType = 1;
                break;
            case 6:
                initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[1].intensiveFactor * initialFOM_Cinput / lengthOfSequence;
                initialC = 25 * initialFOM_Cinput;
                FarmType = 2;
                break;
            case 7:
                initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[1].extensiveFactor * initialFOM_Cinput / lengthOfSequence;
                initialC = 25 * initialFOM_Cinput;
                FarmType = 2;
                break;
            case 8:
                initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[2].intensiveFactor * initialFOM_Cinput / lengthOfSequence;
                initialC = 25 * initialFOM_Cinput;
                FarmType = 3;
                break;
            case 9:
                initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[2].extensiveFactor * initialFOM_Cinput / lengthOfSequence;
                initialC = 25 * initialFOM_Cinput;
                FarmType = 3;
                break;
...
//        if (FarmType > 3)
  //          FarmType -= 3;
        ...
        double pHUMupperLayer = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].pHUMupperLayer;
        double pHUMlowerLayer = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].pHUMlowerLayer;
        ...
        aModel.Initialisation(ClayFraction, offset, amplitude, maxSoilDepth, dampingDepth, initialC,
            GlobalVars.Instance.getConstantFilePath(), GlobalVars.Instance.GeterrorFileName(), InitialCtoN, pHUMupperLayer, pHUMlowerLayer);
        double mineralNFromSpinup = 0;
        spinup(ref mineralNFromSpinup, initialFOM_Cinput, InitialFOMCtoN, averageAirTemperature);
    ...
    public void Write()
    {
        ...
        for (int i = 0; i < theCrops.Count; i++)
        {
            tmp += theCrops[i].GetleachedManureTAN();
        }
        GlobalVars.Instance.writeInformationToFiles("leachedManureTAN", "Leached manure TAN", "kgN/ha/yr", tmp / year, Parens);
        tmp = 0;
        for (int i = 0; i < theCrops.Count; i++)
        {
            tmp += theCrops[i].getLeachedFertiliserN();
        }
        GlobalVars.Instance.writeInformationToFiles("leachedFertiliserN", "Leached fertiliser N", "kgN/ha/yr", tmp / year, Parens);
        tmp = 0;
        for (int i = 0; i < theCrops.Count; i++)
        {
            tmp += theCrops[i].getLeachedUrineN();
        }
        GlobalVars.Instance.writeInformationToFiles("leachedUrineN", "Leached urine N", "kgN/ha/yr", tmp / year, Parens);
        tmp = 0;
        for (int i = 0; i < theCrops.Count; i++)
        {
            tmp += theCrops[i].getLeachedOtherN();
        }
        GlobalVars.Instance.writeInformationToFiles("leachedOtherN", "Leached other N", "kgN/ha/yr", tmp / year, Parens);
        tmp = 0;
        for (int i = 0; i < theCrops.Count; i++)
        {
...
/*        tmp = 0;
   ...
   public void getGrazedFeedItems()
    ...
    public void CalcModelledYield()
    {
        ...
        for (int i = 0; i < theCrops.Count; i++)
        {
            ...
            while ((gotit == false) || (doneOnce == false))//iterate for each crop, until the crop yield stabilises (note special treatment of grazed crops)
            {
                ...
                RunCropCTool(false, false, i, meanTemperature, meanDroughtFactor, 0, ref CropCinputToSoil, ref CropNinputToSoil, ref ManCinputToSoil, ref ManNinputToSoil, 
            ...
            RunCropCTool(false, true, i, meanTemperature, meanDroughtFactor, 0, ref CropCinputToSoil, ref CropNinputToSoil, ref ManCinputToSoil, ref ManNinputToSoil, ref CropsoilCO2_CEmission, ref CropCleached, ref mineralisedN);
    ...
    public bool CheckRotationCBalance(int maxCrops)
    {
        ...
        for (int i = 0; i < maxCrops; i++)
        {
            ...
            if (theCrops[i].GetresidueToNext() != null)
                residueCcarriedOver = theCrops[i].GetresidueToNext().GetModelled_yield() * theCrops[i].GetresidueToNext().composition.GetC_conc()
                    * area;
            else
                residueCcarriedOver = 0;
        }
        finalSoilC = GetCStored();
        CdeltaSoil =  finalSoilC - initialSoilC;
        double diff = CinputToSoil - (soilCO2_CEmission + Cleached + CdeltaSoil);
        ...
        if (Math.Abs(diff) > tolerance)
         ...
         Cbalance = (Cfixed + manureC + faecalC + urineC - (soilCO2_CEmission + Cleached + CdeltaSoil + Charvested + burntC + residueCcarriedOver));
    ...
    public void CheckRotationNBalance(int maxCrops)
    {
        ...
        for (int i = 0; i < maxCrops; i++)
        {
            ...
            if (theCrops[i].GetresidueToNext() != null)
                residueNcarriedOver = theCrops[i].GetresidueToNext().GetModelled_yield() * theCrops[i].GetresidueToNext().composition.GetN_conc()
                    * area;
            else
                residueNcarriedOver = 0;
        }
        ...
        Nbalance = Ninput - (NLost + harvestedN + NdeltaSoil + residualMineralN + residueNcarriedOver);
    ...
    public void spinup(ref double soilNmineralised, double initialFOM_Cinput, double InitialFOMCtoN, double meanTemperature)
    {
        double CatStartSpinup = GetCStored();
        FileInformation file = new FileInformation(GlobalVars.Instance.getConstantFilePath());
        file.setPath("constants(0).spinupYears(-1)");

        int spinupYears = file.getItemInt("Value");
        ...
        for (int j = 0; j < spinupYears; j++)
        {
            ...
            node.Add(aModel.Dynamics(true, 1, startDay + (j + 1) * 365, startDay-1 + (j + 2) * 365, FOM_Cin, HUM_Cin, fomnIn, cultivation, meanTemperature, droughtFactor,
                    ref tempCchange, ref tempCO2Emission, ref tempCleached, ref Nmin, ref tempNleached));
   ...
   public void RunCropCTool(bool diagnostics, bool writeOutput, int cropNo, double meanTemperature, double droughtFactor, double cultivationDepth, ref double CropCinputToSoil, ref double CropNinputToSoil,
       ref double ManCinputToSoil, ref double ManNinputToSoil, ref double CropsoilCO2_CEmission, ref double CropCleached, ref double Nmin)
{
    ...
    if (theCrops[cropNo].Getname() == "Rotational clover grass")
    {
        //GlobalVars.Instance.log(theCrops[cropNo].getStartLongTime().ToString() + " " + theCrops[cropNo].getEndLongTime().ToString(),3);
//        Console.WriteLine(theCrops[cropNo].getStartLongTime().ToString() + " " + theCrops[cropNo].getEndLongTime().ToString());
    }
    XElement tmp= aModel.Dynamics(writeOutput, (int)JDay, theCrops[cropNo].getStartLongTime(), theCrops[cropNo].getEndLongTime(), FOM_Cin, HUM_Cin, fomnIn, cultivation,
        meanTemperature, droughtFactor, ref Cchange, ref CO2Emission, ref tempCleached, ref Nmin, ref orgNleached);
    if (writeOutput == true)
        node.Add(tmp);
