...
public class CropSequenceClass
{
    ...
    public CropSequenceClass(string aPath, int aID, int zoneNr, int currentFarmType, string aParens)
    {
        ...
        double initialFOM_Cinput = GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].InitialFOM;
        ...
        double PupperLayer= GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].PupperLayer;
        double PLoweLayer= GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].PLoweLayer;
        double CNratio=GlobalVars.Instance.theZoneData.thesoilData[soilIndex].theC_ToolData[FarmType - 1].InitialFOMCtoN;
        ...
        aModel.Initialisation(ClayFraction, offset, amplitude, maxSoilDepth, dampingDepth, initialC, initialFOMn,  
            GlobalVars.Instance.getConstantFilePath(), GlobalVars.Instance.GeterrorFileName(),CNratio,  PupperLayer, PLoweLayer);
        double mineralNFromSpinup = 0;
        spinup(ref mineralNFromSpinup, initialFOM_Cinput, InitialFOMCtoN, averageAirTemperature, GlobalVars.Instance.theZoneData.GetdebugIt());
...
/*        tmp = 0;
   ...
   public void getGrazedFeedItems()
    ...
    public void CalcModelledYield()
    {
        ...
        for (int i = 0; i < theCrops.Count; i++)
        {
            ...
            while ((gotit == false) || (doneOnce == false))//iterate for each crop, until the crop yield stabilises (note special treatment of grazed crops)
            {
                ...
                RunCropCTool(false, i, meanTemperature, meanDroughtFactor, 0, ref CropCinputToSoil, ref CropNinputToSoil, ref ManCinputToSoil, ref ManNinputToSoil, 
            ...
            resetC_Tool();
            RunCropCTool(false, i, meanTemperature, meanDroughtFactor, 0, ref CropCinputToSoil, ref CropNinputToSoil, ref ManCinputToSoil, ref ManNinputToSoil, ref CropsoilCO2_CEmission, ref CropCleached, ref mineralisedN);
            ...
            for (int j = 0; j < theCrops[i].GettheProducts().Count; j++)
                GlobalVars.Instance.log("Modelled yield for " + i + " " + theCrops[i].GettheProducts()[j].GetModelled_yield(), 5);
        }
    }
    ...
    public void CheckRotationNBalance(int maxCrops)
    {
        ...
        Nbalance = NinputToSoil - (NdeltaSoil + mineralisedSoilN + orgNleached);
        double diff = 0;
        if (NinputToSoil>0)
            diff=Nbalance / NinputToSoil;
    ...
    public void spinup(ref double soilNmineralised, double initialFOM_Cinput, double InitialFOMCtoN, double meanTemperature, bool diagnostics)
    {            
        int startDay=1;
        int endDay=365;
        ...
        for (int j = 0; j < spinupYears; j++)
        {
            ...
            aModel.Dynamics(1,startDay, endDay, FOM_Cin, HUM_Cin, fomnIn, cultivation, meanTemperature,droughtFactor,
        ...
        if (diagnostics)
        {
            GlobalVars.Instance.log("kg C/ha at start of spinup " + initCStored.ToString() + " kg C/ha at end of spinup " + initialSoilC.ToString(), 5);
            GlobalVars.Instance.log("kg N/ha min at end of spinup " + soilNmineralised.ToString(), 5);
        }
   ...
   public void RunCropCTool(bool diagnostics, int cropNo, double meanTemperature, double droughtFactor, double cultivationDepth, ref double CropCinputToSoil, ref double CropNinputToSoil,
       ref double ManCinputToSoil, ref double ManNinputToSoil, ref double CropsoilCO2_CEmission, ref double CropCleached, ref double Nmin)
{
    ...
    while (JDay > 365)
        JDay = JDay - 365;
    aModel.Dynamics((int)JDay, theCrops[cropNo].getStartLongTime(), theCrops[cropNo].getEndLongTime(), FOM_Cin, HUM_Cin, fomnIn, cultivation,
