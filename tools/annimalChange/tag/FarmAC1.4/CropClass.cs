...
public class CropClass
{
       ...
       public void CalculateHarvestedYields()
    {
        ...
        for (int i = 0; i < theProducts.Count; i++)
        {
            Console.WriteLine(theProducts[i].Harvested);
            if (theProducts[i].composition.GetisGrazed()) //grazed crop
            {
            ...
            else
            {
                ...
                else if (theProducts[i].Harvested == "Incorporated") 
                {
                    surfaceResidueC += theProducts[i].GetExpectedYield() * theProducts[i].composition.GetC_conc();
                    surfaceResidueN += theProducts[i].GetExpectedYield() * theProducts[i].composition.GetN_conc();
                    surfaceResidueDM += theProducts[i].GetExpectedYield();
                }
            }
        }
    }
    ...
    public void CalculateManureInput(bool lockit)
    {
        ...
        for (int i = 0; i < manureApplied.Count; i++)
        {
            ...
            GlobalVars.Instance.log(manureFOMNsurface[manureApplied[i].GetdayOfApplication() - 1].ToString(),5);
        }
    }
