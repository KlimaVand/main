...
public class CropClass
{
    ...
    double LAI = 0;
    double totalTsum = 0;
    /*double cumulativeEvapotranspiration = 0;
    double cumulativeDrainage = 0;
    double cumulativeDroughtIndex = 0;*/
...
//            manureTAN[manureApplied[i].GetdayOfApplication()] += aManure.GetTAN();
    ...
    public void CalculateFertiliserInput(ref double fertiliserC, ref double fertiliserNin)
    {
        fertiliserNin = 0;
        fertiliserC = 0;
        ...
        for (int i = 0; i < fertiliserApplied.Count; i++)
        {
            ...
            if ((applicDay < 0) || (applicDay > duration))
                Console.WriteLine("Error in timing of fertiliser application");
...
/*                    else if (theProducts[2].Harvested == "Incorporated")
    ...
    public void CalcAvailableN(ref double surplusMineralN, double thesoilNMineralisation, ref double relGrowth)
    {
        ...
        double evenNSupply = urineNasFertilizer + soilNMineralisation + nAtm;
        relGrowth = 0;
...
//            double temp = residueFromPrevious.composition.Getamount() * theProducts[i].composition.GetN_conc();
    ...
    public void Write()
    {
        GlobalVars.Instance.writeStartTab("CropClass");
            ...
            GlobalVars.Instance.writeInformationToFiles("AverageDroughtIndex", "AverageDroughtIndex", "kg/ha", GetAverageDroughtIndex(), Parens);
    ...
    public double GetAverageDroughtIndex()
    {
        double averageDroughtIndex = 0;
        for (int i = 0; i < duration; i++)
            averageDroughtIndex += droughtFactor[i];
    ...
    public void CalculateClimate()
    {
        ...
        while (clockit.getLongTime() < theEndDate.getLongTime())
        {
            ...
            int maxRainfreeDays = (int)clockit.tabDaysPerMonth[clockit.GetMonth()];
            if (GlobalVars.Instance.theZoneData.rainDays[clockit.GetMonth()]>0)
                maxRainfreeDays = (int)clockit.tabDaysPerMonth[clockit.GetMonth()] / GlobalVars.Instance.theZoneData.rainDays[clockit.GetMonth()];
        ...
        ref double mineralNsurplus)
    {
        ...
        for (int i = 0; i < duration; i++)
        {
            ...
            if (i == 92)
                Console.Write("");
            if (!zeroLeachingDebugging)
                NleachingToday=mineralNavailable * drainage[i] / soilWater[i];
            cumdrainage += drainage[i];
//            double NuptakeToday = mineralNavailable * transpire[i] / soilWater[i];
            double NuptakeToday = mineralNavailable * (1 -droughtFactor[i]);
        ...
        if (maxCropNuptake > 0) //not bare soil
        {
            ...
            if (relGrowth < 1.0)
            {
                Nfixed = GetNfixation(maxCropNuptake - modelledCropNuptake);
...
//        if (nitrateLeaching > 0)
  //          Console.Write("drain " + cumdrainage.ToString());
        Console.WriteLine("Potevap " + GetcumulativepotEvapoTrans().ToString("F6") + " Cum ppt " + GetcumulativePrecipitation().ToString("F6")
            + " evap " + GetcumulativeEvaporation().ToString("F6") + " ave drought " + GetAverageDroughtIndex() + " cum drain " + GetcumulativeDrainage().ToString("F6"));
        if (mineralNavailable < 0.0)
        {
            string messageString = ("Error - insufficient mineral N available to satisfy immobilisation in soil\n");
            messageString += " Crop sequence name " + cropSequenceName + " \n";
