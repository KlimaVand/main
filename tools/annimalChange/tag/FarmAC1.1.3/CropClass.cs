    ...
    public List<GlobalVars.product> GetProductsWithResidue()
    {
        ...
        for(int i=0;i <theProducts.Count;i++)
            if (theProducts[i].Harvested.Contains("Residue"))
            {
                ...
                newProductGraze.composition.SetfedAtPasture(true);
    ...
    public void GetCropInformation(string path, int index, int zoneNr)
    {
        ...
        for (int i = minProduct; i <= maxProduct; i++)
        {
            if(cropInformation.doesIDExist(i)==true)
            {
                ...
                if (anExample.composition.GetfedAtPasture() == true || anExample.Harvested.Contains("Residue"))
                {
                    anExample.Grazed_yield = anExample.Expected_yield;
                    fedAtPastureProduction += anExample.Expected_yield;
                }
...
/*                    else if (theProducts[2].Harvested == "Incorporated")
    ...
    public void CalcAvailableN(double surplusMineralN, double thesoilNMineralisation)
    {
        ...
        {
            case 1:
                N2ONemission = 0;//CalculateN20(fertiliserN, (totmanureTAN + manureOrgN), 0, soilNMineralisation);
                break;
        }
        ...
        Console.WriteLine("Crop " + name + " Min N " + mineralNSupply.ToString("0.00") + " Nfix " + Nfixed.ToString("0.00") + " soil min " + soilNMineralisation.ToString("0.00")
...
//            double temp = residueFromPrevious.composition.Getamount() * theProducts[i].composition.GetN_conc();
            ...
            for (int i = 0; i < theProducts.Count; i++)
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if ((theProducts[i].Harvested == "Harvested") || ((theProducts[i].composition.GetfedAtPasture()))
    ...
    public void WritePlantFile()
    {
        ...
        for (int j = 0; j < times; j++)
        {
            ...
            GlobalVars.Instance.writePlantFile("fertiliserN2OEmission", "N2O emission from fertiliser N", "kgN/ha", fertiliserN2OEmission, Parens, 0);
