...
public class GlobalVars
{
     ...
     public double[] allFeedAndProductsPotentiel = new double[maxNumberFeedItems];
    ...
    public void initialiseFeedAndProductLists()
    {
        for (int i = 0; i < maxNumberFeedItems; i++)
        {
            ...
            allFeedAndProductsPotentiel[i] = 0;
            aproduct = new feedItem();
    ...
    public void CalcAllFeedAndProductsPotentiel(List<CropSequenceClass> list)
    {
        for (int i = 0; i < list.Count; i++)
        {
            ...
            for (int j = 0; j < crops.Count; j++)
            {
                ...
                for (int k = 0; k < products.Count; k++)
                {
                    ...
                    allFeedAndProductsPotentiel[feedCode] += products[k].Potential_yield;
                }
    ...
    public void Write(bool fullModelRun)
    {
        ...
        writer.WriteEndElement();
        writer.WriteStartElement("allFeedAndProductsPotentiel");
        for (int i = 0; i < maxNumberFeedItems; i++)
        {
            if (allFeedAndProductsPotentiel[i] > 0)
            {
          
                GlobalVars.Instance.writeInformationToFiles("allFeedAndProductsPotentiel", "allFeedAndProductsPotentiel "+i.ToString(), "-", allFeedAndProductsPotentiel[i], parens);
            }
        }
        writer.WriteEndElement();
        if (fullModelRun)
