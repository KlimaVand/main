function result = calculateKandPviaLsqcurvefit(runFile, runNR)
  ...
  start=1;
    ...
    for j=(sizeOfInput-numberOfPossibleObserved+1):(sizeOfInput)
       ...
       if(pFraction==-1)
           ...
           if(withScaling==true)
                foo = @(x,xdata)ctool3(0,x(1),sizeOfInputwithoutNan,0,runNR,observations,0,runFile,numberOfPossibleObserved,WithTransport);
           else
               foo = @(x,xdata)ctool2(0,x(1),sizeOfInputwithoutNan,0,runNR,observations,0,runFile,numberOfPossibleObserved,WithTransport);
           end
       elseif(optimizeAmount==true)
           ...
           sizeOfInputwithoutNan=size(observationsWithoutNan);
           if(withScaling==true)
                foo = @(x,xdata)ctool3(0,x(1),sizeOfInputwithoutNan,0,runNR,observations,3,runFile,numberOfPossibleObserved,WithTransport);
           else
               foo = @(x,xdata)ctool2(0,x(1),sizeOfInputwithoutNan,0,runNR,observations,3,runFile,numberOfPossibleObserved,WithTransport);
           end
       else
...
result=lsqcurvefit(foo,[100],observationsWithoutNan,observationsWithoutNan,[0.0],[300],options)
else
