function result = decompositionFOMC14(numberOfLayers,depth,avgTemperature,amplitude,offset,usingRK,t,i,fomc,FOMdecompositionrate,tF,humification,humc,TransportFom,Co2Fom,AmountFom,outputInformation,DecayRate,WithTransport,year,month)
  ...
  for j=1:numberOfLayers
      ...
      if(usingRK==1)
        FomAfterDecom=exp(-FOMdecompositionrate*t*tempCofficent)*fomc(j,1)-DecayRate*fomc(j,1);
      else
          FomAfterDecom=rk4decay(i*t,fomc(j,1),t,@func,FOMdecompositionrate,tempCofficent)-DecayRate*fomc(j,1);
      end
...
if(WithTransport==true)
    ...
    toLowerLayer=fomc(j,1)*0;
    inCorrentLayer=fomc(j,1)*0;
end
 ...
 %form CO2 and humification
   ...
   for j=1:numberOfLayers
    ...
    if(outputInformation==1)
       fprintf(TransportFom,'%d\t',year);
       fprintf(Co2Fom,'%d\t',year);
       fprintf(AmountFom,'%d\t',year);
    end
    if(outputInformation==1)
       fprintf(TransportFom,'%d\t',month);
       fprintf(Co2Fom,'%d\t',month);
       fprintf(AmountFom,'%d\t',month);
    end
    if(outputInformation==1)
