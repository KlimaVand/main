...
if((iterations-numberOfPossibleObservation*12)<i)
    if(month==12)
        ...
        elseif(fullOutput==3)
            sizeofObserved=size(ObservationWithNan);
            sizeofObserved(1)=sizeofObserved(1)/2;
            ...
            if(placeInObservationWithNan<sizeofObserved(1))
                if(~isnan(ObservationWithNan(placeInObservationWithNan,1)))
                    ...
                    if(data(year,23)==0)
                        amount(placeInOutput+halfway(1),1)=romcPlant(2,1)+humcPlant(2,1)+fomcPlant(2,1)+romcManure(2,1)+humcManure(2,1)+fomcManure(2,1);
                    else
                        amount(placeInOutput+halfway(1),1)=romcPlantC14(2,1)+humcPlantC14(2,1)+fomcPlantC14(2,1)+romcManureC14(2,1)+humcManureC14(2,1)+fomcManureC14(2,1);
                    end
                 placeInOutput=placeInOutput+1;
                end
            else
