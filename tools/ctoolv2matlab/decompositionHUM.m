function result = decompositionHUM(numberOfLayers,depth,avgTemperature,amplitude,offset,usingRK,t,i,HUMdecompositionrate,humc,TransportHum,Co2Hum,AmountHum,ROMfraction,romc,outputInformation,WithTransport,year,month)
 ...
 for j=1:(numberOfLayers)
    ...
    if(usingRK==1)
        humAfterDecom=exp(-HUMdecompositionrate*t*tempCofficent)*humc(j,1);
    else
          humAfterDecom=rk4decay(i*t,humc(j,1),t,@func,HUMdecompositionrate,tempCofficent);
    end
 ...
 % CO2, romification and transport
   ...
   for j=1:numberOfLayers
    ...
    if(outputInformation==1)
       fprintf(TransportHum,'%d\t',year);
       fprintf(Co2Hum,'%d\t',year);
       fprintf(AmountHum,'%d\t',year);
    end
    if(outputInformation==1)
       fprintf(TransportHum,'%d\t',month);
       fprintf(Co2Hum,'%d\t',month);
       fprintf(AmountHum,'%d\t',month);
    end
    if(outputInformation==1)
