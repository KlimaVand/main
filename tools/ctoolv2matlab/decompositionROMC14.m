function romc = decompositionROMC14(numberOfLayers,depth,avgTemperature,amplitude,offset,usingRK,t,i,TransportRom,Co2Rom,AmountRom,romc,ROMdecompositionrate,outputInformation,DecayRate,WithTransport,year,month)
 ...
 for j=1:(numberOfLayers)
     ...
     if(usingRK==1)
        romAfterDecom=exp(-ROMdecompositionrate*t*tempCofficent)*romc(j,1)-DecayRate*romc(j,1);
    else
          romAfterDecom=rk4decay(i*t,romc(j,1),t,@func,ROMdecompositionrate,tempCofficent)-DecayRate*romc(j,1);
    end
 ...
 %CO2 ROM and transport
    ...
    if(outputInformation==1)
       fprintf(TransportRom,'%d\t',year);
       fprintf(Co2Rom,'%d\t',year);
       fprintf(AmountRom,'%d\t',year);
    end
    if(outputInformation==1)
       fprintf(TransportRom,'%d\t',month);
       fprintf(Co2Rom,'%d\t',month);
       fprintf(AmountRom,'%d\t',month);
    end
    if(outputInformation==1)
