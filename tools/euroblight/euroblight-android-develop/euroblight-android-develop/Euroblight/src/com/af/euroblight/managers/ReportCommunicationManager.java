package com.af.euroblight.managers;
...
import com.af.euroblight.handlers.CommunicationManagerHandler.ERRORS_DEFINITION_CODE;
...
public class ReportCommunicationManager extends CommunicationManager {
	...
	public void uploadObservation(String username, 
		...
		report.getPhoto().getBitmap().compress(Bitmap.CompressFormat.JPEG, 75, outputStream);
			...
			public void run() {
				...
				try { 
					...
					if (handler != null) {
						...
						handler.errorHandler(report ,ERRORS_DEFINITION_CODE.COMMUNICATION_ERROR);
					}
				}
	...
	private void handleResponse(Report report, 
		...
		try {
			...
			if (resultCode == 20) {
				...
				handler.errorHandler(report, ERRORS_DEFINITION_CODE.getCode(resultCode));
			} 
		} 
		catch (Exception exception) {
			if (handler != null) {
				handler.errorHandler(report, ERRORS_DEFINITION_CODE.PARSE_ERROR);
			}
		}
	}
}