...
public class CropSequenceClass
{
    ...
    public CropSequenceClass(string aPath, int aID, int zoneNr, int currentFarmType, string aParens)
    {
        ...
        int startk = lengthOfSequence + 1;
        int endk = lengthOfSequence +1;
        ...
        for (int j = 0; j < repeats-1; j++)
        {
            for (int i = 0; i < theCrops.Count; i++)
            {
                ...
                //Console.WriteLine(startk.ToString() + " " + endk.ToString());
                if (theCrops[i].GetEndYear() > theCrops[i].GetStartYear())
                    startk++;
    ...
    public void CalcModelledYield()
    {
        ...
        for (int i = 0; i < theCrops.Count; i++)
        {
            ...
            for (int j = 0; j < theCrops[i].GettheProducts().Count; j++)
                Console.WriteLine("Modelled yield for " + i + " " + theCrops[i].GettheProducts()[j].GetModelled_yield());
        }
    }
   ...
   public void calculateExpectedYield()
    {
        ...
        for (int i = 0; i < ID.Count; i++)
        {
            ...
            GlobalVars.Instance.writeInformationToFiles("Identity", "ExpectedYield", "-", ID[i], "none");
            if(ExpectedYeld0[i]!=-1)
            {
                GlobalVars.Instance.writeStartTab("product");
                GlobalVars.Instance.writeInformationToFiles("ExpectedYield", "ExpectedYield", "-",ExpectedYeld0[i] / NumberOfPlants[i],"none");
               GlobalVars.Instance.writeEndTab();
            }
            if (ExpectedYeld1[i] != -1)
            {
                GlobalVars.Instance.writeStartTab("product");
                GlobalVars.Instance.writeInformationToFiles("ExpectedYield","ExpectedYield", "-", ExpectedYeld1[i] / NumberOfPlants[i],"none");
                GlobalVars.Instance.writeEndTab();
            }
            GlobalVars.Instance.writeEndTab();
        }
