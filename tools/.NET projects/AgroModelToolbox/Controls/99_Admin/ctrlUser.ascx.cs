...
namespace AgroModelToolbox.Controls._99_Admin
{
   public partial class ctrlUser : System.Web.UI.UserControl
   {
      ...
      protected void Page_Load(object sender,EventArgs e)
      {
         ...
         strSelectCommand+=" INNER JOIN AgroModel.dbo.Country ON UserAdditional.CountryID = AgroModel.dbo.Country.CountryID";
      ...
      private string GetUserFullName(string aUserName)
      {
         ...
         try
         {
            using(conSQL=new SqlConnection(con))
            {
               ...
               string strSQLget="SELECT aspnet_Users.UserName, UserAdditional.FirstName + ' ' + UserAdditional.LastName + ' [' + AgroModel.dbo.Country.CountryName + ']' AS FullName";
               strSQLget+=" FROM aspnet_Users";
               strSQLget+=" INNER JOIN UserAdditional ON aspnet_Users.UserId = UserAdditional.UserID";
               strSQLget+=" INNER JOIN AgroModel.dbo.Country ON UserAdditional.CountryID = AgroModel.dbo.Country.CountryID";
